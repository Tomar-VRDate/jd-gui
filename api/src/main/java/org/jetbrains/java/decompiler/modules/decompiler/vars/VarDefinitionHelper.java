...
public class VarDefinitionHelper {
	...
	public VarDefinitionHelper(Statement root,
	                           StructMethod mt,
		...
		for (int i = 0;
			...
			VarVersionPair vpp = new VarVersionPair(varindex,
			                                        0);
			if (varindex != 0 || !thisvar) {
				varproc.setVarName(vpp,
				                   vc.getFreeName(varindex));
			}
		...
		while (!stack.isEmpty()) {
			...
			if (lstVars != null) {
				for (VarExprent var : lstVars) {
					...
					varproc.setVarName(new VarVersionPair(var),
					                   vc.getFreeName(var.getIndex()));
	...
	private static boolean isVarReadFirst(VarVersionPair var,
	                                      Statement stat,
	                                      int index,
	...
	private static boolean isVarReadFirst(VarVersionPair target,
	                                      Exprent exp,
	                                      VarExprent... whitelist) {
		AssignmentExprent ass = exp.type == Exprent.EXPRENT_ASSIGNMENT
		                        ? (AssignmentExprent) exp
		                        : null;
	...
	public void setVarDefinitions() {
		...
		for (Entry<Integer, Statement> en : mapVarDefStatements.entrySet()) {
			...
			varproc.setVarName(new VarVersionPair(index,
			                                      0),
			                   vc.getFreeName(index));
			...
			if (stat.type == Statement.TYPE_DO) {
				...
				if (dstat.getLooptype() == DoStatement.LOOP_FOR) {
					...
					if (dstat.getInitExprent() != null && setDefinition(dstat.getInitExprent(),
					                                                    index)) {
						continue;
					} else {
						List<Exprent> lstSpecial = Arrays.asList(dstat.getConditionExprent(),
						                                         dstat.getIncExprent());
			...
			Statement first = findFirstBlock(stat,
			                                 index);
			...
			if (!defset) {
				VarExprent var = new VarExprent(index,
				                                varproc.getVarType(new VarVersionPair(index,
				                                                                      0)),
				                                varproc);
				var.setDefinition(true);

				LocalVariable lvt = findLVT(index,
				                            stat);
				if (lvt != null) {
					var.setLVT(lvt);
				}
	...
	private LocalVariable findLVT(int index,
	                              Statement stat) {
		if (stat.getExprents() == null) {
	...
	private LocalVariable findLVT(int index,
		...
		return var.getIndex() == index
		       ? var.getLVT()
		       : null;
	}

	private Statement findFirstBlock(Statement stat,
	                                 int varindex) {
		...
		while (!stack.isEmpty()) {
			...
			if (stack.isEmpty() || mapStatementVars.get(st.id)
			                                       .contains(varindex)) {
	...
	private Set<Integer> initStatement(Statement stat) {
		...
		for (Entry<Integer, Integer> en : mapCount.entrySet()) {
			if (en.getValue() > 1) {
				mapVarDefStatements.put(en.getKey(),
				                        stat);
			}
		}

		mapStatementVars.put(stat.id,
		                     set);

		return set;
	}

	private boolean setDefinition(Exprent expr,
	                              int index) {
	...
	private void populateTypeBounds(VarProcessor proc,
	                                Statement stat) {
		Map<VarVersionPair, VarType> mapExprentMinTypes = varproc.getVarVersions()
		                                                         .getTypeProcessor()
		                                                         .getMapExprentMinTypes();
		Map<VarVersionPair, VarType> mapExprentMaxTypes = varproc.getVarVersions()
		                                                         .getTypeProcessor()
					...
					switch (exp.type) {
						...
						case Exprent.EXPRENT_EXIT:
							...
							if (instance != null && instance.type == Exprent.EXPRENT_VAR) {
								...
								VarType newType = new VarType(CodeConstants.TYPE_OBJECT,
								                              0,
								                              target);
								...
								if (!newType.equals(oldMax)) {
									if (oldMax != null && oldMax.type == CodeConstants.TYPE_OBJECT) {
										...
										if (DecompilerContext.getStructContext()
										                     .instanceOf(newType.value,
										                                 oldMax.value)) {
											mapExprentMaxTypes.put(key,
											                       newType);
										}
									} else {
										mapExprentMaxTypes.put(key,
	...
	private VPPEntry mergeVars(Statement stat) {
		...
		populateTypeBounds(varproc,
		                   stat);

		Map<VarVersionPair, VarVersionPair> blacklist = new HashMap<VarVersionPair, VarVersionPair>();
		VPPEntry remap = mergeVars(stat,
		                           parent,
		                           new HashMap<Integer, VarVersionPair>(),
		                           blacklist);
		while (remap != null) {
			...
			if (!remapVar(stat,
			              remap.getKey(),
			              remap.getValue())) {
				blacklist.put(remap.getKey(),
				              remap.getValue());
			}
			remap = mergeVars(stat,
			                  parent,
			                  new HashMap<Integer, VarVersionPair>(),
			                  blacklist);
		}
		...
		if (stat.getExprents() == null) {
          ...
          */
					...
					if (leaked_n.size() > 0) {
						...
						} else if (stat.type == Statement.TYPE_SWITCH || stat.type == Statement.TYPE_SYNCRONIZED) {
							if (obj == stat.getFirst()) {
								leaked.putAll(leaked_n); //First is outside the scope so leak!
							} else {
								leaked_n.clear();
							}
						} else if (stat.type == Statement.TYPE_TRYCATCH || stat.type == Statement.TYPE_CATCHALL) {
							leaked_n.clear(); // Catches can't leak anything mwhahahahah!
						}
						this_vars.putAll(leaked_n);
					}
				} else if (obj instanceof Exprent) {
					VPPEntry ret = processExprent((Exprent) obj,
					                              this_vars,
					                              scoped,
					                              blacklist);
					if (ret != null && isVarReadFirst(ret.getValue(),
					                                  stat,
					                                  i + 1)) {
						return ret;
					}
			...
			for (int i = 0;
			     i < exps.size();
			     i++) {
				VPPEntry ret = processExprent(exps.get(i),
				                              this_vars,
				                              scoped,
				                              blacklist);
				if (ret != null && !isVarReadFirst(ret.getValue(),
	...
	private VPPEntry processExprent(Exprent exp,
	                                Map<Integer, VarVersionPair> this_vars,
	                                Map<Integer, VarVersionPair> leaked,
	                                Map<VarVersionPair, VarVersionPair> blacklist) {
		VarExprent var = null;
	...
	private boolean remapVar(Statement stat,
	                         VarVersionPair from,
	                         VarVersionPair to) {
		if (from.equals(to)) {
		...
		if (stat.getExprents() == null) {
			for (Object obj : stat.getSequentialObjects()) {
				if (obj instanceof Statement) {
					success |= remapVar((Statement) obj,
					                    from,
					                    to);
				} else if (obj instanceof Exprent) {
					if (remapVar((Exprent) obj,
					             from,
					             to)) {
						success = true;
					}
		...
		if (success) {
			Iterator<Exprent> itr = stat.getVarDefinitions()
			                            .iterator();
			while (itr.hasNext()) {
				...
				if (exp.type == Exprent.EXPRENT_VAR) {
					...
					if (from.equals(var.getVarVersionPair())) {
						...
						VarType merged = getMergedType(from,
						                               to);
	...
	private boolean remapVar(Exprent exprent,
	                         VarVersionPair from,
	                         VarVersionPair to) {
	...
	private VarType getMergedType(VarVersionPair from,
	                              VarVersionPair to) {
		Map<VarVersionPair, VarType> minTypes = varproc.getVarVersions()
		                                               .getTypeProcessor()
		                                               .getMapExprentMinTypes();
		Map<VarVersionPair, VarType> maxTypes = varproc.getVarVersions()
		                                               .getTypeProcessor()
		                                               .getMapExprentMaxTypes();
		return getMergedType(minTypes.get(from),
		                     minTypes.get(to),
		                     maxTypes.get(from),
		                     maxTypes.get(to));
	}

	private VarType getMergedType(VarType fromMin,
	                              VarType toMin,
	                              VarType fromMax,
	                              VarType toMax) {
		...
		VarType type = fromMin == null
		               ? toMin
		               : (toMin == null
		                  ? fromMin
		                  : VarType.getCommonSupertype(fromMin,
		                                               toMin));
	...
	private void propogateLVTs(Statement stat) {
		...
		if (varproc.hasLVT()) {
			...
			if (!mt.hasModifier(CodeConstants.ACC_STATIC)) {
				...
				if (lvt != null && lvt.size() > 0) {
					types.put(new VarVersionPair(index,
					                             0),
					          new VarInfo(lvt.get(0),
					                      null));
				}
				index++;
			}
		...
		for (Entry<VarVersionPair, VarInfo> e : types.entrySet()) {
			typeNames.put(e.getKey(),
			              e.getValue()
			               .getCast());
		}

		Map<VarVersionPair, String> renames = this.mt.getVariableNamer()
		                                             .rename(typeNames);

		// Stuff the parent context into enclosed child methods
		StatementIterator.iterate(root,
		                          (exprent) -> {
			                          ...
			                          if (exprent.type == Exprent.EXPRENT_VAR) {
				                          ...
				                          if (_new.isAnonymous()) { //TODO: Check for Lambda here?
					                          ClassNode child = DecompilerContext.getClassProcessor()
					                                                             .getMapRootClasses()
					                                                             .get(_new.getNewType().value);
					                          if (child != null) {
						                          if (_new.isLambda()) {
							                          if (child.lambdaInformation.is_method_reference) {
								                          //methods.add(child.getWrapper().getClassStruct().getMethod(child
								                          // .lambdaInformation.content_method_key));
							                          } else {
								                          methods.add(child.classStruct.getMethod(child.lambdaInformation.content_method_name,
								                                                                  child.lambdaInformation.content_method_descriptor));
							                          }
						                          } else {
							                          methods.addAll(child.classStruct.getMethods());
						                          }
			                          ...
			                          for (StructMethod meth : methods) {
				                          meth.getVariableNamer()
				                              .addParentContext(VarDefinitionHelper.this.mt.getVariableNamer());
			                          }
			                          return 0;
		                          });
		...
		for (Entry<VarVersionPair, VarInfo> e : types.entrySet()) {
			...
			LocalVariable lvt = e.getValue()
			                     .getLVT();
			String rename = renames == null
			                ? null
			                : renames.get(idx);

			if (rename != null) {
				varproc.setVarName(idx,
				                   rename);
			}
	...
	private void findTypes(Statement stat,
	                       Map<VarVersionPair, VarInfo> types) {
		if (stat == null) {
			return;
		}
		...
		if (stat.getExprents() == null) {
			for (Object obj : stat.getSequentialObjects()) {
				if (obj instanceof Statement) {
					findTypes((Statement) obj,
					          types);
				} else if (obj instanceof Exprent) {
					findTypes((Exprent) obj,
					          types);
				}
			}
		} else {
	...
	private void findTypes(Exprent exp,
	                       Map<VarVersionPair, VarInfo> types) {
		List<Exprent> lst = exp.getAllExprents(true);
		lst.add(exp);
		...
		for (Exprent exprent : lst) {
			if (exprent.type == Exprent.EXPRENT_VAR) {
				...
				if (var.isDefinition()) {
					types.put(ver,
					          new VarInfo(var.getLVT(),
					                      var.getVarType()));
				} else {
					VarInfo existing = types.get(ver);
					if (existing == null) {
						existing = new VarInfo(var.getLVT(),
						                       var.getVarType());
	...
	private void applyTypes(Statement stat,
	                        Map<VarVersionPair, LocalVariable> types) {
		...
		if (stat.getExprents() == null) {
			for (Object obj : stat.getSequentialObjects()) {
				if (obj instanceof Statement) {
					applyTypes((Statement) obj,
					           types);
				} else if (obj instanceof Exprent) {
					applyTypes((Exprent) obj,
					           types);
				}
			}
		} else {
	...
	private void applyTypes(Exprent exprent,
	                        Map<VarVersionPair, LocalVariable> types) {
		if (exprent == null) {
			return;
		}
	...
	private void setNonFinal(Statement stat,
	                         Set<VarVersionPair> unInitialized) {
		...
		if (stat.type == Statement.TYPE_DO) {
			...
			if (dostat.getInitExprentList() != null) {
				setNonFinal(dostat.getInitExprent(),
				            unInitialized);
			}
			if (dostat.getIncExprentList() != null) {
				setNonFinal(dostat.getIncExprent(),
				            unInitialized);
			}
		} else if (stat.type == Statement.TYPE_IF) {
			IfStatement ifstat = (IfStatement) stat;
			if (ifstat.getIfstat() != null && ifstat.getElsestat() != null) {
				setNonFinal(ifstat.getFirst(),
				            unInitialized);
				setNonFinal(ifstat.getIfstat(),
				            new HashSet<>(unInitialized));
				setNonFinal(ifstat.getElsestat(),
				            unInitialized);
				return;
			}
		}
	...
	private void setNonFinal(Exprent exp,
	                         Set<VarVersionPair> unInitialized) {
		VarExprent var = null;
		...
		if (exp.type == Exprent.EXPRENT_ASSIGNMENT) {
			...
			if (func.getFuncType() >= FunctionExprent.FUNCTION_IMM && func.getFuncType() <= FunctionExprent.FUNCTION_PPI) {
				if (func.getLstOperands()
					...
					var = (VarExprent) func.getLstOperands()
					                       .get(0);
				}
			}
		}
		...
		if (var != null && !var.isDefinition() && !unInitialized.remove(var.getVarVersionPair())) {
			var.getProcessor()
			   .setVarFinal(var.getVarVersionPair(),
	...
	private static class VPPEntry
		...
		public VPPEntry(VarExprent key,
		                VarVersionPair value) {
			super(new VarVersionPair(key),
			      value);
		}
	}
	...
	private static class VarInfo {
		...
		private VarInfo(LocalVariable lvt,
		                VarType type) {
			if (lvt != null && lvt.getSignature() != null) {
				this.cast = ExprProcessor.getCastTypeName(GenericType.parse(lvt.getSignature()),
				                                          false);
			} else if (lvt != null) {
				this.cast = ExprProcessor.getCastTypeName(lvt.getVarType(),
				                                          false);
			} else if (type != null) {
				this.cast = ExprProcessor.getCastTypeName(type,
				                                          false);
			} else {
				this.cast = "this";
			}
