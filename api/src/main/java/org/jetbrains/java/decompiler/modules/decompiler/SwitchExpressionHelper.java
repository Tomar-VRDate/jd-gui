...
public final class SwitchExpressionHelper {
	...
	private static boolean processStatement(SwitchStatement stat) {
		...
		if (condition.type == Exprent.EXPRENT_INVOCATION) {
			...
			if (invoc.getName()
			         .equals("hashCode") && invoc.getClassname()
			                                     .equals("java/lang/String")) {
				return false; // We don't want to make switch expressions yet as switch processing hasn't happened
			}
		}

		// analyze all case statements with breaks to find the var we want. if it's found in all statements with breaks,
		// and no other var is also found, we can make switch expressions
		...
		for (Statement st : check) {
			...
			if (!stat.containsStatement(breaks.get(0).closure)
			    && breaks.get(0)
			             .getDestination().type != Statement.TYPE_DUMMYEXIT) {
				return false;
			}
		}
		...
		for (Statement caseStat : stat.getCaseStatements()) {
			TryWithResourcesProcessor.findEdgesLeaving(caseStat,
			                                           caseStat,
			                                           edges);
		...
		if (!sucs.isEmpty()) {
			...
			if (suc.type != Statement.TYPE_BASICBLOCK) { // make basic block if it isn't found
				...
				SequenceStatement seq = new SequenceStatement(stat,
				                                              suc);
				...
				for (Statement st : stat.getCaseStatements()) {
					for (StatEdge edge : st.getAllSuccessorEdges()) {
						if (edge.getDestination() == oldSuc) {
							...
							st.addSuccessor(new StatEdge(edge.getType(),
							                             st,
							                             suc,
							                             seq));
						}
				...
				suc.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
				                              suc,
				                              oldSuc,
				                              seq));
			}
			...
			for (Statement st : stat.getCaseStatements()) {
				...
				if (!replacements.isEmpty()) {
					...
					// 2) Preventing the variable assignment tracker from putting variable definitions too early because the
					// assignments are no longer in the phantom statement
					replace(st,
					        replacements);
				}
			}
			...
			VarExprent vExpr = new VarExprent(relevantVar.getIndex(),
			                                  relevantVar.getVarType(),
			                                  relevantVar.getProcessor());
			vExpr.setStack(true); // We want to inline
			AssignmentExprent toAdd = new AssignmentExprent(vExpr,
			                                                new SwitchExprent(stat,
			                                                                  relevantVar.getExprType(),
			                                                                  false),
			                                                null);
			...
			List<Exprent> firstExprents = stat.getFirst()
			                                  .getExprents();
			if (firstExprents != null && !firstExprents.isEmpty()) {
				int i = 0;
				for (Iterator<Exprent> iterator = firstExprents.iterator();
				     iterator.hasNext(); ) {
	...
	private static void findReplacements(Statement stat,
	                                     VarVersionPair var,
	                                     Map<Exprent, YieldExprent> replacements) {
				...
				if (e.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					if (assign.getLeft().type == Exprent.EXPRENT_VAR) {
						if (((VarExprent) assign.getLeft()).getIndex() == var.var) {
							...
							replacements.put(assign,
							                 new YieldExprent(assign.getRight(),
							                                  assign.getExprType()));
						}
					}
				}
	...
	private static void replace(Statement stat,
	                            Map<Exprent, YieldExprent> replacements) {
		for (Map.Entry<Exprent, YieldExprent> entry : replacements.entrySet()) {
			stat.replaceExprent(entry.getKey(),
			                    entry.getValue());
		}
	...
	private static List<Statement> findBreakJumps(SwitchStatement stat) {
		...
		breaks.addAll(((RootStatement) stat.getTopParent()).getDummyExit()
	...
	private static VarExprent findRelevantVar(Map<Statement, List<VarVersionPair>> assignments) {
		...
		for (Map.Entry<Statement, List<VarVersionPair>> entry : assignments.entrySet()) {
			if (entry.getValue() == firstNotNull) {
				List<Exprent> exprents = entry.getKey()
				                              .getExprents();
	...
	public static boolean hasSwitchExpressions(RootStatement statement) {
		return statement.mt.getBytecodeVersion()
		                   .hasSwitchExpressions()
		       && DecompilerContext.getOption(IFernflowerPreferences.SWITCH_EXPRESSIONS);
	}
}
