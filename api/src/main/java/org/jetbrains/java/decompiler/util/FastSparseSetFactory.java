...
public class FastSparseSetFactory<E> {
	...
	public FastSparseSetFactory(Collection<? extends E> set) {
		...
		for (E element : set) {
			...
			colValuesInternal.putWithKey(new int[]{block,
			                                       mask},
			                             element);
	...
	private int[] addElement(E element) {
		...
		int[] pointer = new int[]{lastBlock,
		                          lastMask};
		colValuesInternal.putWithKey(pointer,
		                             element);
	...
	public static final class FastSparseSet<E>
		...
		private static void changeNext(int[] arrnext,
		                               int key,
		                               int oldnext,
		                               int newnext) {
			for (int i = key - 1;
		...
		public void intersection(FastSparseSet<E> set) {
			...
			int minlength = Math.min(extdata.length,
			                         intdata.length);

			for (int i = minlength - 1;
			     i >= 0;
			     i--) {
				intdata[i] &= extdata[i];
			}

			for (int i = intdata.length - 1;
			     i >= minlength;
			     i--) {
				intdata[i] = 0;
			}
	...
	public static final class FastSparseSetIterator<E>
		...
		private FastSparseSetIterator(FastSparseSet<E> set) {
			colValuesInternal = set.getFactory()
			                       .getInternalValuesCollection();
			data = set.getData();
			next = set.getNext();
			size = colValuesInternal.size();
		}
		...
		public E next() {
			...
			next_pointer = -1;
			return pointer < size
			       ? colValuesInternal.getKey(pointer)
			       : null;
		}
