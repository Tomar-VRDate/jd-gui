...
public class ConstExprent
	...
	static {
		CHAR_ESCAPES.put(0x8,
		                 "\\b");   /* \u0008: backspace BS */
		CHAR_ESCAPES.put(0x9,
		                 "\\t");   /* \u0009: horizontal tab HT */
		CHAR_ESCAPES.put(0xA,
		                 "\\n");   /* \u000a: linefeed LF */
		CHAR_ESCAPES.put(0xC,
		                 "\\f");   /* \u000c: form feed FF */
		CHAR_ESCAPES.put(0xD,
		                 "\\r");   /* \u000d: carriage return CR */
		...
		UNINLINED_DOUBLES.put(Math.E,
		                      bytecode -> getDouble(bytecode,
		                                            "E",
		                                            "java/lang/Math"));
		UNINLINED_DOUBLES.put(-Math.E,
		                      bytecode -> getDouble(bytecode,
		                                            "E",
		                                            "java/lang/Math").prepend("-"));

		// Positive and negative pi
		UNINLINED_DOUBLES.put(Math.PI,
		                      ConstExprent::getPiDouble);
		UNINLINED_DOUBLES.put(-Math.PI,
		                      bytecode -> getPiDouble(bytecode).prepend("-"));
		...
		for (int i = 2;
		     ...
		     i++) {
			int finalI = i;
			UNINLINED_DOUBLES.put(Math.PI / i,
			                      bytecode -> getPiDouble(bytecode).append(" / " + finalI));
			UNINLINED_DOUBLES.put(-Math.PI / i,
			                      bytecode -> getPiDouble(bytecode).append(" / " + finalI)
			                                                       .prepend("-"));
		}
		...
		for (double numerator = 2;
		     numerator < 13;
		     numerator++) {
			for (double denominator = 2;
			     denominator < 13;
			     denominator++) {
				double gcd = gcd(numerator,
				                 denominator);
				if (gcd == 1) {
					double finalNumerator   = numerator;
					double finalDenominator = denominator;
					UNINLINED_DOUBLES.put(Math.PI * (numerator / denominator),
					                      bytecode -> getPiDouble(bytecode).append(" * "
					                                                               + finalNumerator
					                                                               + " / "
					                                                               + finalDenominator));
					UNINLINED_DOUBLES.put(-Math.PI * (numerator / denominator),
					...
					if ((float) Math.PI * (float) numerator / (float) denominator != (float) (Math.PI * numerator
					                                                                          / denominator)) {
						UNINLINED_FLOATS.put((float) Math.PI * ((float) numerator / (float) denominator),
						                     bytecode -> getPiDouble(bytecode).append(" * "
						                                                              + finalNumerator
						                                                              + "F / "
						                                                              + finalDenominator
		...
		UNINLINED_DOUBLES.put(180.0 / Math.PI,
		                      bytecode -> getPiDouble(bytecode).prepend("180.0 / "));
		UNINLINED_DOUBLES.put(-180.0 / Math.PI,
		                      bytecode -> getPiDouble(bytecode).prepend("-180.0 / "));

		UNINLINED_FLOATS.put((float) (180.0F / Math.PI),
		                     bytecode -> getPiDouble(bytecode).prepend("180.0F / "));
		UNINLINED_FLOATS.put((float) (-180.0F / Math.PI),
		                     bytecode -> getPiDouble(bytecode).prepend("-180.0F / "));

		UNINLINED_FLOATS.put(180.0F / (float) Math.PI,
		                     bytecode -> getPiDouble(bytecode).prepend("180.0F / (float)"));
		UNINLINED_FLOATS.put(-180.0F / (float) Math.PI,
		                     bytecode -> getPiDouble(bytecode).prepend("-180.0F / (float)"));

		// Positive and negative pi / 180
		UNINLINED_DOUBLES.put(Math.PI / 180.0,
		                      bytecode -> getPiDouble(bytecode).append(" / 180.0"));
		UNINLINED_DOUBLES.put(-Math.PI / 180.0,
		                      bytecode -> getPiDouble(bytecode).append(" / 180.0")
		                                                       .prepend("-"));

		UNINLINED_FLOATS.put((float) (Math.PI / 180.0),
		                     bytecode -> getPiDouble(bytecode).append(" / 180.0")
		                                                      .prepend("(float) "));
		UNINLINED_FLOATS.put((float) (-Math.PI / 180.0),
		                     bytecode -> getPiDouble(bytecode).append(" / 180.0")
		...
		UNINLINED_DOUBLES.forEach((key, valueFunction) -> {
			UNINLINED_FLOATS.put(key.floatValue(),
			                     bytecode -> {
				                     TextBuffer doubleValue = valueFunction.apply(bytecode);
				                     if (doubleValue.count(" ",
				                                           0)
				                         > 0) { // As long as all uninlined double values with more than one expression have a
					                     // space in it, this'll work.
					                     doubleValue.prepend("(")
					                                .append(")");
				                     }
		...
		UNINLINED_DOUBLES.put(Double.POSITIVE_INFINITY,
		                      bytecode -> getDouble(bytecode,
		                                            "POSITIVE_INFINITY",
		                                            "java/lang/Double"));
		UNINLINED_DOUBLES.put(Double.NEGATIVE_INFINITY,
		                      bytecode -> getDouble(bytecode,
		                                            "NEGATIVE_INFINITY",
		                                            "java/lang/Double"));
		UNINLINED_DOUBLES.put(Double.MAX_VALUE,
		                      bytecode -> getDouble(bytecode,
		                                            "MAX_VALUE",
		                                            "java/lang/Double"));
		UNINLINED_DOUBLES.put(Double.MIN_NORMAL,
		                      bytecode -> getDouble(bytecode,
		                                            "MIN_NORMAL",
		                                            "java/lang/Double"));
		UNINLINED_DOUBLES.put(Double.MIN_VALUE,
		                      bytecode -> getDouble(bytecode,
		                                            "MIN_VALUE",
		                                            "java/lang/Double"));
		UNINLINED_DOUBLES.put(-Double.MAX_VALUE,
		...
		UNINLINED_FLOATS.put(Float.POSITIVE_INFINITY,
		                     bytecode -> getFloat(bytecode,
		                                          "POSITIVE_INFINITY",
		                                          "java/lang/Float"));
		UNINLINED_FLOATS.put(Float.NEGATIVE_INFINITY,
		                     bytecode -> getFloat(bytecode,
		                                          "NEGATIVE_INFINITY",
		                                          "java/lang/Float"));
		UNINLINED_FLOATS.put(Float.MAX_VALUE,
		                     bytecode -> getFloat(bytecode,
		                                          "MAX_VALUE",
		                                          "java/lang/Float"));
		UNINLINED_FLOATS.put(Float.MIN_NORMAL,
		                     bytecode -> getFloat(bytecode,
		                                          "MIN_NORMAL",
		                                          "java/lang/Float"));
		UNINLINED_FLOATS.put(Float.MIN_VALUE,
		                     bytecode -> getFloat(bytecode,
		                                          "MIN_VALUE",
		                                          "java/lang/Float"));
		UNINLINED_FLOATS.put(-Float.MAX_VALUE,
	...
	private ConstExprent(VarType constType,
	                     Object value,
	                     boolean boolPermitted,
	                     BitSet bytecodeOffsets) {
		super(EXPRENT_CONST);
		...
		if (constType.equals(VarType.VARTYPE_CLASS) && value != null) {
			...
			List<VarType> args = Collections.singletonList(new VarType(stringVal,
			                                                           !stringVal.startsWith("[")));
			this.constType = new GenericType(constType.type,
			                                 constType.arrayDim,
			                                 constType.value,
			                                 null,
			                                 args,
			                                 GenericType.WILDCARD_NO);
		}
	}

	private static VarType guessType(int val,
	                                 boolean boolPermitted) {
		if (boolPermitted) {
	...
	private static double gcd(double a,
	                          double b) {
		return b == 0
		       ? a
		       : gcd(b,
	...
	private static TextBuffer getPiDouble(BitSet bytecode) {
		return getDouble(bytecode,
		                 "PI",
		                 "java/lang/Math");
	}

	private static TextBuffer getDouble(BitSet bytecode,
	                                    String name,
	                                    String className) {
		return new FieldExprent(name,
		                        className,
		                        true,
		                        null,
		                        FieldDescriptor.DOUBLE_DESCRIPTOR,
		                        bytecode).toJava(0);
	}

	private static TextBuffer getFloat(BitSet bytecode,
	                                   String name,
	                                   String className) {
		return new FieldExprent(name,
		                        className,
		                        true,
		                        null,
		                        FieldDescriptor.FLOAT_DESCRIPTOR,
		                        bytecode).toJava(0);
	}
	...
	public static String convertStringToJava(String value,
	                                         boolean ascii) {
	...
	public static ConstExprent getZeroConstant(int type) {
		switch (type) {
			case CodeConstants.TYPE_INT:
				return new ConstExprent(VarType.VARTYPE_INT,
				                        0,
				                        null);
			case CodeConstants.TYPE_LONG:
				return new ConstExprent(VarType.VARTYPE_LONG,
				                        0L,
				                        null);
			case CodeConstants.TYPE_DOUBLE:
				return new ConstExprent(VarType.VARTYPE_DOUBLE,
				                        0d,
				                        null);
			case CodeConstants.TYPE_FLOAT:
				return new ConstExprent(VarType.VARTYPE_FLOAT,
				                        0f,
				                        null);
		}
	...
	@Override
	public Exprent copy() {
		return new ConstExprent(constType,
		                        value,
		                        bytecode);
	}
		...
		switch (constType.type) {
			...
			case CodeConstants.TYPE_INT:
				...
				if (!literal) {
					if (intVal == Integer.MAX_VALUE) {
						return buf.append(new FieldExprent("MAX_VALUE",
						                                   "java/lang/Integer",
						                                   true,
						                                   null,
						                                   FieldDescriptor.INTEGER_DESCRIPTOR,
						                                   bytecode).toJava(0));
					} else if (intVal == Integer.MIN_VALUE) {
						return buf.append(new FieldExprent("MIN_VALUE",
						                                   "java/lang/Integer",
						                                   true,
						                                   null,
						                                   FieldDescriptor.INTEGER_DESCRIPTOR,
				...
				if (!literal) {
					if (longVal == Long.MAX_VALUE) {
						return buf.append(new FieldExprent("MAX_VALUE",
						                                   ...
						                                   true,
						                                   null,
						                                   FieldDescriptor.LONG_DESCRIPTOR,
						                                   bytecode).toJava(0));
			...
			case CodeConstants.TYPE_FLOAT:
				...
				if (!literal) {
					if (Float.isNaN(floatVal)) {
						return buf.append(new FieldExprent("NaN",
						                                   "java/lang/Float",
						                                   true,
						                                   null,
						                                   FieldDescriptor.FLOAT_DESCRIPTOR,
						...
						return buf.append(UNINLINED_FLOATS.get(floatVal)
						                                  .apply(bytecode));
					}
				} else {
				...
				return buf.append(trimZeros(Float.toString(floatVal)))
				          .append('F');
			...
			case CodeConstants.TYPE_DOUBLE:
				...
				if (!literal) {
					...
					// Try to convert the double representation of the value to the float representation, to output the cleanest
					// version of the value.
					// This patch is based on work in ForgeFlower submitted by Pokechu22.
					float floatRepresentation = (float) doubleVal;
					if (floatRepresentation == doubleVal) {
						if (Float.toString(floatRepresentation)
						         .length() < Double.toString(doubleVal)
						                           .length()) {
							// Check the uninlined values to see if we have one of those
							if (UNINLINED_FLOATS.containsKey(floatRepresentation)) {
								return buf.append(UNINLINED_FLOATS.get(floatRepresentation)
								                                  .apply(bytecode));
							} else {
								// Return the standard representation if the value is not able to be uninlined
								return buf.append(Float.toString(floatRepresentation))
								          .append("F");
							}
						}
					}
			...
			case CodeConstants.TYPE_OBJECT:
				if (constType.equals(VarType.VARTYPE_STRING)) {
					return buf.append(convertStringToJava(value.toString(),
					                                      ascii))
					          .enclose("\"",
					                   "\"");
				} else if (constType.equals(VarType.VARTYPE_CLASS)) {
					String stringVal = value.toString();
					VarType type = new VarType(stringVal,
					                           !stringVal.startsWith("["));
					return buf.append(ExprProcessor.getCastTypeName(type))
					          .append(".class");
				}
		}
	...
	public boolean equals(Object o) {
		...
		return InterpreterUtil.equalObjects(constType,
		                                    cn.getConstType()) && InterpreterUtil.equalObjects(value,
		                                                                                       cn.getValue());
	}

	@Override
	public int hashCode() {
		int result = constType != null
		             ? constType.hashCode()
		             : 0;
