...
public final class GenericMain {
	...
	public static GenericClassDescriptor parseClassSignature(String qualifiedName,
	                                                         String signature) {
		String original = signature;
		try {
			GenericClassDescriptor descriptor = new GenericClassDescriptor();

			signature = parseFormalParameters(signature,
			...
			DecompilerContext.getLogger()
			                 .writeMessage("Invalid signature: " + original,
	...
	public static GenericFieldDescriptor parseFieldSignature(String signature) {
		try {
			...
			DecompilerContext.getLogger()
			                 .writeMessage("Invalid signature: " + signature,
			                               IFernflowerLogger.Severity.WARN);
			return null;
		}
	}
	...
	public static GenericMethodDescriptor parseMethodSignature(String signature) {
		...
		try {
			...
			String parameters = signature.substring(1,
			                                        to);
			...
			return new GenericMethodDescriptor(typeParameters,
			                                   typeParameterBounds,
			                                   parameterTypes,
			                                   returnType,
			                                   exceptionTypes);
	...
	private static String parseFormalParameters(String signature,
		...
		String value = signature.substring(1,
		                                   index);
		...
		while (value.length() > 0) {
			...
			String param = value.substring(0,
			                               to);
