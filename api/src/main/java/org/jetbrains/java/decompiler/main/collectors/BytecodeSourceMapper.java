...
public class BytecodeSourceMapper {
	...
	public void addMapping(String className,
	                       ...
	                       int bytecodeOffset,
	                       int sourceLine) {
		Map<String, Map<Integer, Integer>> class_mapping = mapping.computeIfAbsent(className,
		                                                                           k -> new LinkedHashMap<>()); // need
		// to preserve order
		Map<Integer, Integer> method_mapping = class_mapping.computeIfAbsent(methodName,
		                                                                     k -> new HashMap<>());

		// don't overwrite
		method_mapping.putIfAbsent(bytecodeOffset,
		                           sourceLine);
	}

	public void addTracer(String className,
	                      String methodName,
	                      BytecodeMappingTracer tracer) {
		for (Entry<Integer, Integer> entry : tracer.getMapping()
		                                           .entrySet()) {
			addMapping(className,
			           methodName,
			           entry.getKey(),
			           entry.getValue());
		}
	...
	public void dumpMapping(TextBuffer buffer,
		...
		for (Entry<String, Map<String, Map<Integer, Integer>>> class_entry : mapping.entrySet()) {
			...
			for (Entry<String, Map<Integer, Integer>> method_entry : class_mapping.entrySet()) {
				...
				buffer.appendIndent(1)
				      .append("method '" + method_entry.getKey() + "' {" + lineSeparator);
		...
		for (Entry<Integer, Integer> entry : sorted.entrySet()) {
			buffer.append(entry.getKey())
			      .append(" <-> ")
			      .append(entry.getValue() + offset_total + 1)
			      .appendLineSeparator();
		}
