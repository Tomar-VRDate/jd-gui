 ...
 * Including the entire classpath is generally needed for this to work as it needs to know which classes implement
 * AutoCloseable.
 *
...
public final class TryWithResourcesProcessor {
	...
	public static boolean makeTryWithResource(CatchAllStatement finallyStat) {
		...
		Statement toCheck = finallyStat.getHandler()
		                               .getFirst();
		if (toCheck.type != Statement.TYPE_IF
		    || ((IfStatement) toCheck).getIfstat() == null
		    || ((IfStatement) toCheck).getIfstat().type != Statement.TYPE_IF) {
			return false;
		}
		...
		if (elseBlock.getExprents() != null
		    && elseBlock.getExprents()
		                .size() == 1) {
			Exprent exp = elseBlock.getExprents()
			                       .get(0);
		...
		if (var != null) {
			...
			for (StatEdge edge : finallyStat.getAllPredecessorEdges()) {
				if (edge.getDestination()
				        .equals(finallyStat) && edge.getSource().type == Statement.TYPE_BASICBLOCK) {
					ass = findResourceDef(var,
					                      edge.getSource());
					if (ass != null) {
				...
				if (stat2.type == Statement.TYPE_TRYCATCH) {
					...
					if (ass.getRight()
						...
						if (child.getFirst() != null) {
							fixResourceAssignment(resourceDef,
							                      child.getFirst());
						}
					}

					if (resourceDef.getRight()
					               .getExprType()
					               .equals(VarType.VARTYPE_NULL)) {
						return false;
					}
					...
					initBlock.getExprents()
					         .remove(ass);
					child.getResources()
					     .add(0,
					          resourceDef);

					if (!finallyStat.getVarDefinitions()
					                .isEmpty()) {
						child.getVarDefinitions()
						     .addAll(0,
						             finallyStat.getVarDefinitions());
					}

					stat.replaceStatement(finallyStat,
					                      child);
					removeRedundantThrow(initBlock,
					                     child);
					return true;
				}
			}
	...
	public static boolean makeTryWithResourceJ11(CatchStatement tryStatement) {
		...
		if (tryStatement.getStats()
		                .size() < 2) {
			return false;
		}

		Statement inner = tryStatement.getStats()
		                              .get(1); // Get catch block
		...
		if (inner.type == Statement.TYPE_SEQUENCE) {
			...
			if (inner.type == Statement.TYPE_IF) {
				Exprent ifCase = ((IfStatement) inner).getHeadexprent()
				                                      .getCondition();
				...
				if (ifCase.type == Exprent.EXPRENT_FUNCTION) {
					...
					Exprent check = func.getLstOperands()
					                    .get(0);
						...
						if (inner.type == Statement.TYPE_TRYCATCH) {
							...
							if (inTry.type == Statement.TYPE_BASICBLOCK) {
								...
								if (isCloseable(first)) {
									...
									if (!closeable.getVarVersionPair()
									              .equals(((VarExprent) check).getVarVersionPair())) {
										closeable = null;
									}
								}
							}
		...
		if (edge.getSource().type == Statement.TYPE_BASICBLOCK) {
			AssignmentExprent assignment = findResourceDef(closeable,
			                                               edge.getSource());
	...
	private static boolean isValid(Statement stat,
		...
		if (nullable) {
			...
			if (stat.type == Statement.TYPE_IF) {
				...
				Exprent condition = ifStat.getHeadexprent()
				                          .getCondition();
				...
				if (condition.type == Exprent.EXPRENT_FUNCTION) {
					...
					if (func.getFuncType() == FunctionExprent.FUNCTION_NE
					    && func.getLstOperands()
					    ...
					    && func.getLstOperands()
					           ...
					           .getExprType()
					           .equals(VarType.VARTYPE_NULL)) {
						return func.getLstOperands()
						           .get(0).type == Exprent.EXPRENT_VAR && ((VarExprent) func.getLstOperands()
						                                                                    .get(0)).getVarVersionPair()
			...
			if (stat.type == Statement.TYPE_BASICBLOCK) {
				if (stat.getExprents() != null && !stat.getExprents()
				                                       .isEmpty()) {
					Exprent exprent = stat.getExprents()
					                      .get(0);
					...
					if (exprent.type == Exprent.EXPRENT_INVOCATION) {
						...
						return inst.type == Exprent.EXPRENT_VAR && inst.equals(closeable) && isCloseable(exprent);
					}
				}
			}
		}
	...
	private static void removeClose(Statement statement,
	                                boolean nullable) {
		if (nullable) {
			// Breaking out of parent, remove label
			// TODO: The underlying problem is that empty labeled basic blocks remove their label but the edge is marked as
			//  labeled and explicit.
			// label1: {
			//   ...
			...
			if (!edges.isEmpty() && edges.get(0).closure == statement.getParent()) {
				...
				for (StatEdge edge : statement.getFirst()
				                              .getAllSuccessorEdges()) {
					edge.getDestination()
					    .removePredecessor(edge);
				}

				for (StatEdge edge : ((IfStatement) statement).getIfstat()
				                                              .getAllSuccessorEdges()) {
					edge.getDestination()
					    .removePredecessor(edge);
	...
	private static Set<Statement> findExitpoints(Statement stat) {
		...
		findEdgesLeaving(stat.getFirst(),
		                 stat,
		                 edges);

		return edges.stream()
		            .map(StatEdge::getDestination)
	...
	private static FunctionExprent unwrapNegations(FunctionExprent func) {
		while (func.getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT) {
			Exprent expr = func.getLstOperands()
			                   .get(0);
	...
	private static AssignmentExprent findResourceDef(VarExprent var,
		...
		for (Exprent exp : prevStatement.getExprents()) {
			if (exp.type == Exprent.EXPRENT_ASSIGNMENT) {
				...
				if (ass.getLeft().type
				    == Exprent.EXPRENT_VAR) { // cannot use equals as var's varType may be unknown and not match
					VarExprent left = (VarExprent) ass.getLeft();
					if (left.getVarVersionPair()
					        .equals(var.getVarVersionPair())) {
						return ass;
					}
				}
			}
	...
	private static void fixResourceAssignment(AssignmentExprent ass,
	                                          Statement statement) {
		if (statement.getExprents() != null) {
			for (Exprent exp : statement.getExprents()) {
				if (exp.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					if (ass.getLeft()
					       .equals(toRemove.getLeft()) && !toRemove.getRight()
					                                               .getExprType()
					                                               .equals(VarType.VARTYPE_NULL)) {
	...
	private static boolean removeRedundantThrow(BasicBlockStatement initBlock,
	                                            CatchStatement catchStat) {
		if (catchStat.getStats()
		             .size() > 1) {
			boolean   removed = false;
			Statement temp    = null;
			int       i       = 1;
			for (;
			     i < catchStat.getStats()
			                  .size();
			     ++i) {
				temp = catchStat.getStats()
				                .get(i);

				if (temp.type == Statement.TYPE_BASICBLOCK && temp.getExprents() != null) {
					if (temp.getExprents()
					        .size() >= 2 && catchStat.getVars()
					                                 .get(i - 1)
					                                 .getVarType().value.equals("java/lang/Throwable")) {
						if (temp.getExprents()
						        .get(temp.getExprents()
						                 .size() - 1).type == Exprent.EXPRENT_EXIT) {
							ExitExprent exitExprent = (ExitExprent) temp.getExprents()
							                                            .get(temp.getExprents()
							                                                     .size() - 1);
							if (exitExprent.getExitType() == ExitExprent.EXIT_THROW && exitExprent.getValue()
							                                                                      .equals(catchStat.getVars()
							                                                                                       .get(i - 1))) {

								catchStat.getExctStrings()
								         .remove(i - 1);
								catchStat.getVars()
								         .remove(i - 1);
								catchStat.getStats()
			...
			if (removed
			    && temp.getExprents()
			           .get(temp.getExprents()
			                    .size() - 2).type == Exprent.EXPRENT_ASSIGNMENT) {
				AssignmentExprent assignmentExp = (AssignmentExprent) temp.getExprents()
				                                                          .get(temp.getExprents()
				                                                                   .size() - 2);
				if (assignmentExp.getLeft()
				                 .getExprType().value.equals("java/lang/Throwable")) {
					for (Exprent exprent : initBlock.getExprents()) {
						if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
							AssignmentExprent toRemove = (AssignmentExprent) exprent;
							if (toRemove.getLeft()
							            .equals(assignmentExp.getLeft())) {
								initBlock.getExprents()
								         .remove(toRemove);
								return true;
							}
