...
public class StructModuleAttribute
	...
	private static List<ExportsEntry> readExports(DataInputFullStream data,
	                                              ConstantPool pool)
					throws
					IOException {
		...
		for (int i = 0;
			...
			List<String> exportsToModules = readStringList(data,
			                                               pool);
			String packageName = pool.getPrimitiveConstant(packageNameIndex)
			                         .getString();
			exports.add(new ExportsEntry(packageName,
			                             exportsFlags,
			                             exportsToModules));
		}
		return exports;
	...
	private static List<OpensEntry> readOpens(DataInputFullStream data,
	                                          ConstantPool pool)
					throws
					IOException {
		...
		for (int i = 0;
			...
			List<String> opensToModules = readStringList(data,
			                                             pool);
			String packageName = pool.getPrimitiveConstant(packageNameIndex)
			                         .getString();
			opens.add(new OpensEntry(packageName,
			                         opensFlags,
			                         opensToModules));
		}
		return opens;
	...
	private static List<String> readUses(DataInputFullStream data,
	                                     ConstantPool pool)
					throws
					IOException {
	...
	private static List<ProvidesEntry> readProvides(DataInputFullStream data,
	                                                ConstantPool pool)
					throws
					IOException {
		...
		for (int i = 0;
			...
			String interfaceName = pool.getPrimitiveConstant(interfaceNameIndex)
			                           .getString();
			List<String> implementationNames = readStringList(data,
			                                                  pool);
			provides.add(new ProvidesEntry(interfaceName,
			                               implementationNames));
		}
		return provides;
	}

	private static List<String> readStringList(DataInputFullStream data,
	                                           ConstantPool pool)
					throws
					IOException {
		...
		if (count <= 0) {
			...
			for (int i = 0;
				...
				strings.add(pool.getPrimitiveConstant(index)
				                .getString());
			}
			return strings;
		}
	...
	public void initContent(DataInputFullStream data,
		...
		this.moduleName = pool.getPrimitiveConstant(moduleNameIndex)
		                      .getString();
		this.moduleFlags = data.readUnsignedShort();

		int moduleVersionIndex = data.readUnsignedShort();
		if (moduleVersionIndex != 0) {
			moduleVersion = pool.getPrimitiveConstant(moduleVersionIndex)
			                    .getString();
		}

		this.requires = readRequires(data,
		                             pool);
		this.exports = readExports(data,
		                           pool);
		this.opens = readOpens(data,
		                       pool);
		this.uses = readUses(data,
		                     pool);
		this.provides = readProvides(data,
		                             pool);
	}

	public List<RequiresEntry> readRequires(DataInputFullStream data,
	                                        ConstantPool pool)
					throws
					IOException {
		...
		for (int i = 0;
			...
			String moduleName = pool.getPrimitiveConstant(moduleNameIndex)
			                        .getString();
			String version = versionIndex == 0
			                 ? null
			                 : pool.getPrimitiveConstant(versionIndex)
			                       .getString();
			requires.add(new RequiresEntry(moduleName,
			                               requiresFlags,
			                               version));
		}
		return requires;
	...
	public static final class RequiresEntry {
		...
		public RequiresEntry(String moduleName,
		                     int flags,
	...
	public static final class ExportsEntry {
		...
		public ExportsEntry(String packageName,
		                    int flags,
		                    List<String> exportToModules) {
	...
	public static final class OpensEntry {
		...
		public OpensEntry(String packageName,
		                  int flags,
		                  List<String> exportToModules) {
			this.packageName = packageName;
			this.flags = flags;
	...
	public static final class ProvidesEntry {
		...
		public ProvidesEntry(String interfaceName,
