...
public class JrtFinder {
	public static void addJrt(StructContext ctx) {
		...
		try {
			...
			if (objectUrl == null) {
				logger.writeMessage("Could not locate Java runtime",
				                    Severity.WARN);
				return;
			}
			...
			switch (objectUri.getScheme()) {
				...
				case "jrt":
					...
					break;
				default:
					logger.writeMessage("Unknown file system for Java runtime: " + objectUri.getScheme(),
					                    Severity.WARN);
			}
