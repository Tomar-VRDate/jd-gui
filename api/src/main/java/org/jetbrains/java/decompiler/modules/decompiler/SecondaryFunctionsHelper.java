...
public final class SecondaryFunctionsHelper {
	...
	private static final int[] funcsnot = new int[]{FunctionExprent.FUNCTION_NE,
	...
	static {
		mapNumComparisons.put(FunctionExprent.FUNCTION_EQ,
		                      new Integer[]{FunctionExprent.FUNCTION_LT,
		                                    FunctionExprent.FUNCTION_EQ,
		                                    FunctionExprent.FUNCTION_GT});
		mapNumComparisons.put(FunctionExprent.FUNCTION_NE,
		                      new Integer[]{FunctionExprent.FUNCTION_GE,
	...
	public static boolean identifySecondaryFunctions(Statement stat,
		...
		if (stat.getExprents() == null) {
			...
			if (stat.type == Statement.TYPE_IF) {
				...
				if (ifelsestat.iftype == IfStatement.IFTYPE_IFELSE && ifstat.getExprents() != null && ifstat.getExprents()
				                                                                                            .isEmpty() && (
								    ifstat.getAllSuccessorEdges()
								          .isEmpty()
								    || !ifstat.getAllSuccessorEdges()
								              .get(0).explicit)) {
					...
					ifelsestat.getHeadexprentList()
					          .set(0,
					               ((IfExprent) ifelsestat.getHeadexprent()
					                                      .copy()).negateIf());
		...
		while (replaced) {
			...
			List<Object> lstObjects = new ArrayList<>(stat.getExprents() == null
			                                          ? stat.getSequentialObjects()
			                                          : stat.getExprents());
			...
			for (int i = 0;
				...
				if (obj instanceof Statement) {
					...
					Exprent retexpr = identifySecondaryFunctions((Exprent) obj,
					                                             true,
					                                             varProc);
					if (retexpr != null) {
						if (stat.getExprents() == null) {
							// only head expressions can be replaced!
							stat.replaceExprent((Exprent) obj,
							                    retexpr);
						} else {
							stat.getExprents()
							    .set(i,
	...
	private static Exprent identifySecondaryFunctions(Exprent exprent,
		...
		if (exprent.type == Exprent.EXPRENT_FUNCTION) {
			...
			switch (fexpr.getFuncType()) {
				...
				case FunctionExprent.FUNCTION_LE:
					Exprent expr1 = fexpr.getLstOperands()
					                     .get(0);
					Exprent expr2 = fexpr.getLstOperands()
					                     .get(1);

					if (expr1.type == Exprent.EXPRENT_CONST) {
						expr2 = expr1;
						expr1 = fexpr.getLstOperands()
						             .get(1);
					}
					...
					if (expr1.type == Exprent.EXPRENT_FUNCTION && expr2.type == Exprent.EXPRENT_CONST) {
						...
						if (functype == FunctionExprent.FUNCTION_LCMP
							...
							if (desttype >= 0) {
								if (functype != FunctionExprent.FUNCTION_LCMP) {
									boolean oneForNan = functype == FunctionExprent.FUNCTION_DCMPL
									                    || functype == FunctionExprent.FUNCTION_FCMPL;
									boolean trueForOne = desttype == FunctionExprent.FUNCTION_LT
									                     || desttype == FunctionExprent.FUNCTION_LE;
									...
									if (trueForNan) {
										...
										operands.add(new FunctionExprent(funcsnot[desttype - FunctionExprent.FUNCTION_EQ],
										                                 funcexpr.getLstOperands(),
										                                 funcexpr.bytecode));
										return new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
										                           operands,
										                           funcexpr.bytecode);
									}
								}
								return new FunctionExprent(desttype,
								                           funcexpr.getLstOperands(),
								                           funcexpr.bytecode);
							}
						}
		...
		while (replaced) {
			...
			for (Exprent expr : exprent.getAllExprents()) {
				Exprent retexpr = identifySecondaryFunctions(expr,
				                                             false,
				                                             varProc);
				if (retexpr != null) {
		...
		switch (exprent.type) {
			case Exprent.EXPRENT_FUNCTION:
				...
				switch (fexpr.getFuncType()) {
						...
						for (int i = 0;
							...
							if (operand.type == Exprent.EXPRENT_CONST && operandtype.type != CodeConstants.TYPE_BOOLEAN) {
								...
								if (val == -1) {
									...
									return new FunctionExprent(FunctionExprent.FUNCTION_BIT_NOT,
									                           lstBitNotOperand,
									                           fexpr.bytecode);
								}
							}
					...
					case FunctionExprent.FUNCTION_EQ:
					case FunctionExprent.FUNCTION_NE:
						if (lstOperands.get(0)
						               .getExprType().type == CodeConstants.TYPE_BOOLEAN
						    && lstOperands.get(1)
						                  .getExprType().type == CodeConstants.TYPE_BOOLEAN) {
							for (int i = 0;
							     i < 2;
								...
								if (lstOperands.get(i).type == Exprent.EXPRENT_CONST) {
									...
									if ((fexpr.getFuncType() == FunctionExprent.FUNCTION_EQ && val == 1) || (fexpr.getFuncType()
									                                                                         == FunctionExprent.FUNCTION_NE
									                                                                         && val == 0)) {
					...
					case FunctionExprent.FUNCTION_BOOL_NOT:
						if (lstOperands.get(0).type == Exprent.EXPRENT_CONST) {
							...
							if (val == 0) {
								return new ConstExprent(VarType.VARTYPE_BOOLEAN,
								                        1,
								                        fexpr.bytecode);
							} else {
								return new ConstExprent(VarType.VARTYPE_BOOLEAN,
					...
					case FunctionExprent.FUNCTION_IIF:
						...
						if (expr1.type == Exprent.EXPRENT_CONST && expr2.type == Exprent.EXPRENT_CONST) {
							...
							if (expr1.type == Exprent.EXPRENT_CONST && expr1.getExprType().type == CodeConstants.TYPE_BOOLEAN) {
								...
								if (val) {
									// bl ? true : bl2 <-> bl || bl2
									return new FunctionExprent(FunctionExprent.FUNCTION_COR,
									                           Arrays.asList(lstOperands.get(0),
									                                         lstOperands.get(2)),
									                           fexpr.bytecode);
								} else {
									// bl ? false : bl2 <-> !bl && bl2
									FunctionExprent fnot = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
									                                           lstOperands.get(0),
									                                           fexpr.bytecode);
									return new FunctionExprent(FunctionExprent.FUNCTION_CADD,
									                           Arrays.asList(fnot,
									                                         lstOperands.get(2)),
									                           fexpr.bytecode);
								}
							} else if (expr2.type == Exprent.EXPRENT_CONST
							           && expr2.getExprType().type == CodeConstants.TYPE_BOOLEAN) {
								ConstExprent cexpr2 = (ConstExprent) expr2;
					...
					case FunctionExprent.FUNCTION_DCMPG:
						...
						VarType type = lstOperands.get(0)
						                          .getExprType();

						FunctionExprent iff = new FunctionExprent(FunctionExprent.FUNCTION_IIF,
						                                          Arrays.asList(new FunctionExprent(FunctionExprent.FUNCTION_LT,
						                                                                            Arrays.asList(new VarExprent(var,
						                                                                                                         type,
						                                                                                                         varProc),
						...
						FunctionExprent head = new FunctionExprent(FunctionExprent.FUNCTION_EQ,
						                                           Arrays.asList(new AssignmentExprent(new VarExprent(var,
						                                                                                              type,
						                                                                                              varProc),
						...
						varProc.setVarType(new VarVersionPair(var,
						                                      0),
						                   type);
			...
			case Exprent.EXPRENT_INVOCATION:
				if (!statement_level) { // simplify if exprent is a real expression. The opposite case is pretty absurd, can
					// still happen however (and happened at least once).
	...
	public static Exprent propagateBoolNot(Exprent exprent) {
		...
		if (exprent.type == Exprent.EXPRENT_FUNCTION) {
			...
			if (fexpr.getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT) {
				...
				Exprent param = fexpr.getLstOperands()
				                     .get(0);
				...
				if (param.type == Exprent.EXPRENT_FUNCTION) {
					...
					switch (ftype) {
						case FunctionExprent.FUNCTION_BOOL_NOT:
							Exprent newexpr = fparam.getLstOperands()
							                        .get(0);
						...
						case FunctionExprent.FUNCTION_IIF:
							// Wrap branches
							FunctionExprent fex1 = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
							                                           fparam.getLstOperands()
							                                                 .get(1),
							                                           null);
							FunctionExprent fex2 = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
							                                           fparam.getLstOperands()
							                                                 .get(2),
							                                           null);
						...
						case FunctionExprent.FUNCTION_LE:
							if (!canSimplify) {
								...
								VarType left = operands.get(0)
								                       .getExprType();
								VarType right = operands.get(1)
								                        .getExprType();
								VarType commonSupertype = VarType.getCommonSupertype(left,
								                                                     right);
								if (commonSupertype != null) {
									canSimplify = commonSupertype.type != CodeConstants.TYPE_FLOAT
									              && commonSupertype.type != CodeConstants.TYPE_DOUBLE;
								}
							}
							if (canSimplify) {
	...
	public static boolean updateAssignments(Statement stat) {
		...
		List<Object> objects = new ArrayList<>(stat.getExprents() == null
		                                       ? stat.getSequentialObjects()
		                                       : stat.getExprents());
		...
		for (Object obj : objects) {
			if (obj instanceof Statement) {
				...
				if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					// We only want expressions that are standard assignments where the left hand side is a variable and the
					// right hand side is a function.
					if (assignment.getCondType() == -1
					    && lhs.type == Exprent.EXPRENT_VAR
					    && rhs.type == Exprent.EXPRENT_FUNCTION) {
						...
						if (rhsFunc.getFuncType() <= FunctionExprent.FUNCTION_USHR
						    && funcParams.get(0).type == Exprent.EXPRENT_VAR) {
							// Get the left hand side of the function
							VarExprent lhsVarFunc = (VarExprent) funcParams.get(0);

							// Check if the left hand side of the assignment and the left hand side of the function are the same
							// variable
							// TODO: maybe we should be checking for var version equality too?
							if (lhsVar.getIndex() == lhsVarFunc.getIndex()) {
								// If all the checks succeed, set the assignment to be a compound assignment and set the right hand
								// side to be the 2nd part of the function
