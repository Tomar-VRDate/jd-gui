...
public final class TernaryProcessor {
	public static boolean processTernary(RootStatement root) {
		...
		for (StatEdge edge : root.getDummyExit()
		...
		if (res) {
			...
			LabelHelper.lowContinueLabels(root,
			                              new LinkedHashSet<>());
		}
	...
	private static boolean processIf(IfStatement statement) {
		...
		if (ifStatement.type == Statement.TYPE_IF
		    && elseStatement.type == Statement.TYPE_IF
		    && ifStatement.getExprents() == null
		    ...
		    && ifStatement.getAllSuccessorEdges()
		                  ...
		                  .getDestination() == elseStatement.getAllSuccessorEdges()
		                                                    .get(0)
		                                                    .getDestination()
		    && ifStatement.getFirst()
		                  .getExprents() != null
		    && ifStatement.getFirst()
		                  .getExprents()
		                  .isEmpty()
		    && elseStatement.getFirst()
		                    .getExprents() != null
		    && elseStatement.getFirst()
		                    .getExprents()
		                    .isEmpty()
		    && ((IfStatement) ifStatement).getIfstat() == null
		    && ((IfStatement) elseStatement).getIfstat() == null) {

			Statement destination = ifStatement.getAllSuccessorEdges()
			                                   .get(0)
			                                   .getDestination();
			...
			if (destination.getPredecessorEdges(StatEdge.TYPE_BREAK)
			               .size() > 2) {
				return false;
			}
			...
			if (closure != null && !closure.getAllSuccessorEdges()
			                               .isEmpty()) {
				for (StatEdge edge : closure.getAllSuccessorEdges()
				                            .get(0)
				                            .getDestination()
			...
			StatEdge destEdge = destination.getAllSuccessorEdges()
			                               .get(0);
			List<Statement> labelsNeedRemoving = new ArrayList<>();
			if (destination.getSuccessorEdges(StatEdge.TYPE_REGULAR)
			               .size() == 1) {

				int idx = parent.getStats()
				                .getIndexByKey(destination.id);

				int size = parent.getStats()
				                 .size();
				...
				for (int i = idx + 1;
				     i < size;
				     i++) {
					Statement st = parent.getStats()
					                     .get(i);

					stats.add(st);
				}

				List<StatEdge> succs = parent.getStats()
				                             .getLast()
				                             .getAllSuccessorEdges();
				destEdge = succs.isEmpty()
				           ? null
				           : succs.get(0);

				// fun hack! if we know that we're enclosed in a loop, we can construct a dummy continue edge that flows back
				// into the parent.
				...
				if (destEdge == null && closure != null && closure.type == Statement.TYPE_DO) {
					destEdge = new StatEdge(StatEdge.TYPE_CONTINUE,
					                        statement,
					                        closure,
					                        closure.getParent());
				}
			...
			Exprent condition = statement.getHeadexprent()
			                             .getCondition();
			Exprent ifCond = ((IfStatement) ifStatement).getHeadexprent()
			                                            .getCondition();
			Exprent elseCond = ((IfStatement) elseStatement).getHeadexprent()
			                                                .getCondition();

			// Need to negate!
			Exprent condA = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
			                                    ifCond,
			                                    null);
			Exprent condB = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
			                                    elseCond,
			                                    null);

			// Construct a ternary with the new conditions and set it
			List<Exprent> operands = Arrays.asList(condition,
			                                       condA,
			                                       condB);
			statement.getHeadexprent()
			         .setCondition(new FunctionExprent(FunctionExprent.FUNCTION_IIF,
			                                           operands,
			                                           null));
			...
			Statement blockBefore = bstat1.getAllPredecessorEdges()
			                              .get(0)
			                              .getSource();

			bstat1.getParent()
			      .getStats()
			      .removeWithKey(bstat1.id);
			bstat2.getParent()
			      .getStats()
			      .removeWithKey(bstat2.id);
			...
			for (StatEdge pred : destination.getAllPredecessorEdges()) {
				pred.getSource()
				    .removeSuccessor(pred);
			}

			// Add destination to statement and remove from old parent
			parent.getStats()
			      .removeWithKey(destination.id);
			statement.getStats()
			         .addWithKey(destination,
			                     destination.id);
			...
			StatEdge edgetoDest = new StatEdge(StatEdge.TYPE_REGULAR,
			                                   statement.getFirst(),
			                                   destination);
			// No successors at this point
			statement.getFirst()
			         .addSuccessor(edgetoDest);
			...
			if (destEdge != null) {
				...
				if (isReturnEdge) {
					statement.addSuccessor(new StatEdge(StatEdge.TYPE_BREAK,
					                                    statement,
					                                    destEdge.getDestination()));
				} else {
					StatEdge edge = new StatEdge(destEdge.getType(),
					                             statement,
					                             destEdge.getDestination(),
					                             destEdge.getDestination()
					                                     .getParent());

					// If the edge from the destination is a continue, modify to break- if keep it as continue then it'll create
					// a double continue and cause problems
