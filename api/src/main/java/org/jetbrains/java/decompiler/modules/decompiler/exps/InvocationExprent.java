...
public class InvocationExprent
	...
	private static final VarType             JAVA_NIO_BUFFER = new VarType(CodeConstants.TYPE_OBJECT,
	                                                                       0,
	                                                                       "java/nio/Buffer");
	...
	static {
		...
		UNBOXING_METHODS.put("booleanValue",
		                     "java/lang/Boolean");
		UNBOXING_METHODS.put("byteValue",
		                     "java/lang/Byte");
		UNBOXING_METHODS.put("shortValue",
		                     "java/lang/Short");
		UNBOXING_METHODS.put("intValue",
		                     "java/lang/Integer");
		UNBOXING_METHODS.put("longValue",
		                     "java/lang/Long");
		UNBOXING_METHODS.put("floatValue",
		                     "java/lang/Float");
		UNBOXING_METHODS.put("doubleValue",
		                     "java/lang/Double");
		UNBOXING_METHODS.put("charValue",
		                     "java/lang/Character");
	}
	...
	public InvocationExprent(int opcode,
		...
		if (opcode == CodeConstants.opc_invokedynamic || invocationTyp == CONSTANT_DYNAMIC) {
			...
			if (dynamicInvocationType == CodeConstants.CONSTANT_MethodHandle_REF_invokeStatic) {
				...
				// FIXME: remove the first parameter completely from the list. It's the object type for a virtual lambda
				//  method.
	...
	private static void appendBootstrapArgument(TextBuffer buf,
	                                            PooledConstant arg) {
		if (arg instanceof PrimitiveConstant) {
			...
			if (prim.type == CodeConstants.CONSTANT_Class) {
				...
				buf.append('"')
				   .append(ConstExprent.convertStringToJava(stringValue,
				                                            false))
				   .append('"');
			} else {
	...
	public VarType getInferredExprType(VarType upperBound) {
		...
		StructClass mthCls = DecompilerContext.getStructContext()
		                                      .getClass(classname);
		...
		if (desc != null && mthCls != null) {
			...
			if (desc.getSignature() != null || isGenNew) {
				...
				List<String> fparams = isGenNew
				                       ? mthCls.getSignature().fparameters
				                       : desc.getSignature().typeParameters;
				VarType ret = isGenNew
				              ? mthCls.getSignature().genericType
				              : desc.getSignature().returnType;
				...
				if (upperBound != null && !upperBound.equals(VarType.VARTYPE_OBJECT) && (upperBound.type
				                                                                         != CodeConstants.TYPE_GENVAR
				                                                                         || named.containsKey(upperBound))) {
					VarType ub = upperBound; // keep original
					VarType r  = ret;
					if (ub.type != CodeConstants.TYPE_GENVAR
					    && r.type != CodeConstants.TYPE_GENVAR
					    && !ub.value.equals(r.value)) {
						if (DecompilerContext.getStructContext()
						                     .instanceOf(ub.value,
						                                 r.value)) {
							ub = GenericType.getGenericSuperType(ub,
							                                     r);
						} else {
							r = GenericType.getGenericSuperType(r,
							                                    ub);
						}
					}

					if (r.type == CodeConstants.TYPE_GENVAR) {
						upperBoundsMap.put(r.resizeArrayDim(0),
						                   upperBound.resizeArrayDim(upperBound.arrayDim - r.arrayDim));
					} else {
						gatherGenerics(ub,
						               r,
						               tempMap);
						tempMap.forEach((from, to) -> {
							if (!genericsMap.containsKey(from)) {
								if (to != null && (to.type != CodeConstants.TYPE_GENVAR || named.containsKey(to))) {
									if (isMappingInBounds(from,
									                      to,
									                      named,
									                      bounds)) {
										upperBoundsMap.put(from,
										                   to);
									}
								}
							}
				...
				fparams.stream()
				       ...
				       .forEach(t -> upperBoundsMap.put(t,
				                                        GenericType.DUMMY_VAR));
				if (mthCls.getSignature() != null) {
					mthCls.getSignature().fparameters.stream()
					                                 .map(p -> "T" + p + ";")
					                                 .map(GenericType::parse)
					                                 .filter(t -> !upperBoundsMap.containsKey(t))
					                                 .forEach(t -> upperBoundsMap.put(t,
				...
				if (instance != null && !isNew) {
					...
					VarType instUB = mthCls.getSignature() != null
					                 ? mthCls.getSignature().genericType.remap(upperBoundsMap)
					                 : upperBound;
					VarType instType;
					...
					if (instType.isGeneric() && instType.type != CodeConstants.TYPE_GENVAR) {
						...
						cls = DecompilerContext.getStructContext()
						                       .getClass(instType.value);
						if (cls != null && cls.getSignature() != null) {
							cls.getSignature().genericType.mapGenVarsTo(ginstance,
							                                            tempMap);
							tempMap.forEach((from, to) -> {
								if (!fparams.contains(from.value)) {
									processGenericMapping(from,
									                      to,
									                      named,
									                      bounds);
								}
				...
				if (upperBound == null && isGenNew) {
					...
					if (currentCls != null) {
						if (mthCls.equals(currentCls.classStruct)) {
							mthCls.getSignature().genericType.getAllGenericVars()
							                                 .forEach(var -> genericsMap.put(var,
							                                                                 var));
						} else {
							...
							if (hierarchy.containsKey(mthCls.qualifiedName)) {
								hierarchy.get(mthCls.qualifiedName)
								         .forEach(genericsMap::put);
							}
						}
					}
				}
				...
				if (!lstParameters.isEmpty() && desc.getSignature() != null) {
					...
					if (newNode != null) {
						if (isNew) {
							mask = ExprUtil.getSyntheticParametersMask(newNode,
							                                           stringDescriptor,
							                                           lstParameters.size());
							start = newNode.classStruct.hasModifier(CodeConstants.ACC_ENUM)
							        ? 2
							        : 0;
						} else if (!newNode.enclosingClasses.isEmpty()) {
							start = !newNode.classStruct.hasModifier(CodeConstants.ACC_STATIC)
							        ? 1
							        : 0;
						}
					}
					...
					for (int i = start;
						...
						if ((mask == null || mask.get(i) == null)) {
							...
							if (paramType.isGeneric()) {
								...
								VarType paramUB = paramType.remap(hierarchyMap)
								                           .remap(combined);
								...
								if (lstParameters.get(i).type == EXPRENT_FUNCTION
									...
									argtype = ((FunctionExprent) lstParameters.get(i)).getLstOperands()
									                                                  .get(0)
									                                                  .getInferredExprType(paramUB);
								} else {
									argtype = lstParameters.get(i)
									                       .getInferredExprType(paramUB);
								}

								StructClass paramCls = DecompilerContext.getStructContext()
								                                        .getClass(paramType.value);
								cls = argtype.type != CodeConstants.TYPE_GENVAR
								      ? DecompilerContext.getStructContext()
								                         .getClass(argtype.value)
								      : null;

								if (cls != null && paramCls != null) {
									if (paramType.isGeneric() && !paramType.value.equals(argtype.value)) {
										argtype = GenericType.getGenericSuperType(argtype,
										                                          paramType);
									}
									...
									if (paramType.isGeneric() && argtype.isGeneric()) {
										...
										genParamType.mapGenVarsTo(genArgType,
										                          tempMap);
										tempMap.forEach((from, to) -> {
									...
									processGenericMapping(paramType,
									                      argtype,
									                      named,
									                      bounds);
								}
				...
				if (!genericsMap.isEmpty()) {
					...
					if (!skipArgs && (!isNew || isGenNew)) {
						...
						boolean suppress = (!missing || !isInvocationInstance) && (upperBound == null
						                                                           || !newRet.isGeneric()
						                                                           || DecompilerContext.getStructContext()
						                                                                               .instanceOf(newRet.value,
						                                                                                           upperBound.value));

						if (!suppress || DecompilerContext.getOption(IFernflowerPreferences.EXPLICIT_GENERIC_ARGUMENTS)) {
							getGenericArgs(fparams,
							               genericsMap,
							               genericArgs);
						} else if (isGenNew) {
				...
				if (ret.isGeneric() && ((GenericType) ret).getAllGenericVars()
				                                          .isEmpty()) {
					return ret;
				}
			}
	...
	public CheckTypesResult checkExprTypeBounds() {
		...
		if (instance != null) {
			result.addMinTypeExprent(instance,
			                         VarType.getMinTypeInFamily(instance.getExprType().typeFamily));
			result.addMaxTypeExprent(instance,
			                         instance.getExprType());
		}
		...
		for (int i = 0;
			...
			result.addMinTypeExprent(parameter,
			                         VarType.getMinTypeInFamily(leftType.typeFamily));
			result.addMaxTypeExprent(parameter,
			                         leftType);
		}
	...
	public TextBuffer toJava(int indent) {
		...
		if (isStatic || invocationTyp == INVOKE_DYNAMIC || invocationTyp == CONSTANT_DYNAMIC) {
			if (isBoxingCall() && canIgnoreBoxing && !forceBoxing) {
				...
				ExprProcessor.getCastedExprent(lstParameters.get(0),
				                               descriptor.params[0],
				                               buf,
				                               indent,
				                               false,
			...
			if (invocationTyp == CONSTANT_DYNAMIC) {
				buf.append('(')
				   .append(ExprProcessor.getCastTypeName(descriptor.ret))
				   .append(')');
			}
			...
			if (instance != null && instance.type == Exprent.EXPRENT_VAR) {
				...
				if (varProc != null) {
					this_classname = varProc.getThisVars()
					                        .get(varPair);
				}
			...
			if (CodeConstants.isReturnPolymorphic(classname,
			                                      name) && !descriptor.ret.equals(VarType.VARTYPE_VOID)) {
				buf.append('(')
				   .append(ExprProcessor.getCastTypeName(descriptor.ret))
				   .append(')');
			}
			...
			if (functype == TYP_GENERAL) {
				if (super_qualifier != null) {
					TextUtil.writeQualifiedSuper(buf,
					                             super_qualifier);
				} else if (instance != null) {
					StructClass cl = DecompilerContext.getStructContext()
					                                  .getClass(classname);

					VarType leftType = new VarType(CodeConstants.TYPE_OBJECT,
					                               0,
					                               classname);
					...
					if (isUnboxingCall() && !forceUnboxing) {
						...
						if (instance.type == Exprent.EXPRENT_FUNCTION) {
							...
							if (func.getFuncType() == FunctionExprent.FUNCTION_CAST
							    && func.getLstOperands()
							           .get(1).type == Exprent.EXPRENT_CONST) {
								ConstExprent _const = (ConstExprent) func.getLstOperands()
								                                         .get(1);
								boolean skipCast = false;

								if (func.getLstOperands()
								        .get(0).type == Exprent.EXPRENT_VAR) {
									VarType inferred = func.getLstOperands()
									                       .get(0)
									                       .getInferredExprType(leftType);
									skipCast = (inferred.type != CodeConstants.TYPE_OBJECT && inferred.type != CodeConstants.TYPE_GENVAR)
									           || DecompilerContext.getStructContext()
									                               .instanceOf(inferred.value,
									                                           this.classname);
								...
								if (skipCast) {
									buf.append(func.getLstOperands()
									               .get(0)
									               .toJava(indent));
									return buf;
								}
					...
					if (instance.type == EXPRENT_FUNCTION
						...
						// This happens when there are two checkcast opcodes in succession [TestDoubleCast], so this unwraps casts
						// of the same var type
						...
						if (skippedCast) {
							...
							Exprent exp = ((FunctionExprent) instance).getLstOperands()
							                                          .get(0);
							while (exp.type == EXPRENT_FUNCTION
					...
					else if (JAVA_NIO_BUFFER.equals(descriptor.ret)
					         && !JAVA_NIO_BUFFER.equals(rightType)
					         && DecompilerContext.getStructContext()
					                             .instanceOf(rightType.value,
					                                         JAVA_NIO_BUFFER.value)) {
						buf.append("((")
	...
	public TextBuffer appendParamList(int indent) {
		...
		VarType[] types = Arrays.copyOf(descriptor.params,
			...
			if (par.type == Exprent.EXPRENT_INVOCATION) {
				...
				if (inv.isBoxingCall()) {
					...
					if (types[i].typeFamily == CodeConstants.TYPE_FAMILY_INTEGER) {
						types[i] = "java/lang/Short".equals(inv.classname)
						           ? VarType.VARTYPE_SHORT
						           : "java/lang/Byte".equals(inv.classname)
						             ? VarType.VARTYPE_BYTE
						             : "java/lang/Integer".equals(inv.classname)
						               ? VarType.VARTYPE_INT
						               : VarType.VARTYPE_CHAR;
					}

					int count = 0;
					StructClass stClass = DecompilerContext.getStructContext()
					                                       .getClass(classname);
					if (stClass != null) {
						nextMethod:
						for (StructMethod mt : stClass.getMethods()) {
							if (name.equals(mt.getName()) && (currCls == null || canAccess(currCls.classStruct,
							                                                               mt))) {
					...
					if (stClass != null) {
						for (StructMethod mt : stClass.getMethods()) {
							if (name.equals(mt.getName())
							    && (currCls == null || canAccess(currCls.classStruct,
							                                     mt))
		...
		if (desc == null) {
			...
			if (genericsMap.isEmpty() && instance != null && functype != TYP_INIT) {
				...
				if (instType.isGeneric() && instType.type != CodeConstants.TYPE_GENVAR) {
					...
					StructClass cls = DecompilerContext.getStructContext()
					                                   .getClass(instType.value);
					if (cls != null && cls.getSignature() != null) {
						cls.getSignature().genericType.mapGenVarsTo(ginstance,
						                                            genericsMap);
		...
		if (desc != null && desc.getSignature() != null) {
			...
			for (int x = start;
				...
				if (mask == null || mask.get(x) == null) {
					VarType type = desc.getSignature().parameterTypes.get(y++)
					                                                 .remap(genericsMap);
        ...
        else if (desc != null && desc.getSignature() != null && genericArgs.size() != 0) { // and from the inferred
        generic arguments
        ...
        */
				...
				if (i == parameters.size() - 1
				    && lstParameters.get(i)
				                    .getExprType() == VarType.VARTYPE_NULL
	...
	private boolean isVarArgCall() {
		StructClass cl = DecompilerContext.getStructContext()
		                                  .getClass(classname);
		if (cl != null) {
			StructMethod mt = cl.getMethod(InterpreterUtil.makeUniqueKey(name,
			                                                             stringDescriptor));
			if (mt != null) {
			...
			Method mtd = ClasspathHelper.findMethod(classname,
			                                        name,
			                                        descriptor);
	...
	public boolean isBoxingCall() {
		if (isStatic && "valueOf".equals(name) && lstParameters.size() == 1) {
			int paramType = lstParameters.get(0)
			                             .getExprType().type;
			...
			if (lstParameters.get(0).type == Exprent.EXPRENT_CONST) {
				...
				if (lstParameters.get(0)
				                 .getExprType().typeFamily == CodeConstants.TYPE_FAMILY_INTEGER) {
			...
			for (StructMethod mt : cls.getMethods()) {
				if (name.equals(mt.getName())) {
					...
					if (matches(md.params,
					            descriptor.params) && (currCls == null || canAccess(currCls.classStruct,
					                                                                mt))) {
						matches.add(mt);
					}
				}
	...
	private boolean matches(VarType[] left,
	                        VarType[] right) {
	...
	private boolean canAccess(StructClass currCls,
	                          StructMethod mt) {
		if (mt.hasModifier(CodeConstants.ACC_PUBLIC)) {
			return true;
		} else if (mt.hasModifier(CodeConstants.ACC_PRIVATE)) {
			return mt.getClassQualifiedName()
			         .equals(currCls.qualifiedName);
		} else if (mt.hasModifier(CodeConstants.ACC_PROTECTED)) {
			boolean samePackage = isInSamePackage(currCls.qualifiedName,
			                                      mt.getClassQualifiedName());
			return samePackage || DecompilerContext.getStructContext()
			                                       .instanceOf(currCls.qualifiedName,
			                                                   mt.getClassQualifiedName());
		} else {
			return isInSamePackage(currCls.qualifiedName,
			                       mt.getClassQualifiedName());
		}
	}

	private boolean isInSamePackage(String class1,
	                                String class2) {
		...
		String pkg1 = class1.substring(0,
		                               pos1);
		String pkg2 = class2.substring(0,
		                               pos2);
	...
	private BitSet getAmbiguousParameters(List<StructMethod> matches) {
		...
		if (CodeConstants.areParametersPolymorphic(classname,
		                                           name)) {
			missed.set(0,
			           lstParameters.size());
			return missed;
		...
		if (mt != null) {
			...
			if (md.params.length == lstParameters.size()) {
				...
				for (int i = 0;
					...
					// Check if the current parameters and method descriptor are of the same type, or if the descriptor's type
					// is a superset of the parameter's type.
					// This check ensures that parameters that can be safely passed don't have an unneeded cast on them, such as
					// System.out.println((int)5);.
					// TODO: The root cause of the above issue seems to be threading related- When debugging line by line it
					//  doesn't cast, but when running normally it does. More digging needs to be done to figure out why this
					//  happens.
					if ((!(md.params[i].equals(exp.getExprType()) || md.params[i].isSuperset(exp.getExprType()))) || (exp.type
					                                                                                                  == EXPRENT_NEW
			...
			for (int i = 0;
				...
				if (!missed.get(i)) {
					...
					if (exp.type == EXPRENT_NEW) {
						...
						if (newExp.isLambda() && !newExp.isMethodReference() && !DecompilerContext.getStructContext()
						                                                                          .instanceOf(md.params[i].value,
						                                                                                      exp.getExprType().value)) {
		...
		for (int i = 0;
		     i < descriptor.params.length;
		     i++) {
			VarType paramType = descriptor.params[i];
			for (StructMethod mtt : mtds) {

				GenericMethodDescriptor gen
								= mtt.getSignature(); //TODO: Find synthetic flags for params, as Enum generic signatures do no
				// contain the String,int params
				if (gen != null && gen.parameterTypes.size() > i && gen.parameterTypes.get(i)
				                                                                      .isGeneric()) {
					Exprent exp = lstParameters.get(i);
	...
	private void processGenericMapping(VarType from,
	                                   VarType to,
	                                   Map<VarType, List<VarType>> named,
	                                   Map<VarType, List<VarType>> bounds) {
		if (VarType.VARTYPE_NULL.equals(to) || (to != null
		                                        && to.type == CodeConstants.TYPE_GENVAR
		                                        && !named.containsKey(to))) {
			return;
		}

		VarType current = genericsMap.get(from);
		if (!genericsMap.containsKey(from)) {
			putGenericMapping(from,
			                  to,
			                  named,
			                  bounds);
			...
			if (current.type != CodeConstants.TYPE_GENVAR && to.type == CodeConstants.TYPE_GENVAR) {
				if (named.containsKey(to)) {
					VarType bound = named.get(to)
					                     .get(0);
					if (!bound.equals(VarType.VARTYPE_OBJECT) && DecompilerContext.getStructContext()
					                                                              .instanceOf(bound.value,
					                                                                          current.value)) {
						return;
					}
				}
			...
			if (to.isGeneric() && current.isGeneric() && GenericType.isAssignable(to,
			                                                                      current,
			                                                                      named)) {
				putGenericMapping(from,
				                  to,
	...
	private void putGenericMapping(VarType from,
	                               VarType to,
	                               Map<VarType, List<VarType>> named,
	                               Map<VarType, List<VarType>> bounds) {
		if (isMappingInBounds(from,
	...
	private boolean isMappingInBounds(VarType from,
	                                  VarType to,
	                                  Map<VarType, List<VarType>> named,
	                                  Map<VarType, List<VarType>> bounds) {
		...
		java.util.function.BiFunction<VarType, VarType, Boolean> verifier = (newTo, bound) -> {
				...
				if (mapped != null && !mapped.equals(bound)) {
					...
					if (bound.type != CodeConstants.TYPE_GENVAR) {
						return DecompilerContext.getStructContext()
						                        .instanceOf(newTo.value,
						                                    bound.value);
					}
				}

				return isMappingInBounds(bound,
				                         newTo,
				                         named,
				                         bounds);
			}
			...
			if (bound.isGeneric() && !((GenericType) bound).getArguments()
			                                               .isEmpty()) {
				GenericType genbound = (GenericType) bound;
				VarType     _new     = newTo;

				if (!newTo.value.equals(bound.value)) {
					_new = GenericType.getGenericSuperType(newTo,
					                                       bound);
				}

				if (!_new.isGeneric()
				    || ((GenericType) _new).getArguments()
				                           .size() != genbound.getArguments()
				                                              .size()) {
					return false;
				}
				...
				for (int i = 0;
				     i < genbound.getArguments()
				                 .size();
				     ++i) {
					VarType boundArg = genbound.getArguments()
					                           .get(i);
					VarType newArg = genNew.getArguments()
					                       .get(i);
					...
					if (!boundArg.equals(newArg)) {
						...
						if (bounds.containsKey(boundArg) && isMappingInBounds(boundArg,
						                                                      newArg,
						                                                      named,
						                                                      bounds)) {
							toAdd.put(boundArg,
				...
				toAdd.forEach((k, v) -> processGenericMapping(k,
				                                              v,
				                                              named,
				                                              bounds));
			}
	...
	private Map<VarType, List<VarType>> getGenericBounds(StructClass mthCls) {
		...
		if (desc.getSignature() != null) {
			for (int x = 0;
			     x < desc.getSignature().typeParameters.size();
			     x++) {
				bounds.putIfAbsent(GenericType.parse("T" + desc.getSignature().typeParameters.get(x) + ";"),
		...
		if (mthCls.getSignature() != null) {
			for (int x = 0;
			     x < mthCls.getSignature().fparameters.size();
			     x++) {
				bounds.putIfAbsent(GenericType.parse("T" + mthCls.getSignature().fparameters.get(x) + ";"),
		...
		while (cn != null) {
			if (cn.classStruct.getSignature() != null) {
				for (int x = 0;
				     x < cn.classStruct.getSignature().fparameters.size();
				     x++) {
					bounds.putIfAbsent(GenericType.parse("T" + cn.classStruct.getSignature().fparameters.get(x) + ";"),
	...
	public boolean equals(Object o) {
		...
		return InterpreterUtil.equalObjects(name,
		                                    it.getName())
		       && InterpreterUtil.equalObjects(classname,
		                                       it.getClassname())
		       && isStatic == it.isStatic()
		       && InterpreterUtil.equalObjects(instance,
		                                       it.getInstance())
		       && InterpreterUtil.equalObjects(descriptor,
		                                       it.getDescriptor())
		       && functype == it.getFunctype()
		       && InterpreterUtil.equalLists(lstParameters,
		                                     it.getLstParameters());
	}
	...
	@Override
	public boolean match(MatchNode matchNode,
	                     MatchEngine engine) {
		if (!super.match(matchNode,
		                 engine)) {
			return false;
		}

		for (Entry<MatchProperties, RuleValue> rule : matchNode.getRules()
		                                                       .entrySet()) {
