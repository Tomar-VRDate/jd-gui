...
public class FinallyProcessor {
	...
	public FinallyProcessor(MethodDescriptor md,
	...
	private static void insertSemaphore(ControlFlowGraph graph,
		...
		removeExceptionInstructionsEx(handler,
		                              1,
		                              finallytype);
		for (Entry<BasicBlock, Boolean> entry : mapLast.entrySet()) {
			...
			if (entry.getValue()) {
				removeExceptionInstructionsEx(last,
				                              2,
				                              finallytype);
				graph.getFinallyExits()
				     .add(last);
			}
		}

		final int store_length = var <= 3
		                         ? 1
		                         : var <= 128
		                           ? 2
		                           : 4;
		...
		for (BasicBlock block : setTry) {
			...
			for (BasicBlock dest : lstSucc) {
				...
				if (dest != graph.getLast() && !setCopy.contains(dest)) {
					...
					seq.addInstruction(Instruction.create(CodeConstants.opc_bipush,
					                                      false,
					                                      CodeConstants.GROUP_GENERAL,
					                                      bytecode_version,
					                                      new int[]{0},
					                                      1),
					                   -1);
					seq.addInstruction(Instruction.create(CodeConstants.opc_istore,
					                                      false,
					                                      CodeConstants.GROUP_GENERAL,
					                                      bytecode_version,
					                                      new int[]{var},
					...
					for (int j = 0;
					     j < block.getSuccExceptions()
					              .size();
					     j++) {
						BasicBlock hd = block.getSuccExceptions()
						                     .get(j);
						newblock.addSuccessorException(hd);

						ExceptionRangeCFG range = graph.getExceptionRange(hd,
						                                                  block);
	...
	private static void insertBlockBefore(ControlFlowGraph graph,
	                                      BasicBlock oldblock,
	                                      BasicBlock newblock) {
		...
		for (BasicBlock pred : lstTemp) {
			pred.replaceSuccessor(oldblock,
			                      newblock);
		}
	...
	private static BasicBlock getUniqueNext(ControlFlowGraph graph,
	                                        Set<BasicBlock[]> setNext) {
		...
		if (multiple) { // TODO: generic solution
			for (BasicBlock[] arr : setNext) {
				...
				if (block != next) {
					if (InterpreterUtil.equalSets(next.getSuccs(),
					                              block.getSuccs())) {
						...
						if (seqNext.length() == seqBlock.length()) {
							for (int i = 0;
								...
								if (!Instruction.equals(instrNext,
								                        instrBlock)) {
									return null;
								}
								for (int j = 0;
								     j < instrNext.operandsCount();
								     j++) {
	...
	private static void deleteArea(ControlFlowGraph graph,
	                               Area area) {
		...
		for (BasicBlock block : setBlocks) {
			...
			if (graph.getBlocks()
			         .containsKey(block.id)) {

				if (!block.getSuccExceptions()
				          .containsAll(setCommonExceptionHandlers)) {
				...
				for (BasicBlock handler : block.getSuccExceptions()) {
					setRemovedExceptionRanges.add(graph.getExceptionRange(handler,
					                                                      block));
				}
		...
		if (is_outside_range) {
			...
			for (BasicBlock pred : setPredecessors) {
				pred.replaceSuccessor(next,
				                      emptyblock);
			}
		}
	}

	private static void removeExceptionInstructionsEx(BasicBlock block,
	                                                  int blocktype,
	                                                  int finallytype) {
	...
	public boolean iterateGraph(StructClass cl,
		...
		while (!stack.isEmpty()) {
			...
			if (parent != null && parent.type == Statement.TYPE_CATCHALL && stat == parent.getFirst() && !parent.isCopied()) {
				...
				BasicBlock head = fin.getBasichead()
				                     .getBlock();
				BasicBlock handler = fin.getHandler()
				                        .getBasichead()
				                        .getBlock();
				...
				if (catchallBlockIDs.containsKey(handler.id)) {
					...
					Record inf = getFinallyInformation(cl,
					                                   mt,
					                                   root,
					                                   fin);

					if (inf == null) { // inconsistent finally
						catchallBlockIDs.put(handler.id,
						                     null);
						root.addComment("$FF: Could not inline inconsistent finally blocks");
						root.addErrorComment = true;
					} else {
						if (DecompilerContext.getOption(IFernflowerPreferences.FINALLY_DEINLINE) && verifyFinallyEx(graph,
						                                                                                            fin,
						                                                                                            inf)) {
							finallyBlockIDs.put(handler.id,
							                    null);
						} else {
							int varIndex = DecompilerContext.getCounterContainer()
							                                .getCounterAndIncrement(CounterContainer.VAR_COUNTER);
							insertSemaphore(graph,
							                getAllBasicBlocks(fin.getFirst()),
							                head,
							                handler,
							                varIndex,
							                inf,
							...
							root.addComment("$FF: Could not verify finally blocks. A semaphore variable has been added to preserve "
							                + "control flow.");
							root.addErrorComment = true;
						}
	...
	private Record getFinallyInformation(StructClass cl,
	                                     StructMethod mt,
	                                     RootStatement root,
	                                     CatchAllStatement fstat) {
		Map<BasicBlock, Boolean> mapLast = new HashMap<>();

		BasicBlockStatement firstBlockStatement = fstat.getHandler()
		                                               .getBasichead();
		...
		ExprProcessor proc = new ExprProcessor(methodDescriptor,
		                                       varProcessor);
		proc.processStatement(root,
		                      cl);

		SSAConstructorSparseEx ssa = new SSAConstructorSparseEx();
		ssa.splitVariables(root,
		                   mt);

		List<Exprent> lstExprents = firstBlockStatement.getExprents();

		VarVersionPair varpaar = new VarVersionPair((VarExprent) ((AssignmentExprent) lstExprents.get(firstcode == 2
		                                                                                              ? 1
		                                                                                              : 0)).getLeft());
				...
				for (StatEdge edge : blockStatement.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)) {
					if (edge.getType() != StatEdge.TYPE_REGULAR
						...
						if (existingFlag == null || !existingFlag) {
							mapLast.put(blockStatement.getBlock(),
							            isTrueExit);
							break;
						}
					}
		...
		return new Record(firstcode,
		                  mapLast);
	}

	private boolean verifyFinallyEx(ControlFlowGraph graph,
	                                CatchAllStatement fstat,
	                                Record information) {
		...
		BasicBlock first = fstat.getHandler()
		                        .getBasichead()
		                        .getBlock();
		...
		if (finallytype == 3) {
			// empty finally
			removeExceptionInstructionsEx(first,
			                              3,
			                              finallytype);
			...
			if (first.getSeq()
			         .length() == 1 && finallytype > 0) {
				BasicBlock firstsuc = first.getSuccs()
				                           .get(0);
		...
		Collections.sort(starts,
		                 new Comparator<BasicBlock>() {
			                 @Override
			                 public int compare(BasicBlock o1,
			                                    BasicBlock o2) {
				                 return o2.id - o1.id;
			                 }
		                 });
		...
		for (BasicBlock start : starts) {
			...
			Area arr = compareSubgraphsEx(graph,
			                              start,
			                              catchBlocks,
			                              first,
			                              finallytype,
		...
		Collections.sort(lasts,
		                 new Comparator<Entry<BasicBlock, Boolean>>() {
			                 @Override
			                 public int compare(Entry<BasicBlock, Boolean> o1,
			                                    Entry<BasicBlock, Boolean> o2) {
		...
		removeExceptionInstructionsEx(fstat.getHandler()
		                                   .getBasichead()
		                                   .getBlock(),
		                              1,
		                              finallytype);
	...
	private Area compareSubgraphsEx(ControlFlowGraph graph,
		...
		class BlockStackEntry {
			...
			BlockStackEntry(BasicBlock blockCatch,
		...
		stack.add(new BlockStackEntry(startCatch,
		                              startSample,
		                              new ArrayList<>()));
		...
		while (!stack.isEmpty()) {
			...
			if (blockSample.getSuccs()
			               .size() != blockCatch.getSuccs()
			                                    .size()) {
				return null;
			}
			...
			// direct successors
			for (int i = 0;
			     i < blockCatch.getSuccs()
			                   .size();
			     i++) {
				BasicBlock sucCatch = blockCatch.getSuccs()
				                                .get(i);
				BasicBlock sucSample = blockSample.getSuccs()
				                                  .get(i);

				if (catchBlocks.contains(sucCatch) && !setSample.contains(sucSample)) {
					stack.add(new BlockStackEntry(sucCatch,
					                              sucSample,
					                              entry.lstStoreVars));
				}
			}
			...
			if (isLastBlock && blockSample.getSeq()
			                              .isEmpty()) {
				// do nothing, blockSample will be removed anyway
			} else {
				if (blockCatch.getSuccExceptions()
				              .size() == blockSample.getSuccExceptions()
				                                    .size()) {
					for (int i = 0;
					     i < blockCatch.getSuccExceptions()
					                   .size();
					     i++) {
						BasicBlock sucCatch = blockCatch.getSuccExceptions()
						                                .get(i);
						BasicBlock sucSample = blockSample.getSuccExceptions()
						                                  .get(i);

						String excCatch = graph.getExceptionRange(sucCatch,
						                                          blockCatch)
						                       .getUniqueExceptionsString();
						String excSample = graph.getExceptionRange(sucSample,
						                                           blockSample)
						                        .getUniqueExceptionsString();

						// FIXME: compare handlers if possible
						boolean equalexc = excCatch == null
						                   ? excSample == null
						                   : excCatch.equals(excSample);
						...
						if (equalexc) {
							if (catchBlocks.contains(sucCatch) && !setSample.contains(sucSample)) {
								...
								if (sucCatch.getSeq()
								    ...
								    && sucSample.getSeq()
								                .length() > 0) {
									Instruction instrCatch = sucCatch.getSeq()
									                                 .getInstr(0);
									Instruction instrSample = sucSample.getSeq()
									                                   .getInstr(0);

									if (instrCatch.opcode == CodeConstants.opc_astore && instrSample.opcode == CodeConstants.opc_astore) {
										lst = new ArrayList<>(lst);
										lst.add(new int[]{instrCatch.operand(0),
										                  instrSample.operand(0)});
									}
								}
		...
		return new Area(startSample,
		                setSample,
		                getUniqueNext(graph,
		                              new HashSet<>(mapNext.values())));
	}
	...
	private boolean compareBasicBlocksEx(ControlFlowGraph graph,
		...
		for (int i = 0;
			...
			if (!equalInstructions(instrPattern,
			                       instrSample,
			                       lstStoreVars)) {
				return false;
			}
		...
		if (seqPattern.length() < seqSample.length()) { // split in two blocks
			...
			for (int i = seqSample.length() - 1;
			     i >= seqPattern.length();
			     i--) {
				seq.addInstruction(0,
				                   seqSample.getInstr(i),
				                   -1);
			...
			// copy exception edges and extend protected ranges
			for (int j = 0;
			     j < sample.getSuccExceptions()
			               .size();
			     j++) {
				BasicBlock hd = sample.getSuccExceptions()
				                      .get(j);
				newblock.addSuccessorException(hd);
	...
	public boolean equalInstructions(Instruction first,
	                                 Instruction second,
	                                 List<int[]> lstStoreVars) {
			...
			for (int i = 0;
				...
				if (firstOp != secondOp) {
					...
					if (first.opcode == CodeConstants.opc_aload) {
						...
						lstStoreVars.add(new int[]{firstOp,
						                           secondOp});
						return true;
					}
	...
	private static final class Area {
		...
		private Area(BasicBlock start,
		             Set<BasicBlock> sample,
		             BasicBlock next) {
			this.start = start;
			this.sample = sample;
			this.next = next;
