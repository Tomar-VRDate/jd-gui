...
public final class CatchStatement
	...
	private CatchStatement(Statement head,
	                       Statement next,
	                       Set<Statement> setHandlers) {
		this();

		first = head;
		stats.addWithKey(first,
		                 first.id);
			...
			if (setHandlers.contains(stat)) {
				...
				vars.add(new VarExprent(DecompilerContext.getCounterContainer()
				                        ...
				                        new VarType(CodeConstants.TYPE_OBJECT,
				                                    ...
				                                    edge.getExceptions()
				                                        .get(0)),
				                        // FIXME: for now simply the first type. Should get the first common superclass when
				                        //  possible.
	...
	public static Statement isHead(Statement head) {
		...
		if (!setHandlers.isEmpty()) {
			...
			if (!lstHeadSuccs.isEmpty()
			    && lstHeadSuccs.get(0)
			                   .getType() == StatEdge.TYPE_REGULAR) {
				next = lstHeadSuccs.get(0)
				                   .getDestination();
				hnextcount = 2;
			}
			...
			for (StatEdge edge : head.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)) {
				...
				if (edge.getExceptions() != null && setHandlers.contains(stat)) {
					if (stat.getLastBasicType() != LASTBASICTYPE_GENERAL) {
						...
						if (!lstStatSuccs.isEmpty()
						    && lstStatSuccs.get(0)
						                   .getType() == StatEdge.TYPE_REGULAR) {

							Statement statn = lstStatSuccs.get(0)
							                              .getDestination();
			...
			if (hnextcount != 1 && !setHandlers.isEmpty()) {
				...
				if (DecHelper.checkStatementExceptions(lst)) {
					return new CatchStatement(head,
					                          next,
					                          setHandlers);
				}
			}
	...
	public TextBuffer toJava(int indent) {
		...
		if (tryType == NORMAL) {
			...
			if (resources.size() > 1) {
				...
				buf.append(ExprProcessor.listToJava(resources,
				                                    indent + 1));
				buf.appendIndent(indent);
			} else {
				buf.append(resources.get(0)
				                    .toJava(indent + 1));
			}
			buf.append(") {")
			   .appendLineSeparator();
		...
		buf.append(ExprProcessor.jmpWrapper(first,
		                                    indent + 1,
		                                    true));
		buf.appendIndent(indent)
		   .append("}");
		...
		for (int i = 1;
			...
			BasicBlock block = stat.getBasichead()
			                       .getBlock();
			if (!block.getSeq()
			          .isEmpty() && block.getInstruction(0).opcode == CodeConstants.opc_astore) {
			...
			if (exception_types.size() > 1) { // multi-catch, Java 7 style
				for (int exc_index = 1;
				     exc_index < exception_types.size();
				     ++exc_index) {
					VarType exc_type = new VarType(CodeConstants.TYPE_OBJECT,
					                               0,
