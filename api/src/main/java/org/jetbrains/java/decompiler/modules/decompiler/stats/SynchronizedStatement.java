...
public class SynchronizedStatement
	...
	public SynchronizedStatement(Statement head,
	                             Statement body,
	                             Statement exc) {
		...
		this.body = body;
		stats.addWithKey(body,
		                 body.id);

		stats.addWithKey(exc,
		                 exc.id);
	...
	public TextBuffer toJava(int indent) {
		...
		buf.append(ExprProcessor.jmpWrapper(body,
		                                    indent + 1,
		                                    true));
	...
	private void mapMonitorExitInstr(TextBuffer buffer) {
		BasicBlock block = body.getBasichead()
		                       .getBlock();
		if (!block.getSeq()
		          .isEmpty() && block.getLastInstruction().opcode == CodeConstants.opc_monitorexit) {
			Integer offset = block.getOldOffset(block.size() - 1);
			if (offset > -1) {
				buffer.addBytecodeMapping(offset);
			}
		}
	}
	...
	public void removeExc() {
		...
		SequenceHelper.destroyStatementContent(exc,
		                                       true);
