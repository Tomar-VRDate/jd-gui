...
public class FlattenStatementsHelper {
	...
	public DirectGraph buildDirectGraph(RootStatement root) {
		...
		DirectNode node = new DirectNode(DirectNode.NODE_DIRECT,
		                                 dummyexit,
		                                 dummyexit.id.toString());
		node.exprents = new ArrayList<>();
		graph.nodes.addWithKey(node,
		                       node.id);
		mapDestinationNodes.put(dummyexit.id,
		                        new String[]{node.id,
		                                     null});
	...
	private void flattenStatement() {
		...
		class StatementStackEntry {
			...
			StatementStackEntry(Statement statement,
			                    LinkedList<StackEntry> stackFinally,
			...
			if (statEntry.succEdges == null) {
				...
				switch (stat.type) {
					case Statement.TYPE_BASICBLOCK:
						node = new DirectNode(DirectNode.NODE_DIRECT,
						                      stat,
						                      (BasicBlockStatement) stat);
						if (stat.getExprents() != null) {
							node.exprents = stat.getExprents();
						}
						graph.nodes.putWithKey(node,
						                       node.id);
						mapDestinationNodes.put(stat.id,
						                        new String[]{node.id,
						                                     null});
						...
						if (tailExprentList != null) {
							DirectNode tail = new DirectNode(DirectNode.NODE_TAIL,
							                                 stat,
							                                 stat.id + "_tail");
							tail.exprents = tailExprentList;
							graph.nodes.putWithKey(tail,
							                       tail.id);

							mapDestinationNodes.put(-stat.id,
							                        new String[]{tail.id,
							                                     null});
							listEdges.add(new Edge(node.id,
							                       -stat.id,
						...
						if (stat.getLastBasicType() == Statement.LASTBASICTYPE_IF) {
							...
							mapPosIfBranch.put(sourcenode.id,
							                   lstSuccEdges.get(0)
							                               .getDestination().id);
						}
					...
					case Statement.TYPE_TRYCATCH:
						DirectNode firstnd = new DirectNode(DirectNode.NODE_TRY,
						                                    stat,
						                                    stat.id + "_try");
						...
						mapDestinationNodes.put(stat.id,
						                        new String[]{firstnd.id,
						                                     null});
						graph.nodes.putWithKey(firstnd,
						                       firstnd.id);

						LinkedList<StatementStackEntry> lst = new LinkedList<>();

						for (Statement st : stat.getStats()) {
							listEdges.add(new Edge(firstnd.id,
							                       st.id,
							                       StatEdge.TYPE_REGULAR));
							...
							if (stat.type == Statement.TYPE_CATCHALL && ((CatchAllStatement) stat).isFinally()) {
								...
								if (st == stat.getFirst()) { // catch head
									stack.add(new StackEntry((CatchAllStatement) stat,
									                         Boolean.FALSE));
								} else { // handler
									stack.add(new StackEntry((CatchAllStatement) stat,
									                         Boolean.TRUE,
							...
							lst.add(new StatementStackEntry(st,
							                                stack,
							                                null));
						}

						lstStackStatements.addAll(0,
						                          lst);
						break;
					case Statement.TYPE_DO:
						if (statementBreakIndex == 0) {
							...
							lstStackStatements.addFirst(new StatementStackEntry(stat.getFirst(),
							                                                    stackFinally,
							                                                    null));
						...
						switch (looptype) {
							...
							case DoStatement.LOOP_DOWHILE:
								node = new DirectNode(DirectNode.NODE_CONDITION,
								                      stat,
								                      stat.id + "_cond");
							...
							case DoStatement.LOOP_FOREACH:
								DirectNode nodeinit = new DirectNode(DirectNode.NODE_INIT,
								                                     stat,
								                                     stat.id + "_init");
								if (dostat.getInitExprent() != null) {
									nodeinit.exprents = dostat.getInitExprentList();
								}
								graph.nodes.putWithKey(nodeinit,
								                       nodeinit.id);

								DirectNode nodecond = new DirectNode(DirectNode.NODE_CONDITION,
								                                     stat,
								                                     stat.id + "_cond");
								if (looptype != DoStatement.LOOP_FOREACH) {
									nodecond.exprents = dostat.getConditionExprentList();
								}
								graph.nodes.putWithKey(nodecond,
								                       nodecond.id);

								DirectNode nodeinc = new DirectNode(DirectNode.NODE_INCREMENT,
								                                    stat,
								                                    stat.id + "_inc");
								nodeinc.exprents = dostat.getIncExprentList();
								graph.nodes.putWithKey(nodeinc,
								                       nodeinc.id);

								mapDestinationNodes.put(stat.id,
								                        new String[]{nodeinit.id,
								                                     nodeinc.id});
								mapDestinationNodes.put(-stat.id,
								                        new String[]{nodecond.id,
								                                     null});

								listEdges.add(new Edge(nodecond.id,
								                       stat.getFirst().id,
								                       StatEdge.TYPE_REGULAR));
								listEdges.add(new Edge(nodeinit.id,
								                       -stat.id,
								                       StatEdge.TYPE_REGULAR));
								listEdges.add(new Edge(nodeinc.id,
								                       -stat.id,
								                       StatEdge.TYPE_REGULAR));
					...
					case Statement.TYPE_ROOT:
						...
						if (statementBreakIndex <= statsize) {
							...
							for (int i = statementBreakIndex;
							     i < statsize;
							     i++) {
								statEntry.statementIndex = i + 1;
								lstStackStatements.addFirst(statEntry);
								lstStackStatements.addFirst(new StatementStackEntry(stat.getStats()
								                                                        .get(i),
								                                                    stackFinally,
								                                                    (i == 0
								                                                     && tailexprlst != null
								                                                     && tailexprlst.get(0) != null)
								                                                    ? tailexprlst
								                                                    : null));
							...
							if (stat.type == Statement.TYPE_IF
							    ...
							    && !stat.getAllSuccessorEdges()
							            .isEmpty()) {
								lstSuccEdges.add(stat.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)
								                     .get(0));  // exactly one edge
								sourcenode = tailexprlst.get(0) == null
								             ? node
					...
					while (true) {
						...
						if (entry == null) {
							saveEdge(sourcenode,
							         ...
							         edgetype,
							         isFinallyExit
							         ? finallyShortRangeSource
							         : null,
							         finallyLongRangeSource,
							         finallyShortRangeEntry,
							         finallyLongRangeEntry,
							...
							if (entry.state) { // finally handler statement
								...
								if (!catchall.containsStatementStrict(destination)) {
									...
									stack.removeLast();
									stack.add(new StackEntry(catchall,
									                         Boolean.TRUE,
									                         edgetype,
									                         destination,
									                         catchall.getHandler(),
									                         finallyLongRangeEntry == null
									                         ? catchall.getHandler()
									                         : finallyLongRangeEntry,
									                         sourcenode,
									                         finallyLongRangeSource,
									...
									lstStackStatements.addFirst(statEntry);
									lstStackStatements.addFirst(new StatementStackEntry(catchall.getHandler(),
									                                                    stack,
									                                                    null));
	...
	private void saveEdge(DirectNode sourcenode,
		...
		if (edgetype != StatEdge.TYPE_FINALLYEXIT) {
			listEdges.add(new Edge(sourcenode.id,
			                       destination.id,
			                       edgetype));
		}
		...
		if (finallyShortRangeSource != null) {
			...
			mapShortRangeFinallyPathIds.computeIfAbsent(sourcenode.id,
			                                            k -> new ArrayList<>())
			                           .add(new String[]{finallyShortRangeSource.id,
			...
			mapLongRangeFinallyPathIds.computeIfAbsent(sourcenode.id,
			                                           k -> new ArrayList<>())
			                          .add(new String[]{finallyLongRangeSource.id,
			                                            destination.id.toString(),
	...
	private void setEdges() {
		...
		for (Edge edge : listEdges) {
			...
			if (strings == null) {
				DotExporter.toDotFile(graph,
				                      root.mt,
				                      "errorDGraph");

				throw new IllegalStateException("Could not find destination nodes for stat id "
				                                + statid
				                                + " from source "
				                                + sourceid);
			}
			DirectNode dest = graph.nodes.getWithKey(strings[edge.edgetype == StatEdge.TYPE_CONTINUE
			                                                 ? 1
			                                                 : 0]);
			...
			if (mapPosIfBranch.containsKey(sourceid) && !statid.equals(mapPosIfBranch.get(sourceid))) {
				graph.mapNegIfBranch.put(sourceid,
				                         dest.id);
			}
		}
		...
		for (int i = 0;
		     i < 2;
		     i++) {
			for (Entry<String, List<String[]>> ent : (i == 0
			                                          ? mapShortRangeFinallyPathIds
				...
				for (String[] arr : lst) {
					...
					boolean isContinueEdge = arr[i == 0
					                             ? 4
					                             : 3] != null;

					DirectNode dest = graph.nodes.getWithKey(mapDestinationNodes.get(Integer.parseInt(arr[1]))[isContinueEdge
					                                                                                           ? 1
					                                                                                           : 0]);
					DirectNode enter = graph.nodes.getWithKey(mapDestinationNodes.get(Integer.parseInt(arr[2]))[0]);

					newLst.add(new FinallyPathWrapper(arr[0],
					                                  dest.id,
					                                  enter.id));

					if (i == 0 && arr[3] != null) {
						graph.mapFinallyMonitorExceptionPathExits.put(ent.getKey(),
						                                              dest.id);
					}
				}
				...
				if (!newLst.isEmpty()) {
					(i == 0
					 ? graph.mapShortRangeFinallyPaths
					 : graph.mapLongRangeFinallyPaths).put(ent.getKey(),
	...
	public static final class FinallyPathWrapper {
		...
		private FinallyPathWrapper(String source,
		                           String destination,
		                           String entry) {
			this.source = source;
