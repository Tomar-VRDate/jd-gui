...
public class NestedClassProcessor {
	private static void setLambdaVars(ClassNode parent,
		...
		String parent_class_name = parent.getWrapper()
		                                 .getClassStruct().qualifiedName;
		String lambda_class_name = child.simpleName;

		VarType lambda_class_type = new VarType(lambda_class_name,
		                                        true);

		// this pointer
		if (!is_static_lambda_content && DecompilerContext.getOption(IFernflowerPreferences.LAMBDA_TO_ANONYMOUS_CLASS)) {
			varProc.getThisVars()
			       .put(new VarVersionPair(0,
			                               0),
			            parent_class_name);
			varProc.setVarName(new VarVersionPair(0,
			                                      0),
			                   parent.simpleName + ".this");
		}

		// collect all previously used names, excluding "this"
		VarProcessor enclosingVarProc = enclosingMethod.varproc;
		Set<String> usedBefore = new HashSet<>(enclosingVarProc.getVarNamesCollector()
		                                                       .getUsedNames());
		usedBefore.addAll(enclosingVarProc.getVarNames());
		for (VarVersionPair thisVar : enclosingVarProc.getThisVars()
		                                              .keySet()) {
		...
		for (VarVersionPair local : varProc.getUsedVarVersions()) {
			...
			if (usedBefore.contains(name) && !"this".equals(name)) {
				mapNewNames.put(local,
				                enclosingCollector.getFreeName(name));
			}
		}

		enclosingMethod.getOrBuildGraph()
		               .iterateExprents(exprent -> {
			               ...
			               for (Exprent expr : lst) {
				               if (expr.type == Exprent.EXPRENT_NEW) {
					               ...
					               if (new_expr.isLambda() && lambda_class_type.equals(new_expr.getNewType())) {
						               ...
						               int param_index = is_static_lambda_content
						                                 ? 0
						                                 : 1;
						               int varIndex = is_static_lambda_content
						                              ? 0
						                              : 1;

						               for (int i = 0;
						                    i < md_content.params.length;
						                    ++i) {
							               VarVersionPair varVersion = new VarVersionPair(varIndex,
							                                                              0);
							               if (i < vars_count) {
								               Exprent param = inv_dynamic.getLstParameters()
								                                          .get(param_index + i);

								               if (param.type == Exprent.EXPRENT_VAR) {
									               mapNewNames.put(varVersion,
									                               enclosingVarProc.getVarName(new VarVersionPair((VarExprent) param)));
									               lvts.put(varVersion,
									                        ((VarExprent) param).getLVT());
									               if (enclosingVarProc.getVarFinal((new VarVersionPair((VarExprent) param)))
									                   == VarTypeProcessor.VAR_NON_FINAL) {
										               //DecompilerContext.getLogger().writeMessage("Lambda in " + parent.simpleName + "."
										               // + enclosingMethod.methodStruct.getName() + " given non-final var " + (
										               // (VarExprent)param).getName() + "!", IFernflowerLogger.Severity.ERROR);
									               }
								               }
							               } else if (!mapNewNames.containsKey(varVersion)) {
								               mapNewNames.put(varVersion,
								                               enclosingCollector.getFreeName(varProc.getVarName(varVersion)));
							               }
		...
		varProc.getVarNamesCollector()
		       .addNames(enclosingCollector.getUsedNames());
		...
		for (Entry<VarVersionPair, String> entry : mapNewNames.entrySet()) {
			...
			varProc.setVarName(pair,
			                   entry.getValue());
			if (lvt != null) {
				varProc.setVarLVT(pair,
				                  lvt);
			}
		}

		method.getOrBuildGraph()
		      .iterateExprentsDeep(exp -> {
	...
	private static void checkNotFoundClasses(ClassNode root,
	                                         ClassNode node) {
			...
			if ((child.type == ClassNode.CLASS_LOCAL || child.type == ClassNode.CLASS_ANONYMOUS)
				...
				if (!setEnclosing.isEmpty()) {
					StructEnclosingMethodAttribute attr
									= child.classStruct.getAttribute(StructGeneralAttribute.ATTRIBUTE_ENCLOSING_METHOD);
					if (attr != null
					    && attr.getMethodName() != null
					    && node.classStruct.qualifiedName.equals(attr.getClassName())
					    && node.classStruct.getMethod(attr.getMethodName(),
					                                  attr.getMethodDescriptor()) != null) {
						child.enclosingMethod = InterpreterUtil.makeUniqueKey(attr.getMethodName(),
						                                                      attr.getMethodDescriptor());
						continue;
					}
				}
				...
				boolean hasEnclosing = !setEnclosing.isEmpty() && insertNestedClass(root,
				                                                                    child);
	...
	private static boolean insertNestedClass(ClassNode root,
	                                         ClassNode child) {
		Set<String> setEnclosing = child.enclosingClasses;
	...
	private static void computeLocalVarsAndDefinitions(ClassNode node) {
		...
		for (ClassNode nd : node.nested) {
			if (nd.type != ClassNode.CLASS_LAMBDA
				...
				if (mask.isEmpty()) {
					...
					mapVarMasks.put(nd.classStruct.qualifiedName,
					                mask);
				}
			}
		}
		...
		if (clTypes != ClassNode.CLASS_MEMBER) {
			...
			for (MethodWrapper method : node.getWrapper()
			                                .getMethods()) {
				if (method.root != null) { // neither abstract, nor native
					DotExporter.toDotFile(method.getOrBuildGraph(),
					                      method.methodStruct,
					                      "computeLocalVars");
					method.getOrBuildGraph()
					      .iterateExprents(exprent -> {
						      ...
						      for (Exprent expr : lst) {
							      if (expr.type == Exprent.EXPRENT_NEW) {
								      ...
								      if (constructor != null
								          && mapVarMasks.containsKey(constructor.getClassname())) { // non-static inner class
									      // constructor
									      ...
									      if (nestedClassNode.type != ClassNode.CLASS_MEMBER) {
										      ...
										      if (!mapVarFieldPairs.containsKey(refClassName)) {
											      mapVarFieldPairs.put(refClassName,
											                           new HashMap<>());
										      }
										      ...
										      for (int i = 0;
										           i < mask.size();
										           i++) {
											      Exprent param = constructor.getLstParameters()
											                                 .get(i);
											      ...
											      if (param.type == Exprent.EXPRENT_VAR && mask.get(i) != null) {
												      ...
												      pair = new VarFieldPair(mask.get(i).fieldKey,
												                              varPair);
												      // }
											      }
										      ...
										      if (pairMask == null) {
											      ...
											      for (int i = 0;
											           i < pairMask.size();
											           i++) {
												      if (!InterpreterUtil.equalObjects(pairMask.get(i),
												                                        lstTemp.get(i))) {
													      pairMask.set(i,
													                   null);
												      }
		...
		for (Entry<String, Map<String, List<VarFieldPair>>> enclosing : mapVarMasks.entrySet()) {
			...
			if (mapVarFieldPairs.containsKey(enclosing.getKey())) {
				for (List<VarFieldPair> mask : mapVarFieldPairs.get(enclosing.getKey())
				                                               .values()) {
					if (interPairMask == null) {
						interPairMask = new ArrayList<>(mask);
					} else {
						mergeListSignatures(interPairMask,
						                    mask,
						                    false);
					}
				}
			...
			for (List<VarFieldPair> mask : enclosing.getValue()
			                                        .values()) {
				if (interMask == null) {
					interMask = new ArrayList<>(mask);
				} else {
					mergeListSignatures(interMask,
					                    mask,
					                    false);
				}
			}
			...
			if (interPairMask == null) { // member or local and never instantiated
				interPairMask = interMask != null
				                ? new ArrayList<>(interMask)
				                : new ArrayList<>();

				boolean found = false;

				for (int i = 0;
				     i < interPairMask.size();
				     i++) {
			...
			for (Entry<String, List<VarFieldPair>> entry : enclosing.getValue()
			                                                        .entrySet()) {
				mergeListSignatures(entry.getValue(),
				                    interPairMask,
				                    false);

				List<VarVersionPair> mask = new ArrayList<>(entry.getValue()
				                                                 .size());
				for (VarFieldPair pair : entry.getValue()) {
					mask.add(pair != null && !pair.fieldKey.isEmpty()
					         ? pair.varPair
					         : null);
				}
				nestedNode.getWrapper()
				          .getMethodWrapper(CodeConstants.INIT_NAME,
				                            entry.getKey()).synthParameters = mask;
			}
		}
	}

	private static MethodWrapper findEnclosingMethod(ClassNode child) {
		if (child.enclosingMethod == null) {
			return null;
		}
		if (child.parent != null) {
			MethodWrapper fromParent = child.parent.getWrapper()
			                                       .getMethods()
			                                       .getWithKey(child.enclosingMethod);
			if (fromParent != null) {
				return fromParent;
		...
		for (String enclosingClassName : child.enclosingClasses) {
			ClassNode enclosingClass = DecompilerContext.getClassProcessor()
			                                            .getMapRootClasses()
			                                            .get(enclosingClassName);
			if (enclosingClass != null) {
				MethodWrapper fromEnclosing = enclosingClass.getWrapper()
				                                            .getMethods()
				                                            .getWithKey(child.enclosingMethod);
		...
		if (child.type != ClassNode.CLASS_MEMBER) {
			DecompilerContext.getLogger()
			                 .writeMessage("Couldn't find enclosing method \""
			                               + child.enclosingMethod
			                               + "\" of "
			                               + child.classStruct.qualifiedName
			                               + " in "
			                               + child.enclosingClasses,
			                               IFernflowerLogger.Severity.WARN);
		}
		return null;
	}

	private static void insertLocalVars(ClassNode parent,
	                                    ClassNode child) {
		...
		for (MethodWrapper method : child.getWrapper()
			...
			if (method.root != null) { // neither abstract nor native
				...
				if (enclosingMethod != null) {
					method.methodStruct.getVariableNamer()
					                   .addParentContext(enclosingMethod.methodStruct.getVariableNamer());
				}
				...
				if (method.synthParameters != null) {
					...
					for (VarVersionPair pair : method.synthParameters) {
						if (pair != null) {
							VarVersionPair newVar
											= new VarVersionPair(method.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER),
											                     0);

							mapParamsToNewVars.put(varIndex,
							                       newVar);
							...
							if (child.type != ClassNode.CLASS_MEMBER) {
								...
								enclosingMethod.varproc.setVarFinal(pair,
							...
							if (pair.var == -1 || "this".equals(varName) || (varLVT != null && "this".equals(varLVT.getName()))) {
								...
								method.varproc.getThisVars()
								              .put(newVar,
								                   parent.classStruct.qualifiedName);
							}

							mapNewNames.put(newVar,
							                varName);
							mapNewTypes.put(newVar,
							                varType);
							mapNewLVTs.put(newVar,
							               varLVT);
						}
				...
				for (ClassNode classNode = child;
					...
					for (Entry<String, VarVersionPair> entry : classNode.mapFieldsToVars.entrySet()) {
						...
						mapFieldsToNewVars.put(InterpreterUtil.makeUniqueKey(classNode.classStruct.qualifiedName,
						                                                     entry.getKey()),
						                       newVar);
						...
						if (classNode.type != ClassNode.CLASS_MEMBER) {
							...
							enclosing_method.varproc.setVarFinal(entry.getValue(),
							                                     VarTypeProcessor.VAR_EXPLICIT_FINAL);
						}

						if (entry.getValue().var == -1 || "this".equals(varName) || (varLVT != null
						                                                             && "this".equals(varLVT.getName()))) {
							...
							method.varproc.getThisVars()
							              .put(newVar,
							                   classNode.parent.classStruct.qualifiedName);
						}
						...
						if (classNode == child) { // fields higher up the chain were already handled with their classes
							StructField fd = child.classStruct.getFields()
							                                  .getWithKey(entry.getKey());
							child.getWrapper()
							     .getHiddenMembers()
				...
				for (Entry<VarVersionPair, String> entry : mapNewNames.entrySet()) {
					...
					method.varproc.setVarName(pair,
					                          entry.getValue());
					if (type != null) {
						method.varproc.setVarType(pair,
						                          type);
					}
					if (lvt != null) {
						method.varproc.setVarLVT(pair,
						                         lvt);
					}
				}

				iterateExprents(method.getOrBuildGraph(),
				                new ExprentIteratorWithReplace() {
					                @Override
					                public Exprent processExprent(Exprent exprent) {
						                if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
							                ...
							                if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD) {
								                ...
								                if (fExpr.getClassname()
								                         .equals(qName) &&  // process this class only
								                    mapFieldsToNewVars.containsKey(InterpreterUtil.makeUniqueKey(qName,
								                                                                                 fExpr.getName(),
					                ...
					                private Exprent replaceExprent(Exprent exprent) {
						                if (exprent.type == Exprent.EXPRENT_VAR) {
							                ...
							                if (mapParamsToNewVars.containsKey(varIndex)) {
								                ...
								                method.varproc.getExternalVars()
								                              .add(newVar);
								                VarExprent ret = new VarExprent(newVar.var,
								                                                method.varproc.getVarType(newVar),
								                                                method.varproc,
								                                                exprent.bytecode);
							                ...
							                String key = InterpreterUtil.makeUniqueKey(fExpr.getClassname(),
							                                                           fExpr.getName(),
							                                                           fExpr.getDescriptor().descriptorString);
	...
	private static Map<String, List<VarFieldPair>> getMaskLocalVars(ClassWrapper wrapper) {
		...
		for (StructMethod mt : cl.getMethods()) {
			if (CodeConstants.INIT_NAME.equals(mt.getName())) {
				...
				MethodWrapper method = wrapper.getMethodWrapper(CodeConstants.INIT_NAME,
				                                                mt.getDescriptor());
				...
				if (graph != null) { // something gone wrong, should not be null
					...
					for (int i = 0;
					     i < md.params.length;
					     i++) {  // no static methods allowed
						String keyField = getEnclosingVarField(cl,
						                                       method,
						                                       graph,
						                                       varIndex);
						fields.add(keyField == null
						           ? null
						           : new VarFieldPair(keyField,
						                              new VarVersionPair(-1,
						                                                 0))); // TODO: null?
						varIndex += md.params[i].stackSize;
					}

					mapMasks.put(mt.getDescriptor(),
					             fields);
				}
			}
		}
	...
	private static String getEnclosingVarField(StructClass cl,
		...
		if (method.varproc.getVarFinal(new VarVersionPair(index,
		                                                  0)) == VarTypeProcessor.VAR_NON_FINAL) {
			return null;
		}
			...
			for (Exprent exprent : firstNode.exprents) {
				if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					if (assignExpr.getRight().type == Exprent.EXPRENT_VAR
						...
						StructField fd = cl.getField(left.getName(),
						                             left.getDescriptor().descriptorString);
						if (fd != null && cl.qualifiedName.equals(left.getClassname()) && (fd.isSynthetic()
						                                                                   || noSynthFlag
						                                                                      && possiblySyntheticField(fd))) {
							// local (== not inherited) field
							field = InterpreterUtil.makeUniqueKey(left.getName(),
							                                      left.getDescriptor().descriptorString);
							break;
						}
					}
	...
	private static boolean possiblySyntheticField(StructField fd) {
		return fd.getName()
		         .contains("$") && fd.hasModifier(CodeConstants.ACC_FINAL) && fd.hasModifier(CodeConstants.ACC_PRIVATE);
	}

	private static void mergeListSignatures(List<VarFieldPair> first,
	                                        List<VarFieldPair> second,
	                                        boolean both) {
		int i = 1;
		...
		while (first.size() > i && second.size() > i) {
			...
			if (!isEqual(both,
			             fObj,
			             sObj)) {
				first.set(first.size() - i,
				          null);
				if (both) {
					second.set(second.size() - i,
					           null);
				}
			} else if (fObj != null) {
	...
	private static boolean isEqual(boolean both,
	                               VarFieldPair fObj,
	                               VarFieldPair sObj) {
		boolean eq;
	...
	private static void setLocalClassDefinition(MethodWrapper method,
	                                            ClassNode node) {
		RootStatement root = method.root;

		Set<Statement> setStats = new HashSet<>();
		VarType classType = new VarType(node.classStruct.qualifiedName,
		                                true);

		Statement statement = getDefStatement(root,
		                                      classType,
		                                      setStats);
		if (statement == null) {
		...
		Statement first = findFirstBlock(statement,
		                                 setStats);
		...
		VarExprent var = new VarExprent(method.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER),
		                                classType,
		                                method.varproc);
	...
	private static Statement findFirstBlock(Statement stat,
	...
	private static Statement getDefStatement(Statement stat,
	                                         VarType classType,
	                                         Set<? super Statement> setStats) {
		...
		if (stat.getExprents() == null) {
			...
			for (Object obj : stat.getSequentialObjects()) {
				if (obj instanceof Statement) {
					...
					Statement stTemp = getDefStatement(st,
					                                   classType,
					                                   setStats);
		...
		if (retStat != stat) {
			for (Exprent exprent : lst) {
				if (exprent != null && searchForClass(exprent,
				                                      classType)) {
					retStat = stat;
					break;
				}
	...
	private static boolean searchForClass(Exprent exprent,
	                                      VarType classType) {
		List<Exprent> lst = exprent.getAllExprents(true);
		lst.add(exprent);
		...
		for (Exprent expr : lst) {
			...
			switch (expr.type) {
				...
				case Exprent.EXPRENT_INVOCATION:
					res = containsType(((InvocationExprent) expr),
					                   classType);
					break;
				case Exprent.EXPRENT_NEW:
					...
					res = newType.type == CodeConstants.TYPE_OBJECT && classname.equals(newType.value)
					      || containsType(newExpr.getConstructor(),
					                      classType);
					break;
				case Exprent.EXPRENT_VAR:
					VarExprent varExpr = (VarExprent) expr;
					if (varExpr.isDefinition()) {
						res = containsType(varExpr.getInferredExprType(null),
						                   classType);
					}
					break;
			}
	...
	private static boolean containsType(InvocationExprent haystack,
	                                    VarType needle) {
		if (haystack == null) {
			return false;
		}
		if (needle.value.equals(haystack.getClassname())) {
			return true;
		}
	...
	private static boolean containsType(VarType haystack,
	                                    VarType needle) {
	...
	private static void iterateExprents(DirectGraph graph,
	                                    ExprentIteratorWithReplace iter) {
	...
	public void processClass(ClassNode root,
		...
		for (ClassNode child : node.nested) {
			if (child.type == ClassNode.CLASS_LAMBDA) {
				...
				insertLocalVars(node,
				                child);

				if (child.type == ClassNode.CLASS_LOCAL && child.enclosingMethod != null) {
					MethodWrapper enclosingMethodWrapper = node.getWrapper()
					                                           .getMethods()
					                                           .getWithKey(child.enclosingMethod);
					if (enclosingMethodWrapper
					    != null) { // e.g. in case of switch-on-enum. FIXME: some proper handling of multiple enclosing classes
						setLocalClassDefinition(enclosingMethodWrapper,
						                        child);
					}
				}
			}
	 ...
	 * <p>
	 * If a class has multiple switches on multiple enums, the compiler adds the init and try list to the BEGINNING of
  ...
  private void gatherEnumSwitchMaps(ClassNode node) {
    ...
    final int STATIC_FINAL_SYNTHETIC = CodeConstants.ACC_STATIC | CodeConstants.ACC_STATIC | CodeConstants.ACC_FINAL
    | CodeConstants.ACC_SYNTHETIC;
    ...
    for (int x = 0; x < seq.getStats().size();) {
      ...
      if (stat.type != Statement.TYPE_BASICBLOCK || stat.getExprents() == null || stat.getExprents().size() != 1 ||
      stat.getExprents().get(0).type != Exprent.EXPRENT_ASSIGNMENT) {
        break;
      }
      ...
      if (!mapField.getClassname().equals(node.classStruct.qualifiedName) || !potentialFields.contains(mapField
      .getName()) ||
      ...
      if (func.getFuncType() != FunctionExprent.FUNCTION_ARRAY_LENGTH || func.getLstOperands().size() != 1 || func
      .getLstOperands().get(0).type != Exprent.EXPRENT_INVOCATION) {
        break;
      }
      InvocationExprent invoc = (InvocationExprent)func.getLstOperands().get(0);
      if (!"values".equals(invoc.getName()) || !("()[L" + invoc.getClassname() + ";").equals(invoc
      .getStringDescriptor())) {
        break;
      }
      ...
      while (++y < seq.getStats().size()) {
        ...
        if (_try.getVars().size() != 1 || !"java/lang/NoSuchFieldError".equals(_try.getVars().get(0).getVarType()
        .value) ||
            first.type != Statement.TYPE_BASICBLOCK || exprents == null || exprents.size() != 1 || exprents.get(0)
            .type != Exprent.EXPRENT_ASSIGNMENT) {
          break;
        }
        ass = (AssignmentExprent)exprents.get(0);
        if (ass.getRight().type != Exprent.EXPRENT_CONST || (!(((ConstExprent)ass.getRight()).getValue() instanceof
        Integer)) ||
            ass.getLeft().type != Exprent.EXPRENT_ARRAY){
          break;
        }
        ArrayExprent array = (ArrayExprent)ass.getLeft();
        if (array.getArray().type != Exprent.EXPRENT_FIELD || !array.getArray().equals(mapField) || array.getIndex()
        .type != Exprent.EXPRENT_INVOCATION) {
          break;
        }
        invoc = (InvocationExprent)array.getIndex();
        if (!enumName.equals(invoc.getClassname()) || !"ordinal".equals(invoc.getName()) || !"()I".equals(invoc
        .getStringDescriptor()) ||
	...
	private Statement getNewEmptyStatement() {
		BasicBlockStatement bstat = new BasicBlockStatement(new BasicBlock(DecompilerContext.getCounterContainer()
