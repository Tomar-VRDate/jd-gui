...
public class BasicBlockStatement
	...
	public BasicBlockStatement(BasicBlock block) {
		...
		if (id >= coun.getCounter(CounterContainer.STATEMENT_COUNTER)) {
			coun.setCounter(CounterContainer.STATEMENT_COUNTER,
			                id + 1);
		}
	...
	public static BasicBlockStatement create() {
		BasicBlockStatement stat = new BasicBlockStatement(new BasicBlock(DecompilerContext.getCounterContainer()
	...
	public TextBuffer toJava(int indent) {
		TextBuffer tb = ExprProcessor.listToJava(varDefinitions,
		                                         indent);
	...
	public Statement getSimpleCopy() {
		...
		BasicBlock newblock = new BasicBlock(DecompilerContext.getCounterContainer()
		                                                      .getCounterAndIncrement(CounterContainer.STATEMENT_COUNTER));
		...
		for (int i = 0;
		     i < block.getSeq()
		              .length();
		     i++) {
			seq.addInstruction(block.getSeq()
			                        .getInstr(i)
			                        .clone(),
			                   -1);
		}
			...
			for (Exprent exp : exps) {
				...
				for (Exprent exprent : inner) {
					if (exprent.type == Exprent.EXPRENT_FUNCTION
					    && ((FunctionExprent) exprent).getFuncType() == FunctionExprent.FUNCTION_INSTANCEOF) {
						if (((FunctionExprent) exprent).getLstOperands()
						                               .size() > 2) {
							vars.add((VarExprent) ((FunctionExprent) exprent).getLstOperands()
							                                                 .get(2));
						}
					}
				}
	...
	public StartEndPair getStartEndRange() {
		if (block.size() > 0) {
			return new StartEndPair(block.getStartInstruction(),
			                        block.getEndInstruction());
		} else {
			return new StartEndPair(0,
			                        0);
		}
	}
}
