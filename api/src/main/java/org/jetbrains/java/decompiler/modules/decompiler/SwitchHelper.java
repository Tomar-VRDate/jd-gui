...
public final class SwitchHelper {
	public static final int STATIC_FINAL_SYNTHETIC = CodeConstants.ACC_STATIC
	...
	public static boolean simplifySwitches(Statement stat,
		...
		for (int i = 0;
		     i < stat.getStats()
		             .size();
		     ++i) {
			ret |= simplifySwitches(stat.getStats()
			                            .get(i),
			                        mt);
		}
	...
	private static boolean simplify(SwitchStatement switchStatement,
		...
		if (edges.size() == 1
		    && edges.get(0)
		            .getDestination().type == Statement.TYPE_SWITCH) {
			following = (SwitchStatement) edges.get(0)
			                                   .getDestination();
		}
		...
		if (isEnumArray(value)) {
			...
			if (array.getArray().type == Exprent.EXPRENT_FIELD) {
				...
				ClassesProcessor.ClassNode classNode = DecompilerContext.getClassProcessor()
				                                                        .getMapRootClasses()
				                                                        .get(arrayField.getClassname());
				if (classNode != null) {
					ClassWrapper classWrapper = classNode.getWrapper();
					if (classWrapper != null) {
						MethodWrapper wrapper = classWrapper.getMethodWrapper(CodeConstants.CLINIT_NAME,
						                                                      "()V");
				...
				if (classNode != null) {
					ClassWrapper classWrapper = classNode.getWrapper();
					if (classWrapper != null) {
						MethodWrapper wrapper = classWrapper.getMethodWrapper(invocation.getName(),
						                                                      "()[I");
						if (wrapper != null && wrapper.root != null) {
							wrapper.getOrBuildGraph()
							       .iterateExprents(exprent -> {
					...
					if (exprent == null) {
						...
						if (realConst == null) {
							DecompilerContext.getLogger()
							                 .writeMessage("Unable to simplify switch on enum: "
							                               + exprent
							                               + " not found, available: "
							                               + mapping
							                               + " in method "
			...
			switchHeadExprent.replaceExprent(value,
			                                 ((InvocationExprent) array.getIndex()).getInstance()
			                                                                       .copy());
			return true;
			...
			int i = 0;
			for (;
			     i < switchStatement.getCaseStatements()
			                        .size();
			     ++i) {

				Statement curr = switchStatement.getCaseStatements()
				                                .get(i);
				...
				while (curr != null && curr.type == Statement.TYPE_IF) {
					...
					if (condition.type == Exprent.EXPRENT_FUNCTION
					    && ((FunctionExprent) condition).getFuncType() == FunctionExprent.FUNCTION_NE) {
						condition = ((FunctionExprent) condition).getLstOperands()
						                                         .get(0);
					}

					if (condition.type == Exprent.EXPRENT_INVOCATION
					    && ((InvocationExprent) condition).getLstParameters()
					                                      .size() == 1) {
						Exprent assign = ifStat.getIfstat()
						                       .getExprents()
						                       .get(0);
			...
			List<List<Exprent>> realCaseValues = following.getCaseValues()
			                                              .stream()
			                                              .map(l -> l.stream()
			                                                         .map(e -> e instanceof ConstExprent
			                                                                   ? ((ConstExprent) e).getIntValue()
			                                                                   : null)
			                                                         .map(caseMap::get)
			...
			following.getCaseValues()
			         .clear();
			following.getCaseValues()
			         .addAll(realCaseValues);

			Exprent followingVal = ((SwitchHeadExprent) following.getHeadexprent()).getValue();
			following.getHeadexprent()
			         .replaceExprent(followingVal,
			                         ((InvocationExprent) value).getInstance());

			switchStatement.getFirst()
			               .getExprents()
			               .remove(switchStatement.getFirst()
			                                      .getExprents()
			                                      .size() - 1);
			switchStatement.getFirst()
			               .getAllPredecessorEdges()
			               .forEach(switchStatement.getFirst()::removePredecessor);
			switchStatement.getFirst()
			               .getAllSuccessorEdges()
			               .forEach(switchStatement.getFirst()::removeSuccessor);
			switchStatement.getParent()
			               .replaceStatement(switchStatement,
			                                 switchStatement.getFirst());
			return true;
		}
		return false;
	 ...
	 * <p>
	 * Note: SOME compilers name the field starting with $SwitchMap, so if we do not have full context this can be a
	 * guess.
			...
			if (tmp instanceof FieldExprent) {
				...
				if (classNode == null || !"[I".equals(field.getDescriptor().descriptorString)) {
					return field.getName()
					            .startsWith("$SwitchMap") || //This is non-standard but we don't have any more information so..
					       (index instanceof InvocationExprent && ((InvocationExprent) index).getName()
					                                                                         .equals("ordinal"));
				}

				StructField stField;
				if (classNode.getWrapper()
				    == null) { // I have no idea why this happens, according to debug tests it doesn't even return null
					stField = classNode.classStruct.getField(field.getName(),
				...
				if (classNode.getWrapper() == null) {
					isSyncheticClass = (classNode.classStruct.getAccessFlags() & CodeConstants.ACC_SYNTHETIC)
					                   == CodeConstants.ACC_SYNTHETIC;
				} else {
					isSyncheticClass = (classNode.getWrapper()
					                             .getClassStruct()
					                             .getAccessFlags() & CodeConstants.ACC_SYNTHETIC) == CodeConstants.ACC_SYNTHETIC;
				}

				//Exprent init = classNode.getWrapper().getStaticFieldInitializers().getWithKey(InterpreterUtil
				// .makeUniqueKey(field.getName(), field.getDescriptor().descriptorString));
				//Above is null because we haven't preocess the class yet?
				return isSyncheticClass; //TODO: Find a way to check the structure of the initalizer?
			} else if (tmp.type == Exprent.EXPRENT_INVOCATION) {
				InvocationExprent inv = (InvocationExprent) tmp;
				// More nonstandard behavior. Seems like eclipse compiler stuff: https://bugs.eclipse.org/bugs/show_bug.cgi?id=544521
				// TODO: needs tests!
				return inv.getName()
	...
	private static boolean isSwitchOnString(SwitchStatement first,
		...
		if (second != null) {
			...
			if (firstValue.type == Exprent.EXPRENT_INVOCATION
			    && secondValue.type == Exprent.EXPRENT_VAR
			    && first.getCaseStatements()
			            .get(0).type == Statement.TYPE_IF) {
				InvocationExprent invExpr = (InvocationExprent) firstValue;
				VarExprent        varExpr = (VarExprent) secondValue;

				if (invExpr.getName()
				           .equals("hashCode") && invExpr.getClassname()
				                                         .equals("java/lang/String")) {
					boolean matches = true;

					for (int i = 0;
					     matches && i < first.getCaseStatements()
					                         .size();
					     ++i) {
						if (!first.getCaseEdges()
						          .get(i)
						          .contains(first.getDefaultEdge())) {
							Statement curr = first.getCaseStatements()
							                      .get(i);
							while (matches && curr != null) {
								if (curr.type == Statement.TYPE_IF) {
									...
									if (condition.type == Exprent.EXPRENT_INVOCATION) {
										...
										if (condInvocation.getName()
										                  .equals("equals") && condInvocation.getInstance()
										                                                     .equals(invExpr.getInstance())) {
											List<Exprent> block = ifStat.getIfstat()
											                            .getExprents();
