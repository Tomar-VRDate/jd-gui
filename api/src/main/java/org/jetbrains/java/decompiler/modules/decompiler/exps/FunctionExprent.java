...
public class FunctionExprent
	...
	private static final VarType[] TYPES = {VarType.VARTYPE_LONG,
	...
	private static final Set<Integer>  ASSOCIATIVITY               = new HashSet<>(Arrays.asList(FUNCTION_ADD,
	                                                                                             FUNCTION_MUL,
	                                                                                             FUNCTION_AND,
	                                                                                             FUNCTION_OR,
	                                                                                             FUNCTION_XOR,
	...
	public FunctionExprent(int funcType,
		...
		if (funcType >= FUNCTION_BIT_NOT
		    && funcType <= FUNCTION_PPI
		    && funcType != FUNCTION_CAST
	...
	private static VarType getMaxVarType(VarType[] arr) {
		int[] types = {CodeConstants.TYPE_DOUBLE,
		               CodeConstants.TYPE_FLOAT,
		               CodeConstants.TYPE_LONG};
		VarType[] vartypes = {VarType.VARTYPE_DOUBLE,
		                      VarType.VARTYPE_FLOAT,
		                      VarType.VARTYPE_LONG};
	...
	public VarType getExprType() {
		...
		if (funcType <= FUNCTION_NEG
			...
			VarType type1 = lstOperands.get(0)
			                           .getExprType();
			VarType type2 = null;
			if (lstOperands.size() > 1) {
				type2 = lstOperands.get(1)
				                   .getExprType();
			}
			...
			exprType = lstOperands.get(1)
			                      .getExprType();
		} else if (funcType == FUNCTION_IIF) {
			Exprent param1 = lstOperands.get(1);
			Exprent param2 = lstOperands.get(2);
			VarType supertype = VarType.getCommonSupertype(param1.getExprType(),
			                                               param2.getExprType());
			if (param1.type == Exprent.EXPRENT_CONST
			    && param2.type == Exprent.EXPRENT_CONST
			    && supertype.type != CodeConstants.TYPE_BOOLEAN
	...
	public VarType getInferredExprType(VarType upperBound) {
		if (funcType == FUNCTION_CAST) {
			this.needsCast = true;
			VarType right = lstOperands.get(0)
			                           .getInferredExprType(upperBound);
			VarType cast = lstOperands.get(1)
			                          .getExprType();
			...
			if (upperBound != null && (upperBound.isGeneric() || right.isGeneric())) {
				...
				if (types != null) {
					...
					for (VarType type : types) {
						anyMatch |= DecompilerContext.getStructContext()
					...
					this.needsCast = right.type == CodeConstants.TYPE_NULL || !DecompilerContext.getStructContext()
					                                                                            .instanceOf(right.value,
					                                                                                        upperBound.value);
				}
				if (!this.needsCast) {
					if (arrayDim > 0) {
	...
	public CheckTypesResult checkExprTypeBounds() {
		...
		switch (funcType) {
			case FUNCTION_IIF:
				...
				result.addMinTypeExprent(param1,
				                         VarType.VARTYPE_BOOLEAN);
				result.addMinTypeExprent(param2,
			...
			case FUNCTION_I2S:
				...
				result.addMaxTypeExprent(param1,
				                         VarType.VARTYPE_INT);
				break;
			case FUNCTION_IMM:
			case FUNCTION_IPP:
			...
			case FUNCTION_NE: {
				if (type1.type == CodeConstants.TYPE_BOOLEAN) {
					if (type2.isStrictSuperset(type1)) {
						...
						boolean param1_false_boolean = type1.isFalseBoolean() || (param1.type == Exprent.EXPRENT_CONST
						                                                          && !((ConstExprent) param1).hasBooleanValue());
						boolean param2_false_boolean = type1.isFalseBoolean() || (param2.type == Exprent.EXPRENT_CONST
	...
	public Exprent copy() {
		...
		FunctionExprent func = new FunctionExprent(funcType,
		                                           lst,
		                                           bytecode);
	...
	public boolean equals(Object o) {
		...
		return funcType == fe.getFuncType() && InterpreterUtil.equalLists(lstOperands,
		                                                                  fe.getLstOperands()); // TODO: order of operands
		// insignificant
	}
	...
	public TextBuffer toJava(int indent) {
		...
		// If we're an unsigned right shift or lower, this function can be represented as a single leftHand + functionType
		// + rightHand operation.
		if (this.funcType <= FUNCTION_USHR) {
			// Minecraft specific hot fix: If we're doing arithmetic or bitwise math by a char value, we can assume that
			// it's wrong behavior.
			...
			if (this.funcType <= FUNCTION_XOR) {
				...
				// Checks to see if the right expression is a constant and then adjust the type from char to int if the left
				// is an int.
			...
			TextBuffer leftOperand = wrapOperandString(this.lstOperands.get(0),
			                                           false,
			                                           indent,
			                                           true);
			TextBuffer rightOperand = wrapOperandString(this.lstOperands.get(1),
			                                            true,
			                                            indent,
			                                            true);
		...
		if (funcType >= FUNCTION_EQ) {
			...
			buf.append(wrapOperandString(lstOperands.get(0),
			                             false,
			                             indent,
			                             true))
			   .appendPossibleNewline(" ")
			   ...
			   .append(" ")
			   .append(wrapOperandString(lstOperands.get(1),
			                             true,
			                             indent,
			                             true));
			if (!disableNewlineGroupCreation) {
		...
		switch (funcType) {
			case FUNCTION_BIT_NOT:
				return buf.append(wrapOperandString(lstOperands.get(0),
				                                    true,
				                                    indent).prepend("~"));
			case FUNCTION_BOOL_NOT:
				return buf.append(wrapOperandString(lstOperands.get(0),
			...
			case FUNCTION_CAST:
				if (!needsCast) {
					return buf.append(lstOperands.get(0)
					                             .toJava(indent));
				}
				return buf.append(lstOperands.get(1)
				                             .toJava(indent))
				          .enclose("(",
				                   ")")
				          .append(wrapOperandString(lstOperands.get(0),
				                                    true,
				                                    indent));
			case FUNCTION_ARRAY_LENGTH:
				Exprent arr = lstOperands.get(0);

				buf.append(wrapOperandString(arr,
				                             false,
				                             indent));
			...
			case FUNCTION_IIF:
				...
				buf.append(wrapOperandString(lstOperands.get(0),
				   ...
				   .appendPossibleNewline(" ")
				   .append(": ")
				   .append(wrapOperandString(lstOperands.get(2),
				                             true,
				                             indent));
				buf.popNewlineGroup();
				return buf;
			...
			case FUNCTION_INSTANCEOF:
				...
				if (this.lstOperands.size() > 2) {
					...
					buf.append(wrapOperandString(this.lstOperands.get(2),
					                             true,
					                             indent));
				}
				return buf;
	...
	private TextBuffer wrapOperandString(Exprent expr,
	                                     boolean eq,
	                                     int indent) {
		return wrapOperandString(expr,
		                         eq,
		                         indent,
		                         false);
	}
		...
		if (newlineGroup && !parentheses && myprec == exprprec) {
			if (expr.type == Exprent.EXPRENT_FUNCTION) {
				...
				if (funcExpr.getFuncType() == FUNCTION_CAST && !funcExpr.doesCast()) {
					Exprent subExpr = funcExpr.getLstOperands()
					                          .get(0);
