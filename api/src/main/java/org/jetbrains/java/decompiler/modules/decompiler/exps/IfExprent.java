...
public class IfExprent
	...
	private static final int[] FUNC_TYPES = {FunctionExprent.FUNCTION_EQ,
	...
	public IfExprent(int ifType,
		...
		if (ifType <= IF_LE) {
			stack.push(new ConstExprent(0,
			                            true,
			                            null));
		} else if (ifType <= IF_NONNULL) {
			stack.push(new ConstExprent(VarType.VARTYPE_NULL,
			                            null,
			                            null));
		}
		...
		if (ifType == IF_VALUE) {
			...
			condition = new FunctionExprent(FUNC_TYPES[ifType],
			                                stack,
			                                bytecodeOffsets);
		}
	}
	...
	public boolean equals(Object o) {
		...
		return InterpreterUtil.equalObjects(condition,
		                                    ie.getCondition());
	}

	public IfExprent negateIf() {
		condition = new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
		                                condition,
		                                condition.bytecode);
		return this;
	}
