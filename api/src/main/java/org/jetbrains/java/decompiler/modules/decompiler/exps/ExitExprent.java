...
public class ExitExprent
	...
	public ExitExprent(int exitType,
	                   Exprent value,
	                   VarType retType,
	...
	@Override
	public Exprent copy() {
		return new ExitExprent(exitType,
		                       value == null
		                       ? null
		                       : value.copy(),
		                       retType,
	...
	public CheckTypesResult checkExprTypeBounds() {
		...
		if (exitType == EXIT_RETURN && retType.type != CodeConstants.TYPE_VOID) {
			result.addMinTypeExprent(value,
			                         VarType.getMinTypeInFamily(retType.typeFamily));
			result.addMaxTypeExprent(value,
			                         retType);
		}
		...
		if (exitType == EXIT_RETURN) {
			...
			if (retType.type != CodeConstants.TYPE_VOID) {
				...
				if (methodDescriptor != null
				    && methodDescriptor.genericInfo != null
				    && methodDescriptor.genericInfo.returnType != null) {
					ret = methodDescriptor.genericInfo.returnType;
				}
				buf.append(' ');

				ExprProcessor.getCastedExprent(value,
				                               ret,
				                               buf,
				                               indent,
				                               false,
				...
				if (attr != null) {
					...
					for (int i = 0;
					     i < attr.getThrowsExceptions()
					             .size();
					     i++) {
						String exClassName = attr.getExcClassname(i,
						                                          node.classStruct.getPool());
					...
					if (classname != null) {
						VarType exType = new VarType(classname,
						                             true);
						buf.append("throw ");
						ExprProcessor.getCastedExprent(value,
						                               exType,
			...
			return buf.append(value.toJava(indent))
			          .prepend("throw ");
		}
	}
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof ExitExprent)) {
			return false;
		}

		ExitExprent et = (ExitExprent) o;
		return exitType == et.getExitType() && InterpreterUtil.equalObjects(value,
		                                                                    et.getValue());
	}
