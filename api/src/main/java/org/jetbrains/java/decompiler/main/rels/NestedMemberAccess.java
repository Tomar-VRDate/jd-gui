...
public class NestedMemberAccess {
	...
	private static boolean sameTree(ClassNode caller,
	                                ClassNode callee) {
	...
	private static Exprent replaceFunction(final InvocationExprent invexpr,
	                                       final Exprent source) {
		...
		FieldExprent fieldExprent = (FieldExprent) functionExprent.getLstOperands()
		                                                          .get(0);
		...
		fieldExprent.replaceExprent(fieldExprent.getInstance(),
		                            lstParameters.get(0));
		return functionExprent;
	}
	...
	private void computeMethodTypes(ClassNode node) {
		...
		for (MethodWrapper method : node.getWrapper()
		                                .getMethods()) {
			computeMethodType(node,
			                  method);
		}
	}

	private void computeMethodType(ClassNode node,
						...
						switch (exprCore.type) {
							case Exprent.EXPRENT_FIELD:
								...
								if ((parcount == 1 && !fexpr.isStatic()) || (parcount == 0 && fexpr.isStatic())) {
									if (fexpr.getClassname()
									         .equals(node.classStruct.qualifiedName)) {  // FIXME: check for private flag of the field
										if (fexpr.isStatic() || (fexpr.getInstance().type == Exprent.EXPRENT_VAR
							...
							case Exprent.EXPRENT_FUNCTION:
								...
								if (functionExprent.getFuncType() >= FunctionExprent.FUNCTION_IMM
									...
									if (functionExprent.getLstOperands()
						...
						if (type == MethodAccess.METHOD) { // FIXME: check for private flag of the method
							...
							if ((isStatic
								...
								int index = isStatic
								            ...
								            : 1;
								for (int i = 0;
								     i < invexpr.getLstParameters()
								                .size();
								     i++) {
									Exprent parexpr = invexpr.getLstParameters()
									                         .get(i);
									if (parexpr.type != Exprent.EXPRENT_VAR || ((VarExprent) parexpr).getIndex() != index) {
										equalpars = false;
										break;
									}
									index += mtdesc.params[i + (isStatic
									                            ? 0
									                            : 1)].stackSize;
								}
	...
	private void eliminateStaticAccess(ClassNode node) {
		...
		for (MethodWrapper meth : node.getWrapper()
		                              .getMethods()) {
			...
			if (meth.root != null) {
				...
				while (!stack.isEmpty()) {  // TODO: replace with interface iterator?
					...
					for (int i = 0;
						...
						replaced |= replaceInvocations(node,
						                               meth,
						                               exprent);

						if (exprent.type == Exprent.EXPRENT_INVOCATION) {
							Exprent ret = replaceAccessExprent(node,
							                                   meth,
							                                   (InvocationExprent) exprent);
	...
	private boolean replaceInvocations(ClassNode caller,
	                                   MethodWrapper meth,
	                                   Exprent exprent) {
		...
		while (true) {
			...
			for (Exprent expr : exprent.getAllExprents()) {
				if (expr.type == Exprent.EXPRENT_INVOCATION) {
					Exprent newexpr = replaceAccessExprent(caller,
					                                       meth,
	...
	private Exprent replaceAccessExprent(ClassNode caller,
	                                     MethodWrapper methdest,
		...
		if (node != null && node.getWrapper() != null) {
			methsource = node.getWrapper()
			                 .getMethodWrapper(invexpr.getName(),
		...
		if (node.classStruct.qualifiedName.equals(caller.classStruct.qualifiedName)
		    && methsource.methodStruct.getName()
		                              .equals(methdest.methodStruct.getName())
		    && methsource.methodStruct.getDescriptor()
		                              .equals(methdest.methodStruct.getDescriptor())) {
		...
		switch (type) {
			case FIELD_GET:
				...
				if (exsource.getValue().type == Exprent.EXPRENT_VAR) { // qualified this
					...
					VarExprent ret = new VarExprent(index,
					                                var.getVarType(),
					                                methdest.varproc);
					methdest.varproc.setVarName(new VarVersionPair(index,
					                                               0),
					                            varname);

					retexprent = ret;
				} else { // field
					FieldExprent ret = (FieldExprent) exsource.getValue()
					                                          .copy();
					if (!ret.isStatic()) {
						ret.replaceExprent(ret.getInstance(),
						                   invexpr.getLstParameters()
						                          .get(0));
					}
					retexprent = ret;
			...
			case FIELD_SET:
				...
				if (source.type == Exprent.EXPRENT_EXIT) {
					...
					ret = (AssignmentExprent) extex.getValue()
					                               .copy();
				} else {
				...
				if (fexpr.isStatic()) {
					ret.replaceExprent(ret.getRight(),
					                   invexpr.getLstParameters()
					                          .get(0));
				} else {
					ret.replaceExprent(ret.getRight(),
					                   invexpr.getLstParameters()
					                          .get(1));
					fexpr.replaceExprent(fexpr.getInstance(),
					                     invexpr.getLstParameters()
					                            .get(0));
				}
				...
				break;
			case FUNCTION:
				retexprent = replaceFunction(invexpr,
				                             source);
				break;
			case METHOD:
				if (source.type == Exprent.EXPRENT_EXIT) {
				...
				int index = 0;
				if (!invret.isStatic()) {
					invret.replaceExprent(invret.getInstance(),
					                      invexpr.getLstParameters()
					                             .get(0));
					index = 1;
				}
				...
				for (int i = 0;
				     i < invret.getLstParameters()
				               .size();
				     i++) {
					invret.replaceExprent(invret.getLstParameters()
					                            .get(i),
					                      invexpr.getLstParameters()
					                             .get(i + index));
				}
		...
		if (retexprent != null) {
			...
			if (hide) {
				node.getWrapper()
				    .getHiddenMembers()
				    .add(InterpreterUtil.makeUniqueKey(invexpr.getName(),
				                                       invexpr.getStringDescriptor()));
			}
		}
