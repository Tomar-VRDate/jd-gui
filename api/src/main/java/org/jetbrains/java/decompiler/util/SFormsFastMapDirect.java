...
public class SFormsFastMapDirect {
	...
	public SFormsFastMapDirect getCopy() {
		...
		for (int i = 2;
			...
			if (length > 0) {
				...
				@SuppressWarnings("unchecked") FastSparseSet<Integer>[] arrnew = new FastSparseSet[length];
				int[] arrnextnew = Arrays.copyOf(arrnext,
				                                 length);
	...
	public void putInternal(final int key,
	                        final FastSparseSet<Integer> value,
	                        boolean remove) {
	...
	private FastSparseSet<Integer>[] ensureCapacity(int index,
	                                                int size,
	                                                boolean exact) {
