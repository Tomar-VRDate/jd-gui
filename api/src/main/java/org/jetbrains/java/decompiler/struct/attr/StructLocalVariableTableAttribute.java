...
public class StructLocalVariableTableAttribute
		...
		if (len > 0) {
			...
			for (int i = 0;
				...
				localVariables.add(new LocalVariable(start_pc,
				                                     length,
				                                     pool.getPrimitiveConstant(nameIndex)
				                                         .getString(),
				                                     pool.getPrimitiveConstant(descriptorIndex)
				                                         .getString(),
				                                     varIndex));
			}
	...
	public String getName(int index,
	                      int visibleOffset) {
		return matchingVars(index,
		                    visibleOffset).map(v -> v.name)
		                                  .findFirst()
		                                  .orElse(null);
	}
	...
	public String getDescriptor(int index,
	                            int visibleOffset) {
		return matchingVars(index,
		                    visibleOffset).map(v -> v.descriptor)
		                                  .findFirst()
		                                  .orElse(null);
	}
	...
	public Stream<LocalVariable> matchingVars(Statement stat) {
		...
		return getRange(values.nextSetBit(0),
		                values.length() - 1);
	}

	public Stream<LocalVariable> getRange(int start,
	                                      int end) {
		return localVariables.stream()
		                     .filter(v -> v.getStart() >= start && v.getEnd() <= end);
	}

	public boolean containsName(String name) {
		return localVariables.stream()
		                     .anyMatch(v -> Objects.equals(v.name,
		                                                   name));
	}
	...
	private void versionVariables(List<LocalVariable> vars) {
		for (LocalVariable var : vars) {
			...
			version = version == null
			          ? 1
			          : version++;
			indexVersion.put(var.index,
			                 version);
			var.version = new VarVersionPair(var.index,
			                                 version.intValue());
		}
	}

	public void mergeSignatures(StructLocalVariableTypeTableAttribute lvtt) {
		lvtt.backingAttribute.localVariables.stream()
		                                    .forEach(type -> localVariables.stream()
		                                                                   .filter(t -> t.compareTo(type) == 0)
		                                                                   .findFirst()
		                                                                   .ifPresent(lv -> lv.signature
						                                                                                    = type.descriptor));
	}
	...
	public static class LocalVariable
		...
		private LocalVariable(int start_pc,
		                      int length,
		                      String name,
		                      String descriptor,
		                      int index) {
			...
			this.index = index;
			this.version = new VarVersionPair(index,
			                                  0);
		}
		...
		public int compareTo(LocalVariable o) {
			...
			if (this.start_pc != o.start_pc) {
				return this.start_pc - o.start_pc;
			}
		...
		public String toString() {
			return "'(" + index + "," + start_pc + '-' + getEnd() + ")" + descriptor + (signature != null
			                                                                            ? "<" + signature + "> "
			                                                                            : " ") + name + "'";
		}

		public LocalVariable rename(String newName) {
			LocalVariable ret = new LocalVariable(start_pc,
			                                      length,
			                                      newName,
			                                      descriptor,
			                                      index);
