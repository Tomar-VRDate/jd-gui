...
public class ConstantPool
	...
	public ConstantPool(DataInputStream in)
		...
		BitSet[] nextPass = {new BitSet(size),
		                     new BitSet(size),
		                     new BitSet(size)};
		...
		for (int i = 1;
			...
			switch (tag) {
				case CodeConstants.CONSTANT_Utf8:
					pool.add(new PrimitiveConstant(CodeConstants.CONSTANT_Utf8,
					                               in.readUTF()));
					break;

				case CodeConstants.CONSTANT_Integer:
					pool.add(new PrimitiveConstant(CodeConstants.CONSTANT_Integer,
					                               Integer.valueOf(in.readInt())));
					break;

				case CodeConstants.CONSTANT_Float:
					pool.add(new PrimitiveConstant(CodeConstants.CONSTANT_Float,
					                               in.readFloat()));
					break;

				case CodeConstants.CONSTANT_Long:
					pool.add(new PrimitiveConstant(CodeConstants.CONSTANT_Long,
					                               in.readLong()));
					pool.add(null);
					i++;
					break;
				...
				case CodeConstants.CONSTANT_Package:
					pool.add(new PrimitiveConstant(tag,
					                               in.readUnsignedShort()));
					nextPass[0].set(i);
					break;

				case CodeConstants.CONSTANT_NameAndType:
					pool.add(new LinkConstant(tag,
					                          in.readUnsignedShort(),
					                          in.readUnsignedShort()));
					nextPass[0].set(i);
					break;
	...
	public String[] getClassElement(int elementType,
	                                String className,
	                                int nameIndex,
	                                int descriptorIndex) {
		...
		if (interceptor != null) {
			...
			String newDescriptor = buildNewDescriptor(elementType == FIELD,
			                                          descriptor);
	...
	public PrimitiveConstant getPrimitiveConstant(int index) {
		...
		if (cn != null && interceptor != null) {
			if (cn.type == CodeConstants.CONSTANT_Class) {
				...
				if (newName != null) {
					cn = new PrimitiveConstant(CodeConstants.CONSTANT_Class,
					                           newName);
				}
			}
		}
	...
	public LinkConstant getLinkConstant(int index) {
		...
		if (ln != null && interceptor != null && (ln.type == CodeConstants.CONSTANT_Fieldref
			...
			String newDescriptor = buildNewDescriptor(ln.type == CodeConstants.CONSTANT_Fieldref,
			                                          ln.descriptor);
			//TODO: Fix newElement being null caused by ln.classname being a leaf class instead of the class that declared
			// the field/method.
			//See the comments of IDEA-137253 for more information.
			if (newClassName != null || newElement != null || newDescriptor != null) {
				String className = newClassName == null
				                   ? ln.classname
				                   : newClassName;
				String elementName = newElement == null
				                     ? ln.elementname
				                     : newElement.split(" ")[1];
				String descriptor = newDescriptor == null
				                    ? ln.descriptor
				                    : newDescriptor;
				ln = new LinkConstant(ln.type,
				                      className,
				                      elementName,
				                      descriptor);
			}
	...
	public String buildNewClassname(String className) {
		VarType vt = new VarType(className,
		                         true);
	...
	private String buildNewDescriptor(boolean isField,
	                                  String descriptor) {
		if (isField) {
			return FieldDescriptor.parseDescriptor(descriptor)
			                      .buildNewDescriptor(this);
		} else {
			return MethodDescriptor.parseDescriptor(descriptor)
			                       .buildNewDescriptor(this);
		}
	}
}
