...
public class ExprProcessor
	...
	private static final VarType[]             consts     = {VarType.VARTYPE_INT,
	                                                         VarType.VARTYPE_FLOAT,
	                                                         VarType.VARTYPE_LONG,
	                                                         VarType.VARTYPE_DOUBLE,
	                                                         VarType.VARTYPE_CLASS,
	                                                         VarType.VARTYPE_STRING};
	private static final VarType[]             varTypes   = {VarType.VARTYPE_INT,
	                                                         VarType.VARTYPE_LONG,
	                                                         VarType.VARTYPE_FLOAT,
	                                                         VarType.VARTYPE_DOUBLE,
	                                                         VarType.VARTYPE_OBJECT};
	private static final VarType[]             arrTypes   = {VarType.VARTYPE_INT,
	                                                         VarType.VARTYPE_LONG,
	                                                         VarType.VARTYPE_FLOAT,
	                                                         VarType.VARTYPE_DOUBLE,
	                                                         VarType.VARTYPE_OBJECT,
	...
	private static final int[]                 func1      = {FunctionExprent.FUNCTION_ADD,
	                                                         FunctionExprent.FUNCTION_SUB,
	                                                         FunctionExprent.FUNCTION_MUL,
	                                                         FunctionExprent.FUNCTION_DIV,
	                                                         FunctionExprent.FUNCTION_REM};
	private static final int[]                 func2      = {FunctionExprent.FUNCTION_SHL,
	...
	private static final int[]                 func3      = {FunctionExprent.FUNCTION_I2L,
	...
	private static final int[]                 func4      = {FunctionExprent.FUNCTION_LCMP,
	                                                         FunctionExprent.FUNCTION_FCMPL,
	                                                         FunctionExprent.FUNCTION_FCMPG,
	                                                         FunctionExprent.FUNCTION_DCMPL,
	                                                         FunctionExprent.FUNCTION_DCMPG};
	private static final int[]                 func5      = {IfExprent.IF_EQ,
	                                                         IfExprent.IF_NE,
	                                                         IfExprent.IF_LT,
	                                                         IfExprent.IF_GE,
	                                                         IfExprent.IF_GT,
	                                                         IfExprent.IF_LE};
	private static final int[]                 func6      = {IfExprent.IF_ICMPEQ,
	                                                         IfExprent.IF_ICMPNE,
	                                                         IfExprent.IF_ICMPLT,
	                                                         IfExprent.IF_ICMPGE,
	                                                         IfExprent.IF_ICMPGT,
	                                                         ...
	                                                         IfExprent.IF_ACMPEQ,
	                                                         IfExprent.IF_ACMPNE};
	private static final int[]                 func7      = {IfExprent.IF_NULL,
	                                                         IfExprent.IF_NONNULL};
	private static final int[]                 func8      = {MonitorExprent.MONITOR_ENTER,
	                                                         MonitorExprent.MONITOR_EXIT};
	private static final int[]                 arrTypeIds = {CodeConstants.TYPE_BOOLEAN,
	...
	private static final int[]                 negIfs     = {IfExprent.IF_NE,
	                                                         IfExprent.IF_EQ,
	                                                         IfExprent.IF_GE,
	                                                         IfExprent.IF_LT,
	                                                         IfExprent.IF_LE,
	                                                         ...
	                                                         IfExprent.IF_ACMPNE,
	                                                         IfExprent.IF_ACMPEQ};
	private static final String[]              typeNames  = {"byte",
	                                                         "char",
	                                                         "double",
	                                                         "float",
	                                                         "int",
	...
	static {
		mapConsts.put(opc_arraylength,
		              FunctionExprent.FUNCTION_ARRAY_LENGTH);
		mapConsts.put(opc_checkcast,
		              FunctionExprent.FUNCTION_CAST);
		mapConsts.put(opc_instanceof,
		              FunctionExprent.FUNCTION_INSTANCEOF);
	}

	private final MethodDescriptor methodDescriptor;
	private final VarProcessor     varProcessor;

	public ExprProcessor(MethodDescriptor md,
		...
		if (lst != null) {
			for (int i = 1;
			     i < stat.getStats()
			             .size();
			     i++) {
				map.put(flatthelper.getMapDestinationNodes()
				                   .get(stat.getStats()
				                            .get(i).id)[0],
				        lst.get(i - 1));
			}
	...
	public static boolean endsWithSemicolon(Exprent expr) {
		int type = expr.type;
		return !(type == Exprent.EXPRENT_SWITCH_HEAD || type == Exprent.EXPRENT_MONITOR || type == Exprent.EXPRENT_IF || (
						type == Exprent.EXPRENT_VAR
	...
	private static void addDeletedGotoInstructionMapping(Statement stat,
		...
		if (stat instanceof BasicBlockStatement) {
			...
			if (!offsets.isEmpty() && offsets.size() > block.getSeq()
			                                                .length()) { // some instructions have been deleted, but we
				// still have offsets
			...
			if (edge.getType() != StatEdge.TYPE_REGULAR
				...
				switch (edge.getType()) {
					case StatEdge.TYPE_BREAK:
						addDeletedGotoInstructionMapping(stat,
						                                 buf);
						buf.append("break");
						break;
				...
				if (edge.labeled) {
					buf.append(" label")
					   .append(edge.closure.id.toString());
				}
				buf.append(";")
				   .appendLineSeparator();
			}
	...
	public static TextBuffer listToJava(List<? extends Exprent> lst,
	                                    int indent) {
	...
	public static ConstExprent getDefaultArrayValue(VarType arrType) {
		...
		if (arrType.type == CodeConstants.TYPE_OBJECT || arrType.arrayDim > 0) {
			defaultVal = new ConstExprent(VarType.VARTYPE_NULL,
			                              null,
			                              null);
		} else if (arrType.type == CodeConstants.TYPE_FLOAT) {
			defaultVal = new ConstExprent(VarType.VARTYPE_FLOAT,
			                              0f,
			                              null);
		} else if (arrType.type == CodeConstants.TYPE_LONG) {
			defaultVal = new ConstExprent(VarType.VARTYPE_LONG,
			                              0L,
			                              null);
		} else if (arrType.type == CodeConstants.TYPE_DOUBLE) {
			defaultVal = new ConstExprent(VarType.VARTYPE_DOUBLE,
			                              0d,
			                              null);
		} else { // integer types
			defaultVal = new ConstExprent(0,
			                              true,
			                              null);
		}
		return defaultVal;
	...
	public static boolean getCastedExprent(Exprent exprent,
	                                       ...
	                                       int indent,
	                                       boolean castNull) {
		return getCastedExprent(exprent,
		                        leftType,
		                        buffer,
		                        indent,
		                        castNull,
	...
	public static boolean getCastedExprent(Exprent exprent,
		...
		if (unbox) {
			...
			if (exprent.type == Exprent.EXPRENT_INVOCATION) {
				...
				if (invocationExprent.isBoxingCall() && !invocationExprent.shouldForceBoxing()) {
					exprent = invocationExprent.getLstParameters()
					                           .get(0);
		...
		boolean cast = castAlways
		               ...
		               || (castNull
		                   ...
		                   && !UNDEFINED_TYPE_STRING.equals(getTypeName(leftType)))
		...
		if (cast) {
			buffer.append('(')
			      .append(getCastTypeName(leftType))
			      .append(')');
		}

		if (castLambda) {
			buffer.append('(')
			      .append(getCastTypeName(rightType))
			      .append(')');
		}
	...
	private static boolean lambdaNeedsCast(VarType left,
	                                       NewExprent exprent) {
	...
	public void processStatement(RootStatement root,
	                             StructClass cl) {
		...
		map.put(null,
		        new PrimitiveExprsList());
		mapData.put(dgraph.first,
		            map);
		...
		while (!stack.isEmpty()) {
			...
			if (mapCatch.containsKey(node.id)) {
				...
				data = mapData.get(node)
				              .get(buildEntryPointKey(entrypoints));
			}
			...
			String currentEntrypoint = entrypoints.isEmpty()
			                           ? null
			                           : entrypoints.getLast();
			...
			for (DirectNode nd : node.succs) {
				...
				if (isSuccessor) {
					Map<String, PrimitiveExprsList> mapSucc = mapData.computeIfAbsent(nd,
					                                                                  k -> new HashMap<>());
					...
					if (succ_entry_index
					    >= 0) { // we are in a loop (e.g. continue in a finally block), drop all entry points in the list
						// beginning with succ_entry_index
					...
					if (!mapSucc.containsKey(ndentrykey)) {
						...
						mapSucc.put(ndentrykey,
						            copyVarExprents(data.copyStack()));
		...
		for (int i = 0;
			...
			switch (instr.opcode) {
				case opc_aconst_null:
					pushEx(stack,
					       exprlist,
					       new ConstExprent(VarType.VARTYPE_NULL,
					                        null,
					                        bytecode_offsets));
					break;
				case opc_bipush:
				case opc_sipush:
					pushEx(stack,
					       exprlist,
					       new ConstExprent(instr.operand(0),
					                        true,
					                        bytecode_offsets));
					break;
				case opc_lconst_0:
				case opc_lconst_1:
					pushEx(stack,
					       exprlist,
					       new ConstExprent(VarType.VARTYPE_LONG,
					                        (long) (instr.opcode - opc_lconst_0),
					                        bytecode_offsets));
					break;
				case opc_fconst_0:
				case opc_fconst_1:
				case opc_fconst_2:
					pushEx(stack,
					       exprlist,
					       new ConstExprent(VarType.VARTYPE_FLOAT,
					                        (float) (instr.opcode - opc_fconst_0),
					                        bytecode_offsets));
					break;
				case opc_dconst_0:
				case opc_dconst_1:
					pushEx(stack,
					       exprlist,
					       new ConstExprent(VarType.VARTYPE_DOUBLE,
					                        (double) (instr.opcode - opc_dconst_0),
					                        bytecode_offsets));
					break;
				case opc_ldc:
				case opc_ldc_w:
				case opc_ldc2_w:
					...
					if (cn instanceof PrimitiveConstant) {
						pushEx(stack,
						       exprlist,
						       new ConstExprent(consts[cn.type - CONSTANT_Integer],
						                        ((PrimitiveConstant) cn).value,
						                        bytecode_offsets));
				...
				case opc_aload:
					VarExprent varExprent = new VarExprent(instr.operand(0),
					                                       varTypes[instr.opcode - opc_iload],
					                                       varProcessor,
					                                       bytecode_offsets);
					varProcessor.findLVT(varExprent,
					                     bytecode_offset + instr.length);
					pushEx(stack,
					       exprlist,
					       varExprent);
					break;
				...
				case opc_saload:
					...
					pushEx(stack,
					       exprlist,
					       new ArrayExprent(arr,
					                        index,
					                        arrTypes[instr.opcode - opc_iaload],
					                        bytecode_offsets),
					       vartype);
					break;
				case opc_istore:
				...
				case opc_astore:
					...
					varExprent = new VarExprent(varindex,
					                            varTypes[instr.opcode - opc_istore],
					                            varProcessor,
					                            bytecode_offsets);
					varProcessor.findLVT(varExprent,
					                     bytecode_offset + instr.length);
					AssignmentExprent assign = new AssignmentExprent(varExprent,
					                                                 expr,
					                                                 bytecode_offsets);
					exprlist.add(assign);
					break;
				...
				case opc_sastore:
					...
					AssignmentExprent arrassign = new AssignmentExprent(new ArrayExprent(arr_store,
					                                                                     index_store,
					                                                                     arrTypes[instr.opcode - opc_iastore],
					                                                                     bytecode_offsets),
					                                                    value,
					                                                    bytecode_offsets);
				...
				case opc_drem:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(func1[(instr.opcode - opc_iadd) / 4],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_ishl:
				...
				case opc_lxor:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(func2[(instr.opcode - opc_ishl) / 2],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_ineg:
				...
				case opc_dneg:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(FunctionExprent.FUNCTION_NEG,
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_iinc:
					VarExprent vevar = new VarExprent(instr.operand(0),
					                                  VarType.VARTYPE_INT,
					                                  varProcessor,
					                                  bytecode_offsets);
					varProcessor.findLVT(vevar,
					                     bytecode_offset + instr.length);
				...
				case opc_i2s:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(func3[instr.opcode - opc_i2l],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_lcmp:
				...
				case opc_dcmpg:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(func4[instr.opcode - opc_lcmp],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_ifeq:
				...
				case opc_ifgt:
				case opc_ifle:
					exprlist.add(new IfExprent(negIfs[func5[instr.opcode - opc_ifeq]],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_if_icmpeq:
				...
				case opc_if_acmpeq:
				case opc_if_acmpne:
					exprlist.add(new IfExprent(negIfs[func6[instr.opcode - opc_if_icmpeq]],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_ifnull:
				case opc_ifnonnull:
					exprlist.add(new IfExprent(negIfs[func7[instr.opcode - opc_ifnull]],
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_tableswitch:
				case opc_lookupswitch:
					exprlist.add(new SwitchHeadExprent(stack.pop(),
					                                   bytecode_offsets));
					break;
				case opc_ireturn:
				case opc_lreturn:
				...
				case opc_monitorexit:
					exprlist.add(new MonitorExprent(func8[instr.opcode - opc_monitorenter],
					                                stack.pop(),
					                                bytecode_offsets));
					break;
				case opc_checkcast:
				...
				case opc_arraylength:
					pushEx(stack,
					       exprlist,
					       new FunctionExprent(mapConsts.get(instr.opcode),
					                           stack,
					                           bytecode_offsets));
					break;
				case opc_getstatic:
				...
				case opc_putfield:
					...
					Exprent exprfield = new FieldExprent(pool.getLinkConstant(instr.operand(0)),
					                                     instr.opcode == opc_putstatic
					                                     ? null
					                                     : stack.pop(),
					                                     bytecode_offsets);
					exprlist.add(new AssignmentExprent(exprfield,
					                                   valfield,
					                                   bytecode_offsets));
					break;
				...
				case opc_multianewarray:
					int dimensions = (instr.opcode == opc_new)
					                 ...
					                 : (instr.opcode == opc_anewarray)
					                   ? 1
					                   : instr.operand(1);
					VarType arrType = new VarType(pool.getPrimitiveConstant(instr.operand(0))
					                                  .getString(),
					                              true);
				...
				case opc_newarray:
					pushEx(stack,
					       exprlist,
					       new NewExprent(new VarType(arrTypeIds[instr.operand(0) - 4],
					                                  1),
					                      stack,
					                      1,
					                      bytecode_offsets));
				...
				case opc_dup_x1:
					...
					break;
				case opc_dup_x2:
					if (stack.getByOffset(-2)
					         .getExprType().stackSize == 2) {
						insertByOffsetEx(-2,
						                 stack,
						                 exprlist,
						                 -1);
	...
	private void pushEx(ListStack<Exprent> stack,
	...
	private void pushEx(ListStack<Exprent> stack,
		...
		exprlist.add(new AssignmentExprent(var,
		                                   exprent,
		                                   null));
		stack.push(var.copy());
	}

	private void insertByOffsetEx(int offset,
		...
		for (int i = -1;
			...
			VarExprent varnew = new VarExprent(base + i + 1,
			                                   varex.getExprType(),
			                                   varProcessor);
			varnew.setStack(true);
			exprlist.add(new AssignmentExprent(varnew,
			                                   varex,
			                                   null));
			lst.add(0,
			        (VarExprent) varnew.copy());
		}

		Exprent exprent = lst.get(lst.size() + copyoffset)
		                     .copy();
		VarExprent var = new VarExprent(base + offset,
		                                exprent.getExprType(),
		                                varProcessor);
		var.setStack(true);
		exprlist.add(new AssignmentExprent(var,
