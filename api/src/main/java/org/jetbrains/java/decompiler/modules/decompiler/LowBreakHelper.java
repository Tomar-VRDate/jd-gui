...
public class LowBreakHelper {
		...
		while (true) {
			...
			for (StatEdge edge : stat.getLabelEdges()) {
				if (edge.getType() == StatEdge.TYPE_BREAK) {
					Statement minclosure = getMinClosure(stat,
					                                     edge.getSource());
					if (minclosure != stat) {
						minclosure.addLabeledEdge(edge);
						edge.labeled = isBreakEdgeLabeled(edge.getSource(),
						                                  minclosure);
						found = true;
						break;
					}
	...
	public static boolean isBreakEdgeLabeled(Statement source,
		...
		if (closure.type == Statement.TYPE_DO || closure.type == Statement.TYPE_SWITCH) {
			...
			if (parent == closure) {
				...
				return isBreakEdgeLabeled(parent,
				                          closure) || (parent.type == Statement.TYPE_DO
	...
	public static Statement getMinClosure(Statement closure,
	                                      Statement source) {
		...
		while (true) {
			...
			switch (closure.type) {
				case Statement.TYPE_SEQUENCE:
					Statement last = closure.getStats()
					                        .getLast();

					if (isOkClosure(closure,
					                source,
					                last)) {
						newclosure = last;
					}
				...
				case Statement.TYPE_IF:
					...
					} else if (isOkClosure(closure,
					                       source,
					                       ifclosure.getElsestat())) {
	...
	private static boolean isOkClosure(Statement closure,
	                                   Statement source,
	                                   Statement stat) {
		...
		while (true) {
			...
			for (StatEdge edge : stat.getLabelEdges()) {
				if (edge.explicit && edge.labeled && edge.getType() == StatEdge.TYPE_BREAK) {
					...
					Statement newclosure = getMaxBreakLift(stat,
					                                       edge);
	...
	private static Statement getNextBreakLift(Statement stat,
		...
		while (closure != null && !closure.containsStatementStrict(edge.getDestination())) {
			...
			boolean labeled = isBreakEdgeLabeled(edge.getSource(),
			                                     closure);
