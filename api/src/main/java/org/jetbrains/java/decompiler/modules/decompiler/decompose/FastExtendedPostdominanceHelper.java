...
public class FastExtendedPostdominanceHelper {
	...
	private void filterOnExceptionRanges(DominatorTreeExceptionFilter filter) {
		for (int head : new HashSet<>(mapExtPostdominators.keySet())) {
			...
			for (Iterator<Integer> it = set.iterator();
			     it.hasNext(); ) {
				if (!filter.acceptStatementPair(head,
				                                it.next())) {
					it.remove();
				}
			}
	...
	private void removeErroneousNodes() {
		...
		iterateReachability((node, mapSets) -> {
			                    ...
			                    for (int id : setReachability) {
				                    ...
				                    mapExtPostdominators.get(id)
				                                        .complement(setReachabilityCopy);
			                    }
		...
		for (Statement stat : statement.getStats()) {
			if (stat.getPredecessorEdges(Statement.STATEDGE_DIRECT_ALL)
			        .isEmpty() && !stat.getPredecessorEdges(StatEdge.TYPE_EXCEPTION)
			                           .isEmpty()) { // exception handler
	...
	private void calcDefaultReachableSets() {
		...
		for (Statement stat : statement.getStats()) {
			mapExtPostdominators.put(stat.id,
			                         factory.spawnEmptySet());
		}
			                    ...
			                    for (StatEdge sucedge : node.getAllSuccessorEdges()) {
				                    if ((sucedge.getType() & edgetype) != 0) {
					                    if (mapSets.containsKey(sucedge.getDestination().id)) {
						                    ...
						                    if (!InterpreterUtil.equalObjects(setReachability,
						                                                      mapSupportPoints.get(node.id))) {
							                    mapSupportPoints.put(node.id,
							                                         setReachability);
							                    return true;
						                    }
					                    }
	...
	private void iterateReachability(IReachabilityAction action,
	                                 int edgetype) {
	...
	private interface IReachabilityAction {
		boolean action(Statement node,
		               HashMap<Integer, FastFixedSet<Integer>> mapSets);
	}
}
