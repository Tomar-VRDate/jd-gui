...
public final class ConcatenationHelper {
	...
	private static final VarType builderType = new VarType(CodeConstants.TYPE_OBJECT,
	                                                       0,
	                                                       "java/lang/StringBuilder");
	private static final VarType bufferType  = new VarType(CodeConstants.TYPE_OBJECT,
	                                                       0,
		...
		if (stat.getExprents() != null) {
			for (int i = 0;
			     i < stat.getExprents()
			             .size();
			     ++i) {
				Exprent ret = simplifyStringConcat(stat.getExprents()
				                                       .get(i));
				if (ret != null) {
					stat.getExprents()
					    .set(i,
	...
	public static Exprent contractStringConcat(Exprent expr) {
		...
		if (expr.type == Exprent.EXPRENT_INVOCATION) {
			...
			if ("toString".equals(iex.getName())) {
				...
				List<Exprent> parameters = extractParameters(iex.getBootstrapArguments(),
				                                             iex);
				...
				if (parameters.size() >= 2) {
					return createConcatExprent(parameters,
					                           expr.bytecode);
				}
			}
		}
		...
		while (true) {
			...
			switch (exprTmp.type) {
				case Exprent.EXPRENT_INVOCATION:
					...
					if (isAppendConcat(iex,
					                   cltype)) {
						lstOperands.add(0,
						                iex.getLstParameters()
						                   .get(0));
						exprTmp = iex.getInstance();
						found = 1;
					}
				...
				case Exprent.EXPRENT_NEW:
					...
					if (isNewConcat(nex,
					                cltype)) {
						VarType[] params = nex.getConstructor()
		...
		return createConcatExprent(lstOperands,
		                           expr.bytecode);
	}

	private static Exprent createConcatExprent(List<Exprent> lstOperands,
	                                           BitSet bytecode) {
			...
			if (constant.type == CodeConstants.CONSTANT_String) {
				...
				for (int i = 0;
					...
					if (c == TAG_CONST || c == TAG_ARG) {
						...
						// as a constant first:
						if (acc.length() > 0) {
							res.add(new ConstExprent(VarType.VARTYPE_STRING,
							                         acc.toString(),
							                         expr.bytecode));
							acc.setLength(0);
						}
	...
	private static boolean isAppendConcat(InvocationExprent expr,
	                                      VarType cltype) {
	...
	private static boolean isNewConcat(NewExprent expr,
	                                   VarType cltype) {
		if (expr.getNewType()
		        .equals(cltype)) {
			VarType[] params = expr.getConstructor()
			                       .getDescriptor().params;
