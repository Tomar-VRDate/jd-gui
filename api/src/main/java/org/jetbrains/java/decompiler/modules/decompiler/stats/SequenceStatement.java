...
public class SequenceStatement
	...
	public SequenceStatement(List<? extends Statement> lst) {
		...
		lastBasicType = lst.get(lst.size() - 1)
		                   .getLastBasicType();
	...
	private SequenceStatement(Statement head,
	                          Statement tail) {
	...
	public static Statement isHead2Block(Statement head) {
		...
		if (edge != null && edge.getType() == StatEdge.TYPE_REGULAR) {
			...
			if (stat != head
			    && stat.getPredecessorEdges(StatEdge.TYPE_REGULAR)
			           .size() == 1
			    && !stat.isMonitorEnter()) {
				...
				if (stat.getLastBasicType() == Statement.LASTBASICTYPE_GENERAL) {
					if (DecHelper.checkStatementExceptions(Arrays.asList(head,
					                                                     stat))) {
						return new SequenceStatement(head,
						                             stat);
					}
				}
			}
	...
	public TextBuffer toJava(int indent) {
		...
		for (int i = 0;
			...
			TextBuffer str = ExprProcessor.jmpWrapper(st,
			                                          indent,
			                                          false);
			buf.append(str);
