...
public final class AssertProcessor {
	...
	private static final VarType CLASS_ASSERTION_ERROR = new VarType(CodeConstants.TYPE_OBJECT,
	                                                                 0,
	...
	public static void buildAssertions(ClassNode node) {
		...
		if (field != null) {
			...
			String key = InterpreterUtil.makeUniqueKey(field.getName(),
			                                           field.getDescriptor());
	...
	private static StructField findAssertionField(ClassNode node) {
		...
		for (StructField fd : wrapper.getClassStruct()
		                             .getFields()) {

			String keyField = InterpreterUtil.makeUniqueKey(fd.getName(),
			                                                fd.getDescriptor());
			...
			if (wrapper.getStaticFieldInitializers()
				...
				// access flags set
				if (fd.hasModifier(CodeConstants.ACC_STATIC) && fd.hasModifier(CodeConstants.ACC_FINAL) && (noSynthFlag
				                                                                                            || fd.isSynthetic())) {
					...
					if (VarType.VARTYPE_BOOLEAN.equals(fdescr.type)) {
						...
						Exprent initializer = wrapper.getStaticFieldInitializers()
						                             .getWithKey(keyField);
						if (initializer.type == Exprent.EXPRENT_FUNCTION) {
							...
							if (fexpr.getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT
							    && fexpr.getLstOperands()
							            .get(0).type == Exprent.EXPRENT_INVOCATION) {

								InvocationExprent invexpr = (InvocationExprent) fexpr.getLstOperands()
								                                                     .get(0);
								...
								if (invexpr.getInstance() != null
									...
									if (VarType.VARTYPE_CLASS.equals(cexpr.getConstType())) {
										...
										while (nd != null) {
											if (nd.getWrapper()
											      .getClassStruct().qualifiedName.equals(cexpr.getValue())) {
												break;
											}
											nd = nd.parent;
										}
	...
	private static boolean replaceAssertions(Statement statement,
	                                         String classname,
	                                         String key) {
	...
	private static boolean replaceAssertion(Statement parent,
	                                        IfStatement stat,
	                                        String classname,
	                                        String key) {
		...
		Object[] exprres = getAssertionExprent(stat.getHeadexprent()
		                                           .getCondition()
		                                           .copy(),
		                                       classname,
		                                       key,
		...
		lstParams.add(retcond == null
		              ? ascond
		              : retcond);
		if (!throwError.getLstParameters()
		               .isEmpty()) {
			lstParams.add(throwError.getLstParameters()
			                        .get(0));
		}
		...
		if (stat.iftype == IfStatement.IFTYPE_IFELSE || (first.getExprents() != null && !first.getExprents()
		                                                                                      .isEmpty())) {
			...
			if (first.getExprents() != null && !first.getExprents()
			                                         .isEmpty()) {
			...
			for (int i = 0;
				...
				sequence.getStats()
				        .get(i)
				        .addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
				                                   sequence.getStats()
				                                           .get(i),
				                                   sequence.getStats()
				                                           .get(i + 1)));
		...
		newstat.getVarDefinitions()
		       .addAll(stat.getVarDefinitions());
		parent.replaceStatement(stat,
		                        newstat);
	...
	private static Object[] getAssertionExprent(Exprent exprent,
	                                            String classname,
	                                            String key,
	                                            boolean throwInIf) {
		...
		if (exprent.type == Exprent.EXPRENT_FUNCTION) {
			...
			if (fexpr.getFuncType() == desiredOperation) {
				...
				for (int i = 0;
					...
					if (isAssertionField(param,
					                     classname,
					                     key,
					                     throwInIf)) {
						return new Object[]{fexpr.getLstOperands().get(1 - i),
						                    true};
					}
				}
				...
				for (int i = 0;
					...
					Object[] res = getAssertionExprent(param,
					                                   classname,
					                                   key,
					                                   throwInIf);
					if ((Boolean) res[1]) {
						...
						return new Object[]{fexpr,
						                    true};
					}
				}
			} else if (isAssertionField(fexpr,
			                            classname,
			                            key,
			                            throwInIf)) {
				// assert false;
		...
		return new Object[]{exprent,
		                    false};
	}

	private static boolean isAssertionField(Exprent exprent,
	                                        String classname,
	                                        String key,
	                                        boolean throwInIf) {
		if (throwInIf) {
			if (exprent.type == Exprent.EXPRENT_FUNCTION) {
				...
				if (fparam.getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT
					...
					FieldExprent fdparam = (FieldExprent) fparam.getLstOperands()
					                                            .get(0);
					return classname.equals(fdparam.getClassname()) && key.equals(InterpreterUtil.makeUniqueKey(fdparam.getName(),
					                                                                                            fdparam.getDescriptor().descriptorString));
				}
			}
		} else {
