...
public final class PatternMatchProcessor {
	public static boolean matchInstanceof(RootStatement root) {
		boolean res = matchInstanceofRec(root,
		                                 root);
	...
	private static boolean matchInstanceofRec(Statement statement,
	                                          RootStatement root) {
		boolean res = false;
		for (Statement stat : statement.getStats()) {
			if (matchInstanceofRec(stat,
			                       root)) {
				res = true;
			}
		}
		...
		if (statement instanceof IfStatement) {
			res |= handleIf((IfStatement) statement,
			                root);
		}
	...
	private static boolean handleIf(IfStatement statement,
	                                RootStatement root) {
		...
		for (Exprent exprent : exprents) {
			if (exprent.type == Exprent.EXPRENT_FUNCTION) {
				...
				if (iof.getFuncType() == FunctionExprent.FUNCTION_INSTANCEOF) {
					Exprent source = iof.getLstOperands()
					                    .get(0);
					Exprent target = iof.getLstOperands()
					                    .get(1);
					...
					if (statement.getIfstat() != null
					    && statement.getIfstat()
					                .getExprents() != null
					    && statement.getIfstat()
					                .getExprents()
					                .size() > 1) {
						Exprent first = statement.getIfstat()
						                         .getExprents()
						                         .get(0);
						...
						if (first.type == Exprent.EXPRENT_ASSIGNMENT) {
							...
							Exprent left = first.getAllExprents()
							                    .get(0);
							Exprent right = first.getAllExprents()
							                     .get(1);
							...
							if (right.type == Exprent.EXPRENT_FUNCTION) {
								if (((FunctionExprent) right).getFuncType() == FunctionExprent.FUNCTION_CAST) {
									Exprent casted = right.getAllExprents()
									                      .get(0);
									...
									if (source.equals(casted)) {
										...
										if (left.type == Exprent.EXPRENT_VAR && target.getExprType()
										                                              .equals(left.getExprType())) {
											List<VarVersionPair> vvs = new ArrayList<>();

											// We need to make sure we're not assigning to previously assigned variables.
											// This gets all predecessors of the if statement and gathers all the variable assignments
											// inside.
											// TODO: cache this
											findVarsInPredecessors(vvs,
											                       statement.getIfstat());
	...
	private static void findVarsInPredecessors(List<VarVersionPair> vvs,
	                                           Statement root) {
