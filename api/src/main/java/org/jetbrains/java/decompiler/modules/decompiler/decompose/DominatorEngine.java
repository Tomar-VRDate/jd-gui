...
public class DominatorEngine {
	...
	private static Integer getCommonIDom(Integer key1,
	                                     Integer key2,
	...
	private void orderStatements() {
		...
		for (Statement stat : statement.getReversePostOrderList()) {
			colOrderedIDoms.addWithKey(null,
			                           stat.id);
		}
	}
	...
	private void calcIDoms() {
		...
		colOrderedIDoms.putWithKey(statement.getFirst().id,
		                           statement.getFirst().id);
		...
		while (true) {
			...
			for (int id : lstIds) {
				...
				Statement stat = statement.getStats()
				                          .getWithKey(id);
				Integer idom = null;

				for (StatEdge edge : stat.getAllPredecessorEdges()) {
					if (colOrderedIDoms.getWithKey(edge.getSource().id) != null) {
						idom = getCommonIDom(idom,
						                     edge.getSource().id,
						                     colOrderedIDoms);
					}
				}

				Integer oldidom = colOrderedIDoms.putWithKey(idom,
				                                             id);
	...
	public boolean isDominator(Integer node,
	                           Integer dom) {
