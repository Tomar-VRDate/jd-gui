...
public class AssignmentExprent
	...
	public AssignmentExprent(Exprent left,
	                         Exprent right,
	...
	public CheckTypesResult checkExprTypeBounds() {
		...
		if (typeLeft.typeFamily > typeRight.typeFamily) {
			result.addMinTypeExprent(right,
			                         VarType.getMinTypeInFamily(typeLeft.typeFamily));
		} else if (typeLeft.typeFamily < typeRight.typeFamily) {
			result.addMinTypeExprent(left,
			                         typeRight);
		} else {
			result.addMinTypeExprent(left,
			                         VarType.getCommonSupertype(typeLeft,
			                                                    typeRight));
		}
	...
	@Override
	public Exprent copy() {
		return new AssignmentExprent(left.copy(),
		                             right.copy(),
		                             bytecode);
	}
	...
	public TextBuffer toJava(int indent) {
		...
		if (condType == CONDITION_NONE && !leftType.isSuperset(rightType) && (rightType.equals(VarType.VARTYPE_OBJECT)
		                                                                      || leftType.type
		...
		buffer.append(condType == CONDITION_NONE
		              ? " = "
		              : OPERATORS[condType])
		      .append(res);
	...
	public boolean equals(Object o) {
		...
		return InterpreterUtil.equalObjects(left,
		                                    as.getLeft())
		       && InterpreterUtil.equalObjects(right,
		                                       as.getRight())
