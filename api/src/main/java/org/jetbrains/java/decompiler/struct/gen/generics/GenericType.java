...
public class GenericType
	...
	public static final GenericType   DUMMY_VAR        = new GenericType(CodeConstants.TYPE_GENVAR,
	                                                                     0,
	                                                                     "",
	                                                                     null,
	                                                                     null,
		...
		while (index < signature.length()) {
			switch (signature.charAt(index)) {
				...
				case 'T':
					...
					value = signature.substring(index + 1,
					                            signature.length() - 1);
					break loop;
				...
				case 'L':
					...
					if (cl.length() == signature.length()) {
						...
						if (argStart >= 0) {
							value = cl.substring(0,
							                     argStart);
							params = parseArgumentsList(cl.substring(argStart + 1,
							                                         cl.length() - 1));
						} else {
							value = cl;
						}
						...
						while (signature.length() > 0) {
							...
							if (cl.length() == signature.length()) {
								...
								if (parent == null && params == null) {
									...
									parent = new GenericType(CodeConstants.TYPE_OBJECT,
									                         0,
									                         value,
									                         parent,
									                         params,
				...
				default:
					value = signature.substring(index,
					                            index + 1);
	...
	private static List<VarType> parseArgumentsList(String value) {
		...
		while (value.length() > 0) {
			...
			args.add(typeStr.length() == 0
			         ? null
			         : GenericType.parse(typeStr,
			                             wildcard));
	...
	public static VarType withWildcard(VarType var,
		...
		if (var.isGeneric()) {
			...
			return new GenericType(genVar.type,
			                       genVar.arrayDim,
			                       genVar.value,
			                       genVar.parent,
			                       genVar.arguments,
		...
		return new GenericType(var.type,
		                       var.arrayDim,
		                       var.value,
		                       null,
		                       Collections.emptyList(),
	...
	public static boolean isAssignable(VarType from,
			...
			if (from.type == CodeConstants.TYPE_GENVAR && from.type != to.type && named.containsKey(from)) {
				return named.get(from)
				            ...
				            .anyMatch(bound -> {
					            ...
					            return areArgumentsAssignable(bound,
					                                          to,
					                                          named);
				            });
			} else {
		...
		if (to.isGeneric() && !from.value.equals(to.value)) {
			VarType _new = getGenericSuperType(from,
			                                   to);
		...
		return areArgumentsAssignable(from,
		                              to,
		                              named);
	}
	...
	public static boolean areArgumentsAssignable(VarType from,
		...
		if (from.isGeneric() && to.isGeneric()) {
			...
			for (int i = 0;
				...
				int tWild = t.isGeneric()
				            ? ((GenericType) t).wildcard
				            : WILDCARD_NO;

				if (f == null) {
					StructClass cls = DecompilerContext.getStructContext()
					                                   .getClass(genFrom.value);
					VarType bounds = cls.getSignature().fbounds.get(i)
					                                           .get(0);
				...
				int fWild = f.isGeneric()
				            ? ((GenericType) f).wildcard
				            : WILDCARD_NO;

				if (tWild == WILDCARD_EXTENDS) {
					if (fWild == WILDCARD_SUPER || !DecompilerContext.getStructContext()
					                                                 .instanceOf(f.value,
					                                                             t.value)) {
						return false;
					}
				} else if (tWild == WILDCARD_SUPER) {
					if (fWild == WILDCARD_EXTENDS || !DecompilerContext.getStructContext()
					                                                   .instanceOf(t.value,
					                                                               f.value)) {
						return false;
					}
	...
	public static VarType getGenericSuperType(VarType derivedType,
	                                          VarType superType) {
		StructClass dcls = DecompilerContext.getStructContext()
		                                    .getClass(derivedType.value);
		StructClass scls = DecompilerContext.getStructContext()
		                                    .getClass(superType.value);
		...
		if (dcls != null && scls != null) {
			...
			if (hierarchy.containsKey(scls.qualifiedName) && scls.getSignature() != null) {
				...
				if (derivedType.isGeneric() && dcls.getSignature() != null) {
					dcls.getSignature().genericType.mapGenVarsTo((GenericType) derivedType,
					                                             tempMap);
					...
					for (int i = 0;
					     i < dcls.getSignature().fparameters.size();
					     ++i) {
						...
						if (tempMap.get(param) == null) {
							...
							if (!bounds.isEmpty()) {
								VarType replacement = bounds.get(0)
								                            .remap(tempMap);
				...
				return scls.getSignature().genericType.remap(hierarchy.get(scls.qualifiedName))
				                                      .remap(tempMap);
			}
		}
		return derivedType;
	...
	public String getCastName() {
		String clsName = null;
		if (parent == null) {
			clsName = DecompilerContext.getImportCollector()
			                           .getShortName(value.replace('/',
			                                                       '.'));
		} else if (parent.isGeneric()) {
			clsName = ((GenericType) parent).getCastName() + "." + value.substring(parent.value.length() + 1);
		} else {
			clsName = DecompilerContext.getImportCollector()
			                           .getShortName(parent.value.replace('/',
	...
	public VarType remap(Map<VarType, VarType> map) {
		...
		if (main != this) {
			int wild = main != null && main.isGeneric()
			           ? ((GenericType) main).getWildcard()
			           : WILDCARD_NO;
			if (main != null && getWildcard() != WILDCARD_NO && wild != getWildcard()) {
				main = withWildcard(main,
				                    getWildcard());
			}
			return main;
		}
		...
		if (changed) {
			return new GenericType(main.type,
			                       main.arrayDim,
			                       main.value,
			                       parent,
			                       newArgs,
	...
	public boolean equalsExact(Object o) {
		...
		if (type != gt.type || arrayDim != gt.arrayDim || wildcard != gt.wildcard || !InterpreterUtil.equalObjects(value,
		                                                                                                           gt.value)) {
			return false;
		}
	...
	public void mapGenVarsTo(GenericType other,
	                         Map<VarType, VarType> map) {
		if (arguments.size() == other.arguments.size()) {
			for (int i = 0;
				...
				if (thisArg != null && !DUMMY_VAR.equals(otherArg)) {
					if (thisArg.type == CodeConstants.TYPE_GENVAR) {
						...
						int oWild = otherArg == null || !otherArg.isGeneric()
						            ? WILDCARD_NO
						            : ((GenericType) otherArg).wildcard;

						if (tWild == oWild && tWild != WILDCARD_NO) {
							thisArg = withWildcard(thisArg,
							                       WILDCARD_NO);
							otherArg = withWildcard(otherArg,
							                        WILDCARD_NO);
						}
						...
						if (otherArg == null && thisArg.arrayDim == 0) {
							...
							if (!map.containsKey(thisArg)) {
								...
								int cWild = curr == null || !curr.isGeneric()
								            ? WILDCARD_NO
								            : ((GenericType) curr).wildcard;
								if (oWild != cWild) {
									map.put(thisArg,
									        withWildcard(otherArg,
									                     WILDCARD_NO));
								}
							}
						}
					} else if (thisArg.isGeneric() && otherArg != null && otherArg.isGeneric()) {
						((GenericType) thisArg).mapGenVarsTo((GenericType) otherArg,
						                                     map);
					}
				}
			}
			...
			if (other.parent != null && other.parent.isGeneric()) {
				GenericType parent = this.parent != null && this.parent.isGeneric()
				                     ? (GenericType) this.parent
				                     : null;
				if (parent == null) {
					StructClass cls = DecompilerContext.getStructContext()
					                                   .getClass(other.parent.value);
					parent = cls.getSignature().genericType;
				}

				parent.mapGenVarsTo((GenericType) other.parent,
				                    map);
			}
		}
	}
