...
public class ClasspathScanner {
			...
			for (final String path : prop.split(File.pathSeparator)) {
				...
				if (file.exists() && (file.getName()
					...
					DecompilerContext.getLogger()
					                 .writeMessage("Adding File to context from classpath: " + file,
					                               Severity.INFO);
					ctx.addSpace(file,
					             false);
	...
	private static void addAllModulePath(StructContext ctx) {
		for (ModuleReference module : ModuleFinder.ofSystem()
		                                          .findAll()) {
			String name = module.descriptor()
			                    .name();
			try {
				ModuleReader reader = module.open();
				DecompilerContext.getLogger()
				                 .writeMessage("Reading Module: " + name,
				                               Severity.INFO);
				reader.list()
				      .forEach(cls -> {
					      ...
					      try {
						      ...
						      if (!bb.isPresent()) {
							      DecompilerContext.getLogger()
							                       .writeMessage("    Error Reading Class: " + cls,
							                                     Severity.ERROR);
							      return;
						      }
				...
				DecompilerContext.getLogger()
				                 .writeMessage("Error loading module " + name,
				                               e);
			}
		}
	}
