...
public final class DoStatement
	...
	public static Statement isHead(Statement head) {
		...
		if (head.getLastBasicType() == LASTBASICTYPE_GENERAL && !head.isMonitorEnter()) {
			...
			// continues
			if (head.type != TYPE_DO && (edge == null || edge.getType() != StatEdge.TYPE_REGULAR) && head.getContinueSet()
			                                                                                             .contains(head.getBasichead())) {
				return new DoStatement(head);
			}
		}
	...
	public TextBuffer toJava(int indent) {
		...
		switch (looptype) {
			...
			case LOOP_DOWHILE:
				...
				buf.append(conditionExprent.get(0)
				                           .toJava(indent));
				buf.appendPossibleNewline("",
				                          true);
				buf.popNewlineGroup();
			...
			case LOOP_FOR:
				...
				if (initExprent.get(0) != null) {
					buf.append(initExprent.get(0)
					                      .toJava(indent));
				}
				buf.append(";")
				   .appendPossibleNewline(" ")
				   .append(conditionExprent.get(0)
				                           .toJava(indent))
				   .append(";")
				   .appendPossibleNewline(" ")
				   .append(incExprent.get(0)
				                     .toJava(indent))
				   .appendPossibleNewline("",
				                          true);
				buf.popNewlineGroup();
			...
			case LOOP_FOREACH:
				buf.appendIndent(indent)
				   .append("for(")
				   .append(initExprent.get(0)
				                      .toJava(indent));
				incExprent.get(0)
				          .getInferredExprType(null); //TODO: Find a better then null? For now just calls it to clear casts if
				// needed
				buf.append(" : ")
				   .append(incExprent.get(0)
				                     .toJava(indent))
