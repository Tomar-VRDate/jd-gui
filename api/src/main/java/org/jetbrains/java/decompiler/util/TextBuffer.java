...
public class TextBuffer {
	...
	private final String                           myIndent
	                                                                       =
					(String) DecompilerContext.getProperty(IFernflowerPreferences.INDENT_STRING);
	private final int                              myPreferredLineLength
	                                                                       =
					DecompilerContext.getIntOption(IFernflowerPreferences.PREFERRED_LINE_LENGTH);
	private final NewlineGroup                     myRootGroup             = new NewlineGroup(null,
	                                                                                          0,
	                                                                                          0,
	                                                                                          0);
	private final StringBuilder                    myStringBuilder;
	private final Map<BytecodeMappingKey, Integer> myBytecodeOffsetMapping = new LinkedHashMap<>();
	// bytecode offset -> offset in text
	private final DebugTrace                       myDebugTrace
	                                                                       =
					DecompilerContext.getOption(IFernflowerPreferences.UNIT_TEST_MODE)
	                                                                         ? new DebugTrace(this)
	                                                                         : null;
	 ...
	 * Possible newlines are grouped into newline groups. In a newline group, either none of the replacements are
	 * replaced
	 * with newlines, or all of them are (never only some of them). Possible newlines should not be added outside a
	 * group.
	...
	public TextBuffer pushNewlineGroup(int baseIndent,
	                                   int extraIndent) {
		NewlineGroup group = new NewlineGroup(myCurrentGroup,
	...
	public TextBuffer appendPossibleNewline(String alternative) {
		return appendPossibleNewline(alternative,
		                             false);
	}
	...
	public TextBuffer appendPossibleNewline(String alternative,
	                                        boolean dedent) {
		myCurrentGroup.myReplacements.add(new NewlineGroup.Replacement(myStringBuilder.length(),
		                                                               alternative.length(),
		                                                               dedent));
	...
	public TextBuffer enclose(String left,
	                          String right) {
		prepend(left);
		append(right);
		return this;
	...
	public void addBytecodeMapping(int bytecodeOffset) {
		...
		myBytecodeOffsetMapping.putIfAbsent(new BytecodeMappingKey(bytecodeOffset,
		                                                           null,
		                                                           null),
		                                    myStringBuilder.length());
	}
	...
	public void addBytecodeMapping(BitSet bytecodeOffsets) {
		...
		for (int i = bytecodeOffsets.nextSetBit(0);
		     i >= 0;
	...
	public void clearUnassignedBytecodeMappingData() {
		myBytecodeOffsetMapping.keySet()
		                       .removeIf(key -> key.myClass == null);
	}

	public Map<Pair<String, String>, BytecodeMappingTracer> getTracers() {
		List<Integer> newlineOffsets = new ArrayList<>();
		for (int i = myStringBuilder.indexOf(myLineSeparator);
		     i != -1;
		     i = myStringBuilder.indexOf(myLineSeparator,
		                                 i + 1)) {
		...
		myBytecodeOffsetMapping.forEach((key, textOffset) -> {
			if (key.myClass == null) {
				throw new IllegalStateException("getTracers called when not all bytecode offsets have a valid class and "
				                                + "method");
			}
			BytecodeMappingTracer tracer = tracers.computeIfAbsent(Pair.of(key.myClass,
			                                                               key.myMethod),
			                                                       k -> new BytecodeMappingTracer());
			int lineNo = Collections.binarySearch(newlineOffsets,
			                                      textOffset);
			if (lineNo < 0) {
				lineNo = -lineNo - 1;
			}
	 ...
	 */
	private void reformatGroup(NewlineGroup group,
	                           List<Integer> offsetMapping,
	                           int extraIndent) {
		...
		int lastNewline = myStringBuilder.lastIndexOf(myLineSeparator,
		                                              actualStart);
		int nextNewline = myStringBuilder.indexOf(myLineSeparator,
		                                          actualStart);
		// Find the end of the first line of this group, if the group were to not be reformatted
		int firstPartEnd = nextNewline == -1
		                   ? actualStart + group.myLength
		                   : Math.min(nextNewline,
		                              actualStart + group.myLength);
		// Go through all lines in the group, and find the longest one (without reformatting)
		int groupEndWithoutNewlines = lastNewline == -1
		                              ? firstPartEnd
		                              : firstPartEnd - lastNewline;
		while (nextNewline != -1 && nextNewline <= actualStart + group.myLength) {
			int lineStart = nextNewline;
			int lineEnd = nextNewline = myStringBuilder.indexOf(myLineSeparator,
			                                                    nextNewline + 1);
		...
		for (int pos = group.myStart;
		     pos <= group.myStart + group.myLength;
		     pos++) {
			...
			if (pos + offset + myLineSeparator.length() < myStringBuilder.length() && myStringBuilder.substring(pos + offset,
			                                                                                                    pos
			                                                                                                    + offset
			                                                                                                    + myLineSeparator.length())
			                                                                                         .equals(myLineSeparator)) {
				for (int i = 0;
				     i < extraIndent;
				     i++) {
					myStringBuilder.insert(pos + offset + myLineSeparator.length(),
					                       myIndent);
				}
			...
			while (anotherPass) {
				...
				if (addNewLines
				    && replacementIndex < group.myReplacements.size()
				    && pos == group.myReplacements.get(replacementIndex).myStart) {
					NewlineGroup.Replacement replacement = group.myReplacements.get(replacementIndex);
					myStringBuilder.replace(pos + offset,
					                        pos + offset + replacement.myLength,
					                        myLineSeparator);
					if (replacement.myDedent) {
						extraIndent = originalExtraIndent;
					}
					for (int i = 0;
					     i < group.myBaseIndent + extraIndent;
					     i++) {
						myStringBuilder.insert(pos + offset + myLineSeparator.length(),
						                       myIndent);
					}
					offset += myIndent.length() * (group.myBaseIndent + extraIndent) + myLineSeparator.length()
					          - replacement.myLength;
					replacementIndex++;
					anotherPass = true;
				}
				// offset may have changed, update the offset mapping here because the child groups rely on the value being
				// correct in the list
				offsetMapping.set(offsetMapping.size() - 1,
				                  offset);
		...
		if (!ALLOW_TO_STRING) {
			if (DecompilerContext.getOption(IFernflowerPreferences.UNIT_TEST_MODE)) {
				...
				DecompilerContext.getLogger()
				                 .writeMessage("Usage of TextBuffer.toString",
	...
	private String addOriginalLineNumbers() {
		...
		while ((lineEnd = myStringBuilder.indexOf(myLineSeparator,
		                                          lineStart)) > 0) {
			++count;
	...
	private void appendLines(StringBuilder res,
	                         String[] srcLines,
	                         int from,
	                         int to,
	                         int requiredLineNumber) {
	...
	private boolean currentGroupExists() {
		for (NewlineGroup group = myCurrentGroup;
		     group != myRootGroup;
	...
	public TextBuffer append(TextBuffer buffer,
		...
		if (buffer.myLineToOffsetMapping != null && !buffer.myLineToOffsetMapping.isEmpty()) {
			...
			for (Map.Entry<Integer, Integer> entry : buffer.myLineToOffsetMapping.entrySet()) {
				myLineToOffsetMapping.put(entry.getKey(),
				                          entry.getValue() + myStringBuilder.length());
			}
		}
		buffer.myBytecodeOffsetMapping.forEach((key, value) -> {
			if (key.myClass == null) {
				key = new BytecodeMappingKey(key.myBytecodeOffset,
				                             className,
				                             methodKey);
			}
			myBytecodeOffsetMapping.putIfAbsent(key,
			                                    value + myStringBuilder.length());
		});
	...
	public int count(String substring,
		...
		while ((p = myStringBuilder.indexOf(substring,
		                                    p)) > 0) {
			++count;
			p += length;
		}
	...
	public void dumpOriginalLineNumbers(int[] lineMapping) {
		if (lineMapping.length > 0) {
			...
			for (int i = 0;
			     i < lineMapping.length;
			     i += 2) {
				int key = lineMapping[i + 1];
				Set<Integer> existing = myLineMapping.computeIfAbsent(key,
				                                                      k -> new TreeSet<>());
	...
	private static final class BytecodeMappingKey {
		...
		public BytecodeMappingKey(int bytecodeOffset,
		                          String className,
		                          String methodKey) {
		...
		public boolean equals(Object o) {
			...
			return myBytecodeOffset == that.myBytecodeOffset && Objects.equals(myClass,
			                                                                   that.myClass) && Objects.equals(myMethod,
			                                                                                                   that.myMethod);
		}

		@Override
		public int hashCode() {
			return Objects.hash(myBytecodeOffset,
			                    myClass,
			                    myMethod);
		}
	...
	private static final class NewlineGroup {
		...
		NewlineGroup(NewlineGroup parent,
		             int start,
		             int baseIndent,
		             int extraIndent) {
		...
		void truncate(int stringLength) {
			...
			for (Iterator<NewlineGroup> itr = myChildren.iterator();
			     itr.hasNext(); ) {
		...
		NewlineGroup copy() {
			NewlineGroup copy = new NewlineGroup(myParent,
			                                     myStart,
			                                     myBaseIndent,
			                                     myExtraIndent);
	...
	// it's really important that this class does not directly or indirectly reference the TextBuffer, or we will create
	// memory leaks
	static class DebugTrace
		...
		private void onDeletion() {
			if (myPreventDeletion && (!DotExporter.DUMP_DOTS && !DotExporter.DUMP_ERROR_DOTS)) {
				throw new AssertionError("TextBuffer was garbage collected without being added to another TextBuffer, data "
				                         + "loss occurred. See cause for the creation trace",
				                         myCreationTrace);
			}
		}
	}
