...
public class StructInnerClassesAttribute
	...
	public void initContent(DataInputFullStream data,
		...
		if (len > 0) {
			...
			for (int i = 0;
				...
				String innerName = pool.getPrimitiveConstant(innerNameIdx)
				                       .getString();
				String outerName = outerNameIdx != 0
				                   ? pool.getPrimitiveConstant(outerNameIdx)
				                         .getString()
				                   : null;
				String simpleName = simpleNameIdx != 0
				                    ? pool.getPrimitiveConstant(simpleNameIdx)
				                          .getString()
				                    : null;

				entries.add(new Entry(outerNameIdx,
				                      simpleNameIdx,
				                      accessFlags,
				                      innerName,
				                      outerName,
	...
	public static final class Entry {
		...
		private Entry(int outerNameIdx,
		              int simpleNameIdx,
		              int accessFlags,
		              String innerName,
