...
public class SSAUConstructorSparseEx {
	...
	//private HashMap<String, HashMap<Integer, FastSet<Integer>>> inVarVersions = new HashMap<String, HashMap<Integer,
	// FastSet<Integer>>>();

	// node id, var, version (direct branch)
	private final HashMap<String, SFormsFastMapDirect> outVarVersions = new HashMap<>();
	//private HashMap<String, HashMap<Integer, FastSet<Integer>>> outVarVersions = new HashMap<String, HashMap<Integer,
	// FastSet<Integer>>>();

	// node id, var, version (negative branch)
	private final HashMap<String, SFormsFastMapDirect> outNegVarVersions = new HashMap<>();
	//private HashMap<String, HashMap<Integer, FastSet<Integer>>> outNegVarVersions = new HashMap<String,
	// HashMap<Integer, FastSet<Integer>>>();

	// node id, var, version
	private final HashMap<String, SFormsFastMapDirect> extraVarVersions = new HashMap<>();
	//private HashMap<String, HashMap<Integer, FastSet<Integer>>> extraVarVersions = new HashMap<String,
	// HashMap<Integer, FastSet<Integer>>>();
	...
	private final HashMap<VarVersionPair, VarVersionPair> varAssignmentMap = new HashMap<>();
	...
	private static boolean mapsEqual(SFormsFastMapDirect map1,
	                                 SFormsFastMapDirect map2) {
		...
		for (Entry<Integer, FastSparseSet<Integer>> ent2 : map2.entryList()) {
			if (!InterpreterUtil.equalObjects(map1.get(ent2.getKey()),
			                                  ent2.getValue())) {
				return false;
			}
		}
	...
	public void splitVariables(RootStatement root,
	                           StructMethod mt) {

		FlattenStatementsHelper flatthelper = new FlattenStatementsHelper();
		DirectGraph             dgraph      = flatthelper.buildDirectGraph(root);

		DotExporter.toDotFile(dgraph,
		                      mt,
		                      "ssauSplitVariables");
		...
		extraVarVersions.put(dgraph.first.id,
		                     createFirstMap(mt,
		                                    root));
	...
	private void ssaStatements(DirectGraph dgraph,
		...
		for (DirectNode node : dgraph.nodes) {
			...
			SFormsFastMapDirect[] varmaparr = new SFormsFastMapDirect[]{varmap,
			                                                            null};
			...
			// quick solution: 'dummy' field variables should not cross basic block borders (otherwise problems e.g. with
			// finally loops - usage without assignment in a loop)
			// For the full solution consider adding a dummy assignment at the entry point of the method
			boolean allow_field_propagation = node.succs.isEmpty() || (node.succs.size() == 1
			                                                           && node.succs.get(0).preds.size() == 1);
			...
			boolean this_updated = !mapsEqual(varmaparr[0],
			                                  outVarVersions.get(node.id)) || (outNegVarVersions.containsKey(node.id)
			                                                                   && !mapsEqual(varmaparr[1],
			                                                                                 outNegVarVersions.get(node.id)));

			if (this_updated) {

				outVarVersions.put(node.id,
				                   varmaparr[0]);
				if (dgraph.mapNegIfBranch.containsKey(node.id)) {
					outNegVarVersions.put(node.id,
					                      varmaparr[1]);
				}
	...
	private void processExprent(Exprent expr,
	                            SFormsFastMapDirect[] varmaparr,
	                            Statement stat,
	                            boolean calcLiveVars) {
			...
			case Exprent.EXPRENT_FUNCTION:
				...
				switch (func.getFuncType()) {
					case FunctionExprent.FUNCTION_IIF:
						processExprent(func.getLstOperands()
						                   .get(0),
						               varmaparr,
						               stat,
						               calcLiveVars);
						...
						SFormsFastMapDirect[] varmaparrNeg = new SFormsFastMapDirect[]{varmapFalse,
						                                                               null};
						processExprent(func.getLstOperands()
						                   .get(2),
						               varmaparrNeg,
					...
					case FunctionExprent.FUNCTION_CADD:
						...
						SFormsFastMapDirect[] varmaparrAnd = new SFormsFastMapDirect[]{new SFormsFastMapDirect(varmaparr[0]),
						                                                               null};
						...
						// false map
						varmaparr[1] = mergeMaps(varmaparr[varmaparr[1] == null
						                                   ? 0
						                                   : 1],
						                         varmaparrAnd[varmaparrAnd[1] == null
						                                      ? 0
						                                      : 1]);
						// true map
					...
					case FunctionExprent.FUNCTION_COR:
						...
						SFormsFastMapDirect[] varmaparrOr =
										new SFormsFastMapDirect[]{new SFormsFastMapDirect(varmaparr[varmaparr[1]
						                                                                                                == null
						                                                                                                ? 0
						                                                                                                : 1]),
						                                                              null};
						...
						// false map
						varmaparr[1] = varmaparrOr[varmaparrOr[1] == null
						                           ? 0
						                           : 1];
						// true map
						varmaparr[0] = mergeMaps(varmaparr[0],
						                         varmaparrOr[0]);
		...
		if (expr.type == Exprent.EXPRENT_FIELD) {
			...
			if (mapFieldVars.containsKey(expr.id)) {
				...
				mapFieldVars.put(expr.id,
				                 index);

				// ssu graph
				ssuversions.createNode(new VarVersionPair(index,
				                                          1));
			}

			setCurrentVar(varmap,
			              index,
			              1);
		} else if (expr.type == Exprent.EXPRENT_INVOCATION
		           || (expr.type == Exprent.EXPRENT_ASSIGNMENT
		               && ((AssignmentExprent) expr).getLeft().type
		                  == Exprent.EXPRENT_FIELD)
		           || (expr.type
		               == Exprent.EXPRENT_NEW
		               && ((NewExprent) expr).getNewType().type
		                  == CodeConstants.TYPE_OBJECT)
			...
			if (expr.type == Exprent.EXPRENT_FUNCTION) {
				...
				if (fexpr.getFuncType() >= FunctionExprent.FUNCTION_IMM
					...
					if (fexpr.getLstOperands()
					         .get(0).type == Exprent.EXPRENT_FIELD) {
						ismmpp = true;
					}
				}
			}
		...
		if (varassign != null) {
			...
			if (varassign.getVersion() == 0) {
				...
				// ssu graph
				ssuversions.createNode(new VarVersionPair(varindex,
				                                          nextver),
				                       varassign.getLVT());
				...
				if (calcLiveVars) {
					varMapToGraph(new VarVersionPair(varindex,
					                                 varassign.getVersion()),
					              varmap);
				}
				setCurrentVar(varmap,
				              varindex,
			...
			if (assexpr.getRight().type == Exprent.EXPRENT_VAR) {
				...
				varAssignmentMap.put(varassign.getVarVersionPair(),
				                     rightpaar);
			} else if (assexpr.getRight().type == Exprent.EXPRENT_FIELD) {
				int index = mapFieldVars.get(assexpr.getRight().id);
				VarVersionPair rightpaar = new VarVersionPair(index,
				                                              0);
				varAssignmentMap.put(varassign.getVarVersionPair(),
				                     rightpaar);
			}
			...
			switch (func.getFuncType()) {
				...
				case FunctionExprent.FUNCTION_PPI:
					...
					if (func.getLstOperands()
						...
						VarExprent var = (VarExprent) func.getLstOperands()
						                                  .get(0);
						int varindex = var.getIndex();
						VarVersionPair varpaar = new VarVersionPair(varindex,
						                                            var.getVersion());
						...
						if (phantomver == null) {
							...
							int nextver = getNextFreeVersion(varindex,
							                                 null);
							phantomver = new VarVersionPair(varindex,
							                                nextver);
							...
							if (vernode.preds.size() == 1) {
								vers.add(vernode.preds.iterator()
								                      .next().source.version);
							} else {
								for (VarVersionEdge edge : vernode.preds) {
									vers.add(edge.source.preds.iterator()
									                          .next().source.version);
								}
							}
							vers.add(nextver);
							createOrUpdatePhiNode(varpaar,
							                      vers,
							                      stat);
							phantomppnodes.put(varpaar,
							                   phantomver);
						}
						if (calcLiveVars) {
							varMapToGraph(varpaar,
							              varmap);
						}
						setCurrentVar(varmap,
						              varindex,
			...
			if (cardinality == 1) { // size == 1
				if (current_vers != 0) {
					...
					// split last version
					int usever = getNextFreeVersion(varindex,
					                                stat);
					...
					// ssu graph
					int lastver = vers.iterator()
					                  .next();
					VarVersionNode prenode = ssuversions.nodes.getWithKey(new VarVersionPair(varindex,
					                                                                         lastver));
					VarVersionNode usenode = ssuversions.createNode(new VarVersionPair(varindex,
					                                                                   usever));
					VarVersionEdge edge = new VarVersionEdge(VarVersionEdge.EDGE_GENERAL,
					                                         prenode,
					                                         usenode);
				...
				createOrUpdatePhiNode(new VarVersionPair(varindex,
				                                         current_vers),
				                      vers,
				                      stat);
	...
	private void createOrUpdatePhiNode(VarVersionPair phivar,
	                                   FastSparseSet<Integer> vers,
	                                   Statement stat) {
		...
		int ppvers = phantomppnodes.containsKey(phivar)
		             ? phantomppnodes.get(phivar).version
		             : -1;
		...
		if (lstPreds.size() == 1) {
			...
			for (VarVersionEdge edge : lstPreds) {
				int verssrc = edge.source.preds.iterator()
				                               .next().source.version;
		...
		for (int ver : vers) {
			...
			int tempver = getNextFreeVersion(phivar.var,
			                                 stat);

			VarVersionNode tempnode = new VarVersionNode(phivar.var,
			                                             tempver);

			colnodes.add(tempnode);
			colpaars.add(new VarVersionPair(phivar.var,
			                                tempver));
			...
			edge = new VarVersionEdge(VarVersionEdge.EDGE_GENERAL,
			                          tempnode,
			                          phinode);
		...
		ssuversions.addNodes(colnodes,
		                     colpaars);
	}

	private void varMapToGraph(VarVersionPair varpaar,
	...
	private int getNextFreeVersion(int var,
		...
		if (stat != null) { // null iff phantom version
			...
			if (firstRangeId != null) {
				mapVersionFirstRange.put(new VarVersionPair(var,
				                                            nextver),
				                         firstRangeId);
			}
		}
	...
	private void mergeInVarMaps(DirectNode node,
	                            DirectGraph dgraph) {
		...
		for (DirectNode pred : node.preds) {
			SFormsFastMapDirect mapOut = getFilteredOutMap(node.id,
			                                               pred.id,
			                                               dgraph,
			                                               node.id);
			if (mapNew.isEmpty()) {
		...
		inVarVersions.put(node.id,
		                  mapNew);
	}

	private SFormsFastMapDirect getFilteredOutMap(String nodeid,
	                                              String predid,
	                                              DirectGraph dgraph,
	                                              String destid) {
		...
		if (nodeid.equals(dgraph.mapNegIfBranch.get(predid))) {
			if (outNegVarVersions.containsKey(predid)) {
				mapNew = outNegVarVersions.get(predid)
				                          .getCopy();
			}
		} else if (outVarVersions.containsKey(predid)) {
			mapNew = outVarVersions.get(predid)
			                       .getCopy();
		}
		...
		if (isFinallyExit) {
			...
			for (FinallyPathWrapper finwrap : dgraph.mapShortRangeFinallyPaths.get(predid)) {
				...
				if (recFinally) {
					// recursion
					map = getFilteredOutMap(finwrap.entry,
					                        finwrap.source,
					                        dgraph,
					                        destid);
				} else {
				...
				if (!mapTrueSource.isEmpty() && !mapNew.isEmpty()) { // FIXME: what for??
					...
					for (Entry<Integer, FastSparseSet<Integer>> ent : mapExitVar.entryList()) {
						for (int version : ent.getValue()) {
							...
							VarVersionPair exitvar = new VarVersionPair(varindex,
							                                            version);
							...
							if (phantomvar == null) {
								int newversion = getNextFreeVersion(exitvar.var,
								                                    null);
								phantomvar = new VarVersionPair(exitvar.var,
								                                newversion);
								...
								VarVersionEdge edge = new VarVersionEdge(VarVersionEdge.EDGE_PHANTOM,
								                                         exitnode,
								                                         phantomnode);
								exitnode.addSuccessor(edge);
								phantomnode.addPredecessor(edge);

								mapPhantom.put(exitvar,
								               phantomvar);
							}
	...
	private void setCurrentVar(SFormsFastMapDirect varmap,
	                           int var,
	                           int vers) {
	...
	private void setCatchMaps(Statement stat,
	                          DirectGraph dgraph,
		...
		switch (stat.type) {
			...
			case Statement.TYPE_TRYCATCH:
				...
				for (int i = 1;
					...
					extraVarVersions.put(dgraph.nodes.getWithKey(flatthelper.getMapDestinationNodes()
					                                                        .get(stat.getStats()
					                                                                 .get(i).id)[0]).id,
					                     map);
	...
	private SFormsFastMapDirect createFirstMap(StructMethod mt,
		...
		int paramcount = md.params.length + (thisvar
		                                     ? 1
		                                     : 0);
