...
public abstract class Exprent
	...
	public Exprent(int type) {
		...
		this.id = DecompilerContext.getCounterContainer()
		                           .getCounterAndIncrement(CounterContainer.EXPRESSION_COUNTER);
	}

	protected static void measureBytecode(BitSet values,
	                                      Exprent exprent) {
		if (exprent != null) {
	...
	public static List<? extends Exprent> sortIndexed(List<? extends Exprent> lst) {
		...
		Comparator<VarExprent> comp = new Comparator<VarExprent>() {
			public int compare(VarExprent o1,
			                   VarExprent o2) {
	...
	public final List<Exprent> getAllExprents(boolean recursive) {
		...
		getAllExprents(recursive,
		               lst);

		return lst;
	}

	private List<Exprent> getAllExprents(boolean recursive,
	                                     List<Exprent> list) {
		int start = list.size();
		getAllExprents(list);
		int end = list.size();
	...
	protected void gatherGenerics(VarType upperBound,
	                              VarType ret,
	                              Map<VarType, VarType> genericsMap) {
		// List<T> -> List<String>
		if (upperBound != null && upperBound.isGeneric() && ret.isGeneric() && upperBound.arrayDim == ret.arrayDim) {
			int left = ((GenericType) upperBound).getArguments()
			                                     .size();
			int right = ((GenericType) ret).getArguments()
			                               .size();
			if (left == right) {
				((GenericType) ret).mapGenVarsTo((GenericType) upperBound,
				                                 genericsMap);
			}
		}
	}

	protected void getGenericArgs(List<String> fparams,
	                              Map<VarType, VarType> genericsMap,
	                              List<VarType> genericArgs) {
	...
	protected Map<VarType, List<VarType>> getNamedGenerics() {
		...
		while (true) {
			GenericClassDescriptor cls = class_ == null
			                             ? null
			                             : class_.classStruct.getSignature();
			if (cls != null) {
				for (int x = 0;
				     x < cls.fparameters.size();
				     x++) {
					ret.put(GenericType.parse("T" + cls.fparameters.get(x) + ";"),
					        cls.fbounds.get(x));
				}
			}

			GenericMethodDescriptor mtd = method == null
			                              ? null
			                              : method.methodStruct.getSignature();
			if (mtd != null) {
				for (int x = 0;
				     x < mtd.typeParameters.size();
				     x++) {
					ret.put(GenericType.parse("T" + mtd.typeParameters.get(x) + ";"),
					        mtd.typeParameterBounds.get(x));
				}
			}
			...
			method = class_.enclosingMethod == null
			         ? null
			         : class_.parent.getWrapper()
			                        .getMethods()
			                        .getWithKey(class_.enclosingMethod);
			class_ = class_.parent;
		}
		return ret;
	}

	protected void wrapInCast(VarType left,
	                          VarType right,
	                          TextBuffer buf,
	                          int precedence) {
		boolean needsCast = !left.isSuperset(right) && (right.equals(VarType.VARTYPE_OBJECT)
		                                                || left.type != CodeConstants.TYPE_OBJECT);
		...
		if (left.isGeneric() || right.isGeneric()) {
			...
			if (types != null) {
				...
				for (VarType type : types) {
					...
					anyMatch |= right.value == null /*null const doesn't need cast*/ || DecompilerContext.getStructContext()
	...
	public IMatchable findObject(MatchNode matchNode,
		...
		if (position != null) {
			if (position.matches("-?\\d+")) {
				return lstAllExprents.get((lstAllExprents.size() + Integer.parseInt(position))
				                          % lstAllExprents.size()); // care for negative positions
			}
	...
	public boolean match(MatchNode matchNode,
		...
		for (Entry<MatchProperties, RuleValue> rule : matchNode.getRules()
			...
			if (key == MatchProperties.EXPRENT_RET && !engine.checkAndSetVariableValue((String) rule.getValue().value,
			                                                                           this)) {
				return false;
			}
		}
