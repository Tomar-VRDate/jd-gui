...
public class JavaFileTypeFactoryProvider
	...
	@Override
	public Collection<Type> make(API api,
	...
	protected static class JavaMethod
		...
		public JavaMethod(JavaType type,
		                  int access,
		                  String name,
		                  String descriptor) {
			this.type = type;
	...
	protected static class Listener
		...
		protected void enterTypeDeclaration(ParserRuleContext ctx,
		                                    int access) {
			String name = ctx.getToken(JavaParser.Identifier,
			                           0)
			                 .getText();
			...
			if (superType == null) {
				superQualifiedTypeName = ((access & JavaType.FLAG_INTERFACE) == 0)
				                         ? "java/lang/Object"
				                         : "";
			} else {
				superQualifiedTypeName = resolveInternalTypeName(superType.classOrInterfaceType()
				                                                          .Identifier());
			}
			...
			if (currentType == null) {
				String internalTypeName = packageName.isEmpty()
				                          ? name
				                          : packageName + "/" + name;
				...
				currentType = new JavaType(access,
				                           internalTypeName,
				                           ...
				                           displayPackageName,
				                           null);
				types.put(internalTypeName,
				          currentType);
				rootTypes.add(currentType);
				nameToInternalTypeName.put(name,
				                           internalTypeName);
				...
				JavaType subType = new JavaType(access,
				                                internalTypeName,
				                                superQualifiedTypeName,
				                                outerName,
				                                displayTypeName,
		...
		public void enterClassBodyDeclaration(JavaParser.ClassBodyDeclarationContext ctx) {
			if (ctx.getChildCount() == 2) {
				...
				if (first instanceof TerminalNode) {
					if (((TerminalNode) first).getSymbol()
					                          .getType() == JavaParser.STATIC) {
						currentType.getMethods()
						           .add(new JavaMethod(currentType,
						                               JavaType.FLAG_STATIC,
						                               "<clinit>",
						                               "()V"));
					}
				}
		...
		public void enterConstDeclaration(JavaParser.ConstDeclarationContext ctx) {
			...
			int access = getClassBodyDeclarationAccessFlag(ctx.getParent()
			                                                  .getParent());
		...
		protected String createParamDescriptors(JavaParser.FormalParameterListContext formalParameterList) {
			...
			if (formalParameterList != null) {
				...
				for (JavaParser.FormalParameterContext formalParameter : formalParameters) {
					...
					paramDescriptors.append(createDescriptor(formalParameter.type(),
		...
		protected int getAccessFlag(JavaParser.ClassOrInterfaceModifierContext ctx) {
			if (ctx.getChildCount() == 1) {
				...
				if (first instanceof TerminalNode) {
					switch (((TerminalNode) first).getSymbol()
					                              .getType()) {
						case JavaParser.STATIC:
							return JavaType.FLAG_STATIC;
						case JavaParser.FINAL:
							return JavaType.FLAG_FINAL;
						case JavaParser.ABSTRACT:
							return JavaType.FLAG_ABSTRACT;
						case JavaParser.PUBLIC:
							return JavaType.FLAG_PUBLIC;
						case JavaParser.PROTECTED:
							return JavaType.FLAG_PROTECTED;
						case JavaParser.PRIVATE:
							return JavaType.FLAG_PRIVATE;
					}
				}
			}
