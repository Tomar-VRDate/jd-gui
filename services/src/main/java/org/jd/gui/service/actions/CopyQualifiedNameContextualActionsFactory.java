...
public class CopyQualifiedNameContextualActionsFactory
	...
	public Collection<Action> make(API api,
	                               Container.Entry entry,
	                               String fragment) {
		return Collections.singletonList(new CopyQualifiedNameAction(api,
		                                                             entry,
		                                                             fragment));
	}
	...
	public static class CopyQualifiedNameAction
		...
		protected static final ImageIcon ICON = new ImageIcon(CopyQualifiedNameAction.class.getClassLoader()
		...
		public CopyQualifiedNameAction(API api,
		                               Container.Entry entry,
		                               String fragment) {
			this.api = api;
			this.entry = entry;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (typeFactory != null) {
				...
				if (type != null) {
					...
					if (fragment != null) {
						...
						if (dashIndex != -1) {
							...
							if (dashIndex == lastDashIndex) {
								...
								String name = fragment.substring(dashIndex + 1,
								                                 lastDashIndex);
								...
								if (descriptor.startsWith("(")) {
									for (Type.Method method : type.getMethods()) {
										if (method.getName()
										          .equals(name) && method.getDescriptor()
										                                 .equals(descriptor)) {
											sb.append('.')
									...
									for (Type.Field field : type.getFields()) {
										if (field.getName()
										         .equals(name) && field.getDescriptor()
										                               .equals(descriptor)) {
											sb.append('.')
					...
					Toolkit.getDefaultToolkit()
					       .getSystemClipboard()
					       .setContents(new StringSelection(sb.toString()),
					                    null);
					return;
				}
			}
			...
			String path = entry.getUri()
			                   .getPath();
			String rootPath = entry.getContainer()
			                       .getRoot()
			                       .getUri()
			                       .getPath();
			...
			Toolkit.getDefaultToolkit()
			       .getSystemClipboard()
			       .setContents(new StringSelection(qualifiedName),
			                    null);
		}
	}
}
