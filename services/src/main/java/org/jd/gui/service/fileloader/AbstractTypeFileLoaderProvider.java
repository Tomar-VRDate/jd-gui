...
public abstract class AbstractTypeFileLoaderProvider
	...
	protected boolean load(API api,
	                       File file,
	                       String pathInFile) {
		// Search root path
		...
		if (!pathSuffix.isEmpty()) {
			...
			// Create panel
			JComponent mainPanel = load(api,
			                            rootFile,
			                            Paths.get(rootFile.toURI()));

			if (mainPanel instanceof UriOpenable) {
				try {
					// Open page
					pathSuffix = file.getAbsolutePath()
					                 .substring(rootFile.getAbsolutePath()
					                                    .length())
					                 .replace(File.separator,
					                          "/");
					URI rootUri = rootFile.toURI();
					URI uri = new URI(rootUri.getScheme(),
					                  rootUri.getHost(),
					                  rootUri.getPath() + '!' + pathSuffix,
					                  null);
