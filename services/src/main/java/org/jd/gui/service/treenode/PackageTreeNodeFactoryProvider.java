...
public class PackageTreeNodeFactoryProvider
	...
	protected static final ImageIcon ICON = new ImageIcon(PackageTreeNodeFactoryProvider.class.getClassLoader()
	...
	public <T extends DefaultMutableTreeNode & ContainerEntryGettable & UriGettable> T make(API api,
		...
		while (entries.size() == 1) {
			Container.Entry child = entries.iterator()
			                               .next();
			if (!child.isDirectory() || (api.getTreeNodeFactory(child) != this) || (entry.getContainer()
			                                                                        != child.getContainer())) {
				break;
			}
			entry = child;
