...
public abstract class AbstractXmlPathFinder {
	...
	public AbstractXmlPathFinder(Collection<String> paths) {
		for (String path : paths) {
			if ((path != null) && (path.length() > 0)) {
				...
				if (setOfPaths == null) {
					tagNameToPaths.put(lastTagName,
			...
			while (reader.hasNext()) {
				...
				switch (reader.getEventType()) {
					case XMLStreamReader.START_ELEMENT:
						sb.append('/')
						  .append(tagName = reader.getLocalName());
						offset = reader.getLocation()
						               .getCharacterOffset();
						break;
					case XMLStreamReader.END_ELEMENT:
						sb.setLength(sb.length() - reader.getLocalName()
						                                 .length() - 1);
						break;
	...
	public abstract void handle(String path,
	                            String text,
	                            int position);
}
