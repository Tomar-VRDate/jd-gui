...
public abstract class TypePage
	...
	public static void matchFragmentAndAddDocumentRange(String fragment,
	                                                    HashMap<String, DeclarationData> declarations,
		...
		if ((fragment.indexOf('?') != -1) || (fragment.indexOf('*') != -1)) {
			...
			if (lastDash == -1) {
				...
				for (Map.Entry<String, DeclarationData> entry : declarations.entrySet()) {
					if (entry.getKey()
					         .endsWith(slashAndTypeName) || entry.getKey()
					                                             .equals(typeName)) {
						ranges.add(new DocumentRange(entry.getValue().startPosition,
						                             entry.getValue().endPosition));
					}
				}
			} else {
				String prefix = fragment.substring(0,
				                                   lastDash + 1);
				...
				if (fragment.charAt(0) == '*') {
					...
					for (Map.Entry<String, DeclarationData> entry : declarations.entrySet()) {
						...
						if ((key.indexOf(slashAndTypeNameAndName) != -1) || (key.startsWith(typeNameAndName))) {
							...
							if (matchDescriptors.apply(suffix,
							                           key.substring(index))) {
								ranges.add(new DocumentRange(entry.getValue().startPosition,
								                             entry.getValue().endPosition));
							}
			...
			if (data != null) {
				ranges.add(new DocumentRange(data.startPosition,
				                             data.endPosition));
			} else if (fragment.endsWith("-<clinit>-()V")) {
				// 'static' bloc not found ==> Select type declaration
				String typeName = fragment.substring(0,
				                                     fragment.indexOf('-'));
	...
	public static void matchQueryAndAddDocumentRange(Map<String, String> parameters,
	                                                 HashMap<String, DeclarationData> declarations,
	                                                 TreeMap<Integer, HyperlinkData> hyperlinks,
	                                                 ArrayList<StringData> strings,
	                                                 List<DocumentRange> ranges) {
			...
			if (highlightFlags.indexOf('s') != -1) {
				...
				for (StringData data : strings) {
					if (matchScope(highlightScope,
						...
						while (matcher.find()) {
							ranges.add(new DocumentRange(offset + matcher.start(),
							                             offset + matcher.end()));
						}
					}
				}
				...
				for (Map.Entry<String, DeclarationData> entry : declarations.entrySet()) {
					...
					if (matchScope(highlightScope,
						...
						if ((t && declaration.isAType()) || (c && declaration.isAConstructor())) {
							matchAndAddDocumentRange(pattern,
							                         getMostInnerTypeName(declaration.typeName),
				...
				for (Map.Entry<Integer, HyperlinkData> entry : hyperlinks.entrySet()) {
					...
					if (matchScope(highlightScope,
						...
						if ((t && reference.isAType()) || (c && reference.isAConstructor())) {
							matchAndAddDocumentRange(pattern,
							                         getMostInnerTypeName(reference.typeName),
							                         hyperlink.startPosition,
							                         hyperlink.endPosition,
							                         ranges);
						}
	...
	public static boolean matchScope(String scope,
	                                 String type) {
	...
	public static void matchAndAddDocumentRange(Pattern pattern,
	                                            String text,
	                                            int start,
	                                            int end,
	                                            List<DocumentRange> ranges) {
		if (pattern.matcher(text)
		           .matches()) {
			ranges.add(new DocumentRange(start,
			                             end));
		}
	}
		...
		if (hyperlinkReferenceData.reference.enabled) {
			try {
				...
				if (entries.contains(entry)) {
					api.openURI(new URI(uri.getScheme(),
					                    uri.getAuthority(),
					                    uri.getPath(),
					                    fragment));
				} else {
	...
	public boolean openUri(URI uri) {
		...
		if (fragment != null) {
			matchFragmentAndAddDocumentRange(fragment,
			                                 declarations,
			                                 ranges);
		}
		...
		if (query != null) {
			...
			if (parameters.containsKey("lineNumber")) {
				...
				try {
					...
					if (textArea.getDocument()
					            .getLength() > pos) {
						ranges.add(new DocumentRange(pos,
						                             pos));
					}
				} catch (NumberFormatException e) {
					assert ExceptionUtil.printStackTrace(e);
				...
				matchQueryAndAddDocumentRange(parameters,
				                              declarations,
				                              hyperlinks,
				                              strings,
				                              ranges);
	...
	public void indexesChanged(Collection<Future<Indexes>> collectionOfFutureIndexes) {
		...
		for (ReferenceData reference : references) {
			...
			if (reference.name == null) {
				...
				try {
					// Recursive search
					typeName = searchTypeHavingMember(typeName,
					                                  reference.name,
	...
	@SuppressWarnings("unchecked")
	protected String searchTypeHavingMember(String typeName,
	                                        String name,
	                                        String descriptor,
	                                        Container.Entry entry) {
		...
		if (sameContainerEntries.size() > 0) {
			return searchTypeHavingMember(typeName,
			                              name,
			                              descriptor,
			                              sameContainerEntries);
		} else {
			...
			if (type != null) {
				if (descriptor.indexOf('(') == -1) {
					...
					for (Type.Field field : type.getFields()) {
						if (field.getName()
						         .equals(name) && DescriptorMatcher.matchFieldDescriptors(field.getDescriptor(),
						                                                                  descriptor)) {
							// Field found
							return typeName;
						}
					...
					for (Type.Method method : type.getMethods()) {
						if (method.getName()
						          .equals(name) && DescriptorMatcher.matchMethodDescriptors(method.getDescriptor(),
						                                                                    descriptor)) {
							// Method found
							return typeName;
						}
				...
				String typeOwnerName = searchTypeHavingMember(type.getSuperName(),
				                                              name,
				                                              descriptor,
				                                              entry);
	...
	public static class StringData {
		...
		public StringData(int startPosition,
		                  int length,
		                  String text,
		                  String owner) {
	...
	public static class DeclarationData {
		...
		public DeclarationData(int startPosition,
		                       int length,
		                       String typeName,
		                       String name,
		                       String descriptor) {
	...
	public static class HyperlinkReferenceData
		...
		public HyperlinkReferenceData(int startPosition,
		                              int length,
	...
	protected static class ReferenceData {
		 ...
		 * Used to high light items matching with URI like
		 * "file://dir1/dir2/file?highlightPattern=hello&highlightFlags=drtcmfs&highlightScope=type".
		 */
		public String  owner;
		/**
		...
		public ReferenceData(String typeName,
		                     String name,
		                     String descriptor,
		                     String owner) {
			this.typeName = typeName;
