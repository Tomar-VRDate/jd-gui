...
public abstract class HyperlinkPage
	...
	public HyperlinkPage() {
		MouseAdapter listener = new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if ((e.getClickCount() == 1) && ((e.getModifiers() & (Event.ALT_MASK | Event.META_MASK | Event.SHIFT_MASK))
				                                 == 0)) {
					int offset = textArea.viewToModel(new Point(e.getX(),
					                                            e.getY()));
					if (offset != -1) {
						...
						if (entry != null) {
							...
							if ((entryData != null)
							    && (offset < entryData.endPosition)
							    && (offset >= entryData.startPosition)
							    && isHyperlinkEnabled(entryData)) {
								openHyperlink(e.getXOnScreen(),
								              e.getYOnScreen(),
								              entryData);
							}
						}
	...
	public void addHyperlink(HyperlinkData hyperlinkData) {
		hyperlinks.put(hyperlinkData.startPosition,
		               hyperlinkData);
	}
	...
	protected abstract void openHyperlink(int x,
	                                      int y,
	...
	public static class HyperlinkData {
		...
		public HyperlinkData(int startPosition,
		                     int endPosition) {
	...
	public class HyperlinkSyntaxTextArea
		...
		public boolean getUnderlineForToken(Token t) {
			...
			if (entry != null) {
				HyperlinkData entryData = entry.getValue();
				if ((entryData != null)
				    && (t.getOffset() < entryData.endPosition)
				    && (t.getOffset() >= entryData.startPosition)
				    && isHyperlinkEnabled(entryData)) {
					return true;
				}
			}
