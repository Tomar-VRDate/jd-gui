...
public class JavaFilePage
	...
	public JavaFilePage(API api,
		...
		ANTLRJavaParser.parse(new ANTLRInputStream(text),
		                      declarationListener);
	...
	public static class TypeDeclarationData
		...
		public TypeDeclarationData(int startPosition,
		                           int length,
		                           String type,
		                           String name,
		                           String descriptor,
	...
	public class DeclarationListener
		...
		public void enterImportDeclaration(JavaParser.ImportDeclarationContext ctx) {
			...
			String typeName = identifiers.get(identifiers.size() - 1)
			                             .getSymbol()
			                             .getText();
		...
		public void enterTypeDeclaration(ParserRuleContext ctx) {
			...
			int position = identifier.getSymbol()
			                         .getStartIndex();
			...
			JavaParser.TypeContext superType = ctx.getRuleContext(JavaParser.TypeContext.class,
			                                                      0);
			String superInternalTypeName = (superType != null)
			                               ? resolveInternalTypeName(superType.classOrInterfaceType()
			                                                                  .Identifier())
			                               : null;
			TypeDeclarationData data = new TypeDeclarationData(position,
			                                                   typeName.length(),
			                                                   currentInternalTypeName,
			                                                   null,
			                                                   null,
			...
			declarations.put(currentInternalTypeName,
			                 data);
			typeDeclarations.put(position,
			                     data);
		}
		...
		public void enterClassBodyDeclaration(JavaParser.ClassBodyDeclarationContext ctx) {
			if (ctx.getChildCount() == 2) {
				...
				if (first instanceof TerminalNode) {
					...
					if (f.getSymbol()
					     .getType() == JavaParser.STATIC) {
						String name = f.getText();
						int position = f.getSymbol()
						                .getStartIndex();
						declarations.put(currentInternalTypeName + "-<clinit>-()V",
						                 new TypePage.DeclarationData(position,
						                                              6,
						                                              currentInternalTypeName,
						                                              name,
						                                              "()V"));
		...
		public void enterConstDeclaration(JavaParser.ConstDeclarationContext ctx) {
			...
			for (JavaParser.ConstantDeclaratorContext constantDeclaratorContext : ctx.constantDeclarator()) {
				...
				String descriptor = createDescriptor(typeContext,
				                                     dimensionOnVariable);
				int position = identifier.getSymbol()
				                         .getStartIndex();

				declarations.put(currentInternalTypeName + '-' + name + '-' + descriptor,
				                 new TypePage.DeclarationData(position,
				                                              name.length(),
				                                              currentInternalTypeName,
				                                              name,
		...
		public void enterFieldDeclaration(JavaParser.FieldDeclarationContext ctx) {
			...
			for (JavaParser.VariableDeclaratorContext declaration : ctx.variableDeclarators()
				...
				int position = identifier.getSymbol()
				                         .getStartIndex();
				TypePage.DeclarationData data = new TypePage.DeclarationData(position,
				                                                             name.length(),
				                                                             currentInternalTypeName,
				                                                             name,
				                                                             descriptor);
		...
		public void enterMethodDeclaration(ParserRuleContext ctx,
			...
			String returnDescriptor = createDescriptor(returnType,
			                                           0);
		...
		public void enterConstructorDeclaration(JavaParser.ConstructorDeclarationContext ctx) {
			...
			String       name       = identifier.getText();
			String paramDescriptors = createParamDescriptors(ctx.formalParameters()
			                                                    .formalParameterList());
			String descriptor = paramDescriptors + "V";
			int position = identifier.getSymbol()
			                         .getStartIndex();

			declarations.put(currentInternalTypeName + "-<init>-" + descriptor,
			                 new TypePage.DeclarationData(position,
			                                              name.length(),
			                                              currentInternalTypeName,
			                                              name,
		...
		public String createParamDescriptors(JavaParser.FormalParameterListContext formalParameterList) {
			...
			if (formalParameterList != null) {
				...
				for (JavaParser.FormalParameterContext formalParameter : formalParameters) {
					...
					String descriptor = createDescriptor(formalParameter.type(),
					                                     dimensionOnParameter);
	...
	public class ReferenceListener
		...
		public void enterImportDeclaration(JavaParser.ImportDeclarationContext ctx) {
			List<TerminalNode> identifiers = ctx.qualifiedName()
			                                    .Identifier();
			int position = identifiers.get(0)
			                          .getSymbol()
			                          .getStartIndex();
			String internalTypeName = concatIdentifiers(identifiers);

			addHyperlink(new TypePage.HyperlinkReferenceData(position,
			                                                 internalTypeName.length(),
		...
		public void enterFormalParameters(JavaParser.FormalParametersContext ctx) {
			...
			if (formalParameterList != null) {
				...
				for (JavaParser.FormalParameterContext formalParameter : formalParameters) {
					...
					String descriptor = createDescriptor(formalParameter.type(),
					                                     dimensionOnParameter);
					String name = formalParameter.variableDeclaratorId()
					                             .Identifier()
					                             .getSymbol()
					                             .getText();

					currentContext.nameToDescriptor.put(name,
					                                    descriptor);
				}
			}
		}
		...
		public void enterLocalVariableDeclaration(JavaParser.LocalVariableDeclarationContext ctx) {
			...
			for (JavaParser.VariableDeclaratorContext variableDeclarator : ctx.variableDeclarators()
			                                                                  .variableDeclarator()) {
				JavaParser.VariableDeclaratorIdContext variableDeclaratorId = variableDeclarator.variableDeclaratorId();
				int                                    dimensionOnVariable  = countDimension(variableDeclaratorId.children);
				String descriptor = createDescriptor(typeContext,
				                                     dimensionOnVariable);
				String name = variableDeclarator.variableDeclaratorId()
				                                .Identifier()
				                                .getSymbol()
				                                .getText();
		...
		public void enterCreator(JavaParser.CreatorContext ctx) {
			enterNewExpression(ctx.createdName()
			                      .Identifier(),
			                   ctx.classCreatorRest());
		}

		public void enterInnerCreator(JavaParser.InnerCreatorContext ctx) {
			enterNewExpression(Collections.singletonList(ctx.Identifier()),
			                   ctx.classCreatorRest());
		}

		public void enterNewExpression(List<TerminalNode> identifiers,
		                               JavaParser.ClassCreatorRestContext classCreatorRest) {
			if (identifiers.size() > 0) {
				...
				if (classCreatorRest != null) {
					...
					JavaParser.ExpressionListContext expressionList = classCreatorRest.arguments()
					                                                                  .expressionList();
					String descriptor = (expressionList != null)
					                    ? getParametersDescriptor(expressionList).append('V')
					                                                             .toString()
					                    : "()V";
		...
		public void enterExpression(JavaParser.ExpressionContext ctx) {
			switch (ctx.getChildCount()) {
				case 1:
					TerminalNode identifier0 = getToken(ctx.children,
					...
					if (identifier0 != null) {
						if (isAField(ctx)) {
							...
							if (primaryContext != null) {
								String fieldName = primaryContext.literal()
								                                 .StringLiteral()
								                                 .getText();

								if (currentContext.getDescriptor(fieldName) == null) {
									// Not a local variable or a method parameter
									String fieldTypeName = searchInternalTypeNameForThisFieldName(currentInternalTypeName,
									                                                              fieldName);
									int position = ctx.Identifier()
									                  .getSymbol()
									                  .getStartIndex();

									addHyperlink(new TypePage.HyperlinkReferenceData(position,
									                                                 fieldName.length(),
									                                                 newReferenceData(fieldTypeName,
									                                                                  fieldName,
									                                                                  "?",
									                                                                  currentInternalTypeName)));
								}
						...
						TerminalNode identifier = ctx.primary()
						                             .Identifier();
					...
					if (getToken(ctx.children,
						...
						if ((identifier3 != null) && isAField(ctx)) {
							...
							if (fieldTypeName != null) {
								int position = identifier3.getSymbol()
								                          .getStartIndex();
		...
		public void enterCallMethodExpression(JavaParser.ExpressionContext ctx,
			...
			if (first instanceof JavaParser.ExpressionContext) {
				...
				switch (f.getChildCount()) {
					case 1:
						...
						if (identifier != null) {
							...
							if (symbol != null) {
								...
								String methodTypeName = searchInternalTypeNameForThisMethodName(currentInternalTypeName,
								                                                                methodName);

								if (methodTypeName != null) {
									int position = symbol.getStartIndex();
									String methodDescriptor = (expressionListContext != null)
									                          ? getParametersDescriptor(expressionListContext).append('?')
									                                                                          .toString()
									                          : "()?";

									addHyperlink(new TypePage.HyperlinkReferenceData(position,
									                                                 methodName.length(),
									                                                 newReferenceData(methodTypeName,
									                                                                  methodName,
									                                                                  methodDescriptor,
									                                                                  currentInternalTypeName)));
								}
							...
							Token symbol = primary.getChild(TerminalNode.class,
							                                0)
							                      .getSymbol();
							...
							if (symbol != null) {
								switch (symbol.getType()) {
									case JavaParser.THIS:
										...
										addHyperlink(new TypePage.HyperlinkReferenceData(position,
										                                                 4,
										                                                 newReferenceData(currentInternalTypeName,
										                                                                  "<init>",
										                                                                  methodDescriptor,
										                                                                  currentInternalTypeName)));
										break;
									case JavaParser.SUPER:
										...
										if (data instanceof TypeDeclarationData) {
											...
											methodDescriptor = (expressionListContext != null)
											                   ? getParametersDescriptor(expressionListContext).append('?')
											                                                                   .toString()
											                   : "()?";
					...
					case 3:
						...
						if ((dot instanceof TerminalNode) && (((TerminalNode) dot).getSymbol()
						                                                          .getType() == JavaParser.DOT)) {
					...
					for (int i = 0;
						...
						if (parent.getChild(i) == ctx) {
							...
							if (next instanceof TerminalNode) {
								switch (((TerminalNode) next).getSymbol()
								                             .getType()) {
									case JavaParser.DOT:
				...
				if (pt.getChildCount() == 1) {
					...
					if (identifier != null) {
						String name = identifier.getSymbol()
						                        .getText();
						String descriptor = (currentContext == null)
						                    ? null
						                    : currentContext.getDescriptor(name);
						...
						if (descriptor != null) {
							...
							String internalTypeName = searchInternalTypeNameForThisFieldName(currentInternalTypeName,
							                                                                 name);
						...
						TerminalNode tn = primary.getChild(TerminalNode.class,
						                                   0);
						Token symbol = (tn == null)
						               ? null
						               : tn.getSymbol();
		...
		public String searchInternalTypeNameForThisFieldName(String internalTypeName,
		                                                     String name) {
			...
			if (index != -1) {
				...
				internalTypeName = internalTypeName.substring(0,
				                                              index);

				return searchInternalTypeNameForThisFieldName(internalTypeName,
				                                              name);
			}
		...
		public String searchInternalTypeNameForThisMethodName(String internalTypeName,
		                                                      String name) {
			String prefix = internalTypeName + '-' + name + "-(";
			...
			if (index != -1) {
				...
				return searchInternalTypeNameForThisMethodName(internalTypeName,
				                                               name);
			}
		...
		public TerminalNode getToken(List<ParseTree> children,
		                             int type,
		                             int i) {
		...
		public void enterLiteral(JavaParser.LiteralContext ctx) {
			...
			if (stringLiteral != null) {
				String str = stringLiteral.getSymbol()
				                          .getText();
				int position = stringLiteral.getSymbol()
				                            .getStartIndex();

				strings.add(new TypePage.StringData(position,
				                                    str.length(),
				                                    str,
				                                    currentInternalTypeName));
			}
