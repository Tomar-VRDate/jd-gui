...
public class DynamicPage
	...
	public DynamicPage(API api,
		...
		if (source == null) {
			...
			add(page = new ClassFilePage(api,
			                             entry));
			// Try to load source in background
			api.loadSource(entry,
			               this);
		} else {
			// Display original source code
			add(page = new JavaFilePage(api,
			                            new DelegatedEntry(entry,
			                                               source)));
		}
	}
	...
	public boolean highlightText(String text,
		...
		return page.highlightText(text,
		                          caseSensitive);
	}
	...
	// --- UriOpenable --- //
	@Override
	public boolean openUri(URI uri) {return page.openUri(lastOpenedUri = uri);}
	...
	public void sourceLoaded(String source) {
		SwingUtilities.invokeLater(() -> {
			...
			Point viewPosition = page.getScrollPane()
			                         .getViewport()
			                         .getViewPosition();
	...
	protected static class DelegatedEntry
		...
		DelegatedEntry(Container.Entry entry,
		               String source) {
			this.entry = entry;
			this.source = source;
		}
