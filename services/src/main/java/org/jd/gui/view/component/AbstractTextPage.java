...
public class AbstractTextPage
	...
	protected static final ImageIcon COLLAPSED_ICON = new ImageIcon(AbstractTextPage.class.getClassLoader()
	                                                                                      .getResource("org/jd/gui"
	                                                                                                   + "/images/plus"
	                                                                                                   + ".png"));
	protected static final ImageIcon EXPANDED_ICON  = new ImageIcon(AbstractTextPage.class.getClassLoader()
	                                                                                      .getResource("org/jd/gui"
	                                                                                                   + "/images/minus"
	                                                                                                   + ".png"));
	...
	public AbstractTextPage() {
		...
		textArea.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					SearchEngine.markAll(textArea,
					                     newSearchContext(textArea.getSelectedText(),
					                                      true,
					                                      true,
					                                      true,
					                                      false));
		...
		scrollPane.addMouseWheelListener(e -> {
			if ((e.getModifiers() & (Event.META_MASK | Event.CTRL_MASK)) != 0) {
				...
				int offset = textArea.viewToModel(new Point(x,
				                                            y));

				// Update font size
				if (e.getWheelRotation() > 0) {
					DECREASE_FONT_SIZE_ACTION.actionPerformedImpl(null,
					                                              textArea);
				} else {
					INCREASE_FONT_SIZE_ACTION.actionPerformedImpl(null,
					                                              textArea);
				}
				...
				if (preferences != null) {
					preferences.put(FONT_SIZE_KEY,
					                String.valueOf(textArea.getFont()
					                                       .getSize()));
				}
				...
				try {
					...
					// Scroll
					Point viewPosition = scrollPane.getViewport()
					                               .getViewPosition();
					viewPosition.y = Math.max(viewPosition.y + newY - y,
					                          0);
		...
		gutter.setFoldIcons(COLLAPSED_ICON,
		                    EXPANDED_ICON);
		gutter.setFoldIndicatorForeground(gutter.getBorderColor());

		add(scrollPane,
		    BorderLayout.CENTER);
		add(new RoundMarkErrorStrip(textArea),
	...
	public void setCaretPositionAndCenter(DocumentRange range) {
		...
		if (!foldsExpanded) {
			try {
				...
				if (rec != null) {
					// Visible
					setCaretPositionAndCenter(start,
					                          end,
					                          rec);
				} else {
					// Not visible yet
	...
	protected void setCaretPositionAndCenter(int start,
	                                         int end,
	                                         Rectangle r) {
		if (end != start) {
			try {
	...
	public int getMaximumLineNumber() {
		try {
			return textArea.getLineOfOffset(textArea.getDocument()
			                                        .getLength()) + 1;
		} catch (BadLocationException e) {
			assert ExceptionUtil.printStackTrace(e);
			return 0;
	...
	// --- ContentSearchable --- //
	public boolean highlightText(String text,
	                             boolean caseSensitive) {
		if (text.length() > 1) {
			textArea.setMarkAllHighlightColor(SEARCH_HIGHLIGHT_COLOR);
			textArea.setCaretPosition(textArea.getSelectionStart());

			SearchContext context = newSearchContext(text,
			                                         caseSensitive,
			                                         false,
			                                         true,
			                                         false);
			SearchResult result = SearchEngine.find(textArea,
			                                        context);

			if (!result.wasFound()) {
				textArea.setCaretPosition(0);
				result = SearchEngine.find(textArea,
				                           context);
			}
	...
	protected SearchContext newSearchContext(String searchFor,
	                                         boolean matchCase,
	                                         boolean wholeWord,
			...
			if (parameters.containsKey("lineNumber")) {
				...
				try {
					...
					if (textArea.getDocument()
					            .getLength() > pos) {
						setCaretPositionAndCenter(new DocumentRange(pos,
						                                            pos));
						return true;
					}
				} catch (NumberFormatException e) {
				...
				if ((highlightFlags.indexOf('s') != -1) && parameters.containsKey("highlightPattern")) {
					...
					SearchContext context = newSearchContext(searchFor,
					                                         true,
					                                         false,
					                                         true,
					                                         true);
					...
					if (result.getMatchRange() != null) {
						textArea.setCaretPosition(result.getMatchRange()
						                                .getStartOffset());
					}
	...
	protected Map<String, String> parseQuery(String query) {
		...
		try {
			for (String param : query.split("&")) {
				...
				if (index == -1) {
					parameters.put(URLDecoder.decode(param,
					                                 "UTF-8"),
					               "");
				} else {
					String key = param.substring(0,
					                             index);
					String value = param.substring(index + 1);
					parameters.put(URLDecoder.decode(key,
					                                 "UTF-8"),
		...
		if (fontSize != null) {
			try {
				textArea.setFont(textArea.getFont()
				                         .deriveFont(Float.parseFloat(fontSize)));
			} catch (Exception e) {
				assert ExceptionUtil.printStackTrace(e);
			}
		}
