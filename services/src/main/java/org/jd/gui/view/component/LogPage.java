...
public class LogPage
	...
	protected void parseLine(String content,
	                         int index,
	                         int eol) {
		int start = content.indexOf("at ",
		                            index);

		if ((start != -1) && (start < eol)) {
			int leftParenthesisIndex = content.indexOf('(',
			                                           start);

			if ((leftParenthesisIndex != -1) && (leftParenthesisIndex < eol)) {
				addHyperlink(new LogHyperlinkData(start + 3,
	...
	protected void openHyperlink(int x,
		...
		if (logHyperlinkData.enabled) {
			try {
				...
				// Open link
				String text = getText();
				String typeAndMethodNames = text.substring(hyperlinkData.startPosition,
				...
				String internalTypeName = typeAndMethodNames.substring(0,
				                                                       lastDotIndex)
				                                            .replace('.',
				                                                     '/');
				...
				int rightParenthesisIndex = text.indexOf(')',
				                                         leftParenthesisIndex);
				String lineNumberOrNativeMethodFlag = text.substring(leftParenthesisIndex,
	...
	public void indexesChanged(Collection<Future<Indexes>> collectionOfFutureIndexes) {
		...
		for (Map.Entry<Integer, HyperlinkData> entry : hyperlinks.entrySet()) {
			...
			String typeAndMethodNames = text.substring(entryData.startPosition,
	...
	public static class LogHyperlinkData
		...
		public LogHyperlinkData(int startPosition,
		                        int endPosition) {
			super(startPosition,
			      endPosition);
		}
