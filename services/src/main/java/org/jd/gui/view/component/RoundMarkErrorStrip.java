...
public class RoundMarkErrorStrip
	...
	public void addNotify() {
		...
		textArea.addPropertyChangeListener(RSyntaxTextArea.PARSER_NOTICES_PROPERTY,
		                                   listener);
		textArea.addPropertyChangeListener(RSyntaxTextArea.MARK_OCCURRENCES_PROPERTY,
		                                   listener);
		textArea.addPropertyChangeListener(RSyntaxTextArea.MARKED_OCCURRENCES_CHANGED_PROPERTY,
		                                   listener);
		textArea.addPropertyChangeListener(RSyntaxTextArea.MARK_ALL_OCCURRENCES_CHANGED_PROPERTY,
		                                   listener);
		refreshMarkers();
	}
	...
	private Color getBrighterColor(Color c) {
		...
		if (brighter == null) {
			...
			brighter = new Color(r,
			                     g,
			                     b);
			brighterColors.put(c,
			                   brighter);
		}
		return brighter;
	}
	...
	private void refreshMarkers() {
		...
		for (ParserNotice notice : notices) {
			if (notice.getLevel()
			          .isEqualToOrWorseThan(levelThreshold) || (notice instanceof TaskNotice)) {
		...
		if (getShowMarkedOccurrences() && textArea.getMarkOccurrences()) {
			...
			addMarkersForRanges(occurrences,
			                    markerMap,
	...
	public void removeNotify() {
		...
		textArea.removePropertyChangeListener(RSyntaxTextArea.PARSER_NOTICES_PROPERTY,
		                                      listener);
		textArea.removePropertyChangeListener(RSyntaxTextArea.MARK_OCCURRENCES_PROPERTY,
		                                      listener);
		textArea.removePropertyChangeListener(RSyntaxTextArea.MARKED_OCCURRENCES_CHANGED_PROPERTY,
		                                      listener);
		textArea.removePropertyChangeListener(RSyntaxTextArea.MARK_ALL_OCCURRENCES_CHANGED_PROPERTY,
		                                      listener);
	}
