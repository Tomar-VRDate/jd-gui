...
public class ModuleInfoFilePage
	...
	static {
		...
		atmf.putMapping(SYNTAX_STYLE_JAVA_MODULE,
		                ModuleInfoTokenMaker.class.getName());
	}

	public ModuleInfoFilePage(API api,
		...
		if (hyperlinkReferenceData.reference.enabled) {
			try {
				...
				switch (moduleInfoReferenceData.type) {
					case TYPE:
						entries = IndexesUtil.findInternalTypeName(collectionOfFutureIndexes,
						                                           fragment = moduleInfoReferenceData.typeName);
						break;
					case PACKAGE:
						entries = IndexesUtil.find(collectionOfFutureIndexes,
						                           "packageDeclarations",
						                           moduleInfoReferenceData.typeName);
						fragment = null;
						break;
					default: // MODULE
						entries = IndexesUtil.find(collectionOfFutureIndexes,
						                           "javaModuleDeclarations",
						                           moduleInfoReferenceData.name);
						fragment = moduleInfoReferenceData.typeName;
						break;
				}
	...
	public boolean openUri(URI uri) {
		...
		if ((fragment != null) && (declarations.size() == 1)) {
			DeclarationData declaration = declarations.entrySet()
			                                          .iterator()
			                                          .next()
			                                          .getValue();

			if (fragment.equals(declaration.typeName)) {
				ranges.add(new DocumentRange(declaration.startPosition,
				...
				if (highlightFlags.indexOf('r') != -1) {
					...
					for (Map.Entry<Integer, HyperlinkData> entry : hyperlinks.entrySet()) {
						...
						if (t && (moduleInfoReferenceData.type == TYPE)) {
							matchAndAddDocumentRange(pattern,
							                         getMostInnerTypeName(moduleInfoReferenceData.typeName),
			...
			try {
				for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
					if (futureIndexes.isDone()) {
						...
						switch (moduleInfoReferenceData.type) {
							case TYPE:
								index = futureIndexes.get()
								                     .getIndex("typeDeclarations");
								key = reference.typeName;
								break;
							case PACKAGE:
								index = futureIndexes.get()
								                     .getIndex("packageDeclarations");
								key = reference.typeName;
								break;
							default: // MODULE
								index = futureIndexes.get()
								                     .getIndex("javaModuleDeclarations");
								key = reference.name;
								break;
						}
	...
	protected static class ModuleInfoReferenceData
		...
		public ModuleInfoReferenceData(int type,
		                               String typeName,
		                               String name,
		                               String descriptor,
		                               String owner) {
	...
	public static class ModuleInfoTokenMaker
		...
		public void addToken(Segment segment,
			...
			if (tokenType == Token.IDENTIFIER) {
				int value = wordsToHighlight.get(segment,
				                                 start,
				                                 end);
				if (value != -1) {
					tokenType = value;
	...
	public class ModuleInfoFilePrinter
		...
		public void printReference(int type,
			...
			String key = (type == MODULE)
			             ? name
			             : internalTypeName;
			ReferenceData reference = referencesCache.get(key);

			if (reference == null) {
				reference = new ModuleInfoReferenceData(type,
				                                        internalTypeName,
				                                        name,
				                                        descriptor,
				                                        ownerInternalName);
			...
			addHyperlink(new HyperlinkReferenceData(stringBuffer.length(),
			                                        name.length(),
			                                        reference));
