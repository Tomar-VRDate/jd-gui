...
public class ManifestFilePage
	...
	public ManifestFilePage(API api,
		...
		if (startLineIndex != -1) {
			...
			int startIndex = skipSeparators(text,
			                                startLineIndex + "Main-Class:".length());
			int endIndex = searchEndIndexOfValue(text,
			                                     startLineIndex,
			                                     startIndex);
			String typeName = text.substring(startIndex,
			                                 endIndex);
			String internalTypeName = typeName.replace('.',
			                                           '/');
			addHyperlink(new ManifestHyperlinkData(startIndex,
			                                       endIndex,
			                                       internalTypeName + "-main-([Ljava/lang/String;)V"));
		}
		...
		if (startLineIndex != -1) {
			...
			int startIndex = skipSeparators(text,
			                                startLineIndex + "Premain-Class:".length());
			int endIndex = searchEndIndexOfValue(text,
			                                     startLineIndex,
			                                     startIndex);
			String typeName = text.substring(startIndex,
	...
	public int skipSeparators(String text,
	                          int index) {
	...
	public int searchEndIndexOfValue(String text,
	                                 int startLineIndex,
	                                 int startIndex) {
		int length = text.length();
		...
		while (index < length) {
			...
			switch (text.charAt(index)) {
				case '\r':
					...
					} else if ((index - startLineIndex >= 70)
					           && (index + 2 < length)
					           && (text.charAt(index + 1) == '\n')
					           && (text.charAt(index + 2) == ' ')) {
						// Multiline value
	...
	protected void openHyperlink(int x,
		...
		if (data.enabled) {
			try {
				...
				// Open link
				String text = getText();
				String textLink = getValue(text,
				                           hyperlinkData.startPosition,
				                           hyperlinkData.endPosition);
	...
	public void indexesChanged(Collection<Future<Indexes>> collectionOfFutureIndexes) {
		...
		for (Map.Entry<Integer, HyperlinkData> entry : hyperlinks.entrySet()) {
			...
			String textLink = getValue(text,
			                           entryData.startPosition,
			                           entryData.endPosition);
			String internalTypeName = textLink.replace('.',
			                                           '/');
			boolean enabled = IndexesUtil.containsInternalTypeName(collectionOfFutureIndexes,
			                                                       internalTypeName);
	...
	public static class ManifestHyperlinkData
		...
		ManifestHyperlinkData(int startPosition,
		                      int endPosition,
		                      String fragment) {
			super(startPosition,
			      endPosition);
