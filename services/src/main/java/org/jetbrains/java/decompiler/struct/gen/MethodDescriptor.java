...
public final class MethodDescriptor {
	...
	private MethodDescriptor(VarType[] params,
	                         VarType ret,
	                         String descriptor) {
		this.params = params;
		this.ret = ret;
	...
	public static MethodDescriptor parseDescriptor(String descriptor) {
		...
		if (parenth > 1) {
			String parameters = descriptor.substring(1,
			                                         parenth);
			...
			while (index < len) {
				switch (parameters.charAt(index)) {
					case '[':
						...
						break;
					case 'L':
						ind = parameters.indexOf(";",
						                         index);
						lst.add(parameters.substring(indexFrom < 0
						                             ? index
						                             : indexFrom,
						                             ind + 1));
						index = ind;
		...
		return new MethodDescriptor(params,
		                            ret,
		                            descriptor);
	}

	public static MethodDescriptor parseDescriptor(StructMethod struct,
	                                               ClassNode node) {
		...
		if (sig != null) {
			if (node != null) {
				MethodWrapper methodWrapper = node.getWrapper()
				                                  .getMethodWrapper(struct.getName(),
				                                                    struct.getDescriptor());
				boolean init         = CodeConstants.INIT_NAME.equals(struct.getName())
				                       && node.type != ClassNode.CLASS_ANONYMOUS;
				long    actualParams = md.params.length;
				List<VarVersionPair> sigFields = methodWrapper == null
				                                 ? null
				                                 : methodWrapper.synthParameters;
				if (sigFields != null) {
					actualParams = sigFields.stream()
					                        .filter(Objects::isNull)
					                        .count();
				} else if (init) {
					// && isEnum
					if (DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_ENUM) && DecompilerContext.getStructContext()
					                                                                                           .getClass(struct.getClassQualifiedName())
					                                                                                           .hasModifier(CodeConstants.ACC_ENUM)) {
						actualParams -= 2;
					}
				}
				if (actualParams != sig.parameterTypes.size()) {
					String message = "Inconsistent generic signature in method "
					                 + struct.getName()
					                 + " "
					                 + struct.getDescriptor()
					                 + " in "
					                 + struct.getClassQualifiedName();
	...
	private static VarType buildNewType(VarType type,
	                                    NewClassNameBuilder builder) {
		if (type.type == CodeConstants.TYPE_OBJECT) {
			String newClassName = builder.buildNewClassname(type.value);
			if (newClassName != null) {
				return new VarType(type.type,
				                   type.arrayDim,
				                   newClassName);
			}
		}
	...
	public String buildNewDescriptor(NewClassNameBuilder builder) {
		...
		if (params.length > 0) {
			...
			for (int i = 0;
			     i < params.length;
			     i++) {
				VarType substitute = buildNewType(params[i],
				                                  builder);
				if (substitute != null) {
		...
		VarType newRet = ret;
		VarType substitute = buildNewType(ret,
		                                  builder);
		if (substitute != null) {
			newRet = substitute;
			updated = true;
	...
	public boolean equals(Object o) {
		...
		return ret.equals(md.ret) && Arrays.equals(params,
		                                           md.params);
	}
