...
public final class ClassReference14Processor {
	...
	static {
		...
		invFor.setLstParameters(Collections.singletonList(new VarExprent(0,
		                                                                 VarType.VARTYPE_STRING,
		                                                                 null)));
		BODY_EXPR = new ExitExprent(ExitExprent.EXIT_RETURN,
		                            invFor,
		...
		NewExprent newExpr = new NewExprent(new VarType(CodeConstants.TYPE_OBJECT,
		                                                0,
		...
		invCause.setLstParameters(Collections.singletonList(new VarExprent(2,
		                                                                   new VarType(CodeConstants.TYPE_OBJECT,
		                                                                               0,
		                                                                               "java/lang/ClassNotFoundException"),
		                                                                   null)));
		HANDLER_EXPR = new ExitExprent(ExitExprent.EXIT_THROW,
		                               invCause,
		                               null,
		                               null,
		                               null);
	...
	public static void processClassReferences(ClassNode node) {
		...
		if (!setFound.isEmpty()) {
			for (ClassWrapper wrp : setFound) {
				...
				wrp.getHiddenMembers()
				   .add(InterpreterUtil.makeUniqueKey(mt.getName(),
				                                      mt.getDescriptor()));
			}
		}
	}
	...
	private static void processClassRec(ClassNode node,
	                                    Map<ClassWrapper, MethodWrapper> mapClassMeths,
	                                    Set<? super ClassWrapper> setFound) {
		...
		for (int j = 0;
		     j < 2;
		     j++) {
			VBStyleCollection<Exprent, String> initializers = j == 0
			...
			for (int i = 0;
				...
				for (Entry<ClassWrapper, MethodWrapper> ent : mapClassMeths.entrySet()) {
					...
					String cl = isClass14Invocation(exprent,
					                                ent.getKey(),
					                                ent.getValue());
					if (cl != null) {
						initializers.set(i,
	...
	private static void mapClassMethods(ClassNode node,
	                                    Map<ClassWrapper, MethodWrapper> map) {
			...
			if ((noSynthFlag || mt.isSynthetic())
				...
				if (root != null && root.getFirst().type == Statement.TYPE_TRYCATCH) {
					...
					if (cst.getStats()
					    ...
					    && cst.getStats()
					          .get(1).type == Statement.TYPE_BASICBLOCK
					    && cst.getVars()
					          .get(0)
					          .getVarType()
					          .equals(new VarType(CodeConstants.TYPE_OBJECT,
					                              0,
					                              "java/lang/ClassNotFoundException"))) {

						BasicBlockStatement body = (BasicBlockStatement) cst.getFirst();
						BasicBlockStatement handler = (BasicBlockStatement) cst.getStats()
						                                                       .get(1);
						...
						if (body.getExprents()
						    ...
						    && handler.getExprents()
						              .size() == 1) {
							if (BODY_EXPR.equals(body.getExprents()
							                         .get(0)) && HANDLER_EXPR.equals(handler.getExprents()
							                                                                .get(0))) {
								map.put(wrapper,
								        method);
								break;
	...
	private static boolean replaceInvocations(Exprent exprent,
		...
		while (true) {
			...
			for (Exprent expr : exprent.getAllExprents()) {
				String cl = isClass14Invocation(expr,
				                                wrapper,
				                                meth);
				if (cl != null) {
	...
	private static String isClass14Invocation(Exprent exprent,
	                                          ClassWrapper wrapper,
	                                          MethodWrapper meth) {
		if (exprent.type == Exprent.EXPRENT_FUNCTION) {
			...
			if (fexpr.getFuncType() == FunctionExprent.FUNCTION_IIF) {
				if (fexpr.getLstOperands()
				         .get(0).type == Exprent.EXPRENT_FUNCTION) {
					FunctionExprent headexpr = (FunctionExprent) fexpr.getLstOperands()
					                                                  .get(0);
					if (headexpr.getFuncType() == FunctionExprent.FUNCTION_EQ) {
						if (headexpr.getLstOperands()
						            .get(0).type == Exprent.EXPRENT_FIELD
						    && headexpr.getLstOperands()
						               .get(1).type == Exprent.EXPRENT_CONST
						    && ((ConstExprent) headexpr.getLstOperands()
						                               .get(1)).getConstType()
						                                       .equals(VarType.VARTYPE_NULL)) {

							FieldExprent field = (FieldExprent) headexpr.getLstOperands()
							                                            .get(0);
							ClassNode fieldnode = DecompilerContext.getClassProcessor()
							                                       .getMapRootClasses()
							                                       .get(field.getClassname());

							if (fieldnode != null
							    && fieldnode.classStruct.qualifiedName.equals(wrapper.getClassStruct().qualifiedName)) { // source
								// class
								StructField fd = wrapper.getClassStruct()
								                        .getField(field.getName(),
								                                  field.getDescriptor().descriptorString);  // FIXME: can be null!
								// why??

								if (fd != null && fd.hasModifier(CodeConstants.ACC_STATIC) && (fd.isSynthetic()
								                                                               || DecompilerContext.getOption(IFernflowerPreferences.SYNTHETIC_NOT_SET))) {

									if (fexpr.getLstOperands()
									         .get(1).type == Exprent.EXPRENT_ASSIGNMENT && fexpr.getLstOperands()
									                                                            .get(2)
									                                                            .equals(field)) {
										AssignmentExprent asexpr = (AssignmentExprent) fexpr.getLstOperands()
										                                                    .get(1);

										if (asexpr.getLeft()
										          .equals(field) && asexpr.getRight().type == Exprent.EXPRENT_INVOCATION) {
											InvocationExprent invexpr = (InvocationExprent) asexpr.getRight();

											if (invexpr.getClassname()
											           .equals(wrapper.getClassStruct().qualifiedName)
											    && invexpr.getName()
											              .equals(meth.methodStruct.getName())
											    && invexpr.getStringDescriptor()
											              .equals(meth.methodStruct.getDescriptor())) {

												if (invexpr.getLstParameters()
													...
													return ((ConstExprent) invexpr.getLstParameters()
													                              .get(0)).getValue()
													                                      .toString();
												}
											}
