...
public class VarTypeProcessor {
	...
	public VarTypeProcessor(StructMethod mt,
	                        MethodDescriptor md) {
		method = mt;
		methodDescriptor = md;
	}
			...
			for (Exprent expr : lst) {
				if (expr.type == Exprent.EXPRENT_VAR) {
					...
					if (constExpr.getConstType().typeFamily == CodeConstants.TYPE_FAMILY_INTEGER) {
						constExpr.setConstType(new ConstExprent(constExpr.getIntValue(),
						                                        constExpr.isBoolPermitted(),
						                                        null).getConstType());
					}
				}
			}
	...
	public void calculateVarTypes(RootStatement root,
	                              DirectGraph graph) {
		setInitVars(root);
	...
	private void setInitVars(RootStatement root) {
		...
		if (thisVar) {
			...
			VarType clType = new VarType(CodeConstants.TYPE_OBJECT,
			                             0,
			                             cl.qualifiedName);
			mapExprentMinTypes.put(new VarVersionPair(0,
			                                          1),
			                       clType);
			mapExprentMaxTypes.put(new VarVersionPair(0,
			                                          1),
			                       clType);
		}
		...
		for (int i = 0;
		     i < md.params.length;
		     i++) {
			mapExprentMinTypes.put(new VarVersionPair(varIndex + (thisVar
			                                                      ? 1
			                                                      : 0),
			                                          1),
			                       md.params[i]);
			mapExprentMaxTypes.put(new VarVersionPair(varIndex + (thisVar
			                                                      ? 1
			                                                      : 0),
			                                          1),
			                       md.params[i]);
		...
		while (!stack.isEmpty()) {
			...
			if (lstVars != null) {
				for (VarExprent var : lstVars) {
					mapExprentMinTypes.put(new VarVersionPair(var.getIndex(),
					                                          1),
					                       var.getVarType());
					mapExprentMaxTypes.put(new VarVersionPair(var.getIndex(),
					                                          1),
					                       var.getVarType());
				}
			}
	...
	private boolean processVarTypes(DirectGraph graph) {
		return graph.iterateExprents(exprent -> checkTypeExprent(exprent)
		                                        ? 0
		                                        : 1);
	}
	...
	private boolean checkTypeExprent(Exprent exprent) {
		...
		if (exprent.type == Exprent.EXPRENT_CONST) {
			...
			if (constExpr.getConstType().typeFamily <= CodeConstants.TYPE_FAMILY_INTEGER) { // boolean or integer
				VarVersionPair pair = new VarVersionPair(constExpr.id,
				                                         -1);
				if (!mapExprentMinTypes.containsKey(pair)) {
					mapExprentMinTypes.put(pair,
					                       constExpr.getConstType());
				}
			}
		}
		...
		if (result != null) {
			for (CheckTypesResult.ExprentTypePair entry : result.getLstMaxTypeExprents()) {
				if (entry.type.typeFamily != CodeConstants.TYPE_FAMILY_OBJECT) {
					changeExprentType(entry.exprent,
					                  entry.type,
					                  1);
				}
			}
			...
			for (CheckTypesResult.ExprentTypePair entry : result.getLstMinTypeExprents()) {
				res &= changeExprentType(entry.exprent,
				                         entry.type.type == CodeConstants.TYPE_UNKNOWN
	...
	// true -> Do nothing
	// false -> cancel iteration
	private boolean changeExprentType(Exprent exprent,
	                                  VarType newType,
	                                  int minMax) {
		...
		switch (exprent.type) {
			case Exprent.EXPRENT_CONST:
				...
				if (newType.typeFamily > CodeConstants.TYPE_FAMILY_INTEGER
					...
					VarType minInteger = new ConstExprent((Integer) constExpr.getValue(),
					                                      false,
					                                      null).getConstType();
				...
				return changeVarExprentType(exprent,
				                            newType,
				                            minMax,
				                            new VarVersionPair(exprent.id,
				                                               -1));
			case Exprent.EXPRENT_VAR:
				return changeVarExprentType(exprent,
				                            newType,
				                            minMax,
				                            new VarVersionPair((VarExprent) exprent));

			case Exprent.EXPRENT_ASSIGNMENT:
				return changeExprentType(((AssignmentExprent) exprent).getRight(),
				                         newType,
				                         minMax);

			case Exprent.EXPRENT_FUNCTION:
				return changeFunctionExprentType(newType,
				                                 minMax,
				                                 (FunctionExprent) exprent);
		}

		return true;
	}

	private boolean changeVarExprentType(Exprent exprent,
	                                     VarType newType,
	                                     int minMax,
	                                     VarVersionPair pair) {
		if (minMax == 0) { // min
			...
			if (currentMinType == null || newType.typeFamily > currentMinType.typeFamily) {
				...
				newMinType = VarType.getCommonSupertype(currentMinType,
				                                        newType);
			}
			...
			return currentMinType == null || (newMinType.typeFamily <= currentMinType.typeFamily
			...
			if (currentMaxType == null || newType.typeFamily < currentMaxType.typeFamily) {
				...
				newMaxType = VarType.getCommonMinType(currentMaxType,
				                                      newType);
			}

			mapExprentMaxTypes.put(pair,
			                       newMaxType);
		}
		return true;
	}

	private boolean changeFunctionExprentType(VarType newType,
	                                          int minMax,
		...
		switch (func.getFuncType()) {
			...
			case FunctionExprent.FUNCTION_XOR:
				return changeExprentType(func.getLstOperands()
				                             .get(offset),
				                         newType,
				                         minMax) & changeExprentType(func.getLstOperands()
				                                                         .get(offset + 1),
				                                                     newType,
				                                                     minMax);
		}
	...
	public void setVarType(VarVersionPair pair,
	                       VarType type) {
		mapExprentMinTypes.put(pair,
		                       type);
	}
