...
public final class IdeaNotNullHelper {
	...
	public static boolean removeHardcodedChecks(Statement root,
	                                            StructMethod mt) {
		...
		while (findAndRemoveParameterCheck(root,
		                                   mt)) { // iterate until nothing found. Each invocation removes one parameter
			// check.
			checks_removed = true;
		}
		...
		while (findAndRemoveReturnCheck(root,
		                                mt)) { // iterate until nothing found. Each invocation handles one method exit
			// check.
			checks_removed = true;
		}
	...
	private static boolean findAndRemoveParameterCheck(Statement stat,
	                                                   StructMethod mt) {
		...
		if (st.type == Statement.TYPE_IF) {
			...
			Exprent if_condition = ifstat.getHeadexprent()
			                             .getCondition();
			...
			if (ifbranch != null
			    ...
			    && ifbranch.getExprents()
			               .size() == 1
			    && ifbranch.getExprents()
			               .get(0).type == Exprent.EXPRENT_INVOCATION) {

				FunctionExprent func = (FunctionExprent) if_condition;
				Exprent first_param = func.getLstOperands()
				                          .get(0);
				Exprent second_param = func.getLstOperands()
				                           .get(1);
				...
				if (second_param.type == Exprent.EXPRENT_CONST
					...
					if (first_param.type == Exprent.EXPRENT_VAR && ((InvocationExprent) ifbranch.getExprents()
					                                                                            .get(0)).getName()
					                                                                                    .equals(
																																																			"$$$reportNull$$$0")) {
						...
						if (param_annotations != null) {
							...
							int index = thisvar
							            ...
							            : 0;
							for (int i = 0;
							     i < method_param_number;
							     i++) {

								if (index == var.getIndex()) {
									if (param_annotations_lists.size() >= method_param_number - i) {
										int shift = method_param_number
										            - param_annotations_lists.size(); // NOTE: workaround for compiler bug, count
										// annotations starting with the last parameter
	...
	private static void removeParameterCheck(Statement stat) {
		...
		if (ifstat.getElsestat() == null) {
			...
			if (!ifbranch.getAllSuccessorEdges()
			             .isEmpty()) {
				ifbranch.removeSuccessor(ifbranch.getAllSuccessorEdges()
				                                 .get(0));
			}

			// Replace statement with empty block
			BasicBlockStatement block = new BasicBlockStatement(new BasicBlock(DecompilerContext.getCounterContainer()
	...
	private static boolean findAndRemoveReturnCheck(Statement stat,
	                                                StructMethod mt) {
		...
		return is_notnull_check && removeReturnCheck(stat,
		                                             mt);
	}


	private static boolean removeReturnCheck(Statement stat,
	                                         StructMethod mt) {
		...
		if (parent != null
			...
			if (exprent.type == Exprent.EXPRENT_EXIT) {
				...
				if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
					...
					Exprent if_condition = ifparent.getHeadexprent()
					                               .getCondition();

					if (ifparent.getElsestat() == stat
					    && if_condition.type == Exprent.EXPRENT_FUNCTION
						...
						if (second_param.type == Exprent.EXPRENT_CONST
						    && second_param.getExprType().type == CodeConstants.TYPE_NULL) { // TODO: reversed parameter order
							//if(first_param.type == Exprent.EXPRENT_VAR && ((VarExprent)first_param).getIndex() == var_value
							// .getIndex()) {
							if (first_param.equals(exprent_value)) {        // TODO: check for absence of side effects like method
								// invocations etc.
								if (ifbranch.type == Statement.TYPE_BASICBLOCK
								    ...
								    ifbranch.getExprents()
								            .get(0).type == Exprent.EXPRENT_EXIT) {

									ifparent.getFirst()
									        .removeSuccessor(ifedge);
									ifparent.getFirst()
									        .removeSuccessor(elseedge);

									ifparent.getStats()
									        .removeWithKey(ifbranch.id);
									ifparent.getStats()
									        .removeWithKey(elsebranch.id);
									...
									if (!ifparent.getFirst()
									             .getExprents()
									             .isEmpty()) {
										elsebranch.getExprents()
										          .addAll(0,
										                  ifparent.getFirst()
										                          .getExprents());
									}
			...
			if (exprent.type == Exprent.EXPRENT_EXIT) {
				...
				if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
					...
					int sequence_stats_number = sequence.getStats()
					                                    .size();
					...
					if (sequence_stats_number > 1
					    && sequence.getStats()
					               .getLast() == stat
					    && sequence.getStats()
					               .get(sequence_stats_number - 2).type == Statement.TYPE_IF) {

						IfStatement ifstat = (IfStatement) sequence.getStats()
						                                           .get(sequence_stats_number - 2);
						Exprent if_condition = ifstat.getHeadexprent()
						                             .getCondition();
							...
							if (second_param.type == Exprent.EXPRENT_CONST
								...
								if (first_param.equals(exprent_value)) {        // TODO: check for absence of side effects like method
									...
									if (ifbranch.type == Statement.TYPE_BASICBLOCK
									    ...
									    && ifbranch.getExprents()
									               .get(0).type == Exprent.EXPRENT_INVOCATION
									    && ((InvocationExprent) ifbranch.getExprents()
									                                    .get(0)).getName()
									                                            .equals("$$$reportNull$$$0")) {
