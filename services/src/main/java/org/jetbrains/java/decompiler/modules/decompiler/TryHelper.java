...
public class TryHelper {
	public static boolean enhanceTryStats(RootStatement root,
	                                      StructClass cl) {
		boolean ret = makeTryWithResourceRec(cl,
		                                     root);
	...
	private static boolean makeTryWithResourceRec(StructClass cl,
	                                              Statement stat) {
		if (cl.getVersion()
		      .hasNewTryWithResources()) {
			boolean ret = false;
	...
	// J11+
	// Merges try with resource statements that are nested within each other, as well as try with resources statements
	// nested in a normal try.
		...
		if (inner.type == Statement.TYPE_TRYCATCH) {
			...
			if (((CatchStatement) inner).getTryType() == CatchStatement.RESOURCES) {
				...
				if (inner.getStats()
					...
					stat.getResources()
					    .addAll(((CatchStatement) inner).getResources());
					stat.getVarDefinitions()
					    .addAll(inner.getVarDefinitions());

					// Get inner block of inner try stat
					Statement innerBlock = inner.getStats()
					                            .get(0);
	...
	private static boolean collapseTry(CatchStatement catchStat) {
		...
		if (parent != null && parent.getFirst() != null && parent.getFirst().type == Statement.TYPE_TRYCATCH) {
			...
			if (toRemove.getTryType() == CatchStatement.RESOURCES) {
				...
				catchStat.getResources()
				         .addAll(toRemove.getResources());

				catchStat.getVarDefinitions()
				         .addAll(toRemove.getVarDefinitions());
				parent.replaceStatement(toRemove,
				                        toRemove.getFirst());
