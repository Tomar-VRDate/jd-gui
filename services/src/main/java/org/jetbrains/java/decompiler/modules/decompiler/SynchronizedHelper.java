...
public final class SynchronizedHelper {
	public static boolean cleanSynchronizedVar(Statement stat) {
		...
		if (stat.type == Statement.TYPE_SYNCRONIZED) {
			...
			if (sync.getHeadexprentList()
			        .get(0).type == Exprent.EXPRENT_MONITOR) {
				MonitorExprent mon = (MonitorExprent) sync.getHeadexprentList()
				                                          .get(0);

				for (Exprent e : sync.getFirst()
				                     .getExprents()) {
					if (e.type == Exprent.EXPRENT_ASSIGNMENT) {
						...
						if (ass.getLeft().type == Exprent.EXPRENT_VAR) {
							...
							if (ass.getRight()
							       .equals(mon.getValue()) && !var.isVarReferenced(stat.getParent())) {
								sync.getFirst()
								    .getExprents()
								    .remove(e);
								res = true;
