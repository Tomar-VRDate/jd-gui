...
public class DotExporter {
	private static final String  DOTS_FOLDER       = System.getProperty("DOT_EXPORT_DIR",
	                                                                    null);
	public static final  boolean DUMP_DOTS         = DOTS_FOLDER != null && !DOTS_FOLDER.trim()
	                                                                                    .isEmpty();
	private static final String  DOTS_ERROR_FOLDER = System.getProperty("DOT_ERROR_EXPORT_DIR",
	                                                                    null);
	public static final  boolean DUMP_ERROR_DOTS   = DOTS_ERROR_FOLDER != null && !DOTS_ERROR_FOLDER.trim()
	                                                                                                .isEmpty();
	...
	// Statements with no successors or predecessors (but still contained in the tree) will be in a subgraph titled
	// "Isolated statements".
	...
	private static String statToDot(Statement stat,
	                                String name) {
		...
		for (Statement st : stats) {
			if (st.type == Statement.TYPE_IF) {
				...
				if (ifs.getIfEdge() != null) {
					extraData.put(ifs.getIfEdge(),
					              "If Edge");
				}

				if (ifs.getElseEdge() != null) {
					extraData.put(ifs.getElseEdge(),
					              "Else Edge");
				}
			}
		}
		...
		for (Statement st : stats) {
			String sourceId = st.id + (st.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
			                             .isEmpty()
			                           ? ""
			                           : "000000");
			...
			for (StatEdge edge : st.getSuccessorEdges(Statement.STATEDGE_ALL)) {
				String destId = edge.getDestination().id + (edge.getDestination()
				                                                .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                                .isEmpty()
				                                            ? ""
				                                            : "000000");
				...
				for (Entry<StatEdge, String> entry : extraData.entrySet()) {
					if (edge.getSource().id.equals(entry.getKey()
					                                    .getSource().id) && edge.getDestination().id.equals(entry.getKey()
					                                                                                             .getDestination().id)) {
						edgeType = edgeType == null
						           ? entry.getValue()
						           : edgeType + " (" + entry.getValue() + ")";
					}
				}

				buffer.append(sourceId + "->" + destId + (edgeType != null
				                                          ? "[label=\"" + edgeType + "\"]"
				                                          : "") + ";\r\n");

				if (edge.closure != null) {
					String clsId = edge.closure.id + (edge.getDestination()
					                                      .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
					                                      .isEmpty()
					                                  ? ""
					                                  : "000000");
			...
			for (StatEdge labelEdge : st.getLabelEdges()) {
				String src = labelEdge.getSource().id + (labelEdge.getSource()
				                                                  .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                                  .isEmpty()
				                                         ? ""
				                                         : "000000");
				String destId = labelEdge.getDestination().id + (labelEdge.getDestination()
				                                                          .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                                          .isEmpty()
				                                                 ? ""
				                                                 : "000000");
			...
			for (Statement neighbour : st.getNeighbours(Statement.STATEDGE_ALL,
			                                            Statement.DIRECTION_FORWARD)) {
				String destId = neighbour.id + (neighbour.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                         .isEmpty()
				                                ? ""
				                                : "000000");
			...
			for (StatEdge edge : st.getPredecessorEdges(Statement.STATEDGE_ALL)) {
				String destId = edge.getSource().id + (edge.getSource()
				                                           .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                           .isEmpty()
				                                       ? ""
				                                       : "000000");
				...
				buffer.append(sourceId + "->" + destId + "[color=blue" + (edgeType != null
				                                                          ? ",fontcolor=blue,label=\"" + edgeType + "\""
				                                                          : "") + "];\r\n");
			...
			for (Statement s : st.getStats()) {
				String destId = s.id + (s.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                         .isEmpty()
				                        ? ""
				                        : "000000");
				...
				buffer.append(sourceId + " -> " + destId + " [arrowhead=vee,color=red" + (!label.equals("")
				                                                                          ? ",fontcolor=red,label=\""
				                                                                            + label
				                                                                            + "\""
				                                                                          : "") + "];\r\n");
			...
			if (!foundFirst && st.getFirst() != null) {
				String destId = st.getFirst().id + (st.getFirst()
				                                      .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
				                                      .isEmpty()
				                                    ? ""
				                                    : "000000");
				buffer.append(sourceId
				              + "->"
				              + destId
				              + " [arrowhead=vee,color=red,fontcolor=red,label=\"Dangling First statement!\"];\r\n");
			}

			if (isIf) {
				if (!foundIf && ((IfStatement) st).getIfstat() != null) {
					String destId = ((IfStatement) st).getIfstat().id + (st.getFirst()
					                                                       .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
					                                                       .isEmpty()
					                                                     ? ""
					                                                     : "000000");
					buffer.append(sourceId
					              + "->"
					              + destId
					              + " [arrowhead=vee,color=red,fontcolor=red,label=\"Dangling If statement!\"];\r\n");
				}

				if (!foundElse && ((IfStatement) st).getElsestat() != null) {
					String destId = ((IfStatement) st).getElsestat().id + (((IfStatement) st).getElsestat()
					                                                                         .getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
					                                                                         .isEmpty()
					                                                       ? ""
					                                                       : "000000");
					buffer.append(sourceId
					              + "->"
					              + destId
					              + " [arrowhead=vee,color=red,fontcolor=red,label=\"Dangling Else statement!\"];\r\n");
				}
			}
			...
			String node =
							sourceId + " [shape=box,label=\"" + st.id + " (" + getStatType(st) + ")\r\n" + toJava(st) + "\"" + (
							st
							== stat
							? ",color=red"
							: "") + "];\r\n";
	...
	private static String statementHierarchy(Statement stat) {
		...
		recursivelyGraphStatement(buffer,
		                          stat);
	...
	private static void recursivelyGraphStatement(StringBuffer buffer,
	                                              Statement statement) {
		buffer.append("subgraph cluster_" + statement.id + " {\r\n");
		buffer.append("label=\""
		              + statement.id
		              + " ("
		              + getStatType(statement)
		              + ")\r\n"
		              + toJava(statement)
		              + "\""
		              + ";\r\n");
	...
	private static String toJava(Statement statement) {
		try {
			String java = statement.toJava()
			                       .convertToStringAndAllowDataDiscard()
			                       .replace("\"",
			                                "\\\"");
			if (statement instanceof BasicBlockStatement) {
				if (statement.getExprents() == null || statement.getExprents()
				                                                .isEmpty()) {
					java = "<" + (statement.getExprents() == null
					              ? "null"
					              : "empty") + " basic block>\n" + java;
				}
			}
	...
	private static String getStatType(Statement st) {
		switch (st.type) {
			...
			case 7:
				return "Try Catch";
			case 8:
				return "Basic Block #" + ((BasicBlockStatement) st).getBlock().id;
			case 10:
				return "Synchronized";
			case 11:
				return "Placeholder";
	...
	private static void findAllStats(List<Statement> list,
	                                 Statement root) {
	...
	private static String cfgToDot(String name,
	                               ControlFlowGraph graph,
	                               boolean showMultipleEdges) {
		...
		for (int i = 0;
		     i < graph.getExceptions()
		              .size();
		     i++) {
			ExceptionRangeCFG ex = graph.getExceptions()
			                            .get(i);
			buffer.append("subgraph cluster_ex_"
			              + i
			              + " {\r\n\tlabel=\"Exception range for Block "
			              + ex.getHandler().id
			              + " \";\r\n");
	...
	private static String varsToDot(VarVersionsGraph graph) {
		...
		for (int i = 0;
			...
			buffer.append((block.var * 1000 + block.version)
			              + " [shape=box,label=\""
			              + block.var
			              + "_"
			              + block.version
			...
			for (VarVersionEdge edge : block.succs) {
				...
				buffer.append((block.var * 1000 + block.version) + "->" + (dest.var * 1000 + dest.version) + (edge.type
				                                                                                              == VarVersionEdge.EDGE_PHANTOM
				                                                                                              ? " [style"
				                                                                                                + "=dotted]"
				                                                                                              : "") + ";\r\n");
			}
	...
	private static String digraphToDot(DirectGraph graph,
	                                   Map<String, SFormsFastMapDirect> vars) {
	...
	private static String directBlockIdToDot(String id) {
		id = id.replaceAll("_try",
		                   "999");
		id = id.replaceAll("_tail",
		                   "888");

		id = id.replaceAll("_init",
		                   "111");
		id = id.replaceAll("_cond",
		                   "222");
		id = id.replaceAll("_inc",
		                   "333");
		return id;
	}

	private static File getFile(String folder,
	                            StructMethod mt,
	                            String suffix) {
	...
	private static File getFile(String folder,
	                            ...
	                            String subdirectory,
	                            String suffix) {
		File root = new File(folder + mt.getClassQualifiedName() + (subdirectory.isEmpty()
		                                                            ? ""
		                                                            : "/" + subdirectory));
		if (!root.isDirectory()) {
			root.mkdirs();
	...
	public static void toDotFile(DirectGraph dgraph,
	                             StructMethod mt,
	                             String suffix) {
		toDotFile(dgraph,
		          mt,
	...
	public static void toDotFile(DirectGraph dgraph,
		...
		try {
			BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(getFile(DOTS_FOLDER,
			                                                                                 mt,
			                                                                                 suffix)));
			out.write(digraphToDot(dgraph,
			                       vars).getBytes());
			out.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void toDotFile(Statement stat,
	                             StructMethod mt,
	                             String suffix) {
		toDotFile(stat,
		          mt,
	...
	public static void statementHierarchyToDot(Statement stat,
	                                           StructMethod mt,
	                                           String suffix) {
		if (!DUMP_DOTS) {
			return;
	...
	public static void errorToDotFile(Statement stat,
	                                  StructMethod mt,
	                                  String suffix) {
		if (!DUMP_ERROR_DOTS) {
			return;
		...
		try {
			BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(getFile(DOTS_ERROR_FOLDER,
			                                                                                 mt,
			                                                                                 suffix)));
			out.write(statToDot(stat,
			                    suffix).getBytes());
	...
	public static void toDotFile(VarVersionsGraph graph,
	                             StructMethod mt,
	                             String suffix) {
		if (!DUMP_DOTS) {
			return;
	...
	public static void toDotFile(DecompileRecord decompileRecord,
	                             StructMethod mt,
	                             String suffix,
	                             boolean error) {
		if (error) {
		...
		try {
			BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(getFile(error
			                                                                                 ? DOTS_ERROR_FOLDER
			                                                                                 : DOTS_FOLDER,
			                                                                                 mt,
			                                                                                 suffix)));
	...
	public static void toDotFile(ControlFlowGraph graph,
	                             StructMethod mt,
	                             String suffix,
	                             boolean showMultipleEdges) {
		if (!DUMP_DOTS) {
	...
	public static void errorToDotFile(ControlFlowGraph graph,
	                                  StructMethod mt,
	                                  String suffix) {
		if (!DUMP_ERROR_DOTS) {
			return;
