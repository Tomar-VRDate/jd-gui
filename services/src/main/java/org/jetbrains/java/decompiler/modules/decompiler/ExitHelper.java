...
public final class ExitHelper {
			...
			for (int i = 0;
				...
				if (st.type == Statement.TYPE_SEQUENCE
					...
					Statement last = st.getStats()
					                   .getLast();
					Statement secondlast = st.getStats()
					                         .get(st.getStats()
					                                .size() - 2);

					if (last.getExprents() == null || !last.getExprents()
					                                       .isEmpty()) {
						if (!secondlast.hasBasicSuccEdge()) {

							Set<Statement> set = last.getNeighboursSet(Statement.STATEDGE_DIRECT_ALL,
	...
	private static int integrateExits(Statement stat) {
			...
			if (stat.type == Statement.TYPE_IF) {
				...
				if (ifst.getIfstat() == null) {
					...
					if (dest != null) {
						...
						StatEdge newedge = new StatEdge(StatEdge.TYPE_REGULAR,
						                                ifst.getFirst(),
						                                bstat);
						ifst.getFirst()
						    .addSuccessor(newedge);
						...
						StatEdge oldexitedge = dest.getAllSuccessorEdges()
						                           .get(0);
						StatEdge newexitedge = new StatEdge(StatEdge.TYPE_BREAK,
						                                    bstat,
		...
		if (stat.getAllSuccessorEdges()
			...
			if (stat != parent.getFirst() || (parent.type != Statement.TYPE_IF && parent.type != Statement.TYPE_SWITCH)) {
				...
				StatEdge destedge = stat.getAllSuccessorEdges()
				                        .get(0);
				...
				if (dest != null) {
					...
					SequenceStatement block = new SequenceStatement(Arrays.asList(stat,
					                                                              bstat));
					block.setAllParent();
					...
					for (StatEdge edge : dest.getAllPredecessorEdges()) {
						if (!edge.explicit
						    ...
						    && MergeHelper.isDirectPath(edge.getSource()
						                                    .getParent(),
						                                bstat)) {
	...
	private static Statement isExitEdge(StatEdge edge) {
		...
		if (edge.getType() == StatEdge.TYPE_BREAK
		    && dest.type == Statement.TYPE_BASICBLOCK
		    && edge.explicit
		...
		for (StatEdge ed : stat.getAllPredecessorEdges()) {
			if (ed != edge) {
				if (ed.getType() == StatEdge.TYPE_REGULAR) {
					...
					if (source.type == Statement.TYPE_BASICBLOCK
					    || (source.type == Statement.TYPE_IF
	...
	public static boolean adjustReturnType(RootStatement root,
	                                       MethodDescriptor desc) {
		boolean res = false;
		// Get all statements with returns
		for (StatEdge retEdge : root.getDummyExit()
