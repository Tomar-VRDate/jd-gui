...
public class SimplifyExprentsHelper {
	@SuppressWarnings("SpellCheckingInspection") private static final MatchEngine class14Builder = new MatchEngine(
					...
					+ "     exprent type:var index:$var$\n"
					+ "     exprent type:invocation invclass:java/lang/Class signature:forName(Ljava/lang/String;)"
					+ "Ljava/lang/Class;\n"
	...
	private static boolean addArrayInitializer(Exprent first,
	                                           Exprent second) {
						...
						if (aas.getLeft().type == Exprent.EXPRENT_ARRAY) {
							...
							if (arrExpr.getArray().type == Exprent.EXPRENT_VAR
								...
								if (constValue < newExpr.getLstArrayElements()
								                        .size()) {
									Exprent init = newExpr.getLstArrayElements()
									                      .get(constValue);
									if (init.type == Exprent.EXPRENT_CONST) {
										ConstExprent cinit = (ConstExprent) init;
										VarType arrType = newExpr.getNewType()
										                         .decreaseArrayDim();
										...
										if (cinit.equals(defaultVal)) {
											...
											if (!tempExpr.containsExprent(arrVar)) {
												...
												if (tempExpr.type == Exprent.EXPRENT_NEW) {
													...
													if (dims > 1 && !tempNewExpr.getLstArrayElements()
													                            .isEmpty()) {
	...
	private static int isArrayInitializer(List<Exprent> list,
	                                      int index) {
		...
		if (current.type == Exprent.EXPRENT_ASSIGNMENT) {
			...
			if (as.getRight().type == Exprent.EXPRENT_NEW && as.getLeft().type == Exprent.EXPRENT_VAR) {
				...
				if (newExpr.getExprType().arrayDim > 0
				    && newExpr.getLstDims()
				              .size() == 1
				    && newExpr.getLstArrayElements()
				              .isEmpty()
				    && newExpr.getLstDims()
				              .get(0).type == Exprent.EXPRENT_CONST) {

					int size = (Integer) ((ConstExprent) newExpr.getLstDims()
					                                            .get(0)).getValue();
					if (size == 0) {
						return 0;
					}
							...
							if (aas.getLeft().type == Exprent.EXPRENT_ARRAY) {
								...
								if (arrExpr.getArray().type == Exprent.EXPRENT_VAR
									...
									if (constValue < size && !mapInit.containsKey(constValue)) {
										if (!aas.getRight()
										        .containsExprent(arrVar)) {
											mapInit.put(constValue,
											            aas.getRight());
											found = true;
										}
									}
					...
					if ((arrVar.isStack() && fraction > 0) || (size <= 7 && fraction >= 0.3) || (size > 7 && fraction >= 0.7)) {
						...
						VarType arrayType = newExpr.getNewType()
						                           .decreaseArrayDim();
	...
	private static boolean isAssignmentReturn(Exprent first,
	                                          Exprent second) {
			...
			if (assignment.getCondType() == AssignmentExprent.CONDITION_NONE
				...
				if (assignment.getLeft().type == Exprent.EXPRENT_VAR && exit.getValue().type == Exprent.EXPRENT_VAR) {
					...
					if (assignmentLeft.equals(exitValue) && ((!assignmentLeft.isStack() && !exitValue.isStack())
					                                         || assignment.getRight().type == Exprent.EXPRENT_SWITCH)) {
						exit.replaceExprent(exitValue,
						                    assignment.getRight());
						return true;
					}
				}
	...
	private static boolean isStackAssignment2(Exprent first,
		...
		if (first.type == Exprent.EXPRENT_ASSIGNMENT && second.type == Exprent.EXPRENT_ASSIGNMENT) {
			...
			if (asf.getLeft().type == Exprent.EXPRENT_VAR
			    && ass.getRight().type == Exprent.EXPRENT_VAR
			    && asf.getLeft()
			          .equals(ass.getRight())
			    && ((VarExprent) asf.getLeft()).isStack()) {
				if (ass.getLeft().type != Exprent.EXPRENT_VAR || !((VarExprent) ass.getLeft()).isStack()) {
					asf.setRight(new AssignmentExprent(ass.getLeft(),
					                                   asf.getRight(),
					                                   ass.bytecode));
					return true;
				}
	...
	private static boolean isStackAssignment(Exprent first,
	                                         Exprent second) {
		if (first.type == Exprent.EXPRENT_ASSIGNMENT && second.type == Exprent.EXPRENT_ASSIGNMENT) {
			AssignmentExprent asf = (AssignmentExprent) first;
			AssignmentExprent ass = (AssignmentExprent) second;
		...
		if (first.type == Exprent.EXPRENT_ASSIGNMENT) {
			...
			if (as.getRight().type == Exprent.EXPRENT_FUNCTION) {
				...
				if (func.getFuncType() == FunctionExprent.FUNCTION_ADD || func.getFuncType() == FunctionExprent.FUNCTION_SUB) {
					Exprent econd = func.getLstOperands()
					                    .get(0);
					Exprent econst = func.getLstOperands()
					                     .get(1);

					if (econst.type != Exprent.EXPRENT_CONST
					    && econd.type == Exprent.EXPRENT_CONST
					    && func.getFuncType() == FunctionExprent.FUNCTION_ADD) {
						econd = econst;
						econst = func.getLstOperands()
						             .get(0);
					}
					...
					if (econst.type == Exprent.EXPRENT_CONST && ((ConstExprent) econst).hasValueOne()) {
						...
						if (left.type != Exprent.EXPRENT_VAR && left.equals(econd)) {
							int type = func.getFuncType() == FunctionExprent.FUNCTION_ADD
							           ? FunctionExprent.FUNCTION_PPI
							           : FunctionExprent.FUNCTION_MMI;
							FunctionExprent ret = new FunctionExprent(type,
							                                          econd,
							                                          func.bytecode);
	...
	private static boolean isIPPorIMM(Exprent first,
	                                  Exprent second) {
	...
	private static boolean isIPPorIMM2(Exprent first,
	                                   Exprent second) {
		if (first.type != Exprent.EXPRENT_ASSIGNMENT || second.type != Exprent.EXPRENT_ASSIGNMENT) {
			return false;
		}
	...
	private static boolean hasQualifiedNewGetClass(Statement parent,
	                                               Statement child) {
		if (child.type == Statement.TYPE_BASICBLOCK && child.getExprents() != null && !child.getExprents()
		                                                                                    .isEmpty()) {
			Exprent firstExpr = child.getExprents()
			                         .get(child.getExprents()
			                                   .size() - 1);

			if (parent.type == Statement.TYPE_IF) {
				if (isQualifiedNewGetClass(firstExpr,
				                           ((IfStatement) parent).getHeadexprent()
				                                                 .getCondition())) {
					child.getExprents()
					     .remove(firstExpr);
					return true;
	...
	private static boolean isQualifiedNewGetClass(Exprent first,
	                                              Exprent second) {
		if (first.type == Exprent.EXPRENT_INVOCATION) {
			InvocationExprent invocation = (InvocationExprent) first;

			if ((!invocation.isStatic() && invocation.getName()
			                                         .equals("getClass") && invocation.getStringDescriptor()
			                                                                          .equals("()Ljava/lang/Class;")) // J8
			    || (invocation.isStatic()
			        && invocation.getClassname()
			                     .equals("java/util/Objects")
			        && invocation.getName()
			                     .equals("requireNonNull")
			        && invocation.getStringDescriptor()
			                     .equals("(Ljava/lang/Object;)Ljava/lang/Object;"))) { // J9+
				...
				if (invocation.isStatic()) { // Objects.requireNonNull(target) (J9+)
					// detect target type
					target = invocation.getLstParameters()
					                   .get(0);
				...
				while (!lstExprents.isEmpty()) {
					...
					if (expr.type == Exprent.EXPRENT_NEW) {
						...
						if (newExpr.getConstructor() != null && !newExpr.getConstructor()
						                                                .getLstParameters()
						                                                .isEmpty() && (newExpr.getConstructor()
						                                                                      .getLstParameters()
						                                                                      .get(0)
						                                                                      .equals(target)
						                                                               || isUnambiguouslySameParam(invocation.isStatic(),
						                                                                                           target,
	...
	private static boolean isUnambiguouslySameParam(boolean isStatic,
	                                                Exprent target,
	                                                List<Exprent> parameters) {
		boolean firstParamOfSameType = parameters.get(0)
		                                         .getExprType()
		                                         .equals(target.getExprType());
		if (!isStatic) { // X.getClass()/J8, this is less likely to overlap with legitimate use
			return firstParamOfSameType;
		}
		// Calling Objects.requireNonNull and discarding the result is a common pattern in normal code, so we have to be a
		// bit more
		// cautious about stripping calls when a constructor takes parameters of the instance type
		// ex. given a class X, `Objects.requireNonNull(someInstanceOfX); new X(someInstanceOfX)` should not have the rNN
		// stripped.
	...
	private static boolean isConstructorInvocationRemote(List<Exprent> list,
	                                                     int index) {
		Exprent current = list.get(index);
	...
	private static Exprent isLambda(Exprent exprent,
	                                StructClass cl) {
		List<Exprent> lst = exprent.getAllExprents();
		for (Exprent expr : lst) {
			Exprent ret = isLambda(expr,
			                       cl);
			if (ret != null) {
		...
		if (exprent.type == Exprent.EXPRENT_INVOCATION) {
			...
			if (in.getInvocationTyp() == InvocationExprent.INVOKE_DYNAMIC) {
				...
				ClassNode lambda_class = DecompilerContext.getClassProcessor()
				                                          .getMapRootClasses()
				                                          .get(lambda_class_name);

				if (lambda_class != null) { // real lambda class found, replace invocation with an anonymous class
					NewExprent newExpr = new NewExprent(new VarType(lambda_class_name,
					                                                true),
					                                    null,
					                                    0,
					                                    in.bytecode);
					newExpr.setConstructor(in);
					// note: we don't set the instance to null with in.setInstance(null) like it is done for a common
	...
	private static boolean buildIff(Statement stat,
		...
		if (stat.type == Statement.TYPE_IF && stat.getExprents() == null) {
			...
			BitSet ifHeadExprBytecode = (ifHeadExpr == null
			                             ? null
			                             : ifHeadExpr.bytecode);
			...
			if (statement.iftype == IfStatement.IFTYPE_IFELSE) {
				...
				if (ifStatement.getExprents() != null
				    && ifStatement.getExprents()
				                  .size() == 1
				    && elseStatement.getExprents() != null
				    && elseStatement.getExprents()
				                    .size() == 1
				    && ifStatement.getAllSuccessorEdges()
				                  .size() == 1
				    && elseStatement.getAllSuccessorEdges()
				                    .size() == 1
				    && ifStatement.getAllSuccessorEdges()
				                  .get(0)
				                  .getDestination() == elseStatement.getAllSuccessorEdges()
				                                                    .get(0)
				                                                    .getDestination()) {
					Exprent ifExpr = ifStatement.getExprents()
					                            .get(0);
					Exprent elseExpr = elseStatement.getExprents()
					                                .get(0);
					...
					if (ifExpr.type == Exprent.EXPRENT_ASSIGNMENT && elseExpr.type == Exprent.EXPRENT_ASSIGNMENT) {
						...
						if (ifAssign.getLeft().type == Exprent.EXPRENT_VAR && elseAssign.getLeft().type == Exprent.EXPRENT_VAR) {
							...
							if (ifVar.getIndex() == elseVar.getIndex()
							    && ifVar.isStack()) { // ifVar.getIndex() >= VarExprent.STACK_BASE) {
								boolean found = false;

								// Can happen in EliminateLoopsHelper
								if (ssa == null) {
									throw new IllegalStateException("Trying to make ternary but have no SSA-Form! How is this "
									                                + "possible?");
								}

								for (Entry<VarVersionPair, FastSparseSet<Integer>> ent : ssa.getPhi()
								                                                            .entrySet()) {
									if (ent.getKey().var == ifVar.getIndex()) {
										if (ent.getValue()
										       .contains(ifVar.getVersion()) && ent.getValue()
								...
								if (found) {
									List<Exprent> data = new ArrayList<>(statement.getFirst()
									                                              .getExprents());

									List<Exprent> operands = Arrays.asList(statement.getHeadexprent()
									                                                .getCondition(),
									                                       ifAssign.getRight(),
									                                       elseAssign.getRight());
									data.add(new AssignmentExprent(ifVar,
									...
									if (statement.getAllSuccessorEdges()
									             .isEmpty()) {
										StatEdge ifEdge = ifStatement.getAllSuccessorEdges()
										                             .get(0);
										StatEdge edge = new StatEdge(ifEdge.getType(),
										                             statement,
						...
						if (ifExit.getExitType() == elseExit.getExitType()
						    && ifExit.getValue() != null
						    && elseExit.getValue() != null
							...
							if (ifExit.getExitType() == ExitExprent.EXIT_THROW && !ifExit.getValue()
							                                                             .getExprType()
							                                                             .equals(elseExit.getValue()
							                                                                             .getExprType())) {  //
							...
							data.add(new ExitExprent(ifExit.getExitType(),
							                         new FunctionExprent(FunctionExprent.FUNCTION_IIF,
							                                             Arrays.asList(statement.getHeadexprent()
							                                                                    .getCondition(),
							                                                           ifExit.getValue(),
							                                                           elseExit.getValue()),
							                                             ifHeadExprBytecode),
							                         ifExit.getRetType(),
							                         ifHeadExprBytecode,
							                         ifExit.getMethodDescriptor()));
							statement.setExprents(data);

							StatEdge retEdge = ifStatement.getAllSuccessorEdges()
							                              .get(0);
							Statement closure = retEdge.closure == statement
							                    ? statement.getParent()
							                    : retEdge.closure;
							statement.addSuccessor(new StatEdge(StatEdge.TYPE_BREAK,
							                                    statement,
							                                    retEdge.getDestination(),
							                                    closure));
	...
	private static boolean collapseInlinedClass14(Statement stat) {
		...
		if (ret) {
			...
			assignment.replaceExprent(assignment.getRight(),
			                          new ConstExprent(VarType.VARTYPE_CLASS,
			                                           class_name,
			                                           null));

			List<Exprent> data = new ArrayList<>(stat.getFirst()
			                                         .getExprents());
			...
			if (wrapper != null) {
				wrapper.getHiddenMembers()
				       .add(InterpreterUtil.makeUniqueKey(fieldExpr.getName(),
	...
	public boolean simplifyStackVarsStatement(Statement stat,
		...
		if (expressions == null) {
			...
			while (true) {
				...
				for (Statement st : stat.getStats()) {
					res |= simplifyStackVarsStatement(st,
					                                  setReorderedIfs,
					                                  ssa,
					                                  cl);

					changed = IfHelper.mergeIfs(st,
					                            setReorderedIfs) ||  // collapse composed if's
					          buildIff(st,
					                   ssa) ||  // collapse iff ?: statement
					          processClass14
					          && collapseInlinedClass14(st);  // collapse inlined .class property in version 1.4 and before
					...
					if (!st.getStats()
					       .isEmpty() && hasQualifiedNewGetClass(st,
					                                             st.getStats()
					                                               .get(0))) {
						break;
					}
			...
			res = simplifyStackVarsExprents(expressions,
			                                cl);
		}
	...
	private boolean simplifyStackVarsExprents(List<Exprent> list,
		...
		while (index < list.size()) {
			...
			// constructor invocation
			if (isConstructorInvocationRemote(list,
			                                  index)) {
				list.remove(index);
				res = true;
				continue;
			...
			if (DecompilerContext.getOption(IFernflowerPreferences.REMOVE_GET_CLASS_NEW)) {
				if (isQualifiedNewGetClass(current,
				                           next)) {
					list.remove(index);
					res = true;
					continue;
			...
			int arrCount = isArrayInitializer(list,
			                                  index);
			if (arrCount > 0) {
				for (int i = 0;
				     i < arrCount;
			...
			if (addArrayInitializer(current,
			                        next)) {
				list.remove(index + 1);
				res = true;
				continue;
			...
			// expr++ and expr--
			if (isIPPorIMM(current,
			               next) || isIPPorIMM2(current,
			                                    next)) {
				list.remove(index + 1);
				res = true;
				continue;
			...
			if (!firstInvocation && isStackAssignment2(current,
			                                           next)) {
				list.remove(index + 1);
				res = true;
				continue;
