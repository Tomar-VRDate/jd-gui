...
public final class DecHelper {
	...
	public static boolean checkStatementExceptions(List<? extends Statement> lst) {
		...
		for (Statement stat : lst) {
			Set<Statement> setNew = stat.getNeighboursSet(StatEdge.TYPE_EXCEPTION,
		...
		for (int i = 1;
			...
			if (!stat.getPredecessorEdges(StatEdge.TYPE_EXCEPTION)
			         .isEmpty() && !handlers.contains(stat)) {
				return false;
			}
		}
	...
	public static boolean isChoiceStatement(Statement head,
	                                        List<? super Statement> lst) {

		Statement post = null;

		Set<Statement> setDest = head.getNeighboursSet(StatEdge.TYPE_REGULAR,
		                                               Statement.DIRECTION_FORWARD);
		...
		while (true) {
			...
			for (Statement stat : setDest) {
				...
				// preds
				Set<Statement> setPred = stat.getNeighboursSet(StatEdge.TYPE_REGULAR,
				                                               Statement.DIRECTION_BACKWARD);
				setPred.remove(head);
				...
				if (!setDest.containsAll(setPred) || setPred.size() > 1) {
					...
					Statement pred = setPred.iterator()
					                        .next();
					while (lst.contains(pred)) {
						Set<Statement> setPredTemp = pred.getNeighboursSet(StatEdge.TYPE_REGULAR,
						                                                   Statement.DIRECTION_BACKWARD);
						setPredTemp.remove(head);

						if (!setPredTemp.isEmpty()) { // at most 1 predecessor
							pred = setPredTemp.iterator()
							                  .next();
							if (pred == stat) {
				...
				if (lstEdges.size() > 1) {
					Set<Statement> setSucc = stat.getNeighboursSet(Statement.STATEDGE_DIRECT_ALL,
					                                               Statement.DIRECTION_FORWARD);
					...
					if (edge.getType() == StatEdge.TYPE_REGULAR) {
						...
						if (post != statd && !setDest.contains(statd)) {
							if (post != null) {
								...
								Set<Statement> set = statd.getNeighboursSet(StatEdge.TYPE_REGULAR,
								                                            Statement.DIRECTION_BACKWARD);
								if (set.size() > 1) {
									post = statd;
									repeat = true;
	...
	// Finds all catch blocks that this statement can flow to, and retain those that only have a single exception
	// predecessor
	...
	public static Set<Statement> getUniquePredExceptions(Statement head) {
		Set<Statement> setHandlers = new HashSet<>(head.getNeighbours(StatEdge.TYPE_EXCEPTION,
		                                                              Statement.DIRECTION_FORWARD));
		setHandlers.removeIf(statement -> statement.getPredecessorEdges(StatEdge.TYPE_EXCEPTION)
		                                           .size() > 1);
		return setHandlers;
	}
