...
public class StructMethod
	...
	private static final int[]                   opr_iconst    = {-1,
	                                                              0,
	                                                              1,
	                                                              2,
	                                                              3,
	                                                              4,
	                                                              5};
	private static final int[]                   opr_loadstore = {0,
	                                                              1,
	                                                              2,
	                                                              3,
	                                                              0,
	                                                              ...
	                                                              2,
	                                                              3};
	private static final int[]                   opcs_load     = {opc_iload,
	                                                              opc_lload,
	                                                              opc_fload,
	                                                              opc_dload,
	                                                              opc_aload};
	private static final int[]                   opcs_store    = {opc_istore,
	                                                              opc_lstore,
	                                                              opc_fstore,
	                                                              opc_dstore,
	                                                              opc_astore};
	...
	public static StructMethod create(DataInputFullStream in,
	                                  ConstantPool pool,
	                                  String clQualifiedName,
	                                  BytecodeVersion bytecodeVersion,
	                                  boolean own)
					throws
					IOException {
		...
		String[] values = pool.getClassElement(ConstantPool.METHOD,
		                                       clQualifiedName,
		                                       nameIndex,
		                                       descriptorIndex);
		...
		return new StructMethod(accessFlags,
		                        attributes,
		                        values[0],
		                        values[1],
		                        bytecodeVersion,
	...
	public void expandData(StructClass classStruct)
					throws
					IOException {
		if (codeAndExceptions != null && !expanded) {
			seq = parseBytecode(new DataInputFullStream(codeAndExceptions),
			                    classStruct.getPool());
			expanded = true;
		}
	}
	...
	private InstructionSequence parseBytecode(DataInputFullStream in,
	                                          ConstantPool pool)
					throws
					IOException {
		...
		for (int i = 0;
			...
			Instruction instr = Instruction.create(opcode,
			                                       wide,
			                                       group,
			                                       bytecodeVersion,
			                                       ops,
		...
		for (int i = 0;
			...
			if (excclass != 0) {
				handler.exceptionClass = pool.getPrimitiveConstant(excclass)
				                             .getString();
			}
	...
	public IVariableNameProvider getVariableNamer() {
		if (renamer == null) {
			this.renamer = DecompilerContext.getNamingFactory()
			                                .createFactory(this);
		}
		return renamer;
	}
