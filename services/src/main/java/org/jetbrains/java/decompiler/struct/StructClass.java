...
public class StructClass
	...
	private StructClass(int accessFlags,
		...
		this.own = own;
		this.loader = loader;
		this.version = new BytecodeVersion(majorVersion,
		                                   minorVersion);
	...
	public static StructClass create(DataInputFullStream in,
	                                 boolean own,
	                                 LazyLoader loader)
					throws
					IOException {
		...
		BytecodeVersion bytecodeVersion = new BytecodeVersion(majorVersion,
		                                                      minorVersion);
		...
		String qualifiedName = pool.getPrimitiveConstant(thisClassIdx)
		                           .getString();
		...
		for (int i = 0;
			...
			interfaceNames[i] = pool.getPrimitiveConstant(interfaces[i])
			                        .getString();
		}
		...
		for (int i = 0;
		     i < length;
		     i++) {
			StructField field = StructField.create(in,
			                                       pool,
			                                       qualifiedName,
			                                       bytecodeVersion);
			fields.addWithKey(field,
			                  InterpreterUtil.makeUniqueKey(field.getName(),
			                                                field.getDescriptor()));
		}
		...
		for (int i = 0;
		     i < length;
		     i++) {
			StructMethod method = StructMethod.create(in,
			                                          pool,
			                                          qualifiedName,
			                                          bytecodeVersion,
			                                          own);
			String key = InterpreterUtil.makeUniqueKey(method.getName(),
			                                           method.getDescriptor());
			if (methods.containsKey(key)) {
				String fullName = qualifiedName + "." + method.getName() + method.getDescriptor();
				DecompilerContext.getLogger()
				                 .writeMessage("Duplicate method " + fullName,
		...
		Map<String, StructGeneralAttribute> attributes = readAttributes(in,
		                                                                pool,
		                                                                bytecodeVersion);
		...
		if (DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_GENERIC_SIGNATURES)) {
			StructGenericSignatureAttribute signatureAttr
							= (StructGenericSignatureAttribute) attributes.get(StructGeneralAttribute.ATTRIBUTE_SIGNATURE.name);
			if (signatureAttr != null) {
				signature = GenericMain.parseClassSignature(qualifiedName,
				                                            signatureAttr.getSignature());
			}
		}

		StructClass cl = new StructClass(accessFlags,
		                                 attributes,
		                                 qualifiedName,
		                                 superClass,
		                                 own,
	...
	public StructField getField(String name,
	                            String descriptor) {
		return fields.getWithKey(InterpreterUtil.makeUniqueKey(name,
		                                                       descriptor));
	}
	...
	public StructMethod getMethod(String name,
	                              String descriptor) {
		return methods.getWithKey(InterpreterUtil.makeUniqueKey(name,
		                                                        descriptor));
	}

	public StructMethod getMethodRecursive(String name,
	                                       String descriptor) {
		StructMethod ret = getMethod(name,
		                             descriptor);
		...
		if (superClass != null) {
			StructClass cls = DecompilerContext.getStructContext()
			                                   .getClass((String) superClass.value);
			if (cls != null) {
				ret = cls.getMethodRecursive(name,
				                             descriptor);
				if (ret != null) {
					return ret;
				}
	...
	private Map<VarType, VarType> getGenericMap(VarType type) {
		...
		if (gtype.getArguments()
		         .size() != this.signature.fparameters.size()) { //Invalid instance type?
			return Collections.emptyMap();
		}

		Map<VarType, VarType> ret = new HashMap<>();
		for (int x = 0;
		     x < this.signature.fparameters.size();
		     x++) {
			VarType var = gtype.getArguments()
			                   .get(x);
			if (var != null) {
				ret.put(GenericType.parse("T" + this.signature.fparameters.get(x) + ";"),
				        var);
			}
		}
		return ret;
	...
	public Map<String, Map<VarType, VarType>> getAllGenerics() {
		...
		if (this.signature != null) {
			for (VarType intf : this.signature.superinterfaces) {
				...
				if (cls != null) {
					...
					for (Entry<String, Map<VarType, VarType>> e : cls.getAllGenerics()
					                                                 .entrySet()) {
						if (e.getValue()
						     .isEmpty()) {
							ret.put(e.getKey(),
							...
							for (Entry<VarType, VarType> e2 : e.getValue()
							                                   .entrySet()) {
								sub.put(e2.getKey(),
								        sig.getOrDefault(e2.getValue(),
								                         e2.getValue()));
							}
							ret.put(e.getKey(),
							        sub);
		...
		if (this.superClass != null) {
			StructClass cls = DecompilerContext.getStructContext()
			                                   .getClass((String) this.superClass.value);
			if (cls != null) {
				Map<VarType, VarType> sig = this.signature == null
				                            ? Collections.emptyMap()
				                            : cls.getGenericMap(this.signature.superclass);
				if (sig.isEmpty()) {
					ret.putAll(cls.getAllGenerics());
				} else {
					for (Entry<String, Map<VarType, VarType>> e : cls.getAllGenerics()
		...
		this.genericHiarachy = ret.isEmpty()
		                       ? Collections.emptyMap()
		                       : ret;
