...
public class VarProcessor {
	...
	public VarProcessor(StructMethod mt,
	                    MethodDescriptor md) {
		method = mt;
		methodDescriptor = md;
	}
	...
	public void setVarDefinitions(Statement root) {
		...
		new VarDefinitionHelper(root,
		                        method,
		                        this).setVarDefinitions();
	}
	...
	public void setDebugVarNames(Map<VarVersionPair, String> mapDebugVarNames) {
		...
		for (VarVersionPair pair : listVars) {
			...
			if (key != null) {
				...
				if (debugName != null && TextUtil.isValidIdentifier(debugName,
				                                                    method.getBytecodeVersion())) {
					name = debugName;
					lvtName = true;
				}
			}
	...
	public Integer getVarOriginalIndex(int index) {
		...
		final VarVersionPair pair = varVersions.getMapOriginalVarIndices()
		                                       .get(index);
		return pair == null
		       ? null
		       : pair.var;
	...
	public void refreshVarNames(VarNamesCollector vc) {
		...
		for (Entry<VarVersionPair, String> ent : tempVarNames.entrySet()) {
			mapVarNames.put(ent.getKey(),
			                vc.getFreeName(ent.getValue()));
		}
	}
	...
	public VarType getVarType(VarVersionPair pair) {
		return varVersions == null
		       ? null
		       : varVersions.getVarType(pair);
	}

	public void setVarType(VarVersionPair pair,
	                       VarType type) {
		if (varVersions != null) {
			varVersions.setVarType(pair,
			                       type);
		}
	}

	public String getVarName(VarVersionPair pair) {
		return mapVarNames == null
		       ? null
		       : mapVarNames.get(pair);
	}

	public void setVarName(VarVersionPair pair,
	                       String name) {
		mapVarNames.put(pair,
		                name);
	}
	...
	public int getVarFinal(VarVersionPair pair) {
		return varVersions == null
		       ...
		       : varVersions.getVarFinal(pair);
	}

	public void setVarFinal(VarVersionPair pair,
	                        int finalType) {
		varVersions.setVarFinal(pair,
		                        finalType);
	}
	...
	public List<LocalVariable> getCandidates(int origindex) {
		...
		return method.getLocalVariableAttr()
		             .matchingVars(origindex)
		             .collect(Collectors.toList());
	}

	public void findLVT(VarExprent exprent,
	                    int start) {
		if (!hasLVT()) {
			return;
		}

		LocalVariable lvt = method.getLocalVariableAttr()
		                          .getVariables()
		                          .filter(v -> v.getVersion().var == exprent.getIndex() && v.getStart() == start)
		                          .findFirst()
		                          .orElse(null);
	...
	public void copyVarInfo(VarVersionPair from,
	                        VarVersionPair to) {
		setVarName(to,
		           getVarName(from));
		setVarFinal(to,
	...
	public Map<Integer, LocalVariable> getLocalVariables(Statement stat) {
		...
		Map<Integer, LocalVariable> ret = method.getLocalVariableAttr()
		                                        .getVariables()
		                                        .filter(lv -> lv.getEnd() > sep.start && lv.getStart() <= sep.end)
		                                        .collect(Collectors.toMap(lv -> lv.getVersion().var,
		                                                                  lv -> lv,
		                                                                  (lv1, lv2) -> {
			                                                                  //System.out.println("DUPLICATE INDEX FOR
			                                                                  // SCOPE: (" +sep +") " + lv1.toString() + " "
			                                                                  // + lv2.toString());
	...
	public void setVarVersions(RootStatement root) {
		...
		varVersions = new VarVersionsProcessor(method,
		                                       methodDescriptor);
		varVersions.setVarVersions(root,
		                           oldProcessor);
	}

	public void setVarLVT(VarVersionPair var,
	                      LocalVariable lvt) {
		mapVarLVTs.put(var,
		               lvt);
	}
