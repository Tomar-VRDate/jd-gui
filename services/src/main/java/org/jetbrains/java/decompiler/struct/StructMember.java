...
public abstract class StructMember {
	...
	protected StructMember(int accessFlags,
	                       Map<String, StructGeneralAttribute> attributes) {
		this.accessFlags = accessFlags;
		this.attributes = attributes;
	}

	public static Map<String, StructGeneralAttribute> readAttributes(DataInputFullStream in,
	                                                                 ConstantPool pool,
	...
	public static Map<String, StructGeneralAttribute> readAttributes(DataInputFullStream in,
		...
		for (int i = 0;
			...
			String name = pool.getPrimitiveConstant(nameIndex)
			                  .getString();
			...
			if (attribute == null || (!readCode && attribute instanceof StructCodeAttribute)) {
				...
				} else if (StructGeneralAttribute.ATTRIBUTE_LOCAL_VARIABLE_TYPE_TABLE.name.equals(name)
		...
		if (attributes.containsKey(ATTRIBUTE_LOCAL_VARIABLE_TABLE.name)
		    && attributes.containsKey(ATTRIBUTE_LOCAL_VARIABLE_TYPE_TABLE.name)) {
	...
	protected StructGeneralAttribute readAttribute(DataInputFullStream in,
	                                               ConstantPool pool,
	                                               String name)
					throws
					IOException {
