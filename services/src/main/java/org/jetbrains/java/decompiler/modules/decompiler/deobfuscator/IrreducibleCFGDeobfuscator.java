...
public final class IrreducibleCFGDeobfuscator {
	...
	public static boolean isStatementIrreducible(Statement statement) {
		...
		for (Statement stat : statement.getStats()) {
			if (!stat.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)
			         .isEmpty()) {
				return false;
			}

			mapNodes.put(stat.id,
			             new Node(stat.id));
		}
		...
		for (Statement stat : statement.getStats()) {
			...
			for (Statement succ : stat.getNeighbours(StatEdge.TYPE_REGULAR,
	...
	private static Statement getCandidateForSplitting(Statement statement) {
		...
		for (Statement stat : statement.getStats()) {
			...
			Set<Statement> setPreds = stat.getNeighboursSet(StatEdge.TYPE_REGULAR,
			                                                Statement.DIRECTION_BACKWARD);

			if (setPreds.size() > 1) {
				int succCount = stat.getNeighboursSet(StatEdge.TYPE_REGULAR,
				                                      Statement.DIRECTION_FORWARD)
				                    .size();
	...
	public static boolean splitIrreducibleNode(Statement statement) {
		...
		StatEdge enteredge = splitnode.getPredecessorEdges(StatEdge.TYPE_REGULAR)
		                              .iterator()
		                              .next();

		// copy the smallest statement
		Statement splitcopy = copyStatement(splitnode,
		                                    null,
		                                    new HashMap<>());
		...
		for (StatEdge succ : splitnode.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)) {
			splitcopy.addSuccessor(new StatEdge(succ.getType(),
			                                    splitcopy,
			                                    succ.getDestination(),
			                                    succ.closure));
		}
	...
	private static int getStatementSize(Statement statement) {
		...
		if (statement.type == Statement.TYPE_BASICBLOCK) {
			res = ((BasicBlockStatement) statement).getBlock()
			                                       .getSeq()
			                                       .length();
		} else {
			res = statement.getStats()
			               .stream()
			               .mapToInt(IrreducibleCFGDeobfuscator::getStatementSize)
			               .sum();
		}
	...
	private static Statement copyStatement(Statement from,
	                                       Statement to,
		...
		for (int i = 0;
		     i < from.getStats()
		             .size();
		     i++) {
			Statement stold = from.getStats()
			                      .get(i);
			Statement stnew = to.getStats()
			                    .get(i);
			...
			for (StatEdge edgeold : stold.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)) {
				...
				StatEdge edgenew = new StatEdge(edgeold.getType(),
				                                stnew,
