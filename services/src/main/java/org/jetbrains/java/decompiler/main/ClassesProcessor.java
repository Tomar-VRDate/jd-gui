...
public class ClassesProcessor
	...
	private static boolean isAnonymous(StructClass cl,
	                                   StructClass enclosingCl) {
		// checking super class and interfaces
		int[] interfaces = cl.getInterfaces();
		if (interfaces.length > 0) {
			boolean hasNonTrivialSuperClass = cl.superClass != null
			                                  && !VarType.VARTYPE_OBJECT.equals(new VarType(cl.superClass.getString(),
			                                                                                true));
			if (hasNonTrivialSuperClass || interfaces.length > 1) { // can't have multiple 'sources'
				String message = "Inconsistent anonymous class definition: '"
				                 + cl.qualifiedName
		...
		String enclosingMethod = attribute != null
		                         ? attribute.getMethodName()
		                         : null;
		...
		for (StructMethod mt : enclosingCl.getMethods()) {
				...
				if (seq != null) {
					...
					for (int i = 0;
						...
						switch (instr.opcode) {
							case opc_checkcast:
							case opc_instanceof:
								if (cl.qualifiedName.equals(pool.getPrimitiveConstant(instr.operand(0))
								                                .getString())) {
									refCounter++;
									refNotNew = true;
								}
			...
			if (refCounter > 1 || refNotNew) {
				String message = "Inconsistent references to the class '"
				                 + cl.qualifiedName
	...
	private static boolean shouldInitEarly(ClassNode node) {
		if (node.classStruct.hasModifier(CodeConstants.ACC_SYNTHETIC)) {
			if (node.classStruct.getMethods()
			                    .size() == 1 && node.classStruct.getMethods()
			                                                    .get(0)
			                                                    .getName()
			                                                    .equals(CodeConstants.CLINIT_NAME)) {
				return node.classStruct.getFields()
				                       .stream()
				                       .allMatch(stField -> stField.getDescriptor()
				                                                   .equals("[I")
				                                            && stField.hasModifier(SwitchHelper.STATIC_FINAL_SYNTHETIC));
			}
		}
		return false;
	}

	private static void addClassNameToImport(ClassNode node,
	                                         ImportCollector imp) {
		if (node.simpleName != null && node.simpleName.length() > 0) {
			imp.getShortName(node.type == ClassNode.CLASS_ROOT
	...
	private static void destroyWrappers(ClassNode node) {
		...
		node.classStruct.getMethods()
		                .forEach(m -> m.clearVariableNamer());
		...
		for (StructClass cl : context.getOwnClasses()
			...
			if (!mapRootClasses.containsKey(cl.qualifiedName)) {
						...
						for (StructInnerClassesAttribute.Entry entry : inner.getEntries()) {
							...
							if (savedName != null) {
								simpleName = savedName;
							           ...
							           && renamer != null
							           && renamer.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_CLASS,
							                                  simpleName,
							                                  null,
							                                  null)) {
								simpleName = renamer.getNextClassName(innerName,
								                                      simpleName);
								mapNewSimpleNames.put(innerName,
								                      simpleName);
							}

							Inner rec = new Inner();
							rec.simpleName = simpleName;
							rec.type = entry.simpleNameIdx == 0
							           ? ClassNode.CLASS_ANONYMOUS
							           : entry.outerNameIdx == 0
							             ? ClassNode.CLASS_LOCAL
							...
							if (entry.enclosingName != null) {
								...
								if (in != null) {
									StructEnclosingMethodAttribute attr
													= in.getAttribute(StructGeneralAttribute.ATTRIBUTE_ENCLOSING_METHOD);
									if (attr != null) {
							...
							// enclosing class
							String enclClassName = entry.outerNameIdx != 0
							                       ? entry.enclosingName
							                       : cl.qualifiedName;
							...
							if (enclosingClass != null && enclosingClass.isOwn()) { // own classes only
								...
								if (existingRec == null) {
									mapInnerClasses.put(innerName,
									                    rec);
								} else if (!Inner.equal(existingRec,
								                        rec)) {
									int oldPriority = existingRec.source.equals(innerName)
									                  ? 1
									                  : existingRec.source.equals(enclClassName)
									                    ? 2
									                    : 3;
									int newPriority = rec.source.equals(innerName)
									                  ? 1
									                  : rec.source.equals(enclClassName)
									                    ? 2
									                    : 3;
									if (DecompilerContext.getOption(IFernflowerPreferences.WARN_INCONSISTENT_INNER_CLASSES)) {
										...
										DecompilerContext.getLogger()
										                 .writeMessage(message,
										                               IFernflowerLogger.Severity.WARN);
										DecompilerContext.getLogger()
										                 .writeMessage("  " + existingRec.source + ": " + existingRec,
										                               IFernflowerLogger.Severity.WARN);
										DecompilerContext.getLogger()
										                 .writeMessage("  " + rec.source + ": " + rec,
										                               IFernflowerLogger.Severity.WARN);
										if (newPriority < oldPriority) {
											DecompilerContext.getLogger()
											                 .writeMessage("Changing mapping to " + innerName + " -> " + rec,
											                               IFernflowerLogger.Severity.WARN);
										}
									}
									if (newPriority < oldPriority) {
								...
								mapNestedClassReferences.computeIfAbsent(enclClassName,
								                                         k -> new HashSet<>())
								                        .add(innerName);
								// reference to the enclosing class
								mapEnclosingClassReferences.computeIfAbsent(innerName,
								                                            k -> new HashSet<>())
								                           .add(enclClassName);
							}
						}
				...
				if (isWhitelisted(cl.qualifiedName)) {
					ClassNode node = new ClassNode(ClassNode.CLASS_ROOT,
					                               cl);
					node.access = cl.getAccessFlags();
					mapRootClasses.put(cl.qualifiedName,
					                   node);
				}
			}
		}
				...
				if (!mapInnerClasses.containsKey(ent.getKey())) {
					...
					while (!stack.isEmpty()) {
						...
						if (setNestedClasses != null) {
							...
							if (inner == null || inner.getEntries()
							                          .isEmpty()) {
								DecompilerContext.getLogger()
							...
							for (StructInnerClassesAttribute.Entry entry : inner.getEntries()) {
								...
								if (verifyAnonymousClasses
								    && nestedNode.type == ClassNode.CLASS_ANONYMOUS
								    && !isAnonymous(nestedNode.classStruct,
								                    scl)) {
								...
								if (nestedNode.type == ClassNode.CLASS_ANONYMOUS) {
									...
									int[] interfaces = cl.getInterfaces();
									if (interfaces.length > 0) {
										nestedNode.anonymousClassType = new VarType(cl.getInterface(0),
										                                            true);
									} else {
										nestedNode.anonymousClassType = new VarType(cl.superClass.getString(),
										                                            true);
									}
	...
	public void writeClass(StructClass cl,
	                       TextBuffer buffer)
					throws
					IOException {
		...
		boolean moduleInfo = cl.hasModifier(CodeConstants.ACC_MODULE)
		                     && cl.hasAttribute(StructGeneralAttribute.ATTRIBUTE_MODULE);

		DecompilerContext.getLogger()
		                 .startReadingClass(cl.qualifiedName);
		try {
			ImportCollector importCollector = new ImportCollector(root);
			DecompilerContext.startClass(importCollector);

			if (packageInfo) {
				ClassWriter.packageInfoToJava(cl,
				                              buffer);

				importCollector.writeImports(buffer,
				                             false);
			} else if (moduleInfo) {
				TextBuffer moduleBuffer = new TextBuffer(AVERAGE_CLASS_SIZE);
				ClassWriter.moduleInfoToJava(cl,
				                             moduleBuffer);
				...
				try {
					...
					DecompilerContext.getLogger()
					                 .writeMessage("Class " + root.simpleName + " couldn't be written.",
					                               IFernflowerLogger.Severity.WARN,
					                               t);
					...
					if (DecompilerContext.getOption(IFernflowerPreferences.DUMP_EXCEPTION_ON_ERROR)) {
						...
						ClassWriter.collectErrorLines(t,
						                              lines);
				...
				try {
					new NestedClassProcessor().processClass(root,
					                                        root);
				...
				new ClassWriter().classToJava(root,
				                              classBuffer,
				                              0);
				classBuffer.reformat();
				classBuffer.getTracers()
				           .forEach((classAndMethod, tracer) -> {
					           // get the class by name
					           StructClass clazz = DecompilerContext.getStructContext()
					                                                .getClass(classAndMethod.a);
					           ...
					           if (method != null) {
						           ...
						           DecompilerContext.getBytecodeSourceMapper()
						                            .addTracer(classAndMethod.a,
						                                       classAndMethod.b,
						                                       tracer);
					           }
				           });
	...
	private static class Inner {
		...
		private static boolean equal(Inner o1,
		                             Inner o2) {
			return o1.type == o2.type
			       && o1.accessFlags == o2.accessFlags
			       && InterpreterUtil.equalObjects(o1.simpleName,
			                                       o2.simpleName)
			       && InterpreterUtil.equalObjects(o1.enclosingName,
			                                       o2.enclosingName);
		}
		...
		private String getType() {
			return type == ClassNode.CLASS_ANONYMOUS
			       ? "ANONYMOUS"
			       : type == ClassNode.CLASS_LAMBDA
			         ? "LAMBDA"
			         : type == ClassNode.CLASS_LOCAL
			           ? "LOCAL"
			           : type == ClassNode.CLASS_MEMBER
			             ? "MEMBER"
			             : type == ClassNode.CLASS_ROOT
			               ? "ROOT"
			               : "UNKNOWN(" + type + ")";
		}
	}
	...
	public static class ClassNode
		...
		public ClassNode(String content_class_name,
			...
			anonymousClassType = new VarType(lambda_class_name,
			                                 true);
			...
			lambdaInformation.is_content_method_static = (lambdaInformation.content_method_invocation_type
			                                              == CodeConstants.CONSTANT_MethodHandle_REF_invokeStatic); // FIXME
			// : redundant?
		}

		public ClassNode(int type,
		                 StructClass classStruct) {
			this.type = type;
			this.classStruct = classStruct;

			simpleName = classStruct.qualifiedName.substring(classStruct.qualifiedName.lastIndexOf('/') + 1);
			StructEnclosingMethodAttribute enclosingMethodAttr
							= classStruct.getAttribute(StructGeneralAttribute.ATTRIBUTE_ENCLOSING_METHOD);
			if (enclosingMethodAttr != null) {
				String name = enclosingMethodAttr.getMethodName();
				String desc = enclosingMethodAttr.getMethodDescriptor();
				if (name != null && desc != null) {
					this.enclosingMethod = InterpreterUtil.makeUniqueKey(name,
					                                                     desc);
				}
			}
		}
