...
public final class MergeHelper {
	...
	private static void matchDoWhile(DoStatement stat) {
		...
		if (last.type == Statement.TYPE_IF) {
			...
			if (lastif.iftype == IfStatement.IFTYPE_IF && lastif.getIfstat() == null) {
				...
				StatEdge elseedge = lastif.getAllSuccessorEdges()
				                          .get(0);

				if ((ifedge.getType() == StatEdge.TYPE_BREAK
				     && elseedge.getType() == StatEdge.TYPE_CONTINUE
				     && elseedge.closure == stat
				     && isDirectPath(stat,
				                     ifedge.getDestination())) || (ifedge.getType() == StatEdge.TYPE_CONTINUE
				                                                   && elseedge.getType() == StatEdge.TYPE_BREAK
				                                                   && ifedge.closure == stat
				                                                   && isDirectPath(stat,
					...
					IfExprent ifexpr = (IfExprent) lastif.getHeadexprent()
					                                     .copy();
					if (ifedge.getType() == StatEdge.TYPE_BREAK) {
						ifexpr.negateIf();
					}
					...
					if (stat.getConditionExprent() != null) {
						ifexpr.getCondition()
						      .addBytecodeOffsets(stat.getConditionExprent().bytecode);
					}
					ifexpr.getCondition()
					      .addBytecodeOffsets(lastif.getHeadexprent().bytecode);
					...
					if (lastif.getFirst()
					          .getExprents()
					          .isEmpty()) {
						removeLastEmptyStatement(stat,
						                         lastif);
					} else {
						lastif.setExprents(lastif.getFirst()
						                         .getExprents());

						StatEdge newedge = new StatEdge(StatEdge.TYPE_CONTINUE,
						                                lastif,
						                                stat);
					...
					if (stat.getAllSuccessorEdges()
					        .isEmpty()) {
						StatEdge edge = elseedge.getType() == StatEdge.TYPE_CONTINUE
						                ? ifedge
						                : elseedge;
				...
				if (firstif.iftype == IfStatement.IFTYPE_IF) {
					if (firstif.getIfstat() == null) {
						...
						if (isDirectPath(stat,
						                 ifedge.getDestination()) || addContinueOrBreak(stat,
						                                                                ifedge)) {
							// exit condition identified
							stat.setLooptype(DoStatement.LOOP_WHILE);

							// negate condition (while header)
							IfExprent ifexpr = (IfExprent) firstif.getHeadexprent()
							                                      .copy();
							ifexpr.negateIf();
							...
							ifexpr.getCondition()
							      .addBytecodeOffsets(firstif.getHeadexprent().bytecode);
							...
							firstif.getFirst()
							       .removeSuccessor(ifedge);
							firstif.removeSuccessor(firstif.getAllSuccessorEdges()
							                               .get(0));
							...
							if (firstif == stat.getFirst()) {
								BasicBlockStatement bstat
												= new BasicBlockStatement(new BasicBlock(DecompilerContext.getCounterContainer()
								...
								if (!sequence.getStats()
								             .isEmpty()) {
									sequence.setFirst(sequence.getStats()
									                          .get(0));
								} else {
					...
					StatEdge elseEdge = firstif.getAllSuccessorEdges()
					                           .get(0);
					boolean directlyConnectedToExit = directlyConnectedToExit(stat,
					                                                          elseEdge.getDestination()
					                                                                  .getParent(),
					                                                          elseEdge.getDestination());
					if (isDirectPath(stat,
						...
						if (directlyConnectedToExit) {
							...
							if (firstStat.type == Statement.TYPE_SEQUENCE
							    && firstStat.getStats()
							                .size() > 1) {
								// Get continues
								List<StatEdge> continues = firstStat.getStats()
								                                    .getLast()
								                                    .getSuccessorEdges(StatEdge.TYPE_CONTINUE);
								...
								for (int idx = 1;
								     idx < firstStat.getStats()
								                    .size();
								     idx++) {
									Statement st = firstStat.getStats()
									                        .get(idx);
									// Add to temp list
									toAdd.add(st);
								}
	...
	private static boolean isIif(Exprent exprent) {
		...
		while (check.type == Exprent.EXPRENT_FUNCTION
		       && ((FunctionExprent) check).getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT) {
			check = ((FunctionExprent) check).getLstOperands()
			                                 .get(0);
		}
	...
	private static void liftToParent(DoStatement stat,
	                                 List<Statement> toAdd) {
		VBStyleCollection<Statement, Integer> stats = stat.getParent()
		                                                  .getStats();
		if (stat.getParent().type == Statement.TYPE_SEQUENCE) {
			...
			for (Statement st : toAdd) {
				...
				stats.addWithKeyAndIndex(stats.indexOf(stat) + i,
				                         st,
				                         st.id);
			}
	...
	private static boolean directlyConnectedToExit(Statement loop,
	                                               Statement parent,
	                                               Statement endStat) {
		...
		if (successors.size() == 1) {
			...
			// If the destination is connected to the exit and the successor's closure points to our current loop, then we
			// have successfully found a direct path to the exit
			if (destination.type == Statement.TYPE_DUMMYEXIT && successor.closure == loop) {
				return true;
			}

			// If we didn't find a direct path but the destination contained within the parent statement (i.e a neighbor of
			// ours) then check that too [TestLoopBreakException]
			if (parent.containsStatement(destination)) {
				return directlyConnectedToExit(loop,
				                               parent,
				                               destination);
			}
		}
	...
	public static boolean isDirectPath(Statement stat,
	                                   Statement endstat) {
		Set<Statement> setStat = stat.getNeighboursSet(Statement.STATEDGE_DIRECT_ALL,
			...
			if (parent == null) {
				...
				switch (parent.type) {
					...
					case Statement.TYPE_SWITCH:
						...
						for (int i = 0;
						     i
						     < swst.getCaseStatements()
						           .size() - 1;
						     i++) {
							Statement stt = swst.getCaseStatements()
							                    .get(i);
							if (stt == stat) {
								Statement stnext = swst.getCaseStatements()
								                       .get(i + 1);

								if (stnext.getExprents() != null && stnext.getExprents()
								                                          .isEmpty()) {
									stnext = stnext.getAllSuccessorEdges()
									               .get(0)
									               .getDestination();
								}
	...
	private static void matchFor(DoStatement stat) {
		...
		if (lastData == null || lastData.getExprents()
		                                .isEmpty()) {
			return;
		}
		...
		if (lstExpr.size() == 1) {  // single exprent
			if (lastData.getAllPredecessorEdges()
			            .size() > 1) { // break edges
				issingle = true;
			}
		}

		boolean haslast = issingle
		                  || lastDoExprent.type == Exprent.EXPRENT_ASSIGNMENT
		...
		while (true) {
			...
			if (parent.type == Statement.TYPE_SEQUENCE) {
				if (current == parent.getFirst()) {
					...
					preData = current.getNeighbours(StatEdge.TYPE_REGULAR,
					                                Statement.DIRECTION_BACKWARD)
					                 .get(0);
					...
					if (preData != null && !preData.getExprents()
					                               .isEmpty()) {
						initDoExprent = preData.getExprents()
						                       .get(preData.getExprents()
						                                   .size() - 1);
		...
		if (hasinit || issingle) {  // FIXME: issingle sufficient?
			...
			// The nature of the for loop filter makes it so any loop that contains the pattern "1) assignment 2) loop 3)
			// assignment" will become a for loop.
			...
			Statement firstStat = stat.getFirst();
			while (firstStat.type == Statement.TYPE_SEQUENCE
			       && firstStat.getStats()
			                   .size() == 1) {
			...
			// Then filter to make sure the loop body is a basic block (Seems like it always is- but it never hurts to
			// double check!)
			if (firstStat.type == Statement.TYPE_BASICBLOCK) {

				// Last filter to make sure the basic block only has the final third assignment. If so, break out to make a
				// while loop instead as it will produce cleaner output.
				if (firstStat.getExprents()
				             .size() == 1) {
					return;
				}
			}
			...
			if (hasinit) {
				Exprent exp = preData.getExprents()
				                     .remove(preData.getExprents()
				                                    .size() - 1);
			...
			Exprent exp = lastData.getExprents()
			                      .remove(lastData.getExprents()
			                                      .size() - 1);
	...
	private static void cleanEmptyStatements(DoStatement dostat,
	                                         Statement stat) {
		if (stat != null && stat.getExprents()
		                        .isEmpty()) {
			List<StatEdge> lst = stat.getAllSuccessorEdges();
			if (!lst.isEmpty()) {
				stat.removeSuccessor(lst.get(0));
			}
			removeLastEmptyStatement(dostat,
			                         stat);
		}
	}

	private static void removeLastEmptyStatement(DoStatement dostat,
	                                             Statement stat) {
		...
		if (stat == dostat.getFirst()) {
			BasicBlockStatement bstat = new BasicBlockStatement(new BasicBlock(DecompilerContext.getCounterContainer()
			                                                                                    .getCounterAndIncrement(CounterContainer.STATEMENT_COUNTER)));
			bstat.setExprents(new ArrayList<>());
			dostat.replaceStatement(stat,
			                        bstat);
		} else {
			for (StatEdge edge : stat.getAllPredecessorEdges()) {
				edge.getSource()
				    .changeEdgeType(Statement.DIRECTION_FORWARD,
				                    edge,
				                    StatEdge.TYPE_CONTINUE);
	...
	private static Statement getLastDirectData(Statement stat) {
		...
		for (int i = stat.getStats()
		                 .size() - 1;
		     i >= 0;
		     i--) {
			Statement tmp = getLastDirectData(stat.getStats()
			                                      .get(i));
			if (tmp == null || !tmp.getExprents()
			                       .isEmpty()) {
				return tmp;
			}
		}
	...
	private static boolean matchForEach(DoStatement stat) {
		...
		while (true) {
			...
			if (parent.type == Statement.TYPE_SEQUENCE) {
				if (current == parent.getFirst()) {
					...
					if (preData != null && !preData.getExprents()
					                               .isEmpty()) {
						int size = preData.getExprents()
						                  .size();
						for (int x = 0;
						     x < initExprents.length;
						     x++) {
							if (size > x) {
								Exprent exprent = preData.getExprents()
								                         .get(size - 1 - x);
		...
		if (firstData != null
		    && firstData.getExprents()
		                .get(0).type == Exprent.EXPRENT_ASSIGNMENT) {
			firstDoExprent = (AssignmentExprent) firstData.getExprents()
			                                              .get(0);
		}
		lastData = getLastDirectData(stat.getFirst());
		if (lastData != null && !lastData.getExprents()
		                                 .isEmpty()) {
			lastExprent = lastData.getExprents()
			                      .get(lastData.getExprents()
			                                   .size() - 1);
		}
		...
		if (stat.getLooptype() == DoStatement.LOOP_WHILE && initExprents[0] != null && firstDoExprent != null) {
			if (isIteratorCall(initExprents[0].getRight())) {
				...
				if (invc.getClassname()
				        .contains("java/util/stream")) {
					return false;
				}
				...
				if ((!isNextCall(ass.getRight()) && !isNextUnboxing(ass.getRight()))
				...
				if (next.getInstance().type != Exprent.EXPRENT_VAR
				    ...
				    || ((VarExprent) initExprents[0].getLeft()).isVarReferenced(stat,
				                                                                (VarExprent) next.getInstance(),
				                                                                (VarExprent) hnext.getInstance())) {
					return false;
				}
				...
				ass.getRight()
				   .getBytecodeRange(ass.getLeft().bytecode);
				if (stat.getIncExprent() != null) {
					stat.getIncExprent()
					    .getBytecodeRange(holder.getInstance().bytecode);
				}
				if (stat.getInitExprent() != null) {
					stat.getInitExprent()
					    .getBytecodeRange(ass.getLeft().bytecode);
				...
				preData.getExprents()
				       .remove(initExprents[0]);
				firstData.getExprents()
				         .remove(firstDoExprent);

				if (initExprents[1] != null
				    && initExprents[1].getLeft().type == Exprent.EXPRENT_VAR
					...
					if (copy.getIndex() == inc.getIndex()
					    && copy.getVersion() == inc.getVersion()
					    && !inc.isVarReferenced(stat.getTopParent(),
					                            copy)
					    && !isNextCall(initExprents[1].getRight())) {
						preData.getExprents()
						       .remove(initExprents[1]);
						initExprents[1].getBytecodeRange(initExprents[1].getRight().bytecode);
						stat.getIncExprent()
						    .getBytecodeRange(initExprents[1].getRight().bytecode);
				...
				if (firstDoExprent.getRight().type != Exprent.EXPRENT_ARRAY
				...
				//FunctionExprent funcCond  = (FunctionExprent)drillNots(stat.getConditionExprent()); //TODO: Verify this is
				// counter < copy.length
				...
				if (funcRight.getFuncType() != FunctionExprent.FUNCTION_ARRAY_LENGTH
				    || (incType != FunctionExprent.FUNCTION_PPI
				        && incType
				           != FunctionExprent.FUNCTION_IPP)
				...
				VarExprent counter = (VarExprent) funcInc.getLstOperands()
				                                         .get(0);
				...
				if (counter.isVarReferenced(stat.getFirst(),
				                            index)) {
					return false;
				}

				funcRight.getLstOperands()
				         .get(0)
				         .addBytecodeOffsets(initExprents[0].bytecode);
				funcRight.getLstOperands()
				         .get(0)
				         .addBytecodeOffsets(initExprents[1].bytecode);
				funcRight.getLstOperands()
				         .get(0)
				         .addBytecodeOffsets(lastExprent.bytecode);
				firstDoExprent.getLeft()
				              .addBytecodeOffsets(firstDoExprent.bytecode);
				firstDoExprent.getLeft()
				              .addBytecodeOffsets(initExprents[0].bytecode);

				stat.setLooptype(DoStatement.LOOP_FOREACH);
				stat.setInitExprent(firstDoExprent.getLeft());
				stat.setIncExprent(funcRight.getLstOperands()
				                            .get(0));
				preData.getExprents()
				       .remove(initExprents[0]);
				preData.getExprents()
				...
				if (initExprents[2] != null && initExprents[2].getLeft().type == Exprent.EXPRENT_VAR) {
					...
					if (copy.getIndex() == array.getIndex() && copy.getVersion() == array.getVersion()) {
						preData.getExprents()
						       .remove(initExprents[2]);
						initExprents[2].getRight()
						               .addBytecodeOffsets(initExprents[2].bytecode);
						initExprents[2].getRight()
						               .addBytecodeOffsets(stat.getIncExprent().bytecode);
	...
	private static Statement getFirstDirectData(Statement stat) {
		if (stat.getExprents() != null && !stat.getExprents()
		                                       .isEmpty()) {
			return stat;
		}
	...
	private static Exprent getUncast(Exprent exp) {
		if (exp.type == Exprent.EXPRENT_FUNCTION) {
			...
			if (func.getFuncType() == FunctionExprent.FUNCTION_CAST) {
				return getUncast(func.getLstOperands()
				                     .get(0));
			}
		}
		return exp;
	...
	private static boolean isIteratorCall(Exprent exp) {
		...
		if (!DecompilerContext.getStructContext()
		                      .instanceOf(descriptor.ret.value,
		                                  "java/util/Iterator")) {
			return false;
		}
	...
	private static boolean addContinueOrBreak(DoStatement stat,
	                                          StatEdge ifedge) {
		...
		if (outer != null && (outer.type == Statement.TYPE_SWITCH
		                      || ((DoStatement) outer).getLooptype() != DoStatement.LOOP_DO)) {
			Statement parent = stat.getParent();
			if (parent.type != Statement.TYPE_SEQUENCE || parent.getStats()
			                                                    .getLast()
			                                                    .equals(stat)) {
				// need to insert a break or continue after the loop
				if (ifedge.getDestination()
				          .equals(outer)) {
					stat.addSuccessor(new StatEdge(StatEdge.TYPE_CONTINUE,
					                               stat,
					                               ifedge.getDestination(),
					                               outer));
					return true;
				} else if (MergeHelper.isDirectPath(outer,
				                                    ifedge.getDestination())) {
					stat.addSuccessor(new StatEdge(StatEdge.TYPE_BREAK,
					                               stat,
					                               ifedge.getDestination(),
					                               outer));
					return true;
