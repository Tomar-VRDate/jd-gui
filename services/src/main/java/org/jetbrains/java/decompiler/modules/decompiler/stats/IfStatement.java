...
public final class IfStatement
	...
	private IfStatement(Statement head,
	                    int regedges,
	                    Statement postst) {
		...
		switch (regedges) {
			...
			case 1:
				...
				break;
			case 2:
				elsestat = lstHeadSuccs.get(0)
				                       .getDestination();
				ifstat = lstHeadSuccs.get(1)
				                     .getDestination();
				...
				if (ifstat.getPredecessorEdges(StatEdge.TYPE_REGULAR)
				          .size() > 1 || lstSucc.size() > 1) {
					post = ifstat;
				} else if (elsestat.getPredecessorEdges(StatEdge.TYPE_REGULAR)
				                   .size() > 1 || lstSucc1.size() > 1) {
					post = elsestat;
				} else {
					if (lstSucc.size() == 0) {
						post = elsestat;
		...
		ifedge = lstHeadSuccs.get(negated
		                          ? 0
		                          : 1);
		elseedge = (regedges == 2)
		           ? lstHeadSuccs.get(negated
		                              ? 1
		                              : 0)
		           : null;
		...
		if (iftype == IFTYPE_IF) {
			if (regedges == 0) {
				...
				StatEdge edge = lstHeadSuccs.get(negated
				                                 ? 1
				                                 : 0);
				head.removeSuccessor(edge);
			}
		...
		if (ifstat != null) {
			stats.addWithKey(ifstat,
			                 ifstat.id);
		}

		if (elsestat != null) {
			stats.addWithKey(elsestat,
			                 elsestat.id);
		}
	...
	public static Statement isHead(Statement head) {
		...
		if (head.type == TYPE_BASICBLOCK && head.getLastBasicType() == LASTBASICTYPE_IF) {
			int regsize = head.getSuccessorEdges(StatEdge.TYPE_REGULAR)
			                  .size();
	...
	public TextBuffer toJava(int indent) {
		...
		if (ifstat == null) {
			...
			if (ifedge.explicit) {
				...
				if (ifedge.labeled) {
					buf.append(" label")
					   .append(ifedge.closure == null
					           ? "<unknownclosure>"
					           : ifedge.closure.id.toString());
				}
			}
			if (semicolon) {
				buf.append(";")
			...
			buf.append(ExprProcessor.jmpWrapper(ifstat,
			                                    indent + 1,
			                                    true));
		}
		...
		if (elsestat != null) {
			if (elsestat.type == Statement.TYPE_IF
			    && elsestat.varDefinitions.isEmpty()
			    && (elsestat.getFirst()
			                .getExprents() != null
			        && elsestat.getFirst()
			                   .getExprents()
			                   .isEmpty())
			    && !elsestat.isLabeled()
			    && (elsestat.getSuccessorEdges(STATEDGE_DIRECT_ALL)
			                .isEmpty() || !elsestat.getSuccessorEdges(STATEDGE_DIRECT_ALL)
			                                       .get(0).explicit)) { // else if
				buf.appendIndent(indent)
				   .append("} else ");

				TextBuffer content = ExprProcessor.jmpWrapper(elsestat,
				                                              indent,
				                                              false);
				content.setStart(TextUtil.getIndentString(indent)
				                         .length());
				buf.append(content);
	...
	public void initExprents() {
		IfExprent ifexpr = (IfExprent) first.getExprents()
	...
	public void initSimpleCopy() {
		...
		List<StatEdge> lstSuccs = first.getSuccessorEdges(STATEDGE_DIRECT_ALL);
		ifedge = lstSuccs.get((iftype == IFTYPE_IF || negated)
		                      ? 0
		                      : 1);
		if (stats.size() > 1) {
			ifstat = stats.get(1);
	...
	public List<VarExprent> getImplicitlyDefinedVars() {
		...
		List<Exprent> conditionList = getHeadexprent().getCondition()
		                                              .getAllExprents(true);
		...
		for (Exprent condition : conditionList) {
			if (condition.type == Exprent.EXPRENT_FUNCTION) {
				...
				if (func.getFuncType() == FunctionExprent.FUNCTION_INSTANCEOF
				    && func.getLstOperands()
				           .size() > 2) {
					vars.add((VarExprent) func.getLstOperands()
					                          .get(2));
				}
			}
		}
	...
	public StartEndPair getStartEndRange() {
		return StartEndPair.join(super.getStartEndRange(),
		                         ifstat != null
		                         ? ifstat.getStartEndRange()
		                         : null,
		                         elsestat != null
		                         ? elsestat.getStartEndRange()
		                         : null);
	}
	...
	public IMatchable findObject(MatchNode matchNode,
	                             int index) {
		IMatchable object = super.findObject(matchNode,
		                                     index);
		if (object != null) {
			return object;
		}
