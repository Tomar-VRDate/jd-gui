...
public final class StrongConnectivityHelper {
	...
	// List of strongly connected components, each entry is a list of statements that compose the component
	private final Set<Statement>          processed         = new HashSet<>(); // Already processed statements,
	// persistent
	private final ListStack<Statement>    stack             = new ListStack<>();
	// Stack of statements currently being tracked
	...
	// Statement -> lowest index of any statement reachable from this one
	private final Map<Statement, Boolean> onStack           = new HashMap<>();
	// Statement -> whether this statement is on the stack or not
	...
	public StrongConnectivityHelper(Statement stat) {
		...
		for (Statement st : stat.getStats()) {
			if (!this.processed.contains(st) && st.getPredecessorEdges(Statement.STATEDGE_DIRECT_ALL)
			                                      .isEmpty()) {
				visitTree(st);
			}
		}
	...
	public static boolean isExitComponent(List<? extends Statement> lst) {
		...
		for (Statement stat : lst) {
			set.addAll(stat.getNeighbours(StatEdge.TYPE_REGULAR,
	...
	private void visit(Statement stat) {
		...
		this.indexMap.put(stat,
		                  this.index);
		this.lowLinkMap.put(stat,
		                    this.index);
		this.index++;
		this.onStack.put(stat,
		                 true);

		// Get all neighbor successors
		List<Statement> successors = stat.getNeighbours(StatEdge.TYPE_REGULAR,
		                                                Statement.DIRECTION_FORWARD); // TODO: set?
