...
public final class SwitchStatement
	...
	// We need to do this because switch expressions can have code in their case values, so we need to preserve the
	// statement graph.
	...
	private SwitchStatement(Statement head,
	                        Statement poststat) {
		...
		// find post node
		Set<Statement> lstNodes = new HashSet<>(head.getNeighbours(StatEdge.TYPE_REGULAR,
		                                                           DIRECTION_FORWARD));
		...
		defaultEdge = head.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)
		                  .get(0);

		//We need to use set above in case we have multiple edges to the same node. But HashSets iterator is not ordered,
		// so sort
		List<Statement> sorted = new ArrayList<>(lstNodes);
		Collections.sort(sorted,
		                 new Comparator<Statement>() {
			                 @Override
			                 public int compare(Statement o1,
			                                    Statement o2) {
				                 return o1.id - o2.id;
			                 }
		                 });
	...
	public static Statement isHead(Statement head) {
		...
		if (head.type == Statement.TYPE_BASICBLOCK && head.getLastBasicType() == Statement.LASTBASICTYPE_SWITCH) {
			...
			if (DecHelper.isChoiceStatement(head,
			                                lst)) {
				...
				if (DecHelper.checkStatementExceptions(lst)) {
					return new SwitchStatement(head,
					                           post);
				}
			}
		}
	...
	public TextBuffer toJava(int indent) {
		...
		buf.append(ExprProcessor.listToJava(varDefinitions,
		                                    indent));
		...
		buf.append(headexprent.get(0)
		                      .toJava(indent))
		   .append(" {")
		   .appendLineSeparator();

		VarType switch_type = headexprent.get(0)
		                                 .getExprType();

		for (int i = 0;
		     i < caseStatements.size();
		     i++) {
			...
			for (int j = 0;
				...
				if (edges.get(j) == defaultEdge) {
					...
					if (value == null) { // TODO: how can this be null? Is it trying to inject a synthetic case value in
						// switch-on-string processing? [TestSwitchDefaultBefore]
						continue;
					}
			...
			buf.append(ExprProcessor.jmpWrapper(stat,
			                                    indent + 1,
			                                    false));
		}
	...
	public void initExprents() {
		SwitchHeadExprent swexpr = (SwitchHeadExprent) first.getExprents()
		                                                    .remove(first.getExprents()
		                                                                 .size() - 1);
	...
	public void replaceStatement(Statement oldstat,
		...
		for (int i = 0;
			...
			if (caseStatements.get(i) == oldstat) {
				caseStatements.set(i,
				                   newstat);
			}
		}

		super.replaceStatement(oldstat,
		                       newstat);
	}
	...
	public void initSimpleCopy() {
		first = stats.get(0);
		defaultEdge = first.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)
		                   .get(0);
	...
	public void sortEdgesAndNodes() {
		...
		for (int i = 0;
		     i < lstFirstSuccs.size();
		     i++) {
			mapEdgeIndex.put(lstFirstSuccs.get(i),
			                 i == 0
			                 ? lstFirstSuccs.size()
			                 : i);
		}
		...
		int[] values = ((SwitchInstruction) bbstat.getBlock()
		                                          .getLastInstruction()).getValues();
		...
		for (int index = 0;
			...
			if (stat != null) {
				HashSet<Statement> setPreds = new HashSet<>(stat.getNeighbours(StatEdge.TYPE_REGULAR,
				                                                               DIRECTION_BACKWARD));
				setPreds.remove(first);

				if (!setPreds.isEmpty()) {
					Statement pred = setPreds.iterator()
					                         .next(); // assumption: at most one predecessor node besides the head. May not hold
					// true for obfuscated code.
					for (int j = 0;
					     j < nodes.size();
					     j++) {
		...
		for (List<Integer> lst : edges) {
			...
			for (Integer in : lst) {
				...
				lstv.add(index == 0
				         ? null
				         : new ConstExprent(values[index - 1],
				                            false,
				                            null));
			}
			lstEdges.add(lste);
		...
		for (int i = 0;
			...
			if (nodes.get(i) == null) {
				...
				StatEdge sample_edge = lstEdges.get(i)
				                               .get(0);

				bstat.addSuccessor(new StatEdge(sample_edge.getType(),
				                                bstat,
				                                sample_edge.getDestination(),
				                                sample_edge.closure));
