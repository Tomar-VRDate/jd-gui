...
public final class DomHelper {
	private static RootStatement graphToStatement(ControlFlowGraph graph,
		...
		for (BasicBlock block : blocks) {
			stats.addWithKey(new BasicBlockStatement(block),
			                 block.id);
		}
		...
		Statement general;
		if (stats.size() > 1
		    || firstblock.isSuccessor(firstblock)) { // multiple basic blocks or an infinite loop of one block
			general = new GeneralStatement(firstst,
			                               stats,
			                               null);
		} else { // one straightforward basic block
			RootStatement root = new RootStatement(firstst,
			                                       dummyexit,
			                                       mt);
			firstst.addSuccessor(new StatEdge(StatEdge.TYPE_BREAK,
			                                  firstst,
			                                  dummyexit,
			                                  root));
		...
		if (DecompilerContext.getOption(IFernflowerPreferences.EXPERIMENTAL_TRY_LOOP_FIX)) {
			...
			for (ExceptionRangeCFG exception : graph.getExceptions()) {
				if (exception.getProtectedRange()
				             .contains(firstblock)) {
		...
		for (BasicBlock block : blocks) {
			...
			for (BasicBlock succ : block.getSuccs()) {
				...
				stat.addSuccessor(new StatEdge(type,
				                               stat,
				                               stsucc,
				                               (type == StatEdge.TYPE_REGULAR)
				                               ? null
			...
			for (BasicBlock succex : block.getSuccExceptions()) {
				...
				ExceptionRangeCFG range = graph.getExceptionRange(succex,
				                                                  block);
				if (!range.isCircular()) {
		...
		return new RootStatement(general,
		                         dummyexit,
		                         mt);
	}
	...
	public static VBStyleCollection<List<Integer>, Integer> calcPostDominators(Statement container) {
		...
		do {
			...
			for (Statement stat : lstStats) {
				...
				List<Statement> lstSuccs = stat.getNeighbours(StatEdge.TYPE_REGULAR,
				...
				if (!InterpreterUtil.equalObjects(domsSuccs,
				                                  doms)) {

					lists.put(stat,
					          domsSuccs);

					List<Statement> lstPreds = stat.getNeighbours(StatEdge.TYPE_REGULAR,
		...
		for (int i = 0;
		     i < lstRevPost.size();
		     i++) {
			mapSortOrder.put(lstRevPost.get(i).id,
			                 i);
		}
	...
	public static RootStatement parseGraph(ControlFlowGraph graph,
		...
		if (!processStatement(root,
		                      root,
		                      new LinkedHashMap<>())) {
			DotExporter.errorToDotFile(graph,
			                           mt,
			                           "parseGraphFail");
			DotExporter.errorToDotFile(root,
			                           mt,
			                           "parseGraphFailStat");
				...
				for (int i = 0;
					...
					if (current.isMonitorEnter()) {
						...
						if (next.type == Statement.TYPE_CATCHALL) {
							...
							if (ca.getFirst()
							      .isContainsMonitorExit() && ca.getHandler()
								...
								stat.getStats()
								    .removeWithKey(current.id);
								stat.setFirst(stat.getStats()
								                  .get(0));

								// new statement
								SynchronizedStatement sync = new SynchronizedStatement(current,
								                                                       ca.getFirst(),
								                                                       ca.getHandler());
								sync.setAllParent();
								...
								current.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
								                                  current,
								                                  ca.getFirst()));
			...
			for (int reducibility = 0;
				...
				for (int i = 0;
					...
					while (true) {
						...
						if (findSimpleStatements(general,
						                         mapExtPost)) {
							reducibility = 0;
						}
						...
						Statement stat = findGeneralStatement(general,
						                                      forceall,
						                                      mapExtPost);
	...
	private static Statement findGeneralStatement(Statement stat,
	                                              boolean forceall,
	                                              HashMap<Integer, Set<Integer>> mapExtPost) {
		...
		if (forceall) {
			...
			for (Statement st : lstAll) {
				...
				if (set != null) {
					vbPost.addWithKey(new ArrayList<>(set),
					                  st.id); // FIXME: sort order!!
				}
			}
			...
			if (setFirst != null) {
				for (int id : setFirst) {
					...
					if (lst == null) {
						vbPost.addWithKey(lst = new ArrayList<>(),
						                  id);
					}
					lst.add(id);
				}
		...
		for (int k = 0;
			...
			if (!mapExtPost.containsKey(headid) && !(posts.size() == 1 && posts.get(0)
			                                                                   .equals(headid))) {
				continue;
			}
			...
			for (int postId : posts) {
				...
				while (true) {
					...
					for (Statement handler : setHandlers) {
						...
						if (!addhd) {
							List<Statement> hdsupp = handler.getNeighbours(StatEdge.TYPE_EXCEPTION,
							                                               Statement.DIRECTION_BACKWARD);
							...
							while (!lstStack.isEmpty()) {
								...
								if (!(setNodes.contains(st) || (!same && st == post))) {
									...
									if (st != head) {
										...
										setPreds.addAll(st.getNeighbours(StatEdge.TYPE_REGULAR,
										                                 Statement.DIRECTION_BACKWARD));
									}

									// put successors on the stack
									lstStack.addAll(st.getNeighbours(StatEdge.TYPE_REGULAR,
									                                 Statement.DIRECTION_FORWARD));

									// exception edges
									setHandlers.addAll(st.getNeighbours(StatEdge.TYPE_EXCEPTION,
									                                    Statement.DIRECTION_FORWARD));
								}
							}
				...
				boolean excok = true;
				for (Statement handler : setHandlers) {
					if (!handler.getNeighbours(StatEdge.TYPE_EXCEPTION,
					                           Statement.DIRECTION_BACKWARD)
				...
				if (excok) {
					...
					if (setPreds.size() == 0) {
						if ((setNodes.size() > 1 || head.getNeighbours(StatEdge.TYPE_REGULAR,
						                                               Statement.DIRECTION_BACKWARD)
						                                .contains(head)) && setNodes.size() < stats.size()) {
							if (checkSynchronizedCompleteness(setNodes)) {
								res = new GeneralStatement(head,
								                           setNodes,
								                           same
								                           ? null
								                           : post);
	...
	private static boolean checkSynchronizedCompleteness(Set<Statement> setNodes) {
		...
		for (Statement stat : setNodes) {
			if (stat.isMonitorEnter()) {
				...
				if (lstSuccs.size() != 1
				    || lstSuccs.get(0)
				               .getType() != StatEdge.TYPE_REGULAR) {
					return false;
				}

				if (!setNodes.contains(lstSuccs.get(0)
				                               .getDestination())) {
					return false;
				}
			}
					...
					if (!mapExtPost.isEmpty()) {
						...
						for (int key : new ArrayList<>(mapExtPost.keySet())) {
							...
							if (setOldNodes.contains(key)) {
								mapExtPost.computeIfAbsent(newid,
								                           k -> new LinkedHashSet<>())
								          .addAll(set);
