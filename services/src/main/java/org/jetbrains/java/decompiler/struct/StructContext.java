...
public class StructContext {
	...
	public StructContext(IResultSaver saver,
	                     IDecompiledData decompiledData,
	                     LazyLoader loader) {
		this.saver = saver;
		this.decompiledData = decompiledData;
		this.loader = loader;

		ContextUnit defaultUnit = new ContextUnit(ContextUnit.TYPE_FOLDER,
		                                          null,
		                                          "",
		                                          true,
		                                          saver,
	...
	public void reloadContext()
		...
		for (Map.Entry<String, ContextUnit> e : units.entrySet()) {
			...
			for (StructClass cl : unit.getClasses()) {
				classes.put(cl.qualifiedName,
				            new ClassProvider(cl));
			}
		}
	}
	...
	public void addSpace(File file,
	                     boolean isOwn) {
		addSpace("",
		         file,
		         isOwn,
	...
	public void addSpace(FileSystem fs,
	                     boolean isOwn) {
		String scheme = fs.provider()
		                  .getScheme();
		if ("jar".equals(scheme)) {
			String fileUri = fs.getPath("/")
			                   .toUri()
			                   .getSchemeSpecificPart();
			fileUri = fileUri.substring(0,
			                            fileUri.indexOf('!'));
			addSpace(Paths.get(fileUri)
			              .toFile(),
			         false);
		} else {
			try {
				addFileSystem(fs,
				              "",
				              new File(fs.toString()),
				              ContextUnit.TYPE_JAR,
				              isOwn);
			} catch (IOException e) {
				DecompilerContext.getLogger()
				                 .writeMessage("Corrupted file system: " + fs,
				                               e);
	...
	private void addSpace(String path,
	                      File file,
	                      boolean isOwn,
	                      int level) {
		if (file.isDirectory()) {
			...
			if (unit == null) {
				unit = new ContextUnit(ContextUnit.TYPE_FOLDER,
				                       null,
				                       path,
				                       isOwn,
				                       saver,
			...
			if (filename.endsWith(".class")) {
				addClass(unit,
				         ...
				         filename,
				         isOwn,
				         () -> loader.getClassBytes(file.getAbsolutePath(),
				                                    null));
			} else {
				unit.addOtherEntry(file.getAbsolutePath(),
				                   filename);
			}
		}
	}

	private FileSystem getZipFileSystem(File file)
					throws
					IOException {
		try {
			return zipFiles.computeIfAbsent(file,
			                                f -> {
				                                URI uri = null;
				                                try {
					                                uri = new URI("jar:file",
					                                              null,
					                                              f.toURI()
					                                               .getPath(),
					                                              null);
	...
	private void addArchive(String externalPath,
	                        File file,
	                        int type,
	                        boolean isOwn)
					throws
					IOException {
		DecompilerContext.getLogger()
		                 .writeMessage("Adding Archive: " + file.getAbsolutePath(),
		                               Severity.INFO);
	...
	private void addFileSystem(FileSystem fs,
					...
					throws
					IOException {
		ContextUnit unit = units.computeIfAbsent(externalPath + "/" + file,
		                                         k -> new ContextUnit(type,
		                                                              externalPath,
		                                                              file.getName(),
		                                                              isOwn,
		                                                              saver,
		                   ...
		                   new SimpleFileVisitor<Path>() {
			                   ...
			                   public FileVisitResult visitFile(Path path,
				                   ...
				                   if (path.getNameCount() > 2
				                       && "modules".equals(path.getName(0)
				                                               .toString())
				                       && "jrt".equals(path.getFileSystem()
				                                           .provider()
				                                           .getScheme())) {
					                   name = path.subpath(2,
					                                       path.getNameCount())
			                   ...
			                   public FileVisitResult preVisitDirectory(Path dir,
				                   ...
				                   if (dirStr.length() > 1) {
					                   unit.addDirEntry(dirStr.substring(1));
				                   }
				                   return FileVisitResult.CONTINUE;
			                   }
		                   });
	...
	private void addClass(ContextUnit unit,
	                      String name,
	                      String externalPath,
	                      String internalPath,
	                      boolean isOwn,
	...
	private void addClass(String name,
		...
		//      DecompilerContext.getLogger().writeMessage("Inconsistent class name and class file! Expected " + className
		//      + ".class but found " + name + ".class!", Severity.WARN);
		//
		//      name = className;
		//    }
	...
	public boolean instanceOf(String valclass,
	                          String refclass) {
		...
		if (cl.superClass != null && this.instanceOf(cl.superClass.getString(),
		                                             refclass)) {
			return true;
		}
		...
		for (int i = 0;
		     i < interfaces.length;
		     i++) {
			String intfc = cl.getPool()
			                 .getPrimitiveConstant(interfaces[i])
			                 .getString();

			if (this.instanceOf(intfc,
			                    refclass)) {
				return true;
			}
		}
	...
	public StructClass getFirstCommonClass(String firstclass,
	                                       String secondclass) {
		...
		if (fcls != null && scls != null) {
			...
			while (fcls != null) {
				...
				fcls = fcls.superClass == null
				       ? null
				       : this.getClass(fcls.superClass.getString());
			}
		}
	...
	public void loadAbstractMetadata(String string) {
		for (String line : string.split("\n")) {
			...
			for (int x = 0;
			     ...
			     x++) {
				for (int y = 0;
				     y < desc.parameterTypes.get(x).stackSize;
				     y++) {
					params.add(pts[x + 3]);
				}
			}
			this.abstractNames.put(pts[0] + ' ' + pts[1] + ' ' + pts[2],
			                       params);
		}
	}

	public String renameAbstractParameter(String className,
	                                      String methodName,
	                                      String descriptor,
	                                      int index,
	                                      String _default) {
		List<String> params = this.abstractNames.get(className + ' ' + methodName + ' ' + descriptor);
		return params != null && index < params.size()
		       ? params.get(index)
		       : _default;
	}
	...
	class ClassProvider {
		...
		ClassProvider(ContextUnit unit,
		              String externalPath,
		              String internalPath,
		              boolean own,
		              ClassSupplier supplier) {
		...
		public StructClass get() {
			...
			synchronized (this) {
				...
				try {
					DecompilerContext.getLogger()
					                 .writeMessage("  Loading Class: " + internalPath,
					                               Severity.INFO);
					byte[] data = supplier.get();
					StructClass cl = StructClass.create(new DataInputFullStream(data),
					                                    own,
					                                    loader);
					unit.addClass(cl,
					              internalPath);
					loader.addClassLink(cl.qualifiedName,
					                    new LazyLoader.Link(externalPath,
					                                        internalPath,
					                                        data));
					value = cl;
					supplier = null;
