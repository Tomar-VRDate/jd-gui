...
public class VarVersionsGraph {
	...
	private static List<VarVersionNode> getReversedPostOrder(Collection<VarVersionNode> roots) {
		...
		for (VarVersionNode root : roots) {
			...
			addToReversePostOrderListIterative(root,
			                                   lstTemp,
			                                   setVisited);
			lst.addAll(lstTemp);
		}
	...
	private static void addToReversePostOrderListIterative(VarVersionNode root,
		...
		while (!stackNode.isEmpty()) {
			...
			List<VarVersionEdge> lstSuccs = mapNodeSuccs.computeIfAbsent(node,
			                                                             n -> new ArrayList<>(n.succs));
			for (;
			     index < lstSuccs.size();
			     index++) {
	...
	public VarVersionNode createNode(VarVersionPair ver,
	                                 LocalVariable lvt) {
		VarVersionNode node;
		nodes.addWithKey(node = new VarVersionNode(ver.var,
		                                           ver.version,
		                                           lvt),
		                 ver);
		return node;
	...
	public void addNodes(Collection<VarVersionNode> colnodes,
	                     Collection<VarVersionPair> colpaars) {
		nodes.addAllWithKey(colnodes,
		                    colpaars);
	}

	public boolean isDominatorSet(VarVersionNode node,
	                              Set<VarVersionNode> domnodes) {
		if (domnodes.size() == 1) {
			return engine.isDominator(node,
			                          domnodes.iterator()
			                                  .next());
		} else {
