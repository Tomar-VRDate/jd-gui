...
public final class LoopExtractHelper {
	...
	private static boolean extractLoop(DoStatement stat) {
		...
		for (StatEdge edge : stat.getLabelEdges()) {
			if (edge.getType() != StatEdge.TYPE_CONTINUE && edge.getDestination().type != Statement.TYPE_DUMMYEXIT) {
				if (edge.getType() == StatEdge.TYPE_BREAK && isExternStatement(stat,
				                                                               edge.getSource(),
				                                                               edge.getSource())) {
		...
		if (!extractFirstIf(stat,
		                    stats)) {
			return extractLastIf(stat,
			                     stats);
		} else {
			return true;
		}
	...
	private static boolean extractLastIf(DoStatement stat,
		...
		if (last.type == Statement.TYPE_IF) {
			...
			if (lastif.iftype == IfStatement.IFTYPE_IF && lastif.getIfstat() != null) {
				...
				if (lastif.getAllSuccessorEdges()
				          .isEmpty()) {
					return false;
				}
					...
					if (isExternStatement(stat,
						...
						if (stat.getStats()
							...
							if (ifstat.type == Statement.TYPE_SEQUENCE) {
								removeFrom = ifstat.getStats()
								                   .getLast();
								continues.addAll(ifstat.getStats()
								                       .getLast()
							...
							if (continues.size() == 1
								...
								while (check.getSuccessorEdges(StatEdge.TYPE_REGULAR)
								            .isEmpty()) {
								...
								extractIfBlockIntoLoop(stat,
								                       firstif,
								                       check.getSuccessorEdges(StatEdge.TYPE_REGULAR)
								                            .get(0)
								                            .getDestination());
	...
	private static boolean isExternStatement(DoStatement loop,
	                                         Statement block,
	                                         Statement stat) {
	...
	private static void extractIfBlock(DoStatement loop,
	                                   IfStatement ifstat) {
		...
		SequenceStatement block = new SequenceStatement(Arrays.asList(loop,
		                                                              target));
		loop.getParent()
		...
		loop.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
		                               loop,
		                               target));
		...
		// Find predecessor continues and replace their source from the block to the loop, makes continues from the loop
		// body flow to the proper statement
	...
	private static void extractIfBlockIntoLoop(DoStatement loop,
	                                           IfStatement ifStat,
		...
		SequenceStatement block = new SequenceStatement(Arrays.asList(ifStat,
		                                                              target));
		loop.replaceStatement(ifStat,
		                      block);
		block.setAllParent();
		...
		expr.setCondition(new FunctionExprent(FunctionExprent.FUNCTION_BOOL_NOT,
		                                      expr.getCondition(),
		                                      null));
	}
}
