...
public class ClassWriter {
	...
	private static final int                             METHOD_EXCLUDED                 = CodeConstants.ACC_PUBLIC
	                                                                                       | CodeConstants.ACC_ABSTRACT;
	private static final int                             ACCESSIBILITY_FLAGS             = CodeConstants.ACC_PUBLIC
	                                                                                       | CodeConstants.ACC_PROTECTED
	                                                                                       | CodeConstants.ACC_PRIVATE;

	static {
		MODIFIERS = new LinkedHashMap<>();
		MODIFIERS.put(CodeConstants.ACC_PUBLIC,
		              "public");
		MODIFIERS.put(CodeConstants.ACC_PROTECTED,
		              "protected");
		MODIFIERS.put(CodeConstants.ACC_PRIVATE,
		              "private");
		MODIFIERS.put(CodeConstants.ACC_ABSTRACT,
		              "abstract");
		MODIFIERS.put(CodeConstants.ACC_STATIC,
		              "static");
		MODIFIERS.put(CodeConstants.ACC_FINAL,
		              "final");
		MODIFIERS.put(CodeConstants.ACC_STRICT,
		              "strictfp");
		MODIFIERS.put(CodeConstants.ACC_TRANSIENT,
		              "transient");
		MODIFIERS.put(CodeConstants.ACC_VOLATILE,
		              "volatile");
		MODIFIERS.put(CodeConstants.ACC_SYNCHRONIZED,
		              "synchronized");
		MODIFIERS.put(CodeConstants.ACC_NATIVE,
		              "native");
	}
	...
	private static boolean invokeProcessors(TextBuffer buffer,
	                                        ClassNode node) {
		ClassWrapper wrapper = node.getWrapper();
		if (wrapper == null) {
			buffer.append("/* $FF: Couldn't be decompiled. Class "
			              + node.classStruct.qualifiedName
		...
		for (MethodWrapper method : wrapper.getMethods()) {
			if (method.root != null) {
				try {
					SwitchHelper.simplifySwitches(method.root,
					                              method.methodStruct);
				} catch (Throwable e) {
					DecompilerContext.getLogger()
					                 .writeMessage("Method "
					                               + method.methodStruct.getName()
					                               + " "
		...
		try {
			...
			DecompilerContext.getLogger()
			                 .writeMessage("Class " + node.simpleName + " couldn't be written.",
			                               IFernflowerLogger.Severity.WARN,
			                               t);
	...
	public static void packageInfoToJava(StructClass cl,
	                                     TextBuffer buffer) {
		appendAnnotations(buffer,
		                  0,
		                  cl,
		                  -1);

		int index = cl.qualifiedName.lastIndexOf('/');
		String packageName = cl.qualifiedName.substring(0,
		                                                index)
		                                     .replace('/',
		                                              '.');
	...
	public static void moduleInfoToJava(StructClass cl,
	                                    TextBuffer buffer) {
		appendAnnotations(buffer,
		                  0,
		                  cl,
		...
		buffer.append("module ")
		      .append(moduleAttribute.moduleName)
		      .append(" {")
		      .appendLineSeparator();

		writeModuleInfoBody(buffer,
		                    moduleAttribute);
	...
	private static void writeModuleInfoBody(TextBuffer buffer,
	                                        StructModuleAttribute moduleAttribute) {
		...
		if (!providesEntries.isEmpty()) {
			...
			for (StructModuleAttribute.ProvidesEntry provides : providesEntries) {
				buffer.appendIndent(1)
				      ...
				      .appendLineSeparator();
				appendFQClassNames(buffer,
				                   provides.implementationNames.stream()
	...
	private static boolean isSuperClassSealed(StructClass cl) {
		if (cl.superClass != null) {
			StructClass superClass = DecompilerContext.getStructContext()
			                                          .getClass((String) cl.superClass.value);
		...
		for (String iface : cl.getInterfaceNames()) {
			StructClass ifaceClass = DecompilerContext.getStructContext()
			                                          .getClass(iface);
	...
	private static void methodLambdaToJava(ClassNode lambdaNode,
	                                       ...
	                                       int indent,
	                                       boolean codeOnly) {
		MethodWrapper methodWrapper = classWrapper.getMethodWrapper(mt.getName(),
		                                                            mt.getDescriptor());

		MethodWrapper outerWrapper
						= (MethodWrapper) DecompilerContext.getProperty(DecompilerContext.CURRENT_METHOD_WRAPPER);
		DecompilerContext.setProperty(DecompilerContext.CURRENT_METHOD_WRAPPER,
		                              methodWrapper);
		...
		try {
			...
			if (!codeOnly) {
				...
				int index = lambdaNode.lambdaInformation.is_content_method_static
				            ? 0
				            : 1;
				...
				for (int i = 0;
					...
					if (i >= start_index) {
						...
						String parameterName = methodWrapper.varproc.getVarName(new VarVersionPair(index,
						                                                                           0));
						buffer.append(parameterName == null
						              ? "param" + index
			...
			RootStatement root = classWrapper.getMethodWrapper(mt.getName(),
			                                                   mt.getDescriptor()).root;
				...
				if (root != null) { // check for existence
					try {
						...
						buffer.append(childBuf,
						              classWrapper.getClassStruct().qualifiedName,
						              InterpreterUtil.makeUniqueKey(mt.getName(),
						                                            mt.getDescriptor()));
					} catch (Throwable t) {
						String message = "Method "
						                 ...
						                 + mt.getDescriptor()
						                 + " in class "
						                 + lambdaNode.classStruct.qualifiedName
						                 + " couldn't be written.";
						DecompilerContext.getLogger()
	...
	private static String toValidJavaIdentifier(String name) {
		...
		return res.append("/* $FF was: ")
		          .append(name)
		          .append("*/")
		          .toString();
	}

	private static void dumpError(TextBuffer buffer,
		...
		if (exceptions) {
			lines.addAll(ClassWriter.getErrorComment());
			collectErrorLines(wrapper.decompileError,
			                  lines);
			if (bytecode) {
				lines.add("");
			}
	...
	public static void collectErrorLines(Throwable error,
	                                     List<String> lines) {
		...
		if (cause != null) {
			...
			collectErrorLines(cause,
			                  causeLines);
			if (!causeLines.isEmpty()) {
				lines.add("Caused by: " + causeLines.get(0));
				lines.addAll(causeLines.subList(1,
				                                causeLines.size()));
			}
		}
	}

	private static void collectBytecode(MethodWrapper wrapper,
	                                    List<String> lines)
					throws
					IOException {
		...
		StructBootstrapMethodsAttribute bootstrap
						= classNode.classStruct.getAttribute(StructGeneralAttribute.ATTRIBUTE_BOOTSTRAP_METHODS);
			...
			switch (instr.group) {
				case CodeConstants.GROUP_INVOCATION: {
					...
					if (instr.opcode == CodeConstants.opc_invokedynamic && bootstrap != null) {
						appendBootstrapCall(sb,
						                    pool.getLinkConstant(instr.operand(0)),
						                    bootstrap);
					} else {
						appendConstant(sb,
						               pool.getConstant(instr.operand(0)));
					}
					for (int i = 1;
					     i < instr.operandsCount();
					     i++) {
						sb.append(' ')
						  .append(instr.operand(i));
					}
					break;
				}
	...
	private static void appendBootstrapCall(StringBuilder sb,
	                                        LinkConstant target,
	                                        StructBootstrapMethodsAttribute bootstrap) {
		sb.append(target.elementname)
		  .append(' ')
		  .append(target.descriptor);
	...
	private static void appendConstant(StringBuilder sb,
	                                   PooledConstant constant) {
		if (constant == null) {
		...
		if (constant instanceof PrimitiveConstant) {
			...
			sb.append(linkConstant.classname)
			  .append('.')
			  .append(linkConstant.elementname)
			  .append(' ')
			  .append(linkConstant.descriptor);
		}
	}

	private static boolean hideConstructor(ClassNode node,
	                                       boolean init,
		...
		int classAccessFlags = node.type == ClassNode.CLASS_ROOT
		                       ? cl.getAccessFlags()
		                       : node.access;
	...
	private static Map.Entry<VarType, GenericFieldDescriptor> getFieldTypeData(StructField fd) {
		VarType fieldType = new VarType(fd.getDescriptor(),
		                                false);

		GenericFieldDescriptor descriptor = fd.getSignature();
		return new AbstractMap.SimpleImmutableEntry<>(fieldType,
		                                              descriptor);
	}
	...
	private static void appendDeprecation(TextBuffer buffer,
	                                      int indent) {
		buffer.appendIndent(indent)
		      .append("/** @deprecated */")
		      .appendLineSeparator();
	...
	private static void appendRenameComment(TextBuffer buffer,
	                                        String oldName,
	                                        MType type,
	                                        int indent) {
		if (oldName == null) {
	...
	private static String getTypePrintOut(VarType type) {
		String typeText = ExprProcessor.getCastTypeName(type,
		                                                false);
		if (ExprProcessor.UNDEFINED_TYPE_STRING.equals(typeText)
		    && DecompilerContext.getOption(IFernflowerPreferences.UNDEFINED_PARAM_TYPE_OBJECT)) {
			typeText = ExprProcessor.getCastTypeName(VarType.VARTYPE_OBJECT,
			                                         false);
		}
		return typeText;
	}
	...
	private static void appendComment(TextBuffer buffer,
	                                  String comment,
	                                  int indent) {
		buffer.appendIndent(indent)
		      .append("// $FF: ")
	...
	private static void appendJavadoc(TextBuffer buffer,
	                                  String javaDoc,
	                                  int indent) {
		if (javaDoc == null) {
			return;
	...
	static void appendAnnotations(TextBuffer buffer,
	                              int indent,
	                              StructMember mb,
	                              int targetType) {
		...
		for (StructGeneralAttribute.Key<?> key : ANNOTATION_ATTRIBUTES) {
			...
			if (attribute != null) {
				for (AnnotationExprent annotation : attribute.getAnnotations()) {
					String text = annotation.toJava(indent)
	...
	private static boolean searchForMethod(StructClass cl,
	                                       String name,
	                                       MethodDescriptor md,
	                                       boolean search) {
		...
		if (cl.superClass != null) {
			...
			boolean foundInSuperClass = searchForMethod(superClass,
			                                            name,
			                                            md,
			                                            true);
		...
		for (String ifaceName : cl.getInterfaceNames()) {
			StructClass iface = DecompilerContext.getStructContext()
			                                     .getClass(ifaceName);

			boolean foundInIface = searchForMethod(iface,
			                                       name,
			                                       md,
			                                       true);
	...
	private static void appendParameterAnnotations(TextBuffer buffer,
	                                               StructMethod mt,
	                                               int param) {
		Set<String> filter = new HashSet<>();
			...
			if (attribute != null) {
				...
				if (param < annotations.size()) {
					for (AnnotationExprent annotation : annotations.get(param)) {
						String text = annotation.toJava(-1)
						                        .convertToStringAndAllowDataDiscard();
						filter.add(text);
						buffer.append(text)
						      .append(' ');
		...
		appendTypeAnnotations(buffer,
		                      -1,
		                      mt,
		                      TypeAnnotation.METHOD_PARAMETER,
		                      param,
		                      filter);
	}

	private static void appendTypeAnnotations(TextBuffer buffer,
	                                          int indent,
	                                          StructMember mb,
	                                          int targetType,
	                                          int index,
	...
	private static void appendModifiers(TextBuffer buffer,
	                                    int flags,
	                                    int allowed,
	                                    boolean isInterface,
	                                    int excluded) {
		...
		for (int modifier : MODIFIERS.keySet()) {
			if ((flags & modifier) == modifier && (modifier & excluded) == 0) {
				buffer.append(MODIFIERS.get(modifier))
				      .append(' ');
			}
		}
	}
	...
	public static void appendTypeParameters(TextBuffer buffer,
	...
	private static void appendFQClassNames(TextBuffer buffer,
	                                       List<String> names) {
		for (int i = 0;
		     i < names.size();
		     i++) {
	...
	public void classLambdaToJava(ClassNode node,
	                              TextBuffer buffer,
		...
		DecompilerContext.setProperty(DecompilerContext.CURRENT_CLASS_NODE,
		                              node);
		...
		try {
			...
			DecompilerContext.getLogger()
			                 .startWriteClass(node.simpleName);
			...
			if (node.lambdaInformation.is_method_reference) {
				if (!node.lambdaInformation.is_content_method_static && method_object != null) {
					...
					method_object.getInferredExprType(new VarType(CodeConstants.TYPE_OBJECT,
					                                              0,
				...
				buffer.append("::")
				      .append(CodeConstants.INIT_NAME.equals(node.lambdaInformation.content_method_name)
				              ? "new"
				              : node.lambdaInformation.content_method_name);
			} else {
				// lambda method
				StructMethod mt = cl.getMethod(node.lambdaInformation.content_method_key);
				MethodWrapper methodWrapper = wrapper.getMethodWrapper(mt.getName(),
				                                                       mt.getDescriptor());
				MethodDescriptor md_content
				...
				if (!lambdaToAnonymous) {
					...
					boolean firstParameter = true;
					int index = node.lambdaInformation.is_content_method_static
					            ? 0
					            : 1;
					int start_index = md_content.params.length - md_lambda.params.length;
	...
	public void classToJava(ClassNode node,
	                        TextBuffer buffer,
	                        int indent) {
		ClassNode outerNode = (ClassNode) DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_NODE);
		DecompilerContext.setProperty(DecompilerContext.CURRENT_CLASS_NODE,
		...
		try {
			...
			boolean ok = invokeProcessors(buffer,
			                              node);
			...
			DecompilerContext.getLogger()
			                 .startWriteClass(cl.qualifiedName);

			// write class definition
			writeClassDefinition(node,
			                     buffer,
			                     indent);
			...
			for (StructField fd : cl.getFields()) {
				boolean hide = fd.isSynthetic() && DecompilerContext.getOption(IFernflowerPreferences.REMOVE_SYNTHETIC)
				               || wrapper.getHiddenMembers()
				                         .contains(InterpreterUtil.makeUniqueKey(fd.getName(),
				                                                                 fd.getDescriptor()));
				if (hide) {
					continue;
				}
				...
				if (components != null
				    ...
				    && components.stream()
				                 .anyMatch(c -> c.getName()
				                                 .equals(fd.getName()) && c.getDescriptor()
				                                                           .equals(fd.getDescriptor()))) {
			...
			for (int i = 0;
				...
				boolean hide = mt.isSynthetic() && DecompilerContext.getOption(IFernflowerPreferences.REMOVE_SYNTHETIC)
				               ...
				               || wrapper.getHiddenMembers()
				                         .contains(InterpreterUtil.makeUniqueKey(mt.getName(),
				                                                                 mt.getDescriptor()));
				if (hide) {
					continue;
				}
				...
				boolean methodSkipped = !methodToJava(node,
				                                      mt,
				                                      i,
				                                      methodBuffer,
				                                      indent + 1);
			...
			for (ClassNode inner : node.nested) {
				if (inner.type == ClassNode.CLASS_MEMBER) {
					...
					boolean hide = isSynthetic && DecompilerContext.getOption(IFernflowerPreferences.REMOVE_SYNTHETIC)
					               || wrapper.getHiddenMembers()
					                         .contains(innerCl.qualifiedName);
					if (hide) {
						continue;
					}
	...
	private void writeClassDefinition(ClassNode node,
	                                  TextBuffer buffer,
	                                  int indent) {
		if (node.type == ClassNode.CLASS_ANONYMOUS) {
			buffer.append(" {")
		...
		int flags = node.type == ClassNode.CLASS_ROOT
		            ? cl.getAccessFlags()
		            : node.access;
		...
		StructPermittedSubclassesAttribute permittedSubClassesAttr
						= cl.getAttribute(StructGeneralAttribute.ATTRIBUTE_PERMITTED_SUBCLASSES);
		List<String> permittedSubClasses = permittedSubClassesAttr != null
		                                   ? permittedSubClassesAttr.getClasses()
		                                   : Collections.emptyList();
		boolean isSealed = permittedSubClassesAttr != null && !permittedSubClasses.isEmpty();
		boolean isNonSealed = !isSealed && cl.getVersion()
		                                     .hasSealedClasses() && isSuperClassSealed(cl);

		if (isDeprecated) {
			if (!containsDeprecatedAnnotation(cl)) {
				appendDeprecation(buffer,
				                  indent);
			}
		}
		...
		if (javadocProvider != null) {
			appendJavadoc(buffer,
			              javadocProvider.getClassDoc(cl),
			              indent);
		}
		...
		if (descriptor != null && !descriptor.fparameters.isEmpty()) {
			appendTypeParameters(buffer,
			                     descriptor.fparameters,
			                     descriptor.fbounds);
		}

		if (components != null) {
			buffer.append('(');
			RecordHelper.appendRecordComponents(buffer,
			                                    cl,
			                                    components,
			                                    indent);
			buffer.append(')');
		...
		buffer.pushNewlineGroup(indent,
		                        1);

		if (!isEnum && !isInterface && components == null && cl.superClass != null) {
			VarType supertype = new VarType(cl.superClass.getString(),
			                                true);
		...
		if (!isEnum && isSealed) {
			...
			for (int i = 0;
			     i < permittedSubClasses.size();
			     i++) {
				if (i > 0) {
					buffer.append(",");
					buffer.appendPossibleNewline(" ");
				}
				buffer.append(ExprProcessor.getCastTypeName(new VarType(permittedSubClasses.get(i),
				                                                        true)));
			}
		}
	...
	private void fieldToJava(ClassWrapper wrapper,
		...
		if (javadocProvider != null) {
			appendJavadoc(buffer,
			              javadocProvider.getFieldDoc(cl,
			                                          fd),
			              indent);
		}
		appendAnnotations(buffer,
		...
		if (fd.hasModifier(CodeConstants.ACC_STATIC)) {
			initializer = wrapper.getStaticFieldInitializers()
			                     .getWithKey(InterpreterUtil.makeUniqueKey(fd.getName(),
			                                                               fd.getDescriptor()));
		} else {
		...
		if (initializer != null) {
			if (isEnum && initializer.type == Exprent.EXPRENT_NEW) {
				...
				// FIXME: special case field initializer. Can map to more than one method (constructor) and bytecode
				//  instruction.
				ExprProcessor.getCastedExprent(initializer,
				                               descriptor == null
				                               ? fieldType
				                               : descriptor.type,
				                               buffer,
			...
			if (attr != null) {
				PrimitiveConstant constant = cl.getPool()
				                               .getPrimitiveConstant(attr.getIndex());
				buffer.append(" = ");
				buffer.append(new ConstExprent(fieldType,
				                               constant.value,
				                               null).toJava(indent));
			}
		}
	...
	private boolean methodToJava(ClassNode node,
	                             StructMethod mt,
	                             int methodIndex,
	                             TextBuffer buffer,
	                             int indent) {
		...
		try {
			...
			MethodDescriptor md = MethodDescriptor.parseDescriptor(mt,
			                                                       node);
			...
			if (javadocProvider != null) {
				appendJavadoc(buffer,
				              javadocProvider.getMethodDoc(cl,
				                                           mt),
				              indent);
			}
			...
			if (DecompilerContext.getOption(IFernflowerPreferences.OVERRIDE_ANNOTATION)
				...
				boolean isOverride = searchForMethod(cl,
				                                     mt.getName(),
				                                     md,
				                                     false);
				if (isOverride) {
			...
			if (!clInit && !dInit) {
				...
				int index = isEnum && init
				            ...
				            : thisVar
				              ? 1
				              : 0;
				int start = isEnum && init
				            ? 2
				            : 0;
				boolean hasDescriptor = descriptor != null;
				//mask should now have the Outer.this in it... so this *shouldn't* be nessasary.
				//if (init && !isEnum && ((node.access & CodeConstants.ACC_STATIC) == 0) && node.type == ClassNode
				// .CLASS_MEMBER)
				//    index++;
				...
				for (int i = start;
					...
					VarType parameterType = hasDescriptor && paramCount < descriptor.parameterTypes.size()
					                        ? descriptor.parameterTypes.get(paramCount)
					                        : md.params[i];
					if (mask == null || mask.get(i) == null) {
						if (paramCount > 0) {
							buffer.append(",");
						...
						appendParameterAnnotations(buffer,
						                           mt,
						                           paramCount);

						if (methodParameters != null && i < methodParameters.size()) {
							appendModifiers(buffer,
							                methodParameters.get(i).myAccessFlags,
							                CodeConstants.ACC_FINAL,
							                isInterface,
							                0);
						} else if (methodWrapper.varproc.getVarFinal(new VarVersionPair(index,
						                                                                0))
						...
						if (methodParameters != null && i < methodParameters.size()) {
							...
							parameterName = methodWrapper.varproc.getVarName(new VarVersionPair(index,
							                                                                    0));
						}

						if ((flags & (CodeConstants.ACC_ABSTRACT | CodeConstants.ACC_NATIVE)) != 0) {
							String newParameterName = methodWrapper.methodStruct.getVariableNamer()
							                                                    .renameAbstractParameter(parameterName,
							                                                                             index);
							parameterName = !newParameterName.equals(parameterName)
							                ? newParameterName
							                : DecompilerContext.getStructContext()
							                                   .renameAbstractParameter(methodWrapper.methodStruct.getClassQualifiedName(),
							                                                            mt.getName(),
							                                                            mt.getDescriptor(),
							                                                            index - (((flags & CodeConstants.ACC_STATIC)
							                                                                      == 0)
							                                                                     ? 1
							                                                                     : 0),
							                                                            parameterName);
				...
				if ((descriptor != null && !descriptor.exceptionTypes.isEmpty()) || attr != null) {
					...
					for (int i = 0;
						...
						VarType type = useDescriptor
						               ...
						               : new VarType(attr.getExcClassname(i,
						                                                  cl.getPool()),
						                             true);
			...
			if ((flags & (CodeConstants.ACC_ABSTRACT | CodeConstants.ACC_NATIVE))
				...
				if (isAnnotation) {
					...
					if (attr != null) {
						buffer.append(" default ");
						buffer.append(attr.getDefaultValue()
						                  .toJava(0));
					}
				}
				...
				if (root != null && methodWrapper.decompileError == null) { // check for existence
					try {
						...
						if (RecordHelper.isHiddenRecordMethod(cl,
						                                      mt,
						                                      root)) {
							hideMethod = true;
						} else {
							...
							hideMethod = code.length() == 0 && (clInit || dInit || hideConstructor(node,
							                                                                       init,
							                                                                       throwsExceptions,
							                                                                       paramCount,
							                                                                       flags));
