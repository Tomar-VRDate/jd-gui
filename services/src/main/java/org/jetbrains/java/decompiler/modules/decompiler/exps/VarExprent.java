...
public class VarExprent
	...
	public VarVersionPair getVarVersionPair() {
		return new VarVersionPair(index,
		                          version);
	}
  ...
  */
	...
	private void appendDefinitionType(TextBuffer buffer) {
		if (DecompilerContext.getOption(IFernflowerPreferences.USE_DEBUG_VAR_NAMES)) {
			...
			if (method != null) {
				...
				int visibleOffset = bytecode == null
				                    ? -1
				                    : bytecode.length();
				if (originalIndex != null) {
					...
					if (DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_GENERIC_SIGNATURES)) {
						...
						if (attr != null) {
							String signature = attr.getSignature(originalIndex,
							                                     visibleOffset);
							if (signature != null) {
					...
					if (attr != null) {
						String descriptor = attr.getDescriptor(originalIndex,
						                                       visibleOffset);
						if (descriptor != null) {
	...
	public boolean equals(Object o) {
		...
		return index == ve.getIndex() && version == ve.getVersion() && InterpreterUtil.equalObjects(getVarType(),
		                                                                                            ve.getVarType()); //
	...
	public VarType getVarType() {
		...
		if (processor != null) {
			...
			vt = Exprent.inferredLambdaTypes.get()
			                                .get(name);
			if (vt == null) {
				vt = processor.getVarType(getVarVersionPair());
				if (processor.getThisVars()
				             .containsKey(getVarVersionPair())) {
					String qaulName = processor.getThisVars()
					                           .get(getVarVersionPair());
					StructClass cls = DecompilerContext.getStructContext()
					                                   .getClass(qaulName);
					if (cls != null && cls.getSignature() != null) {
						vt = cls.getSignature().genericType;
					} else if (vt == null) {
						vt = new VarType(CodeConstants.TYPE_OBJECT,
						                 0,
						                 qaulName);
					}
				}
	...
	public void setLVT(LocalVariable var) {
		...
		if (processor != null && lvt != null) {
			processor.setVarType(getVarVersionPair(),
			                     lvt.getVarType());
		}
	}
	...
	public String getName() {
		...
		return pair.version == 0
		       ? "var" + pair.var
		       : "var" + pair.var + "_" + version;
	}
		...
		if (stat.getExprents() == null) {
			for (Object obj : stat.getSequentialObjects()) {
				if (obj instanceof Statement) {
					if (isVarReferenced((Statement) obj,
					                    whitelist)) {
						return true;
					}
				} else if (obj instanceof Exprent) {
					if (isVarReferenced((Exprent) obj,
					                    whitelist)) {
						return true;
					}
				}
	...
	public boolean match(MatchNode matchNode,
		...
		RuleValue rule = matchNode.getRules()
		                          .get(MatchProperties.EXPRENT_VAR_INDEX);
		if (rule != null) {
			if (rule.isVariable()) {
