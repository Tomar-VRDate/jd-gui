...
public class ImportCollector {
	...
	public ImportCollector(ClassNode root) {
		...
		if (index >= 0) {
			String packageName = clName.substring(0,
			                                      index);
		...
		while (currentClass != null) {
			...
			currentClass = currentClass.superClass != null
			               ? ctx.getClass(currentClass.superClass.getString())
			               : null;
		...
		collectConflictingShortNames(root,
		                             new HashMap<>());
	}
	...
	public String getShortName(String fullName,
		...
		boolean existsDefaultClass = (context.getClass(currentPackageSlash + shortName) != null
		                              && !packageName.equals(currentPackagePoint)) || // current package
		...
		String mapKey = currCls == null
		                ? ""
		                : currCls.classStruct.qualifiedName;
		Map<String, String> innerClassNames = mapInnerClassNames.getOrDefault(mapKey,
		                                                                      new HashMap<>());
		if (!existsDefaultClass && innerClassNames.containsKey(shortName) && !innerClassNames.get(shortName)
		                                                                                     .equals(fullName)) {
			// if the class being accessed is also an inner class
			// attempt to import the outer class and reference OuterClass.InnerClass
			if (context.getClass(packageName.replace('.',
			                                         '/') + "$" + shortName) != null) {
				...
				if (lastDot >= 0) {
					...
					if (innerClassNames.containsKey(shortName) && !innerClassNames.get(shortName)
					                                                              .equals(packageName + '.' + shortName)) {
		...
		if (existsDefaultClass || (mapSimpleNames.containsKey(shortName)
		                           && !packageName.equals(mapSimpleNames.get(shortName)))) {
			//  don't return full name because if the class is a inner class, full name refers to the parent full name, not
			//  the child full name
			return result == null
			       ? fullName
			       : ((!packageName.isEmpty()
	...
	public void writeImports(TextBuffer buffer,
	...
	private List<String> packImports() {
		return mapSimpleNames.entrySet()
		                     .stream()
		                     .filter(ent ->
	...
	private void collectConflictingShortNames(ClassNode root,
	                                          Map<String, String> rootNames) {
		Map<String, String> names = new HashMap<>(rootNames);
		getSuperClassInnerClasses(root,
		                          names);
		mapInnerClassNames.put(root.classStruct.qualifiedName,
		                       names);

		for (ClassNode nested : root.nested) {
			collectConflictingShortNames(nested,
			                             names);
		}
	}

	private void getSuperClassInnerClasses(ClassNode node,
		...
		while (currentClass != null) {
			...
			Collections.addAll(queue,
			                   currentClass.getInterfaceNames());

			// .. all inner classes for the current class ..
			StructInnerClassesAttribute attribute =
							currentClass.getAttribute(StructGeneralAttribute.ATTRIBUTE_INNER_CLASSES);
			if (attribute != null) {
				for (StructInnerClassesAttribute.Entry entry : attribute.getEntries()) {
					if (entry.enclosingName != null && entry.enclosingName.equals(currentClass.qualifiedName)) {
						names.put(entry.simpleName,
						          entry.innerName.replace('/',
						                                  '.')
						                         .replace('$',
						                                  '.'));
					}
			...
			// .. and traverse through parent.
			do {
				currentClass = queue.isEmpty()
				               ? null
				               : ctx.getClass(queue.removeFirst());
