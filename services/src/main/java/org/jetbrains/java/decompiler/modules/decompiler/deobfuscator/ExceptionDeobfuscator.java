...
public final class ExceptionDeobfuscator {
	...
	public static void restorePopRanges(ControlFlowGraph graph) {
		...
		for (ExceptionRangeCFG range : graph.getExceptions()) {
			...
			if (!found) {
				...
				lstRanges.add(new Range(range.getHandler(),
				                        range.getUniqueExceptionsString(),
				                        new HashSet<>(range.getProtectedRange()),
				                        range));
			}
		}
					...
					if (firstinstr.opcode == CodeConstants.opc_pop || firstinstr.opcode == CodeConstants.opc_astore) {
						...
						for (Range range_super : lstRanges) { // finally or strict superset
							...
							if (range != range_super) {
								...
								if (!setrange.contains(range_super.handler)
								    && !setrange_super.contains(handler)
									...
									if (!setrange_super.isEmpty()) {
										...
										if (seq.length() > 1) {
											...
											for (ExceptionRangeCFG range_ext : graph.getExceptions()) {
												...
												} else if (range_ext.getProtectedRange()
												                    .contains(handler)) {
													newblock.addSuccessorException(range_ext.getHandler());
													range_ext.getProtectedRange()
													         .add(newblock);
												}
											}
										...
										range_super.rangeCFG.getProtectedRange()
										                    .add(newblock);
	...
	private static List<BasicBlock> getReachableBlocksRestricted(BasicBlock start,
		...
		while (!stack.isEmpty()) {
			...
			if (range.getProtectedRange()
			         .contains(block) && engine.isDominator(block,
			                                                start)) {
				lstRes.add(block);
	...
	public static boolean hasObfuscatedExceptions(ControlFlowGraph graph) {
		...
		for (ExceptionRangeCFG range : graph.getExceptions()) {
			mapRanges.computeIfAbsent(range.getHandler(),
			                          k -> new HashSet<>())
	...
	private static boolean splitExceptionRange(ExceptionRangeCFG range,
		...
		for (BasicBlock entry : setEntries) {
			List<BasicBlock> lstSubrangeBlocks = getReachableBlocksRestricted(entry,
			                                                                  range,
			                                                                  engine);
			if (!lstSubrangeBlocks.isEmpty() && lstSubrangeBlocks.size() < range.getProtectedRange()
			                                                                    .size()) {
				// add new range
				ExceptionRangeCFG subRange = new ExceptionRangeCFG(lstSubrangeBlocks,
				                                                   range.getHandler(),
				...
				range.getProtectedRange()
				     .removeAll(lstSubrangeBlocks);
				return true;
			} else {
				// should not happen
				DecompilerContext.getLogger()
				                 .writeMessage("Inconsistency found while splitting protected range",
	...
	public static void insertDummyExceptionHandlerBlocks(ControlFlowGraph graph,
		...
		for (Entry<BasicBlock, Set<ExceptionRangeCFG>> ent : mapRanges.entrySet()) {
			...
			for (ExceptionRangeCFG range : ranges) {
				...
				seq.addInstruction(Instruction.create(CodeConstants.opc_bipush,
				                                      ...
				                                      1),
				                   -1);
				seq.addInstruction(Instruction.create(CodeConstants.opc_pop,
				                                      false,
				                                      CodeConstants.GROUP_GENERAL,
				                                      bytecode_version,
				                                      null,
				...
				for (BasicBlock commonHandler : commonHandlers) {
					ExceptionRangeCFG commonRange = graph.getExceptionRange(commonHandler,
					                                                        handler);

					dummyBlock.addSuccessorException(commonHandler);
					commonRange.getProtectedRange()
					           .add(dummyBlock);
				}
	...
	private static final class Range {
		...
		private Range(BasicBlock handler,
		              String uniqueStr,
		              Set<BasicBlock> protectedRange,
