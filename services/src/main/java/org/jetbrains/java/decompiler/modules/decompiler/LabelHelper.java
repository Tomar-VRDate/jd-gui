...
public final class LabelHelper {
			...
			if (edge.getType() == StatEdge.TYPE_BREAK) {  // FIXME: ?
				for (Statement st : stat.getStats()) {
					if (st.containsStatementStrict(edge.getSource())) {
						if (MergeHelper.isDirectPath(st,
						                             edge.getDestination())) {
	...
	private static void setRetEdgesUnlabeled(RootStatement root) {
		...
		for (StatEdge edge : exit.getAllPredecessorEdges()) {
			List<Exprent> lst = edge.getSource()
			                        .getExprents();
			if (edge.getType() == StatEdge.TYPE_FINALLYEXIT || (lst != null
			                                                    && !lst.isEmpty()
		...
		switch (stat.type) {
			...
			case Statement.TYPE_CATCHALL:
				...
				for (Statement st : stat.getStats()) {
					...
					processEdgesWithNext(st,
					                     mapEdges1,
					                     null);

					if (stat.type == Statement.TYPE_TRYCATCH
					    || st == stat.getFirst()) { // edges leaving a finally catch block are always explicit
						// merge the maps
						if (mapEdges1 != null) {
							for (Entry<Statement, List<StatEdge>> entr : mapEdges1.entrySet()) {
								if (mapEdges.containsKey(entr.getKey())) {
									mapEdges.get(entr.getKey())
									        .addAll(entr.getValue());
								} else {
									mapEdges.put(entr.getKey(),
									             entr.getValue());
								}
							}
						}
			...
			case Statement.TYPE_DO:
				...
				processEdgesWithNext(stat.getFirst(),
				                     mapEdges,
				                     stat);
				break;
			case Statement.TYPE_IF:
				...
				if (ifstat.getIfstat() == null) { // empty if
					processEdgesWithNext(ifstat.getFirst(),
					                     mapEdges,
					                     null);
				} else {
					mapEdges = setExplicitEdges(ifstat.getIfstat());
					processEdgesWithNext(ifstat.getIfstat(),
					                     mapEdges,
					                     null);
					...
					if (ifstat.getElsestat() != null) {
						...
						processEdgesWithNext(ifstat.getElsestat(),
						                     mapEdges1,
						                     null);
					}
			...
			case Statement.TYPE_ROOT:
				...
				processEdgesWithNext(stat.getFirst(),
				                     mapEdges,
				                     ((RootStatement) stat).getDummyExit());
				break;
			...
			case Statement.TYPE_SWITCH:
				...
				for (int i = 0;
					...
					if (stnext.getExprents() != null && stnext.getExprents()
					                                          .isEmpty() && !stnext.getAllSuccessorEdges()
					                                                               .isEmpty()) {
						stnext = stnext.getAllSuccessorEdges()
						               .get(0)
						               .getDestination();
					}
					processEdgesWithNext(stt,
					                     setExplicitEdges(stt),
					                     stnext);
				}

				int last = swst.getCaseStatements()
				               .size() - 1;
				if (last >= 0) { // empty switch possible
					Statement stlast = swst.getCaseStatements()
					                       .get(last);
					if (stlast.getExprents() != null && stlast.getExprents()
					                                          .isEmpty() && !stlast.getAllSuccessorEdges()
					                                                               .isEmpty()) {
						StatEdge edge = stlast.getAllSuccessorEdges()
						                      .get(0);
						mapEdges.put(edge.getDestination(),
						             new ArrayList<>(Collections.singletonList(edge)));
					} else {
						mapEdges = setExplicitEdges(stlast);
						processEdgesWithNext(stlast,
						                     mapEdges,
						                     null);
					}
				}
			...
			case Statement.TYPE_SYNCRONIZED:
				...
				processEdgesWithNext(synstat.getFirst(),
				                     setExplicitEdges(stat.getFirst()),
				                     synstat.getBody()); // FIXME: basic block?
				mapEdges = setExplicitEdges(synstat.getBody());
				processEdgesWithNext(synstat.getBody(),
				                     mapEdges,
				                     null);
		}
	...
	private static void processEdgesWithNext(Statement stat,
	                                         HashMap<Statement, List<StatEdge>> mapEdges,
	                                         Statement next) {
		...
		if (next == null) {
			if (mapEdges.size() == 1) {
				List<StatEdge> lstEdges = mapEdges.values()
				                                  .iterator()
				                                  .next();
				if (lstEdges.size() > 1
				    && mapEdges.keySet()
				               .iterator()
				               .next().type != Statement.TYPE_DUMMYEXIT) {
					...
					StatEdge newedge = new StatEdge(edge_example.getType(),
					                                stat,
					                                edge_example.getDestination(),
					                                closure);
					...
					mapEdges.put(newedge.getDestination(),
					             new ArrayList<>(Collections.singletonList(newedge)));
				}
			}
		} else {
				...
				if (entr.getKey() == next) {
					for (StatEdge edge : entr.getValue()) {
						if (stat.type == Statement.TYPE_DO) {
							...
							// Edges that contain it's own loop as a closure are always explicit, as removing them can alter control
							// flow [TestLoopBreak]
						...
						if (edge.closure != null
						    && edge.closure.type == Statement.TYPE_DO
						    && stat.type == Statement.TYPE_IF
			...
			if (stat.getAllSuccessorEdges()
				...
				for (Entry<Statement, List<StatEdge>> entr : mapEdges.entrySet()) {
					if (entr.getKey().type != Statement.TYPE_DUMMYEXIT && (lstEdges == null
					                                                       || entr.getValue()
					                                                              .size() > lstEdges.size())) {
		...
		if (statedge != null) {
			mapEdges.put(statedge.getDestination(),
			             new ArrayList<>(Collections.singletonList(statedge)));
		}
	}
		...
		if (stat.type == Statement.TYPE_SWITCH) {
			...
			if (last >= 0) { // empty switch possible
				...
				if (stlast.getExprents() != null && stlast.getExprents()
				                                          .isEmpty() && !stlast.getAllSuccessorEdges()
				                                                               .isEmpty()) {
					if (!stlast.getAllSuccessorEdges()
					           .get(0).explicit) {
						List<StatEdge> lstEdges = swst.getCaseEdges()
						                              .get(last);
			...
			if (shieldType) {
				for (StatEdge edge : stat.getLabelEdges()) {
					if (edge.explicit && ((edge.getType() == StatEdge.TYPE_BREAK && sets.breaks.contains(edge.getSource())) || (
									edge.getType() == StatEdge.TYPE_CONTINUE
									&& sets.continues.contains(edge.getSource())))) {
						edge.labeled = false;
					}
				}
			}
	...
	//
	// Also processes labeled continues to make them into breaks if they are the last statement inside the closure
	// statement
	// As so:
	//
	// label:
	...
	// }
	//
	// This is only applicable when this is the last statement within it's parent nest as otherwise it'll be a jump to
	// the next statement instead of a backjump to the loop
		...
		if (stat.type == Statement.TYPE_DO) {
			...
			for (StatEdge edge : lst) {
				...
				if (edge.explicit) {
					...
					if (minclosure != edge.closure && !InlineSingleBlockHelper.isBreakEdgeLabeled(edge.getSource(),
					                                                                              minclosure)) {
						edge.getSource()
