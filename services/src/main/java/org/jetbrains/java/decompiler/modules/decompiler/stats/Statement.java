...
public class Statement
	...
	private static void addToReversePostOrderListIterative(Statement root,
		...
		while (!stackNode.isEmpty()) {
			...
			for (;
				...
				if (!setVisited.contains(succ) && (edge.getType() == StatEdge.TYPE_REGULAR
	...
	private static void addToPostReversePostOrderList(Statement stat,
		...
		if (setVisited.contains(stat)) { // because of not considered exception edges, s. isExitComponent. Should be
			// rewritten, if possible.
			return;
		}
		setVisited.add(stat);

		for (StatEdge prededge : stat.getEdges(StatEdge.TYPE_REGULAR | StatEdge.TYPE_EXCEPTION,
		                                       DIRECTION_BACKWARD)) {
			Statement pred = prededge.getSource();
			if (!setVisited.contains(pred)) {
				addToPostReversePostOrderList(pred,
				                              lst,
				                              setVisited);
			}
		}
	...
	public void collapseNodesToStatement(Statement stat) {
		...
		// post edges
		if (post != null) {
			for (StatEdge edge : post.getEdges(STATEDGE_DIRECT_ALL,
			                                   DIRECTION_BACKWARD)) {
				if (stat.containsStatementStrict(edge.getSource())) {
					edge.getSource()
					    .changeEdgeType(DIRECTION_FORWARD,
					                    edge,
					                    StatEdge.TYPE_BREAK);
		...
		for (StatEdge prededge : head.getAllPredecessorEdges()) {
			...
			prededge.getSource()
			        .changeEdgeNode(DIRECTION_FORWARD,
			                        prededge,
			                        stat);
		...
		// exception edges
		Set<Statement> setHandlers = new HashSet<>(head.getNeighbours(StatEdge.TYPE_EXCEPTION,
		                                                              DIRECTION_FORWARD));
		for (Statement node : setNodes) {
			setHandlers.retainAll(node.getNeighbours(StatEdge.TYPE_EXCEPTION,
			                                         DIRECTION_FORWARD));
		}

		if (!setHandlers.isEmpty()) {

			for (StatEdge edge : head.getEdges(StatEdge.TYPE_EXCEPTION,
			                                   DIRECTION_FORWARD)) {
				Statement handler = edge.getDestination();

				if (setHandlers.contains(handler)) {
					if (!setNodes.containsKey(handler.id)) {
						stat.addSuccessor(new StatEdge(stat,
						                               handler,
						                               edge.getExceptions()));
					}
				}
			...
			for (Statement node : setNodes) {
				for (StatEdge edge : node.getEdges(StatEdge.TYPE_EXCEPTION,
				                                   DIRECTION_FORWARD)) {
		...
		if (post != null && !stat.getNeighbours(StatEdge.TYPE_EXCEPTION,
		                                        DIRECTION_FORWARD)
		                         .contains(post)) { // TODO: second condition redundant?
			stat.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
			                               stat,
			                               post));
		}
	...
	private void addEdgeDirectInternal(int direction,
	                                   StatEdge edge,
	                                   int edgetype) {
		Map<Integer, List<StatEdge>> mapEdges = direction == DIRECTION_BACKWARD
		                                        ? mapPredEdges
		                                        : mapSuccEdges;
		Map<Integer, List<Statement>> mapStates = direction == DIRECTION_BACKWARD
		                                          ? mapPredStates
		                                          : mapSuccStates;

		mapEdges.computeIfAbsent(edgetype,
		                         k -> new ArrayList<>())
		        .add(edge);

		mapStates.computeIfAbsent(edgetype,
		                          k -> new ArrayList<>())
		         .add(direction == DIRECTION_BACKWARD
		              ? edge.getSource()
		              : edge.getDestination());
	}

	private void addEdgeInternal(int direction,
	                             StatEdge edge) {
		int type = edge.getType();

		int[] arrtypes;
		if (type == StatEdge.TYPE_EXCEPTION) {
			arrtypes = new int[]{STATEDGE_ALL,
			                     StatEdge.TYPE_EXCEPTION};
		} else {
			arrtypes = new int[]{STATEDGE_ALL,
			                     STATEDGE_DIRECT_ALL,
			                     type};
		...
		for (int edgetype : arrtypes) {
			addEdgeDirectInternal(direction,
			                      edge,
			                      edgetype);
		}
	}

	private void removeEdgeDirectInternal(int direction,
	                                      StatEdge edge,
	                                      int edgetype) {
		...
		if (lst != null) {
			...
			if (index >= 0) {
				lst.remove(index);
				mapStates.get(edgetype)
				         .remove(index);
			}
		}
	}

	private void removeEdgeInternal(int direction,
	                                StatEdge edge) {
		...
		for (int edgetype : arrtypes) {
			removeEdgeDirectInternal(direction,
			                         edge,
			                         edgetype);
		}
	}
	...
	public void addPredecessor(StatEdge edge) {
		addEdgeInternal(DIRECTION_BACKWARD,
		                edge);
	}
	...
	public void removePredecessor(StatEdge edge) {
		...
		removeEdgeInternal(DIRECTION_BACKWARD,
		                   edge);
	}

	public void addSuccessor(StatEdge edge) {
		addEdgeInternal(DIRECTION_FORWARD,
		                edge);
	...
	public void removeSuccessor(StatEdge edge) {
		...
		removeEdgeInternal(DIRECTION_FORWARD,
		                   edge);
	...
	public HashSet<Statement> buildContinueSet() {
		...
		for (StatEdge edge : getEdges(StatEdge.TYPE_CONTINUE,
		                              DIRECTION_FORWARD)) {
			continueSet.add(edge.getDestination()
			                    .getBasichead());
		}
	...
	public void buildMonitorFlags() {
		...
		switch (type) {
			case TYPE_BASICBLOCK:
				...
				InstructionSequence seq = bblock.getBlock()
				                                .getSeq();
	...
	public List<Statement> getReversePostOrderList(Statement stat) {
		...
		addToReversePostOrderListIterative(stat,
		                                   res);
	...
	public List<Statement> getPostReversePostOrderList(List<Statement> lstexits) {
		...
		for (Statement exit : lstexits) {
			addToPostReversePostOrderList(exit,
			                              res,
			                              setVisited);
		}
	...
	public final void replaceWith(Statement stat) {
		this.parent.replaceStatement(this,
		                             stat);
	}
	...
	public void replaceStatement(Statement oldstat,
		...
		stats.addWithKeyAndIndex(statindex,
		                         newstat,
		                         newstat.id);
	...
	public void changeEdgeNode(int direction,
	                           StatEdge edge,
	                           Statement value) {
	...
	public void changeEdgeType(int direction,
	                           StatEdge edge,
	                           int newtype) {
		...
		if (direction == DIRECTION_FORWARD) {
			edge.getDestination()
			    .changeEdgeType(DIRECTION_BACKWARD,
			                    edge,
			                    newtype);
		}
	...
	private List<StatEdge> getEdges(int type,
	                                int direction) {

		Map<Integer, List<StatEdge>> map = direction == DIRECTION_BACKWARD
		                                   ? mapPredEdges
		                                   : mapSuccEdges;
	...
	public List<Statement> getNeighbours(int type,
	                                     int direction) {

		Map<Integer, List<Statement>> map = direction == DIRECTION_BACKWARD
		                                    ? mapPredStates
		                                    : mapSuccStates;
	...
	public Set<Statement> getNeighboursSet(int type,
	                                       int direction) {
		return new HashSet<>(getNeighbours(type,
		                                   direction));
	}

	public List<StatEdge> getSuccessorEdges(int type) {
		return getEdges(type,
		                DIRECTION_FORWARD);
	}
	...
	// Loops are connected to the next block if it's not an infinite while(true){} loop TODO: many while(true) loops
	//  have breaks in their body. Would that not count?
	public boolean hasBasicSuccEdge() {

		// FIXME: default switch

		return type == TYPE_BASICBLOCK || (type == TYPE_IF && ((IfStatement) this).iftype == IfStatement.IFTYPE_IF) || (type
		                                                                                                                == TYPE_DO
		                                                                                                                && ((DoStatement) this).getLooptype()
		                                                                                                                   != DoStatement.LOOP_DO);
	}
			...
			for (Object obj : this.getSequentialObjects()) {
				if (obj == null) {
					...
					DecompilerContext.getLogger()
					                 .writeMessage("Found unknown class from sequential objects! " + obj.getClass(),
	...
	public StartEndPair getStartEndRange() {
		if (endpoints == null) {
			...
			getOffset(set);
			endpoints = new StartEndPair(set.nextSetBit(0),
			                             set.length() - 1);
		}
		return endpoints;
	}
	...
	@Override
	public IMatchable findObject(MatchNode matchNode,
	                             int index) {
		...
		if (node_type == MatchNode.MATCHNODE_STATEMENT && !this.stats.isEmpty()) {
			...
			if (position != null) {
				if (position.matches("-?\\d+")) {
					return this.stats.get((this.stats.size() + Integer.parseInt(position))
			...
			if (position != null) {
				if (position.matches("-?\\d+")) {
					return this.exprents.get((this.exprents.size() + Integer.parseInt(position))
		...
		for (Entry<MatchProperties, RuleValue> rule : matchNode.getRules()
			...
			switch (rule.getKey()) {
				...
				case STATEMENT_EXPRSIZE:
					...
					break;
				case STATEMENT_RET:
					if (!engine.checkAndSetVariableValue((String) rule.getValue().value,
					                                     this)) {
						return false;
					}
					break;
