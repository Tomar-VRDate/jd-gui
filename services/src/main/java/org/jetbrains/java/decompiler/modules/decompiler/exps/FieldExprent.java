...
public class FieldExprent
	...
	public FieldExprent(LinkConstant cn,
	                    Exprent instance,
		...
		this(cn.elementname,
		     ...
		     instance == null,
		     instance,
		     FieldDescriptor.parseDescriptor(cn.descriptor),
		     bytecodeOffsets);
	}

	public FieldExprent(String name,
	                    String classname,
	                    boolean isStatic,
	                    Exprent instance,
	...
	public VarType getInferredExprType(VarType upperBound) {
		StructClass cl = DecompilerContext.getStructContext()
		                                  .getClass(classname);
		Map<String, Map<VarType, VarType>> types = cl == null
		...
		if (ft != null && ft.getSignature() != null) {
			VarType ret = ft.getSignature().type.remap(types.getOrDefault(cl.qualifiedName,
			                                                              Collections.emptyMap()));
	...
	public int getExprentUse() {
		...
		//+            aint[j1] &= 16777215;
		//return 0; // multiple references to a field considered dangerous in a multithreaded environment, thus no Exprent
		// .MULTIPLE_USES set here
		return instance == null
		       ? Exprent.MULTIPLE_USES
		...
		if (isStatic) {
			...
			if (instance != null && instance.type == Exprent.EXPRENT_VAR) {
				...
				if (currentMethod != null) { // FIXME: remove
					String this_classname = currentMethod.varproc.getThisVars()
					                                             .get(pair);
			...
			if (super_qualifier != null) {
				...
				boolean casted = ExprProcessor.getCastedExprent(instance,
			...
			if (buf.contentEquals(VarExprent.VAR_NAMELESS_ENCLOSURE)) { // FIXME: workaround for field access of an
				// anonymous enclosing class. Find a better way.
				buf.setLength(0);
			} else {
				buf.append(".");
			}
	...
	public boolean match(MatchNode matchNode,
		...
		RuleValue rule = matchNode.getRules()
		                          .get(MatchProperties.EXPRENT_FIELD_NAME);
		if (rule != null) {
			if (rule.isVariable()) {
				return engine.checkAndSetVariableValue((String) rule.value,
				                                       this.name);
			} else {
