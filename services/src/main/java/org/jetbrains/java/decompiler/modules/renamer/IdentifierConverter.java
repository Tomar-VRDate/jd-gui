...
public class IdentifierConverter
	...
	public IdentifierConverter(StructContext context,
	                           IIdentifierRenamer helper,
	...
	private void renameClasses() {
		...
		for (ClassWrapperNode node : lstClasses) {
			...
			renameClassIdentifiers(cl,
			                       names);

			if (!node.getSubclasses()
			         .isEmpty()) {
				classNameMaps.put(cl.qualifiedName,
				                  names);
			}
		}
	}
	...
	private void renameInterfaces() {
		...
		for (ClassWrapperNode node : lstInterfaces) {
			...
			interfaceNameMaps.put(cl.qualifiedName,
			                      names);
		}
	...
	private void renameClass(StructClass cl) {
		...
		if (helper.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_CLASS,
		                       clSimpleName,
		                       null,
		                       null)) {
			...
			interceptor.addName(classOldFullName,
			                    classNewFullName);
		}
	}
	...
	private void renameClassIdentifiers(StructClass cl,
		...
		for (int i = 0;
			...
			if (!cl.isOwn() || mt.hasModifier(CodeConstants.ACC_NATIVE)) {
			...
			} else if (helper.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_METHOD,
			                              classOldFullName,
			                              name,
			                              mt.getDescriptor())) {
				if (isPrivate || !names.containsKey(key)) {
					do {
						name = helper.getNextMethodName(classOldFullName,
						                                name,
						                                mt.getDescriptor());
				...
				interceptor.addName(classOldFullName + " " + mt.getName() + " " + mt.getDescriptor(),
				                    classNewFullName + " " + name + " " + buildNewDescriptor(false,
				                                                                             mt.getDescriptor()));
			}
		}
		...
		for (StructField fd : cl.getFields()) {
			if (helper.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_FIELD,
			                       classOldFullName,
			                       fd.getName(),
			                       fd.getDescriptor())) {
				String newName;
				do {
					newName = helper.getNextFieldName(classOldFullName,
					                                  fd.getName(),
					                                  fd.getDescriptor());
				} while (setFieldNames.contains(newName));

				interceptor.addName(classOldFullName + " " + fd.getName() + " " + fd.getDescriptor(),
				                    classNewFullName + " " + newName + " " + buildNewDescriptor(true,
				                                                                                fd.getDescriptor()));
			}
		}
	}
	...
	private String buildNewDescriptor(boolean isField,
		...
		if (isField) {
			newDescriptor = FieldDescriptor.parseDescriptor(descriptor)
			                               .buildNewDescriptor(this);
		} else {
			newDescriptor = MethodDescriptor.parseDescriptor(descriptor)
			                                .buildNewDescriptor(this);
		}
