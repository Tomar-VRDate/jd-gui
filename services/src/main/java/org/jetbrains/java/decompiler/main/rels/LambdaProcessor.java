...
public class LambdaProcessor {
	...
	public void processClass(ClassNode node)
		...
		if (!cl.getVersion()
		       .hasLambdas()) { // lambda beginning with Java 8
			return;
		}
		...
		for (int i = 0;
		     i < bootstrap.getMethodsNumber();
		     ++i) {
			LinkConstant method_ref = bootstrap.getMethodReference(i); // method handle

			// FIXME: extend for Eclipse etc. at some point
			if (JAVAC_LAMBDA_CLASS.equals(method_ref.classname) && (JAVAC_LAMBDA_METHOD.equals(method_ref.elementname)
			                                                        || JAVAC_LAMBDA_ALT_METHOD.equals(method_ref.elementname))) {
				lambdaMethods.set(i);
			}
		}
			...
			if (seq != null && seq.length() > 0) {
				...
				for (int i = 0;
					...
					if (instr.opcode == CodeConstants.opc_invokedynamic) {
						LinkConstant invoke_dynamic = cl.getPool()
						                                .getLinkConstant(instr.operand(0));
						...
						if (lambdaMethods.get(invoke_dynamic.index1)) { // lambda invocation found
							...
							ClassNode node_lambda = new ClassNode(content_method_handle.classname,
							                                      ...
							                                      content_method_handle.descriptor,
							                                      content_method_handle.index1,
							                                      lambda_class_name,
							                                      lambda_method_name,
							                                      lambda_method_descriptor,
							                                      cl);
							node_lambda.simpleName = cl.qualifiedName
							                         + "##Lambda_"
							                         + invoke_dynamic.index1
							                         + "_"
							...
							clProcessor.getMapRootClasses()
							           .put(node_lambda.simpleName,
							                node_lambda);
		...
		for (ClassNode nd : node.nested) {
			if (nd.type == ClassNode.CLASS_LAMBDA) {
				...
				if (parent_class_name != null) {
					ClassNode parent_class = clProcessor.getMapRootClasses()
