...
public class SwitchExprent
	...
	public SwitchExprent(SwitchStatement backing,
	                     VarType type,
	                     boolean fallthrough) {
	...
	public TextBuffer toJava(int indent) {
		...
		VarType switchType = this.backing.getHeadexprentList()
		                                 .get(0)
		                                 .getExprType();

		buf.append(this.backing.getHeadexprentList()
		                       .get(0)
		                       .toJava(indent))
		   .append(" {")
		   .appendLineSeparator();
		for (int i = 0;
		     i < this.backing.getCaseStatements()
		                     .size();
		     i++) {

			Statement stat = this.backing.getCaseStatements()
			                             .get(i);
			List<StatEdge> edges = this.backing.getCaseEdges()
			                                   .get(i);
			List<Exprent> values = this.backing.getCaseValues()
			                                   .get(i);

			boolean hasDefault = false;
			// As switch expressions can be compiled to a tableswitch, any gaps will contain a jump to the default element.
			// Switch expressions cannot have a case point to the same statement as the default, so we check for default
			// first and don't check for cases if it exists [TestConstructorSwitchExpression1]
			...
			boolean simple = stat.type == Statement.TYPE_BASICBLOCK;
	...
	@Override
	public Exprent copy() {
		return new SwitchExprent(this.backing,
		                         this.type,
		                         this.fallthrough);
	}
