...
public final class DeadCodeHelper {
	...
	public static void removeEmptyBlocks(ControlFlowGraph graph) {
		...
		do {
			...
			for (int i = blocks.size() - 1;
			     i >= 0;
			     i--) {
				BasicBlock block = blocks.get(i);

				if (removeEmptyBlock(graph,
				                     block,
				                     false)) {
					cont = true;
					break;
	...
	private static boolean removeEmptyBlock(ControlFlowGraph graph,
	                                        BasicBlock block,
	                                        boolean merging) {
		...
		if (block.getSeq()
			...
			HashSet<BasicBlock> setExits = new HashSet<>(graph.getLast()
			                                                  .getPreds());

			if (block.getPredExceptions()
			         .isEmpty() && (!setExits.contains(block)
			                        || block.getPreds()
			                                .size() == 1)) {

				if (setExits.contains(block)) {
					BasicBlock pred = block.getPreds()
					                       .get(0);
					...
					if (pred.getSuccs()
					        .size() != 1 || (!pred.getSeq()
					                              .isEmpty()
					                         && pred.getSeq()
					                                .getLastInstr().group == CodeConstants.GROUP_SWITCH)) {
						return false;
					}
				}
				...
				if (setFinallyExits.contains(block)) {
					...
					setFinallyExits.add(setPreds.iterator()
					                            .next());
				}
				...
				if (graph.getFirst() == block) {
					if (setSuccs.size() != 1) {
						...
						graph.setFirst(setSuccs.iterator()
						                       .next());
					}
				}
	...
	public static boolean isDominator(ControlFlowGraph graph,
	                                  BasicBlock block,
	                                  BasicBlock dom) {
		...
		while (!lstNodes.isEmpty()) {
			...
			for (int i = 0;
			     i < node.getPredExceptions()
			             .size();
			     i++) {
				BasicBlock pred = node.getPredExceptions()
				                      .get(i);
				if (pred != dom && !marked.contains(pred)) {
					lstNodes.add(pred);
				}
	...
	public static void extendSynchronizedRangeToMonitorexit(ControlFlowGraph graph) {
		...
		while (true) {
			...
			for (ExceptionRangeCFG range : graph.getExceptions()) {
				...
				BasicBlock predBlock = setPreds.iterator()
				                               .next();
				...
				BasicBlock succBlock = setSuccs.iterator()
				                               .next();
				...
				if (handlerBlock.getSuccs()
				                .size() != 1) {
					continue; // non-unique handler successor
				}
				BasicBlock succHandler = handlerBlock.getSuccs()
				                                     .get(0);
				...
				BasicBlock rangeExitBlock = succBlock.getPreds()
				                                     .get(0);
				for (int j = 0;
				     j < rangeExitBlock.getSuccExceptions()
				                       .size();
				     j++) {
					BasicBlock hd = rangeExitBlock.getSuccExceptions()
					                              .get(j);
					succBlock.addSuccessorException(hd);

					ExceptionRangeCFG rng = graph.getExceptionRange(hd,
					                                                rangeExitBlock);
				...
				for (int counter = 0;
				     counter < handler_monitorexit_index;
				     counter++) {
			...
			if (len > 0 && len < 3) {
				...
				if (ok) {
					...
					if (!block.getPreds()
						...
						BasicBlock predecessor = block.getPreds()
						                              .get(0);

						for (BasicBlock handler : setPredHandlersIntersection) {
							ExceptionRangeCFG range = graph.getExceptionRange(handler,
							                                                  predecessor);
					...
					if (block.getPreds()
					         .size() == 1 && block.getPredExceptions()
					                              .isEmpty()) {

						BasicBlock bpred = block.getPreds()
						                        .get(0);
						if (bpred.getSuccs()
						         .size() == 1) {
							...
							for (BasicBlock succ : bpred.getSuccExceptions()) {
								if (!block.getSuccExceptions()
								          .contains(succ)) {
									ExceptionRangeCFG range = graph.getExceptionRange(succ,
									                                                  bpred);
	...
	public static void mergeBasicBlocks(ControlFlowGraph graph) {
		...
		while (true) {
			...
			for (BasicBlock block : graph.getBlocks()) {
				...
				if (block.getSuccs()
				         .size() == 1) {
					BasicBlock next = block.getSuccs()
					                       .get(0);

					if (next != graph.getLast() && (seq.isEmpty() || seq.getLastInstr().group != CodeConstants.GROUP_SWITCH)) {

						if (next.getPreds()
						        .size() == 1 && next.getPredExceptions()
						                            .isEmpty() && next != graph.getFirst()) {
							...
							if (sameRanges) {
								...
								block.getInstrOldOffsets()
								     .addAll(next.getInstrOldOffsets());
								next.getSeq()
								    .clear();

								removeEmptyBlock(graph,
								                 next,
								                 true);
