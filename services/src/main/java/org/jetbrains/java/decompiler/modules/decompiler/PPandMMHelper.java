...
public class PPandMMHelper {
			...
			if (as.getRight().type == Exprent.EXPRENT_FUNCTION) {
				...
				if (func.getFuncType() >= FunctionExprent.FUNCTION_I2L && func.getFuncType() <= FunctionExprent.FUNCTION_I2S) {
					...
					if (func.getLstOperands()
						...
						func = (FunctionExprent) func.getLstOperands()
						                             .get(0);
					} else {
						return null;
					}
				...
				if (func.getFuncType() == FunctionExprent.FUNCTION_ADD || func.getFuncType() == FunctionExprent.FUNCTION_SUB) {
					...
					if (econst.type == Exprent.EXPRENT_CONST && ((ConstExprent) econst).hasValueOne()) {
						...
						if (exprsEqual(left,
						               econd) && (midlayer == null || midlayer.equals(condtype))) {
							...
							if (!left.equals(econd)) {
								updateVersions(this.dgraph,
								               new VarVersionPair((VarExprent) left),
								               new VarVersionPair((VarExprent) econd));
							}
	...
	private boolean exprsEqual(Exprent e1,
	                           Exprent e2) {
		if (e1 == e2) {
			return true;
		}
	...
	private boolean varsEqual(Exprent e1,
	                          Exprent e2) {
	...
	private void updateVersions(DirectGraph graph,
	                            final VarVersionPair oldVVP,
	                            final VarVersionPair newVVP) {
