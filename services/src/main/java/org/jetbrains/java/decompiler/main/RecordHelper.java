...
public final class RecordHelper {
	public static boolean isHiddenRecordMethod(StructClass cl,
	                                           StructMethod mt,
	                                           RootStatement root) {
		if (cl.getRecordComponents() == null) {
			return false;
		}
		return isSyntheticRecordMethod(mt,
		                               root) || isDefaultRecordMethod(mt,
		                                                              root) || (mt.getName()
		                                                                          .equals(CodeConstants.INIT_NAME)
		                                                                        && !hasAnnotations(mt)
		                                                                        && isDefaultRecordConstructor(cl,
		                                                                                                      root));
	}

	public static void appendRecordComponents(TextBuffer buffer,
	                                          StructClass cl,
	...
	private static boolean isSyntheticRecordMethod(StructMethod mt,
	                                               RootStatement root) {
		...
		return "java/lang/runtime/ObjectMethods".equals(bootstrapMethod.classname)
		       && "bootstrap".equals(bootstrapMethod.elementname);
	}

	// Simple heuristic to check if a method is a default getter generated by a record.
	private static boolean isDefaultRecordMethod(StructMethod mt,
	                                             RootStatement root) {
		Exprent value = getSimpleReturnValue(root);
		...
		return ((VarExprent) instance).getIndex() == 0 && fieldExprent.getName()
		                                                              .equals(mt.getName());
	}

	private static boolean isDefaultRecordConstructor(StructClass cl,
	                                                  RootStatement root) {
		...
		for (int i = 0;
			...
			if (!component.getName()
			              .equals(((FieldExprent) left).getName())) {
				return false;
			}
	...
	private static StructMethod getCanonicalConstructor(StructClass cl) {
		String canonicalConstructorDescriptor = cl.getRecordComponents()
		                                          .stream()
		                                          .map(StructField::getDescriptor)
		                                          .collect(Collectors.joining("",
		                                                                      "(",
		                                                                      ")V"));
		return cl.getMethod(CodeConstants.INIT_NAME,
		                    canonicalConstructorDescriptor);
	}

	private static StructMethod getGetter(StructClass cl,
	                                      StructRecordComponent rc) {
		return cl.getMethod(rc.getName(),
		                    "()" + rc.getDescriptor());
	}
	...
	private static Set<String> getRecordComponentAnnotations(StructClass cl,
	                                                         StructRecordComponent cd,
	                                                         int param) {
		Set<String>        annotations = new LinkedHashSet<>();
		List<StructMember> members     = new ArrayList<>();
		members.add(cd);
		StructMethod getter = getGetter(cl,
		                                cd);
		if (getter != null) {
			members.add(getter);
		}
		...
		for (StructGeneralAttribute.Key<?> key : ClassWriter.PARAMETER_ANNOTATION_ATTRIBUTES) {
			...
			if (param >= paramAnnotations.size()) {
				continue;
			}
	...
	private static void recordComponentToJava(TextBuffer buffer,
	                                          StructClass cl,
	                                          StructRecordComponent cd,
	                                          int param,
	                                          boolean varArgComponent) {
		Set<String> annotations = getRecordComponentAnnotations(cl,
		                                                        cd,
		                                                        param);
		...
		VarType fieldType = new VarType(cd.getDescriptor(),
		                                false);
		...
		buffer.append(ExprProcessor.getCastTypeName(varArgComponent
		                                            ? fieldType.decreaseArrayDim()
		                                            : fieldType));
		if (varArgComponent) {
			buffer.append("...");
		}
