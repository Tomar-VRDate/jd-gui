...
public final class SequenceHelper {
	...
	private static void condenseSequencesRec(Statement stat) {
		...
		if (stat.type == Statement.TYPE_SEQUENCE) {
			...
			for (int i = 0;
				...
				if (st.type == Statement.TYPE_SEQUENCE) {
					...
					if (i == lst.size() - 1 || isSequenceDisbandable(st,
					                                                 lst.get(i + 1))) {
						// move predecessors
						...
						// move successors
						Statement last = st.getStats()
						                   .getLast();
						if (last.getAllSuccessorEdges()
						        .isEmpty() && i < lst.size() - 1) {
							last.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
							                               last,
							                               lst.get(i + 1)));
						} else {
		...
		if (stat.type == Statement.TYPE_SEQUENCE) {
			...
			if (stat.getStats()
				...
				boolean ok = st.getAllSuccessorEdges()
				               .isEmpty();
				if (!ok) {
					StatEdge edge = st.getAllSuccessorEdges()
					                  .get(0);

					ok = stat.getAllSuccessorEdges()
					         .isEmpty();
					if (!ok) {
						StatEdge statedge = stat.getAllSuccessorEdges()
						                        .get(0);
		...
		while (true) {
			for (Statement st : stat.getStats()) {
				if ((st.getStats()
				       .isEmpty() || st.getExprents() != null) && st.type != Statement.TYPE_BASICBLOCK) {
		...
		for (int i = 0;
		     i < stat.getStats()
		             .size();
		     i++) {
			condenseSequencesRec(stat.getStats()
			                         .get(i));
		}
	}

	private static boolean isSequenceDisbandable(Statement block,
	                                             Statement next) {

		Statement last = block.getStats()
		                      .getLast();
	...
	private static void removeEmptyStatements(SequenceStatement sequence) {
		...
		while (true) {
			...
			for (Statement st : sequence.getStats()) {
				...
				if (st.getExprents() != null && st.getExprents()
				                                  .isEmpty()) {

					if (st.getAllSuccessorEdges()
					      .isEmpty()) {
						...
						StatEdge sucedge = st.getAllSuccessorEdges()
						                     .get(0);
	...
	private static void mergeFlatStatements(SequenceStatement sequence) {
		...
		while (true) {
			...
			for (int i = sequence.getStats()
				...
				next = current;
				current = sequence.getStats()
				                  .get(i);

				if (next != null && current.getExprents() != null && !current.getExprents()
				                                                             .isEmpty()) {
	...
	public static void destroyStatementContent(Statement stat,
	                                           boolean self) {

		for (Statement st : stat.getStats()) {
			destroyStatementContent(st,
			                        true);
		}
		stat.getStats()
		    .clear();
