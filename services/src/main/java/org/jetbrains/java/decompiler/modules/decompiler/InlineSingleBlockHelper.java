...
public final class InlineSingleBlockHelper {
	...
	private static void inlineBlock(SequenceStatement seq,
	                                int index) {

		Statement first = seq.getStats()
		                     .get(index);
		Statement pre = seq.getStats()
		                   .get(index - 1);
		pre.removeSuccessor(pre.getAllSuccessorEdges()
		                       .get(0));   // single regular edge

		StatEdge edge = first.getPredecessorEdges(StatEdge.TYPE_BREAK)
		                     .get(0);
		...
		if (parent.type == Statement.TYPE_IF
			...
			parent.replaceStatement(source,
			                        block);
			...
			source.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR,
			                                 source,
			                                 first));
		}
	}

	private static boolean isInlineable(SequenceStatement seq,
	                                    int index) {
		...
		if (lst.size() == 1) {
			...
			if (sameCatchRanges(edge)) {
				...
				boolean noPreSuccessors = pre.getAllSuccessorEdges()
				                             .isEmpty();
	...
	private static boolean noExitLabels(Statement block,
	                                    Statement sequence) {
	...
	public static boolean isBreakEdgeLabeled(Statement source,
	                                         Statement closure) {
		if (closure.type == Statement.TYPE_DO || closure.type == Statement.TYPE_SWITCH) {
			Statement parent = source.getParent();
			return parent != closure && (parent.type == Statement.TYPE_DO
			                             || parent.type == Statement.TYPE_SWITCH
			                             || isBreakEdgeLabeled(parent,
			                                                   closure));
		} else {
			return true;
		}
