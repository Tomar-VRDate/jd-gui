...
public class DominatorTreeExceptionFilter {
	...
	public boolean acceptStatementPair(Integer head,
	                                   Integer exit) {
	...
	private void buildDominatorTree() {
		...
		for (int index = lstKeys.size() - 1;
		     index >= 0;
		     index--) {
			Integer key  = lstKeys.get(index);
			Integer idom = orderedIDoms.get(index);
			mapTreeBranches.computeIfAbsent(idom,
			                                k -> new LinkedHashSet<>())
			               .add(key);
		}

		Integer firstid = statement.getFirst().id;
		mapTreeBranches.get(firstid)
		               .remove(firstid);
	}

	private void buildExceptionRanges() {
		for (Statement stat : statement.getStats()) {
			List<Statement> lstPreds = stat.getNeighbours(StatEdge.TYPE_EXCEPTION,
			...
			if (!lstPreds.isEmpty()) {
				...
				mapExceptionRanges.put(stat.id,
				                       set);
			}
		}
		...
		if (children != null) {
			for (int childid : children) {
				...
				for (int handler : mapChild.keySet()) {
					map.put(handler,
					        map.containsKey(handler)
					        ? id
					        : mapChild.get(handler));
				}
			}
		}
	...
	private void buildFilter(Integer id) {
		...
		mapExceptionRangeUniqueExit.put(id,
		                                map);
	}
