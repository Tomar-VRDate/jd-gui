...
public final class CatchAllStatement
	...
	private CatchAllStatement(Statement head,
	                          Statement handler) {
		...
		for (StatEdge edge : head.getSuccessorEdges(StatEdge.TYPE_EXCEPTION)) {
			...
			if (edge.getExceptions() == null
				...
				if (lstSuccs.isEmpty()
					...
					if (DecHelper.checkStatementExceptions(Arrays.asList(head,
					                                                     exc))) {
						return new CatchAllStatement(head,
						                             exc);
					}
				}
			}
	...
	public TextBuffer toJava(int indent) {
		...
		if (first.type == TYPE_TRYCATCH
		    && first.varDefinitions.isEmpty()
		    && isFinally
		    && !labeled
		    && !first.isLabeled()
		    && (lstSuccs.isEmpty() || !lstSuccs.get(0).explicit)) {
			TextBuffer content = ExprProcessor.jmpWrapper(first,
			                                              indent,
			                                              true);
		...
		if (monitor != null) {
			buf.appendIndent(indent + 1)
			   .append("if (")
			   .append(monitor.toJava(indent))
			   .append(") {")
			   .appendLineSeparator();
		}

		buf.append(ExprProcessor.jmpWrapper(handler,
		                                    indent + 1 + (monitor != null
		                                                  ? 1
		                                                  : 0),
		                                    true));
	...
	public Statement getSimpleCopy() {
		...
		if (!this.vars.isEmpty()) {
			cas.vars.add(new VarExprent(DecompilerContext.getCounterContainer()
			                                             .getCounterAndIncrement(CounterContainer.VAR_COUNTER),
			                            new VarType(CodeConstants.TYPE_OBJECT,
			                                        0,
			                                        "java/lang/Throwable"),
