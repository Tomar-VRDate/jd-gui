...
public final class InitializerProcessor {
	public static void extractInitializers(ClassWrapper wrapper) {
		MethodWrapper method = wrapper.getMethodWrapper(CodeConstants.CLINIT_NAME,
		                                                "()V");
		try {
			if (method != null && method.root != null) {  // successfully decompiled static constructor
				extractStaticInitializers(wrapper,
				                          method);
			}
		} catch (Throwable t) {
			...
			String message = "Method "
			                 ...
			                 + mt.getDescriptor()
			                 + " in class "
			                 + wrapper.getClassStruct().qualifiedName
			                 + " couldn't be written.";
					...
					if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
						...
						if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD
							...
							if (fExpr.getClassname()
							         .equals(wrapper.getClassStruct().qualifiedName)) {
								StructField structField = wrapper.getClassStruct()
								                                 .getField(fExpr.getName(),
					           ...
					           && Statements.isInvocationInitConstructor((InvocationExprent) exprent,
					                                                     method,
					                                                     wrapper,
					                                                     true)) {
						// this() or super()
	...
	private static void hideEmptySuper(ClassWrapper wrapper) {
		for (MethodWrapper method : wrapper.getMethods()) {
			if (CodeConstants.INIT_NAME.equals(method.methodStruct.getName()) && method.root != null) {
				Statement firstData = Statements.findFirstData(method.root);
				if (firstData == null || firstData.getExprents()
				                                  .isEmpty()) {
					return;
				}

				Exprent exprent = firstData.getExprents()
				                           .get(0);
				if (exprent.type == Exprent.EXPRENT_INVOCATION) {
					InvocationExprent invExpr = (InvocationExprent) exprent;
					if (Statements.isInvocationInitConstructor(invExpr,
					                                           method,
					                                           wrapper,
					                                           false)) {
						List<VarVersionPair> mask = ExprUtil.getSyntheticParametersMask(invExpr.getClassname(),
						                                                                invExpr.getStringDescriptor(),
						                                                                invExpr.getLstParameters()
						                                                                       .size());
						boolean hideSuper = true;

						//searching for non-synthetic params
						for (int i = 0;
						     i < invExpr.getDescriptor().params.length;
						     ++i) {
							...
							if (type.type == CodeConstants.TYPE_OBJECT) {
								...
								if (node != null && (node.type == ClassNode.CLASS_ANONYMOUS
								                     || (node.access & CodeConstants.ACC_SYNTHETIC) != 0)) {
					...
					if (type.type == CodeConstants.TYPE_OBJECT) {
						...
						if (node != null && ((node.type == ClassNode.CLASS_ANONYMOUS)
							...
							wrapper.getHiddenMembers()
							       .add(InterpreterUtil.makeUniqueKey(name,
							                                          desc));
						}
					}
				}
	...
	private static void extractStaticInitializers(ClassWrapper wrapper,
		...
		if (firstData != null) {
			boolean inlineInitializers = cl.hasModifier(CodeConstants.ACC_INTERFACE)
			                             || cl.hasModifier(CodeConstants.ACC_ENUM);
			List<AssignmentExprent> exprentsToRemove = new LinkedList<>();//when we loop back through the list,
			// stores ones we need to remove outside iterator loop
			...
			Iterator<Exprent> itr = firstData.getExprents()
			                                 .iterator();
			while (itr.hasNext()) {
				...
				if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD) {
						FieldExprent fExpr = (FieldExprent) assignExpr.getLeft();
						if (fExpr.isStatic() && fExpr.getClassname()
						                             .equals(cl.qualifiedName) && cl.hasField(fExpr.getName(),
						                                                                      fExpr.getDescriptor().descriptorString)) {

							// interfaces fields should always be initialized inline
							String keyField = InterpreterUtil.makeUniqueKey(fExpr.getName(),
							                                                fExpr.getDescriptor().descriptorString);
							boolean exprentIndependent = isExprentIndependent(fExpr,
							                                                  assignExpr.getRight(),
							                                                  method,
							                                                  cl,
							                                                  whitelist,
							                                                  multiAssign,
							                                                  cl.getFields()
							                                                    .getIndexByKey(keyField),
							                                                  true);
							if (inlineInitializers || exprentIndependent) {
								if (!wrapper.getStaticFieldInitializers()
								            .containsKey(keyField)) {
									if (exprentIndependent) {
										wrapper.getStaticFieldInitializers()
										       .addWithKey(assignExpr.getRight(),
										                   keyField);
										...
										if (assignExpr.getRight() instanceof NewExprent) {
											...
											Exprent instance = newExprent.getConstructor()
											                             .getInstance();
											if (instance instanceof VarExprent
											    && nonFieldAssigns.containsKey(((VarExprent) instance).getIndex())) {
												AssignmentExprent nonFieldAssignment
																= nonFieldAssigns.remove(((VarExprent) instance).getIndex());
												newExprent.getConstructor()
												          .setInstance(nonFieldAssignment.getRight());
												...
												DecompilerContext.getLogger()
												                 .writeMessage("Don't know how to handle non independent "
												                               + assignExpr.getRight()
												                                           .getClass()
												                                           .getName(),
											...
											DecompilerContext.getLogger()
											                 .writeMessage("Don't know how to handle non independent " + assignExpr.getRight()
											                                                                                       .getClass()
											                                                                                       .getName(),
											                               IFernflowerLogger.Severity.ERROR);
										}
						...
						DecompilerContext.getLogger()
						                 .writeMessage("Found non field assignment when needing to force inline: " + assignExpr,
						                               IFernflowerLogger.Severity.TRACE);
						if (assignExpr.getLeft() instanceof VarExprent) {
							nonFieldAssigns.put(((VarExprent) assignExpr.getLeft()).getIndex(),
							                    assignExpr);
						} else {
							DecompilerContext.getLogger()
							                 .writeMessage("Left is not VarExprent!",
							                               IFernflowerLogger.Severity.ERROR);
						}
					}
				} else if (inlineInitializers && cl.hasModifier(CodeConstants.ACC_INTERFACE)) {
					DecompilerContext.getLogger()
					                 .writeMessage("Non assignment found in initializer when we're needing to inline all",
					                               IFernflowerLogger.Severity.ERROR);
				}
			}
			if (exprentsToRemove.size() > 0) {
				firstData.getExprents()
				         .removeAll(exprentsToRemove);
			}
		}
	...
	private static void extractDynamicInitializers(ClassWrapper wrapper) {
		...
		boolean isAnonymous = DecompilerContext.getClassProcessor()
		                                       .getMapRootClasses()
		                                       .get(cl.qualifiedName).type == ClassNode.CLASS_ANONYMOUS;
		...
		Set<String> whitelist = new HashSet<String>(wrapper.getStaticFieldInitializers()
		                                                   .getLstKeys());
		int prev_fidx = 0;
		...
		while (true) {
			...
			for (int i = 0;
				...
				Exprent exprent = lst.get(isAnonymous
				                          ? 0
				                          : 1);
				...
				if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
					...
					if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD) {
						...
						if (!fExpr.isStatic() && fExpr.getClassname()
						                              .equals(cl.qualifiedName) && cl.hasField(fExpr.getName(),
						                                                                       fExpr.getDescriptor().descriptorString)) { // check for the physical existence of the field. Could be defined in a superclass.

							String fieldKey = InterpreterUtil.makeUniqueKey(fExpr.getName(),
							                                                fExpr.getDescriptor().descriptorString);
							int fidx = cl.getFields()
							             .getIndexByKey(fieldKey);
							if (prev_fidx <= fidx && isExprentIndependent(fExpr,
							                                              assignExpr.getRight(),
							                                              lstMethodWrappers.get(i),
							                                              cl,
							                                              whitelist,
							                                              new ArrayList<>() /* TODO */,
							                                              fidx,
			...
			if (!wrapper.getDynamicFieldInitializers()
			            .containsKey(fieldWithDescr)) {
	...
	private static boolean isExprentIndependent(FieldExprent field,
	                                            Exprent exprent,
	                                            MethodWrapper method,
	                                            StructClass cl,
	                                            Set<String> whitelist,
	                                            List<String> multiAssign,
	                                            int fidx,
	                                            boolean isStatic) {
		String keyField = InterpreterUtil.makeUniqueKey(field.getName(),
		...
		for (Exprent expr : lst) {
			switch (expr.type) {
				case Exprent.EXPRENT_VAR:
					...
					if (!method.varproc.getExternalVars()
					                   .contains(varPair)) {
						String varName = method.varproc.getVarName(varPair);
						if (!varName.equals("this") && !varName.endsWith(".this")) { // FIXME: remove direct comparison with
							// strings
							return false;
						}
					}
				...
				case Exprent.EXPRENT_FIELD:
					...
					if (cl.hasField(fexpr.getName(),
					                fexpr.getDescriptor().descriptorString)) {
						String key = InterpreterUtil.makeUniqueKey(fexpr.getName(),
						                                           fexpr.getDescriptor().descriptorString);
						if (isStatic) {
							...
							if (!fexpr.isStatic()) {
								return false;
							} else if (cl.getFields()
							             .getIndexByKey(key) >= fidx) {
				...
				case Exprent.EXPRENT_NEW:
					qualifyFieldReferences((NewExprent) expr,
					                       cl,
					                       fidx);
					break;
			}
	...
	private static void qualifyFieldReferences(NewExprent nexpr,
	                                           StructClass cl,
	                                           int fidx) {
		boolean isStatic = cl.getFields()
		                     .get(fidx)
		...
		if (isStatic && nexpr.isLambda() && !nexpr.isMethodReference()) {
			ClassNode child = DecompilerContext.getClassProcessor()
			                                   .getMapRootClasses()
			                                   .get(nexpr.getNewType().value);
			MethodWrapper wrapper = child.parent.getWrapper()
			                                    .getMethods()
			                                    .getWithKey(child.lambdaInformation.content_method_key);

			Set<Exprent> s = new HashSet<>();
			wrapper.getOrBuildGraph()
			       .iterateExprentsDeep(e -> {
			...
			for (Exprent e : s) {
				switch (e.type) {
					case Exprent.EXPRENT_FIELD:
						...
						if (cl.qualifiedName.equals(fe.getClassname()) && fe.isStatic() && cl.hasField(fe.getName(),
						                                                                               fe.getDescriptor().descriptorString)) {
							String key = InterpreterUtil.makeUniqueKey(fe.getName(),
							                                           fe.getDescriptor().descriptorString);
						...
						break;
					case Exprent.EXPRENT_NEW:
						qualifyFieldReferences((NewExprent) e,
						                       cl,
						                       fidx);
						break;
				}
