...
public class StackVarsProcessor {
	...
	private static void replaceSingleVar(Exprent parent,
	                                     VarExprent var,
	                                     Exprent dest,
	                                     SSAUConstructorSparseEx ssau) {
		parent.replaceExprent(var,
		                      dest);
		...
		for (VarVersionPair varpaar : setVars) {
			...
			for (Iterator<Entry<Integer, FastSparseSet<Integer>>> itent = node.live.entryList()
			                                                                       .iterator();
			     itent.hasNext(); ) {
	...
	private static Object[] iterateChildExprent(Exprent exprent,
		...
		for (Exprent expr : exprent.getAllExprents()) {
			while (true) {
				Object[] arr = iterateChildExprent(expr,
				                                   parent,
				                                   next,
				                                   mapVarValues,
				                                   ssau);
		...
		Exprent dest = isReplaceableVar(exprent,
		                                mapVarValues);
		if (dest != null) {
		...
		boolean notdom = getUsedVersions(ssau,
		                                 leftpaar,
		                                 usedVers);
		...
		Map<Integer, Set<VarVersionPair>> mapVars = getAllVarVersions(leftpaar,
		                                                              right,
		                                                              ssau);
		...
		for (VarVersionNode usedvar : usedVers) {
			VarVersionPair usedver = new VarVersionPair(usedvar.var,
			                                            usedvar.version);
			if (isVersionToBeReplaced(usedver,
			                          mapVars,
			                          ssau,
			                          leftpaar) && (right.type == Exprent.EXPRENT_VAR || setAllowedVars.contains(usedver))) {
		...
		if (!notdom && !vernotreplaced) {
			for (VarVersionPair usedver : setTempUsedVers) {
				...
				if (right.type == Exprent.EXPRENT_FIELD && ssau.getMapFieldVars()
	...
	private static boolean getUsedVersions(SSAUConstructorSparseEx ssa,
	                                       VarVersionPair var,
	...
	private static boolean isVersionToBeReplaced(VarVersionPair usedvar,
		...
		if (!InterpreterUtil.equalObjects(ssau.getMapVersionFirstRange()
		                                      .get(leftpaar),
		                                  ssau.getMapVersionFirstRange()
		                                      .get(usedvar))) {
			return false;
		}
		...
		for (Entry<Integer, Set<VarVersionPair>> ent : mapVars.entrySet()) {
			...
			for (int livever : liveverset) {
				VarVersionNode node = ssuversions.nodes.getWithKey(new VarVersionPair(ent.getKey(),
				                                                                      livever));

				if (!ssuversions.isDominatorSet(node,
				                                domset)) {
					isdom = false;
					break;
				}
	...
	private static Map<Integer, Set<VarVersionPair>> getAllVarVersions(VarVersionPair leftvar,
		...
		for (Exprent expr : lst) {
			if (expr.type == Exprent.EXPRENT_VAR) {
				...
				if (leftvar.var != varindex) {
					if (mapLiveVars.containsKey(varindex)) {
						...
						for (int vers : mapLiveVars.get(varindex)) {
							verset.add(new VarVersionPair(varindex,
							                              vers));
						}
						map.put(varindex,
						        verset);
				...
				if (ssau.getMapFieldVars()
				        .containsKey(expr.id)) {
					int varindex = ssau.getMapFieldVars()
					                   .get(expr.id);
					if (mapLiveVars.containsKey(varindex)) {
						Set<VarVersionPair> verset = new HashSet<>();
						for (int vers : mapLiveVars.get(varindex)) {
	...
	private static void setEffectivelyFinalVars(Statement stat,
	                                            SSAUConstructorSparseEx ssau,
	...
	private static void setEffectivelyFinalVars(Statement stat,
		...
		if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
			...
			if (assign.getLeft().type == Exprent.EXPRENT_VAR) {
				...
				varLookupMap.put(var.getVarVersionPair(),
				                 var);
			}
			...
			if (newExpr.isAnonymous()) {
				...
				if (node != null) {
					if (!newExpr.isLambda()) {
								...
								for (int i = Math.max(0,
									...
									if (temp.type == Exprent.EXPRENT_ASSIGNMENT) {
										...
										if (left.type == Exprent.EXPRENT_VAR) {
											...
											if (leftVar.getLVT() != null && paramTypes.contains(leftVar.getLVT()
											                                                           .getVarType())) {
						...
						MethodDescriptor mdContent
										= MethodDescriptor.parseDescriptor(node.lambdaInformation.content_method_descriptor);
						int paramOffset = node.lambdaInformation.is_content_method_static
						                  ? 0
						                  : 1;
						...
						for (int i = 0;
						     i < varsCount;
						     ++i) {
							Exprent param = newExpr.getConstructor()
							                       .getLstParameters()
							                       .get(paramOffset + i);
							if (param.type == Exprent.EXPRENT_VAR) {
								...
								while (true) {
									...
									if (vvnode.var >= VarExprent.STACK_BASE) {
										vvnode = vvnode.preds.iterator()
										                     .next().source;
										VarVersionPair nextVVP = ssau.getVarAssignmentMap()
										                             .get(new VarVersionPair(vvnode.var,
										                                                     vvnode.version));
										...
										if (roots.size() >= allRoots.size()) {
											if (roots.size() == 1) {
												vvnode = roots.get(0);
												vvp = new VarVersionPair(vvnode.var,
												                         vvnode.version);
												VarVersionPair nextVVP = ssau.getVarAssignmentMap()
												                             .get(vvp);
												next = ssau.getSsuversions().nodes.getWithKey(nextVVP);
												...
												if (first.lvt != null && second.lvt != null && first.lvt.getVersion()
												                                                        .equals(second.lvt.getVersion())) {
	...
	public void simplifyStackVars(RootStatement root,
	                              StructMethod mt,
	                              StructClass cl) {
		...
		while (true) {
			...
			ssau.splitVariables(root,
			                    mt);

			if (first) {
				setEffectivelyFinalVars(root,
				                        ssau,
				                        new HashMap<>());
			}

			if (iterateStatements(root,
			                      ssau)) {
				found = true;
			}
		...
		iterateStatements(root,
		                  ssau);

		setVersionsToNull(root);
	}

	private boolean iterateStatements(RootStatement root,
		...
		while (!stack.isEmpty()) {
			...
			if (nd.exprents.isEmpty() && (nd.type == DirectNode.NODE_INIT
			                              || nd.type == DirectNode.NODE_CONDITION
	...
	private int[] iterateExprent(List<Exprent> lstExprents,
		...
		// stack variables only
		if (!left.isStack() && (right.type != Exprent.EXPRENT_VAR
		                        || ((VarExprent) right).isStack())) { // special case catch(... ex)
			return new int[]{-1,
			                 changed};
		}
		...
		Set<VarVersionPair> setNextVars = next == null
		                                  ? null
		                                  : getAllVersions(next);
		...
		if (right.type != Exprent.EXPRENT_CONST
			...
			for (VarVersionNode usedvar : usedVers) {
				if (!setNextVars.contains(new VarVersionPair(usedvar.var,
				                                             usedvar.version))) {
					return new int[]{-1,
					                 changed};
				}
		...
		for (VarVersionNode usedvar : usedVers) {
			...
			if (isVersionToBeReplaced(usedver,
			                          mapVars,
			                          ssau,
			                          leftpaar) && (right.type == Exprent.EXPRENT_CONST
			                                        || right.type == Exprent.EXPRENT_VAR
			                                        || right.type == Exprent.EXPRENT_FIELD
			                                        || setNextVars == null
			                                        || setNextVars.contains(usedver))) {
