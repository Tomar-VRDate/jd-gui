...
public class ClassWrapper {
	...
	public void init() {
		DecompilerContext.setProperty(DecompilerContext.CURRENT_CLASS,
		                              classStruct);
		DecompilerContext.setProperty(DecompilerContext.CURRENT_CLASS_WRAPPER,
		                              this);
		DecompilerContext.getLogger()
		                 .startClass(classStruct.qualifiedName);

		int maxSec = Integer.parseInt(DecompilerContext.getProperty(IFernflowerPreferences.MAX_PROCESSING_METHOD)
		                                               .toString());
		boolean testMode = DecompilerContext.getOption(IFernflowerPreferences.UNIT_TEST_MODE);

		for (StructMethod mt : classStruct.getMethods()) {
			DecompilerContext.getLogger()
			                 .startMethod(mt.getName() + " " + mt.getDescriptor());

			MethodDescriptor md = MethodDescriptor.parseDescriptor(mt,
			                                                       null);
			VarProcessor varProc = new VarProcessor(mt,
			                                        md);
			...
			try {
				if (mt.containsCode()) {
					if (maxSec == 0 || testMode) {
						root = MethodProcessorRunnable.codeToJava(classStruct,
						                                          mt,
						                                          md,
						                                          varProc);
					} else {
						MethodProcessorRunnable mtProc = new MethodProcessorRunnable(classStruct,
						                                                             mt,
						                                                             md,
						                                                             varProc,
					...
					for (int i = 0;
					     i < paramCount;
					     i++) {
						varProc.setVarName(new VarVersionPair(varIndex,
						                                      0),
				...
				if (rootStat != null) {
					DotExporter.errorToDotFile(rootStat,
					                           mt,
					                           "fail");
				}
				...
				if (decompileRecord != null) {
					DotExporter.toDotFile(decompileRecord,
					                      mt,
					                      "failRecord",
					                      true);
				}
			...
			MethodWrapper methodWrapper = new MethodWrapper(root,
			                                                varProc,
			                                                mt,
			                                                counter);
			...
			if (error == null) {
				...
				classStruct.getFields()
				           .forEach(f -> namesCollector.addName(f.getName()));
	...
	public MethodWrapper getMethodWrapper(String name,
	                                      String descriptor) {
