...
public class NewExprent
	...
	public NewExprent(VarType newType,
		...
		this(newType,
		     getDimensions(arrayDim,
		                   stack),
		     bytecodeOffsets);
	}

	public NewExprent(VarType newType,
	                  List<Exprent> lstDims,
	                  BitSet bytecodeOffsets) {
		super(EXPRENT_NEW);
		...
		if (newType.type == CodeConstants.TYPE_OBJECT && newType.arrayDim == 0) {
			ClassNode node = DecompilerContext.getClassProcessor()
			                                  .getMapRootClasses()
			                                  .get(newType.value);
	...
	private static List<Exprent> getDimensions(int arrayDim,
	                                           ListStack<Exprent> stack) {
	...
	private static TextBuffer getQualifiedNewInstance(String classname,
	                                                  List<Exprent> lstParams,
	                                                  int indent) {
		ClassNode node = DecompilerContext.getClassProcessor()
		                                  .getMapRootClasses()
		                                  .get(classname);
		...
		if (node != null
		    ...
		    && node.type != ClassNode.CLASS_LOCAL
			...
			if (!lstParams.isEmpty()) {
				...
				if (enclosing.type == Exprent.EXPRENT_VAR) {
					...
					StructClass current_class
									= ((ClassNode) DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_NODE)).classStruct;
					String this_classname = varEnclosing.getProcessor()
					                                    .getThisVars()
	...
	private static VarType getLambdaReturnType(ClassNode node,
	                                           StructMethod desc,
	                                           VarType upperBound,
		...
		if (wrapper != null) {
			MethodWrapper mt = wrapper.getMethodWrapper(node.lambdaInformation.content_method_name,
			                                            node.lambdaInformation.content_method_descriptor);
			...
			if (mt != null && mt.root != null) {
				...
				for (int i = 0;
				     ...
				     i++) {
					if (i >= start_index) {
						VarVersionPair vpp = new VarVersionPair(index,
						                                        0);
						VarType curType = mt.varproc.getVarType(vpp);
						VarType infType = desc.getSignature().parameterTypes.get(j++)
						                                                    .remap(genericsMap);
						...
						if (infType != null && !infType.equals(VarType.VARTYPE_VOID)) {
							if (!curType.equals(infType) || (infType.isGeneric() && !((GenericType) infType).equalsExact(curType))) {
								...
								inferredLambdaTypes.put(varName,
								                        infType);
							}
						}
					}
					...
					if (lstExpr != null && !lstExpr.isEmpty()) {
						...
						if (expr.type == Exprent.EXPRENT_EXIT) {
							...
							if (ex.getExitType() == ExitExprent.EXIT_RETURN) {
								VarType realRetType = ex.getValue()
								                        .getInferredExprType(upperBound);
	...
	public VarType getExprType() {
		return anonymous
		       ? DecompilerContext.getClassProcessor()
		                          .getMapRootClasses()
		                          .get(newType.value).anonymousClassType
		       : newType;
	}
	...
	public VarType getInferredExprType(VarType upperBound) {
		...
		if (!lambda && newType.type == CodeConstants.TYPE_OBJECT) {
			StructClass node = DecompilerContext.getStructContext()
			                                    .getClass(newType.value);

			if (node != null && node.getSignature() != null) {
				if (anonymous) {
					if (VarType.VARTYPE_OBJECT.equals(node.getSignature().superclass)
					    && !node.getSignature().superinterfaces.isEmpty()) {
					...
					if (constructor != null) {
						...
						return ret.type != CodeConstants.TYPE_VOID
						       ? ret
						       : getExprType();
					} else {
						Map<VarType, VarType> genericsMap = new HashMap<>();
						this.gatherGenerics(upperBound,
						                    sig.genericType,
						                    genericsMap);
						this.getGenericArgs(sig.fparameters,
						                    genericsMap,
						                    genericArgs);
			           ...
			           && !lstArrayElements.isEmpty()
			           && newType.value.equals(VarType.VARTYPE_OBJECT.value)) {
				VarType first = lstArrayElements.get(0)
				                                .getInferredExprType(null);
				if (first.type == CodeConstants.TYPE_GENVAR) {
					boolean matches = true;
					for (int i = 1;
					     i < lstArrayElements.size();
					     ++i) {
						VarType type = lstArrayElements.get(i)
						                               .getInferredExprType(null);
		...
		if (lambda) {
			...
			if (node != null) {
				...
				StructClass cls = DecompilerContext.getStructContext()
				                                   .getClass(classType.value);
				MethodDescriptor desc = MethodDescriptor.parseDescriptor(node.lambdaInformation.method_descriptor);
				StructClass methodCls = DecompilerContext.getStructContext()
				                                         .getClass(node.lambdaInformation.content_class_name);

				if (cls != null && cls.getSignature() != null && methodCls != null) {
					StructMethod refMethod = cls.getMethod(getLambdaMethodKey());
					StructMethod method = methodCls.getMethod(node.lambdaInformation.content_method_name,
					                                          node.lambdaInformation.content_method_descriptor);
					...
					if (method != null && refMethod != null && refMethod.getSignature() != null) {
						...
						gatherGenerics(upperBound,
						               ret,
						               genericsMap);

						HashMap<VarType, VarType> instanceMap = new HashMap<>();
						if (isMethodReference() && methodCls.getSignature() != null) {
							VarType first = ret.getArguments()
							                   .get(0);
							if (constructor.getInstance() != null) {
								VarType instanceType = constructor.getInstance()
								                                  .getInferredExprType(null);
								if (instanceType.isGeneric()) {
									methodCls.getSignature().genericType.mapGenVarsTo((GenericType) instanceType,
									                                                  instanceMap);
								}
							} else if (method.getSignature() != null) {
								for (int i = 0;
								     i < method.getSignature().parameterTypes.size();
								     ++i) {
									...
									if (mtype.type == CodeConstants.TYPE_GENVAR && rtype.type == CodeConstants.TYPE_GENVAR) {
										if (genericsMap.containsKey(rtype)) {
											instanceMap.put(mtype,
											                genericsMap.get(rtype));
										}
									}
								}
							}
						...
						List<VarType> types = method.getSignature() != null
						                      ? method.getSignature().parameterTypes
						                      : Arrays.asList(desc.params);
						for (int i = 0;
						     i < types.size();
						     ++i) {
							if (refMethod.getSignature().parameterTypes.get(i).type == CodeConstants.TYPE_GENVAR) {
								if (!genericsMap.containsKey(refMethod.getSignature().parameterTypes.get(i))) {
									...
									StructClass typeCls = DecompilerContext.getStructContext()
									                                       .getClass(realType.value);
									if (typeCls != null
									    && typeCls.getSignature() != null
									    && !realType.equals(typeCls.getSignature().genericType)) {
										realType = typeCls.getSignature().genericType.resizeArrayDim(realType.arrayDim);
									}
									genericsMap.put(refMethod.getSignature().parameterTypes.get(i),
									                realType);
								}
							}
						}
						...
						if (refMethod.getSignature().returnType.type == CodeConstants.TYPE_GENVAR) {
							...
							if (method.getName()
							          .equals(CodeConstants.INIT_NAME)) {
								if (methodCls.getSignature() != null) {
									genericsMap.put(key,
									                methodCls.getSignature().genericType);
								} else {
									genericsMap.put(key,
									                GenericType.parse("L" + methodCls.qualifiedName + ";"));
								}
							} else if (method.getSignature() != null || !desc.ret.equals(VarType.VARTYPE_OBJECT)) {
								VarType current = genericsMap.get(key);
								VarType returnType = method.getSignature() != null
								                     ? method.getSignature().returnType.remap(instanceMap)
								                     : desc.ret;
								StructClass retCls = returnType == null
								                     ? null
								                     : DecompilerContext.getStructContext()
								                                        .getClass(returnType.value);

								if (!isMethodReference()
								    && retCls != null
								    && retCls.getSignature() != null
								    && !retCls.getSignature().genericType.equalsExact(returnType)) {
									VarType retUB = current != null && current.equals(returnType)
									                ? current
									                : returnType;
									VarType realType = getLambdaReturnType(node,
									                                       refMethod,
									                                       retUB,
									                                       genericsMap);
									if (realType != null) {
								...
								boolean add = current == null
								              ...
								              || (!returnType.equals(genericsMap.get(key)) && (current.type != CodeConstants.TYPE_GENVAR
								                                                               || !named.containsKey(current)));
								if (add) {
									genericsMap.put(key,
									                returnType);
								}
						...
						ret.getAllGenericVars()
						   .forEach(from -> {
							   genericsMap.putIfAbsent(from,
	...
	public CheckTypesResult checkExprTypeBounds() {
		...
		if (newType.arrayDim != 0) {
			for (Exprent dim : lstDims) {
				result.addMinTypeExprent(dim,
				                         VarType.VARTYPE_BYTECHAR);
				result.addMaxTypeExprent(dim,
				                         VarType.VARTYPE_INT);
			}
			...
			if (newType.arrayDim == 1) {
				...
				for (Exprent element : lstArrayElements) {
					result.addMinTypeExprent(element,
					                         VarType.getMinTypeInFamily(leftType.typeFamily));
					result.addMaxTypeExprent(element,
					                         leftType);
				}
			}
	...
	public Exprent copy() {
		...
		NewExprent ret = new NewExprent(newType,
		                                lst,
		                                bytecode);
		ret.setConstructor(constructor == null
		                   ? null
		                   : (InvocationExprent) constructor.copy());
	...
	public TextBuffer toJava(int indent) {
		...
		if (anonymous) {
			ClassNode child = DecompilerContext.getClassProcessor()
			                                   .getMapRootClasses()
			                                   .get(newType.value);
			...
			if (!enumConst && (!lambda || DecompilerContext.getOption(IFernflowerPreferences.LAMBDA_TO_ANONYMOUS_CLASS))) {
				...
				if (selfReference) {
					...
					if (enclosing != null) {
						ClassNode anonymousNode = DecompilerContext.getClassProcessor()
						                                           .getMapRootClasses()
					...
					if (child.getWrapper() != null) {
						GenericClassDescriptor descriptor = child.getWrapper()
						                                         .getClassStruct()
						                                         .getSignature();
						if (descriptor != null) {
							if (descriptor.superinterfaces.isEmpty()) {
								...
								if (descriptor.superinterfaces.size() > 1 && !lambda) {
									DecompilerContext.getLogger()
									                 .writeMessage("Inconsistent anonymous class signature: "
			...
			if (!lambda && constructor != null) {
				appendParameters(buf,
				                 constructor.getGenericArgs());
				buf.append('(')
				   .append(constructor.appendParamList(indent));
			} else {
				appendParameters(buf,
				                 genericArgs);
				buf.append('(');
			...
			if (lambda) {
				...
				Exprent methodObject = constructor == null
				                       ? null
				                       : constructor.getInstance();
				new ClassWriter().classLambdaToJava(child,
				                                    buf,
				                                    methodObject,
				                                    indent);
			} else if (!selfReference) {
				new ClassWriter().classToJava(child,
				                              buf,
				                              indent);
			}
			...
			for (int i = 0;
				...
				ExprProcessor.getCastedExprent(lstArrayElements.get(i),
				                               leftType,
				                               buf,
				                               indent,
				                               false);
			...
			if (!enumConst) {
				...
				if (constructor != null) {
					enclosing = getQualifiedNewInstance(newType.value,
					                                    constructor.getLstParameters(),
					                                    indent);
					if (enclosing != null) {
						buf.append(enclosing)
			...
			if (constructor != null) {
				int start = enumConst
				            ? 2
				            : 0;
				if (!enumConst || start < constructor.getLstParameters()
				                                     .size()) {
					appendParameters(buf,
					                 constructor.getGenericArgs());
					buf.append('(')
			...
			for (int i = 0;
				...
				ExprProcessor.getCastedExprent(element,
				                               leftType,
				                               buf,
				                               indent,
				                               false);
			...
			if (lstArrayElements.size() == 1) {
				VarType elementType = lstArrayElements.get(0)
				                                      .getExprType();
				if (elementType.type == CodeConstants.TYPE_OBJECT
				    && elementType.value.equals("java/lang/Object")
			...
			buf.append("new ")
			   .append(ExprProcessor.getTypeName(newType));
	...
	private void setLambdaGenericTypes() {
		if (inferredLambdaType != null) {
			ClassNode node = DecompilerContext.getClassProcessor()
			                                  ...
			                                  .get(newType.value);
			StructClass cls = DecompilerContext.getStructContext()
			                                   .getClass(inferredLambdaType.value);

			if (node != null && cls != null) {
				StructMethod desc    = cls.getMethod(getLambdaMethodKey());
				ClassWrapper wrapper = node.getWrapper();
				MethodWrapper methodWrapper = wrapper != null
				                              ? wrapper.getMethodWrapper(node.lambdaInformation.content_method_name,
				                                                         node.lambdaInformation.content_method_descriptor)
				                              : null;

				if (desc != null && desc.getSignature() != null && methodWrapper != null && methodWrapper.root != null) {
					if (!desc.getClassQualifiedName()
					         .equals(inferredLambdaType.value)) {
						StructClass candidate = DecompilerContext.getStructContext()
						                                         .getClass(desc.getClassQualifiedName());
						if (candidate.getSignature() != null) {
							cls = candidate;
							inferredLambdaType = GenericType.getGenericSuperType(inferredLambdaType,
							                                                     candidate.getSignature().genericType);
						}
					}

					Map<VarType, VarType> tempMap = new HashMap<>();
					if (inferredLambdaType.isGeneric()) {
						cls.getSignature().genericType.mapGenVarsTo((GenericType) inferredLambdaType,
						                                            tempMap);
					}
					...
					for (int i = 0;
						...
						if (i >= start_index) {
							...
							if (infType != null && !infType.equals(VarType.VARTYPE_VOID)) {
								if (!curType.equals(infType) || (infType.isGeneric()
								                                 && !((GenericType) infType).equalsExact(curType))) {
									methodWrapper.varproc.setVarType(vpp,
									                                 infType);
									...
									while (!nested.isEmpty()) {
										...
										if (childNode.type == ClassNode.CLASS_LAMBDA && !childNode.lambdaInformation.is_method_reference) {
											MethodWrapper enclosedMethod
															= wrapper.getMethodWrapper(childNode.lambdaInformation.content_method_name,
															                           childNode.lambdaInformation.content_method_descriptor);

											if (enclosedMethod != null && paramName.equals(enclosedMethod.varproc.getVarName(vpp))) {
												enclosedMethod.varproc.setVarType(vpp,
												                                  infType);
											}
										}
									}
					...
					if (infType != null && !infType.equals(VarType.VARTYPE_VOID)) {
						if (!curType.equals(infType) || (infType.isGeneric() && !((GenericType) infType).equalsExact(curType))) {
							GenericMethodDescriptor genDesc = new GenericMethodDescriptor(Collections.emptyList(),
	...
	public void replaceExprent(Exprent oldExpr,
		...
		if (constructor != null) {
			constructor.replaceExprent(oldExpr,
			                           newExpr);
		}
	...
	public boolean equals(Object o) {
		...
		return InterpreterUtil.equalObjects(newType,
		                                    ne.getNewType())
		       && InterpreterUtil.equalLists(lstDims,
		                                     ne.getLstDims())
		       && InterpreterUtil.equalObjects(constructor,
		                                       ne.getConstructor())
		       && directArrayInit == ne.directArrayInit
		       && InterpreterUtil.equalLists(lstArrayElements,
		                                     ne.getLstArrayElements());
	}
	...
	public String getLambdaMethodKey() {
		...
		if (node != null && constructor != null) {
			String descriptor = ((PrimitiveConstant) constructor.getBootstrapArguments()
			                                                    .get(0)).getString();
			return InterpreterUtil.makeUniqueKey(node.lambdaInformation.method_name,
			                                     descriptor);
		}
		return "";
	}
