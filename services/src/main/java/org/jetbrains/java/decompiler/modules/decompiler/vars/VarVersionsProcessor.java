...
public class VarVersionsProcessor {
	...
	public VarVersionsProcessor(StructMethod mt,
	                            MethodDescriptor md) {
		method = mt;
		typeProcessor = new VarTypeProcessor(mt,
		                                     md);
	}

	private static void mergePhiVersions(SSAConstructorSparseEx ssa,
	                                     DirectGraph graph) {
		...
		for (Entry<VarVersionPair, FastSparseSet<Integer>> ent : ssa.getPhi()
			...
			for (int version : ent.getValue()) {
				set.add(new VarVersionPair(ent.getKey().var,
				                           version));
			}
		...
		for (Set<VarVersionPair> set : lst) {
			...
			for (VarVersionPair paar : set) {
				phiVersions.put(new VarVersionPair(paar.var,
				                                   paar.version),
				                min);
			}
		}
		...
		for (VarVersionPair paar : new ArrayList<>(mapExprentMinTypes.keySet())) {
			...
			if (type.type == CodeConstants.TYPE_BYTECHAR || type.type == CodeConstants.TYPE_SHORTCHAR) {
				if (maxType != null && maxType.type == CodeConstants.TYPE_CHAR) {
					...
					type = type.type == CodeConstants.TYPE_BYTECHAR
					       ? VarType.VARTYPE_BYTE
					       : VarType.VARTYPE_SHORT;
				}
				mapExprentMinTypes.put(paar,
				                       type);
	...
	private static void simpleMerge(VarTypeProcessor typeProcessor,
	                                DirectGraph graph,
	                                StructMethod mt) {
		Map<VarVersionPair, VarType> mapExprentMaxTypes = typeProcessor.getMapExprentMaxTypes();
		Map<VarVersionPair, VarType> mapExprentMinTypes = typeProcessor.getMapExprentMinTypes();
		...
		for (VarVersionPair pair : mapExprentMinTypes.keySet()) {
			if (pair.version >= 0) {  // don't merge constants
				mapVarVersions.computeIfAbsent(pair.var,
				                               k -> new HashSet<>())
				              .add(pair.version);
			}
		}
		...
		for (Entry<Integer, Set<Integer>> ent : mapVarVersions.entrySet()) {
			...
			if (ent.getValue()
				...
				for (int i = 0;
				     i < lstVersions.size();
				     i++) {
					VarVersionPair firstPair = new VarVersionPair(ent.getKey(),
					                                              lstVersions.get(i));
					...
					for (int j = i + 1;
					     j < lstVersions.size();
					     j++) {
						VarVersionPair secondPair = new VarVersionPair(ent.getKey(),
						                                               lstVersions.get(j));
						...
						if (firstType.equals(secondType)
							...
							VarType type = firstMaxType == null
							               ? secondMaxType
							               : secondMaxType == null
							                 ? firstMaxType
							                 : VarType.getCommonMinType(firstMaxType,
							                                            secondMaxType);

							mapExprentMaxTypes.put(firstPair,
							                       type);
							mapMergedVersions.put(secondPair,
							                      firstPair.version);
							mapExprentMaxTypes.remove(secondPair);
							mapExprentMinTypes.remove(secondPair);

							if (firstType.equals(VarType.VARTYPE_NULL)) {
								mapExprentMinTypes.put(firstPair,
								                       secondType);
								firstType = secondType;
							}

							typeProcessor.getMapFinalVars()
							             .put(firstPair,
	...
	public void setVarVersions(RootStatement root,
		...
		mergePhiVersions(ssa,
		                 graph);

		typeProcessor.calculateVarTypes(root,
		                                graph);
		...
		setNewVarIndices(typeProcessor,
		                 graph,
		                 previousVersionsProcessor);
	}

	private void setNewVarIndices(VarTypeProcessor typeProcessor,
	                              DirectGraph graph,
	                              VarVersionsProcessor previousVersionsProcessor) {
		...
		for (VarVersionPair pair : vvps) {
			...
			if (pair.version >= 0) {
				int newIndex = pair.version == 1
				               ? pair.var
				               : counters.getCounterAndIncrement(CounterContainer.VAR_COUNTER);

				VarVersionPair newVar = new VarVersionPair(newIndex,
				                                           0);

				mapExprentMinTypes.put(newVar,
				                       mapExprentMinTypes.get(pair));
				mapExprentMaxTypes.put(newVar,
				                       mapExprentMaxTypes.get(pair));

				if (mapFinalVars.containsKey(pair)) {
					mapFinalVars.put(newVar,
					                 mapFinalVars.remove(pair));
				}

				mapVarPaar.put(pair,
				               newIndex);
				mapOriginalVarIndices.put(newIndex,
				                          pair);
			}
		}
		...
		graph.iterateExprents(exprent -> {
			...
			for (Exprent expr : lst) {
				if (expr.type == Exprent.EXPRENT_VAR) {
					...
					VarType maxType = mapExprentMaxTypes.get(new VarVersionPair(expr.id,
					                                                            -1));
		...
		if (previousVersionsProcessor != null) {
			...
			for (Entry<Integer, VarVersionPair> entry : mapOriginalVarIndices.entrySet()) {
				...
				value = oldValue != null
				        ? oldValue
				        : value;
				this.mapOriginalVarIndices.put(entry.getKey(),
				                               value);
			}
		} else {
	...
	public void setVarType(VarVersionPair pair,
	                       VarType type) {
		typeProcessor.setVarType(pair,
		                         type);
	}

	public int getVarFinal(VarVersionPair pair) {
		Integer fin = typeProcessor.getMapFinalVars()
		                           .get(pair);
		return fin == null
		       ? VarTypeProcessor.VAR_FINAL
		       : fin;
	}

	public void setVarFinal(VarVersionPair pair,
	                        int finalType) {
		typeProcessor.getMapFinalVars()
		             .put(pair,
		                  finalType);
