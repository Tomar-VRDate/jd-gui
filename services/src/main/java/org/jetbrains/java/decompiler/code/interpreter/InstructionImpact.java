...
public final class InstructionImpact {
	...
	private static final int[] arr_type = new int[]{CodeConstants.TYPE_BOOLEAN,
	...
	public static void stepTypes(DataPoint data,
	                             Instruction instr,
	                             ConstantPool pool) {
		...
		if (arr != null) {
			...
			// Sonderbehandlung
			processSpecialInstructions(data,
			                           instr,
			                           pool);
		}
	}

	private static void processSpecialInstructions(DataPoint data,
	                                               Instruction instr,
	                                               ConstantPool pool) {
		...
		switch (instr.opcode) {
			case CodeConstants.opc_aconst_null:
				stack.push(new VarType(CodeConstants.TYPE_NULL,
				                       0,
				                       null));
				break;
			...
			case CodeConstants.opc_ldc2_w:
				...
				switch (constant.type) {
					...
					case CodeConstants.CONSTANT_String:
						stack.push(new VarType(CodeConstants.TYPE_OBJECT,
						                       0,
						                       "java/lang/String"));
						break;
			...
			case CodeConstants.opc_aaload:
				var1 = stack.pop(2);
				stack.push(new VarType(var1.type,
				                       var1.arrayDim - 1,
				                       var1.value));
				break;
			case CodeConstants.opc_astore:
				data.setVariable(instr.operand(0),
				                 stack.pop());
				break;
			...
			case CodeConstants.opc_newarray:
				stack.pop();
				stack.push(new VarType(arr_type[instr.operand(0) - 4],
				                       1).resizeArrayDim(1));
				break;
			...
			case CodeConstants.opc_multianewarray:
				int dimensions = (instr.opcode == CodeConstants.opc_anewarray)
				                 ? 1
				                 : instr.operand(1);
				stack.pop(dimensions);
				cn = pool.getPrimitiveConstant(instr.operand(0));
				if (cn.isArray) {
					var1 = new VarType(CodeConstants.TYPE_OBJECT,
					                   0,
					                   cn.getString());
