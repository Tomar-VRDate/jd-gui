...
public class ControlFlowGraph
	...
	private static short[] findStartInstructions(InstructionSequence seq) {
		...
		for (ExceptionHandler handler : seq.getExceptionTable()
		                                   .getHandlers()) {
	...
	private static void removeJsrInstructions(ConstantPool pool,
	                                          BasicBlock block,
	                                          DataPoint data) {
		...
		for (int i = 0;
			...
			InstructionImpact.stepTypes(data,
			                            instr,
			                            pool);
		...
		for (int i = 0;
		     i < block.getSuccs()
		              .size();
		     i++) {
			BasicBlock suc = block.getSuccs()
			                      .get(i);
			if (suc.mark != 1) {
				removeJsrInstructions(pool,
				                      suc,
				                      data.copy());
			}
		}
		...
		for (int i = 0;
		     i < block.getSuccExceptions()
		              .size();
		     i++) {
			BasicBlock suc = block.getSuccExceptions()
			                      .get(i);
			if (suc.mark != 1) {
				...
				point.getStack()
				     .push(new VarType(CodeConstants.TYPE_OBJECT,
				                       0,
				                       null));
	...
	private static void addToReversePostOrderListIterative(BasicBlock root,
	                                                       List<? super BasicBlock> lst) {
	...
	public String toString() {
		...
		for (BasicBlock block : blocks) {
			...
			for (BasicBlock aSuc : suc) {
				buf.append(">>>>>>>>(regular) Block ")
				   .append(aSuc.id)
				   .append(new_line_separator);
			}
			suc = block.getSuccExceptions();
			for (BasicBlock handler : suc) {
				ExceptionRangeCFG range = getExceptionRange(handler,
				                                            block);

				if (range == null) {
					buf.append(">>>>>>>>(exception) Block ")
					   .append(handler.id)
					   .append("\t")
	...
	public void inlineJsr(StructClass cl,
	                      StructMethod mt) {
		processJsr();
		removeJsr(cl,
		          mt);
	...
	public void removeBlock(BasicBlock block) {
		...
		while (block.getSuccs()
		            .size() > 0) {
			block.removeSuccessor(block.getSuccs()
			                           .get(0));
		}

		while (block.getSuccExceptions()
		            .size() > 0) {
			block.removeSuccessorException(block.getSuccExceptions()
			                                    .get(0));
		}

		while (block.getPreds()
		            .size() > 0) {
			block.getPreds()
			     .get(0)
		...
		while (block.getPredExceptions()
		            .size() > 0) {
			block.getPredExceptions()
			     .get(0)
			     .removeSuccessorException(block);
		}
		...
		subroutines.entrySet()
		           .removeIf(ent -> ent.getKey() == block || ent.getValue() == block);
	}

	public ExceptionRangeCFG getExceptionRange(BasicBlock handler,
	                                           BasicBlock block) {
		...
		for (int i = exceptions.size() - 1;
		     ...
		     i--) {
			ExceptionRangeCFG range = exceptions.get(i);
			if (range.getHandler() == handler && range.getProtectedRange()
			                                          .contains(block)) {
				return range;
				//ranges.add(range);
			}
	...
	private void buildBlocks(InstructionSequence instrseq) {
		...
		VBStyleCollection<BasicBlock, Integer> colBlocks = createBasicBlocks(states,
		                                                                     instrseq,
		                                                                     mapInstrBlocks);

		blocks = colBlocks;

		connectBlocks(colBlocks,
		              mapInstrBlocks);

		setExceptionEdges(instrseq,
		                  mapInstrBlocks);
	...
	private VBStyleCollection<BasicBlock, Integer> createBasicBlocks(short[] startblock,
		...
		for (int i = 0;
			...
			mapInstrBlocks.put(i,
			                   currentBlock);

			currseq.addInstruction(instrseq.getInstr(i),
			                       instrseq.getOffset(i) - blockoffset);
	...
	private void setExceptionEdges(InstructionSequence instrseq,
		...
		for (ExceptionHandler handler : instrseq.getExceptionTable()
		                                        .getHandlers()) {
			...
			if (mapRanges.containsKey(key)) {
				...
				ExceptionRangeCFG range = new ExceptionRangeCFG(protectedRange,
				                                                handle,
	...
	private void setSubroutineEdges() {
		...
		for (BasicBlock block : blocks) {
			...
			if (block.getSeq()
			         .getLastInstr().opcode == CodeConstants.opc_jsr) {
	...
	private int processJsrRanges() {
		...
		for (Entry<BasicBlock, BasicBlock> ent : subroutines.entrySet()) {
			...
			lstJsrAll.add(new JsrRecord(jsr,
			                            getJsrRange(jsr,
			                                        ret),
			                            ret));
		}
	...
	private Set<BasicBlock> getJsrRange(BasicBlock jsr,
	                                    BasicBlock ret) {
						...
						if (node != jsr) {
							for (int k = 0;
							     k < child.getPreds()
							              .size();
							     k++) {
								if (!DeadCodeHelper.isDominator(this,
								                                child.getPreds()
								                                     .get(k),
								                                dom)) {
									continue CHILD;
							...
							for (int k = 0;
							     k < child.getPredExceptions()
							              .size();
							     k++) {
								if (!DeadCodeHelper.isDominator(this,
								                                child.getPredExceptions()
								                                     .get(k),
								                                dom)) {
									continue CHILD;
								}
	...
	private void splitJsrRange(BasicBlock jsr,
	                           BasicBlock ret,
	                           Set<BasicBlock> common_blocks) {

		List<BasicBlock>         lstNodes    = new LinkedList<>();
		Map<Integer, BasicBlock> mapNewNodes = new HashMap<>();

		lstNodes.add(jsr);
		mapNewNodes.put(jsr.id,
		                jsr);
			...
			for (int j = 0;
				...
				for (int i = lst.size() - 1;
					...
					if (mapNewNodes.containsKey(childid)) {
						node.replaceSuccessor(child,
						                      mapNewNodes.get(childid));
					} else if (common_blocks.contains(child)) {
						// make a copy of the current block
						BasicBlock copy = child.cloneBlock(++last_id);
						// copy all successors
						if (copy.getLastInstruction().opcode == CodeConstants.opc_ret && child.getSuccs()
						                                                                      .contains(ret)) {
							...
							for (int k = 0;
							     k < child.getSuccs()
							              .size();
							     k++) {
								copy.addSuccessor(child.getSuccs()
								                       .get(k));
							}
						}
						for (int k = 0;
						     k < child.getSuccExceptions()
						              .size();
						     k++) {
							copy.addSuccessorException(child.getSuccExceptions()
							                                .get(k));
						}

						lstNodes.add(copy);
						mapNewNodes.put(childid,
						                copy);
		...
		splitJsrExceptionRanges(common_blocks,
		                        mapNewNodes);
	}
			...
			if (setBoth.size() > 0) {
				...
				if (setBoth.size() == lstRange.size()) {
					...
					ExceptionRangeCFG newRange = new ExceptionRangeCFG(lstNewRange,
					                                                   mapNewNodes.get(range.getHandler().id),
	...
	private void removeJsr(StructClass cl,
	                       StructMethod mt) {
		removeJsrInstructions(cl.getPool(),
		                      first,
		                      DataPoint.getInitialDataPoint(mt));
	}
	...
	public List<BasicBlock> getReversePostOrder() {
		...
		addToReversePostOrderListIterative(first,
		                                   res);
	...
	private static final class JsrRecord {
		...
		private JsrRecord(BasicBlock jsr,
