...
public class MonitorExprent
	...
	public MonitorExprent(int monType,
	                      Exprent value,
	...
	@Override
	public Exprent copy() {
		return new MonitorExprent(monType,
		                          value.copy(),
		                          bytecode);
	}
		...
		if (monType == MONITOR_ENTER) {
			...
			if (this.value.type == EXPRENT_CONST && this.value.getExprType() == VarType.VARTYPE_NULL) {
				DecompilerContext.getLogger()
				                 .writeMessage("Created invalid synchronize on null!",
				                               IFernflowerLogger.Severity.WARN);
			}
			return buf.append(value.toJava(indent))
			          .enclose("synchronized(",
			                   ")");
		} else {
			return buf;
		}
	...
	public boolean equals(Object o) {
		...
		return monType == me.getMonType() && InterpreterUtil.equalObjects(value,
		                                                                  me.getValue());
	}
