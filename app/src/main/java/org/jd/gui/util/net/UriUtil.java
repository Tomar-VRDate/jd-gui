...
public class UriUtil {
	 ...
	 */
	public static URI createURI(API api,
		...
		try {
			...
			if (typeFactory != null) {
				...
				if (type != null) {
					path = getOuterPath(collectionOfFutureIndexes,
					                    entry,
					                    type);
				}
			}
	...
	protected static String getOuterPath(Collection<Future<Indexes>> collectionOfFutureIndexes,
	                                     Container.Entry entry,
	                                     Type type) {
			...
			try {
				for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
					if (futureIndexes.isDone()) {
						Collection<Container.Entry> outerEntries = futureIndexes.get()
