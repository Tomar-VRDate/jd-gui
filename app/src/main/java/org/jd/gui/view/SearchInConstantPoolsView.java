...
public class SearchInConstantPoolsView<T extends DefaultMutableTreeNode & ContainerEntryGettable & UriGettable> {
	...
	public SearchInConstantPoolsView(API api,
	                                 JFrame mainFrame,
		...
		SwingUtil.invokeLater(() -> {
			searchInConstantPoolsDialog = new JDialog(mainFrame,
			                                          "Search",
			                                          false);
			...
			searchInConstantPoolsEnterTextField.addKeyListener(new KeyAdapter() {
				...
				public void keyTyped(KeyEvent e) {
					switch (e.getKeyChar()) {
						...
						case ']':
							...
							break;
						default:
							if (Character.isDigit(e.getKeyChar()) && (searchInConstantPoolsEnterTextField.getText()
							                                                                             .length() == 0)) {
				...
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode() == KeyEvent.VK_DOWN) {
						DefaultMutableTreeNode root = (DefaultMutableTreeNode) searchInConstantPoolsTree.getModel()
						                                                                                .getRoot();
			...
			searchInConstantPoolsEnterTextField.getDocument()
			                                   .addDocumentListener(new DocumentListener() {
				                                   ...
				                                   protected void call() {
					                                   changedPatternCallback.accept(searchInConstantPoolsEnterTextField.getText(),
					                                                                 getFlags());
				                                   }
			                                   });
			...
			subsubpanel.setLayout(new GridLayout(2,
			                                     1));
			subsubpanel.add(searchInConstantPoolsCheckBoxType = new JCheckBox("Type",
			                                                                  true));
			...
			subsubpanel.setLayout(new GridLayout(2,
			                                     1));
			subsubpanel.add(searchInConstantPoolsCheckBoxDeclarations = new JCheckBox("Declarations",
			                                                                          true));
			searchInConstantPoolsCheckBoxDeclarations.addItemListener(checkBoxListener);
			subsubpanel.add(searchInConstantPoolsCheckBoxReferences = new JCheckBox("References",
			                                                                        true));
			...
			searchInConstantPoolsTree.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 2) {
						...
						if (node != null) {
							selectedTypeCallback.accept(node.getUri(),
							                            searchInConstantPoolsEnterTextField.getText(),
							                            getFlags());
						}
					}
				}
			...
			Action searchInConstantPoolsOpenActionListener = new AbstractAction() {
				...
				public void actionPerformed(ActionEvent actionEvent) {
					...
					if (selectedTreeNode != null) {
						selectedTypeCallback.accept(selectedTreeNode.getUri(),
						                            searchInConstantPoolsEnterTextField.getText(),
						                            getFlags());
					}
				}
			...
			rootPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
			        .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE,
			                                    0),
			             "SearchInConstantPoolsView.cancel");
			rootPane.getActionMap()
			        .put("SearchInConstantPoolsView.cancel",
			             searchInConstantPoolsCancelActionListener);
	...
	@SuppressWarnings("unchecked")
	protected void populate(DefaultTreeModel model,
	                        T node) {
		// Populate node
		populate(node);
		// Populate children
	...
	@SuppressWarnings("unchecked")
	public void updateTree(Collection<DelegatingFilterContainer> containers,
	...
	protected static class ContainerComparator
