...
public class SelectLocationView<T extends DefaultMutableTreeNode & ContainerEntryGettable & UriGettable> {
	protected static final DelegatingFilterContainerComparator DELEGATING_FILTER_CONTAINER_COMPARATOR
					= new DelegatingFilterContainerComparator();
	...
	public SelectLocationView(API api,
	                          JFrame mainFrame) {
		this.api = api;
		// Build GUI
		SwingUtil.invokeLater(() -> {
			selectLocationDialog = new JDialog(mainFrame,
			                                   "",
			                                   false);
			...
			selectLocationLabel.setBorder(BorderFactory.createEmptyBorder(5,
			                                                              5,
			                                                              0,
			                                                              5));
			selectLocationPanel.add(selectLocationLabel,
			                        BorderLayout.NORTH);

			selectLocationTree = new Tree();
			selectLocationTree.setBorder(BorderFactory.createEmptyBorder(5,
			                                                             5,
			                                                             5,
			                                                             5));
			...
			selectLocationPanel.add(selectLocationTree,
			                        BorderLayout.CENTER);
			...
			rootPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
			        .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE,
			                                    0),
			             "SelectLocationView.cancel");
			rootPane.getActionMap()
			        .put("SelectLocationView.cancel",
			             new AbstractAction() {
				             @Override
	...
	public void show(Point location,
	                 Collection<DelegatingFilterContainer> containers,
	                 int locationCount,
	                 Consumer<URI> selectedEntryCallback,
		...
		SwingUtil.invokeLater(() -> {
			// Init
			T root = (T) selectLocationTree.getModel()
			                               .getRoot();
			...
			for (DelegatingFilterContainer container : sortedContainers) {
				...
				if (factory != null) {
					T node = factory.make(api,
					                      parentEntry);

					if (node != null) {
						root.add(node);
						populate(container.getUris(),
						         node);
					}
				}
			}
			...
			// Expand all nodes
			for (int row = 0;
			     row < selectLocationTree.getRowCount();
			     row++) {
	...
	@SuppressWarnings("unchecked")
	protected void populate(Set<URI> uris,
	...
	protected static class DelegatingFilterContainerComparator
		...
		@Override
		public int compare(DelegatingFilterContainer fcw1,
		                   DelegatingFilterContainer fcw2) {
			return fcw1.getRoot()
			           .getUri()
			           .compareTo(fcw2.getRoot()
			                          .getUri());
		}
	}
}
