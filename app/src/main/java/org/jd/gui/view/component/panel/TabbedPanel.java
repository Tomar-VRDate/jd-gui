...
public class TabbedPanel<T extends JComponent & UriGettable>
				extends JPanel
				implements PreferencesChangeListener {
	protected static final ImageIcon CLOSE_ICON        = new ImageIcon(TabbedPanel.class.getClassLoader()
	                                                                                    .getResource("org/jd/gui/images"
	                                                                                                 + "/close.gif"));
	protected static final ImageIcon CLOSE_ACTIVE_ICON = new ImageIcon(TabbedPanel.class.getClassLoader()
	                                                                                    .getResource("org/jd/gui/images"
	                                                                                                 + "/close_active"
	                                                                                                 + ".gif"));
	...
	protected static Color darker(Color c) {
		return new Color(Math.max((int) (c.getRed() * 0.85),
		                          0),
		                 Math.max((int) (c.getGreen() * 0.85),
		                          0),
		                 Math.max((int) (c.getBlue() * 0.85),
		                          0),
		                 c.getAlpha());
	}
	...
	protected JTabbedPane createTabPanel() {
		JTabbedPane tabPanel = new JTabbedPane() {
			...
			public String getToolTipText(MouseEvent e) {
				int index = indexAtLocation(e.getX(),
				                            e.getY());
				if (index != -1) {
		...
		tabPanel.addMouseListener(new MouseAdapter() {
			...
			protected void showPopupTabMenu(MouseEvent e) {
				if (e.isPopupTrigger()) {
					int index = tabPanel.indexAtLocation(e.getX(),
					                                     e.getY());
					if (index != -1) {
						new PopupTabMenu(tabPanel.getComponentAt(index)).show(e.getComponent(),
						                                                      e.getX(),
						                                                      e.getY());
					}
				}
	...
	public void addPage(String title,
		...
		tab.setBorder(BorderFactory.createEmptyBorder(2,
		                                              0,
		                                              3,
		                                              0));
		tab.setOpaque(false);
	...
	protected class PopupTabMenu
		...
		public PopupTabMenu(Component component) {
			...
			if ((tabbedPane.getTabCount() > 1) && (PlatformService.getInstance()
			                                                      .isMac() || "true".equals(preferences.get(TAB_LAYOUT)))) {
				addSeparator();
			...
			if (component instanceof ContainerEntryGettable) {
				Collection<Action> actions = api.getContextualActions(((ContainerEntryGettable) component).getEntry(),
				                                                      null);
