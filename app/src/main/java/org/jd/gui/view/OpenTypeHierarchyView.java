...
public class OpenTypeHierarchyView {
	protected static final ImageIcon ROOT_CLASS_ICON     = new ImageIcon(OpenTypeHierarchyView.class.getClassLoader()
	                                                                                                .getResource("org/jd"
	                                                                                                             + "/gui"
	                                                                                                             +
	                                                                                                             "/images/generate_class.png"));
	protected static final ImageIcon ROOT_INTERFACE_ICON = new ImageIcon(OpenTypeHierarchyView.class.getClassLoader()
	                                                                                                .getResource("org/jd"
	                                                                                                             + "/gui"
	                                                                                                             +
	                                                                                                             "/images/generate_int.png"));
	...
	public OpenTypeHierarchyView(API api,
	                             JFrame mainFrame,
	                             TriConsumer<Point, Collection<Container.Entry>, String> selectedTypeCallback) {
		this.api = api;
		this.selectedTypeCallback = selectedTypeCallback;
		// Build GUI
		SwingUtil.invokeLater(() -> {
			openTypeHierarchyDialog = new JDialog(mainFrame,
			                                      "Hierarchy Type",
			                                      false);
			...
			openTypeHierarchyTree.addTreeExpansionListener(new TreeExpansionListener() {
				...
				public void treeExpanded(TreeExpansionEvent e) {
					...
					model.reload((TreeNode) e.getPath()
					                         .getLastPathComponent());
			...
			openTypeHierarchyScrollPane.setPreferredSize(new Dimension(400,
			                                                           150));
			panel.add(openTypeHierarchyScrollPane,
			          BorderLayout.CENTER);
			...
			rootPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
			        .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE,
			                                    0),
			             "OpenTypeHierarchyView.cancel");
			rootPane.getActionMap()
			        .put("OpenTypeHierarchyView.cancel",
			             openTypeHierarchyCancelActionListener);
	...
	public void updateTree(Collection<Future<Indexes>> collectionOfFutureIndexes) {
		...
		if (selectedTreeNode != null) {
			updateTree(selectedTreeNode.entry,
			           selectedTreeNode.typeName);
		}
	}

	protected void updateTree(Container.Entry entry,
	                          String typeName) {
		SwingUtil.invokeLater(() -> {
			// Clear tree
			DefaultTreeModel       model = (DefaultTreeModel) openTypeHierarchyTree.getModel();
			...
			TreeNode selectedTreeNode = createTreeNode(entry,
			                                           typeName);
			...
			if (selectedTreeNode != null) {
				...
				if (bounds != null) {
					...
					if (lastRowBounds != null) {
						bounds.y = Math.max(bounds.y - 30,
						                    0);
						bounds.height = Math.min(bounds.height + bounds.y + 60,
						                         lastRowBounds.height + lastRowBounds.y) - bounds.y;
					}
	...
	protected TreeNode createTreeNode(Container.Entry entry,
	                                  String typeName) {
		Type type = api.getTypeFactory(entry)
		               .make(api,
		                     entry,
		                     typeName);
		...
		TreeNode treeNode = new TreeNode(entry,
		                                 typeName,
		                                 entries,
		                                 new TreeNodeBean(type));
	...
	protected TreeNode createParentTreeNode(TreeNode treeNode) {
		Type type = api.getTypeFactory(treeNode.entry)
		               .make(api,
		                     treeNode.entry,
		                     treeNode.typeName);
		...
		if (superTypeName != null) {
			...
			if (superEntry != null) {
				...
				TreeNode superTreeNode = createTreeNode(superEntry,
				                                        superTypeName);
				// Populate parent tree node
				populateTreeNode(superTreeNode,
				                 treeNode);
				// Recursive call
				...
				String package_ = superTypeName.substring(0,
				                                          lastPackageSeparatorIndex)
				                               .replace('/',
				                                        '.');
				String name = superTypeName.substring(lastPackageSeparatorIndex + 1)
				                           .replace('$',
				                                    '.');
				String label = (package_ != null)
				               ? name + " - " + package_
				               : name;
				Icon icon = ((type.getFlags() & Type.FLAG_INTERFACE) == 0)
				            ? ROOT_CLASS_ICON
				            : ROOT_INTERFACE_ICON;
	 ...
	 */
	protected void populateTreeNode(TreeNode superTreeNode,
	                                TreeNode activeTreeNode) {
		superTreeNode.removeAllChildren();

		// Search preferred container: if 'superTreeNode' is a root with an unknown super entry, uses the container of
		// active child node
		...
		for (String subTypeName : subTypeNames) {
			if (subTypeName.equals(activeTypName)) {
				...
				if (entry != null) {
					// Create type
					Type t = api.getTypeFactory(entry)
					            .make(api,
					                  entry,
					                  subTypeName);
					if (t != null) {
						// Create tree node
						treeNodes.add(createTreeNode(entry,
						                             t.getName()));
					}
				}
			}
	...
	protected void onTypeSelected() {
		...
		if (selectedTreeNode != null) {
			...
			Point leftBottom = new Point(listLocation.x + bounds.x,
			                             listLocation.y + bounds.y + bounds.height);
			selectedTypeCallback.accept(leftBottom,
			                            selectedTreeNode.entries,
			                            selectedTreeNode.typeName);
		}
	}
		...
		try {
			for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
				if (futureIndexes.isDone()) {
					Map<String, Collection> subTypeNames = futureIndexes.get()
					                                                    .getIndex("subTypeNames");
	...
	protected List<Container.Entry> getEntries(String typeName) {
		...
		try {
			for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
				if (futureIndexes.isDone()) {
					Map<String, Collection> typeDeclarations = futureIndexes.get()
					                                                        .getIndex("typeDeclarations");
	...
	protected static class TreeNode
		...
		TreeNode(Container.Entry entry,
		         String typeName,
		         List<Container.Entry> entries,
		         Object userObject) {
			super(userObject);
	...
	protected static class TreeNodeBean
		...
		TreeNodeBean(Type type) {
			this.label = (type.getDisplayPackageName() != null)
			             ? type.getDisplayTypeName() + " - " + type.getDisplayPackageName()
			             : type.getDisplayTypeName();
			this.icon = type.getIcon();
		}

		TreeNodeBean(String label,
		             Icon icon) {
			this.label = label;
			this.icon = icon;
		}
