...
public class MainTabbedPanel<T extends JComponent & UriGettable>
	...
	public void create() {
		...
		Color fontColor = panel.getBackground()
		                       .darker();
		...
		JLabel title = newLabel("No files are open",
		                        fontColor);
		...
		box.add(title);
		box.add(newLabel("Open a file with menu \"File > Open File...\"",
		                 fontColor));
		box.add(newLabel("Open recent files with menu \"File > Recent Files\"",
		                 fontColor));
		box.add(newLabel("Drag and drop files from " + getFileManagerLabel(),
		                 fontColor));
		box.add(Box.createVerticalGlue());
	...
	protected String getFileManagerLabel() {
		switch (PlatformService.getInstance()
		                       .getOs()) {
			case Linux:
	...
	public <T extends JComponent & UriGettable> void pageChanged(T page) {
		...
		if (subPage != null) {
			((JComponent) subPage).putClientProperty("currentPage",
			                                         page);
		}
