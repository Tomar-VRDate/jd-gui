...
public class GoToView {
	...
	public GoToView(Configuration configuration,
	                JFrame mainFrame) {
		// Build GUI
		SwingUtil.invokeLater(() -> {
			goToDialog = new JDialog(mainFrame,
			                         "Go to Line",
			                         false);
			...
			rootPane.getActionMap()
			        .put("OpenTypeView.cancel",
			             goToCancelActionListener);

			// Add main listener
			goToEnterLineNumberTextField.getDocument()
			                            .addDocumentListener(new DocumentListener() {
				                            protected final Color backgroundColor = UIManager.getColor("TextField.background");
				                            protected final Color errorBackgroundColor
								                            = Color.decode(configuration.getPreferences()
