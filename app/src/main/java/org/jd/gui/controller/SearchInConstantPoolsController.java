...
public class SearchInConstantPoolsController
	...
	public SearchInConstantPoolsController(API api,
		...
		// Create UI
		this.searchInConstantPoolsView = new SearchInConstantPoolsView(api,
		                                                               mainFrame,
		...
		// Create result cache
		this.cache = new LinkedHashMap<String, Map<String, Collection>>(CACHE_MAX_ENTRIES * 3 / 2,
		                                                                0.7f,
		                                                                true) {
			@Override
	...
	protected static Map<String, Collection> matchTypeEntriesWithChar(char c,
		...
		if ((c == '*') || (c == '?')) {
			...
			for (String typeName : index.keySet()) {
				...
				int lastIndex = Math.max(lastPackageSeparatorIndex,
				                         lastTypeNameSeparatorIndex);
	...
	protected static Map<String, Collection> matchTypeEntriesWithString(String pattern,
	                                                                    Map<String, Collection> index) {
		...
		for (String typeName : index.keySet()) {
			...
			if (p.matcher(typeName.substring(lastIndex))
			     .matches()) {
				map.put(typeName,
				        index.get(typeName));
			}
	...
	protected static Map<String, Collection> matchWithChar(char c,
	                                                       Map<String, Collection> index) {
		if ((c == '*') || (c == '?')) {
			return index;
		} else {
	...
	protected static Map<String, Collection> matchWithString(String pattern,
	                                                         Map<String, Collection> index) {
		Pattern                 p   = createPattern(pattern);
		Map<String, Collection> map = new HashMap<>();
	...
	public void show(Collection<Future<Indexes>> collectionOfFutureIndexes,
		...
		if (hashCode != indexesHashCode) {
			...
			updateTree(searchInConstantPoolsView.getPattern(),
			           searchInConstantPoolsView.getFlags());
	...
	@SuppressWarnings("unchecked")
	protected void updateTree(String pattern,
	                          int flags) {
		...
		executor.execute(() -> {
					...
					for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
						if (futureIndexes.isDone()) {
							...
							if (!matchingEntries.isEmpty()) {
								...
								Container.Entry parentEntry = matchingEntries.iterator()
								                                             .next();
								Container container = null;

								while (parentEntry.getContainer()
								                  .getRoot() != null) {
									container = parentEntry.getContainer();
									parentEntry = container.getRoot()
									                       .getParent();
								}

								// TODO In a future release, display matching strings, types, inner-types, fields and methods, not
								//  only matching files
								...
								delegatingFilterContainers.add(new DelegatingFilterContainer(container,
								                                                             matchingEntries));
							}
						}
					}
			...
			searchInConstantPoolsView.updateTree(delegatingFilterContainers,
			                                     count);
		});
	}
	...
	protected HashSet<Container.Entry> getOuterEntries(Set<Container.Entry> matchingEntries) {
		...
		for (Container.Entry entry : matchingEntries) {
			TypeFactory typeFactory = TypeFactoryService.getInstance()
			                                            .get(entry);

			if (typeFactory != null) {
				Type type = typeFactory.make(api,
				                             entry,
				                             null);
				...
				if ((type != null) && (type.getOuterName() != null)) {
					...
					if (outerTypeEntry == null) {
						...
						for (Container.Entry e : entry.getParent()
						                              .getChildren()) {
							typeFactory = TypeFactoryService.getInstance()
							                                .get(e);

							if (typeFactory != null) {
								type = typeFactory.make(api,
								                        e,
								                        null);

								if (type != null) {
									typeNameToEntry.put(type.getName(),
									                    e);
									if (type.getOuterName() != null) {
										innerTypeNameToOuterTypeName.put(type.getName(),
										                                 type.getOuterName());
									}
								}
							}
						...
						for (Map.Entry<String, String> e : innerTypeNameToOuterTypeName.entrySet()) {
							...
							if (innerTypeEntry != null) {
								...
								if (outerTypeEntry != null) {
									innerTypeEntryToOuterTypeEntry.put(innerTypeEntry,
									                                   outerTypeEntry);
								}
							}
						}
	...
	protected void filter(Indexes indexes,
	                      String pattern,
	                      int flags,
		...
		if ((flags & SearchInConstantPoolsView.SEARCH_TYPE) != 0) {
			if (declarations) {
				match(indexes,
				      ...
				      SearchInConstantPoolsController::matchTypeEntriesWithString,
				      matchingEntries);
			}
			if (references) {
				match(indexes,
		...
		if ((flags & SearchInConstantPoolsView.SEARCH_METHOD) != 0) {
			if (declarations) {
				match(indexes,
				      ...
				      SearchInConstantPoolsController::matchWithString,
				      matchingEntries);
			}
			if (references) {
				match(indexes,
	...
	@SuppressWarnings("unchecked")
	protected void match(Indexes indexes,
	                     String indexName,
	                     String pattern,
		...
		if (patternLength > 0) {
			...
			if (matchedEntries == null) {
				...
				if (index != null) {
					if (patternLength == 1) {
						matchedEntries = matchWithCharFunction.apply(pattern.charAt(0),
						                                             index);
					} else {
						String lastKey = key.substring(0,
						                               key.length() - 1);
						Map<String, Collection> lastMatchedTypes = cache.get(lastKey);
						if (lastMatchedTypes != null) {
							matchedEntries = matchWithStringFunction.apply(pattern,
							                                               lastMatchedTypes);
						} else {
							matchedEntries = matchWithStringFunction.apply(pattern,
							                                               index);
	...
	protected void onTypeSelected(URI uri,
	                              String pattern,
	                              int flags) {
		...
		if (entry != null) {
			...
			try {
				openCallback.accept(new URI(u.getScheme(),
				                            u.getHost(),
				                            u.getPath(),
				                            query,
				                            null));
