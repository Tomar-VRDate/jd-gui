...
public class SelectLocationController {
	...
	public SelectLocationController(API api,
	                                JFrame mainFrame) {
		this.api = api;
		// Create UI
		selectLocationView = new SelectLocationView(api,
		                                            mainFrame);
	}
	...
	public void show(Point location,
	                 ...
	                 Consumer<Container.Entry> selectedLocationCallback,
		...
		for (Container.Entry entry : entries) {
			...
			while (true) {
				Container parentContainer = container.getRoot()
				                                     .getParent()
				                                     .getContainer();
		...
		for (Map.Entry<Container, ArrayList<Container.Entry>> mapEntry : map.entrySet()) {
			...
			delegatingFilterContainers.add(new DelegatingFilterContainer(container,
			                                                             getOuterEntries(mapEntry.getValue())));
		}

		Consumer<URI> selectedEntryCallback = uri -> onLocationSelected(delegatingFilterContainers,
		                                                                uri,
		                                                                selectedLocationCallback);

		selectLocationView.show(location,
		                        delegatingFilterContainers,
		                        entries.size(),
	...
	protected Collection<Container.Entry> getOuterEntries(Collection<Container.Entry> entries) {
		...
		for (Container.Entry entry : entries) {
			...
			TypeFactory factory = TypeFactoryService.getInstance()
			                                        .get(entry);

			if (factory != null) {
				Type type = factory.make(api,
				                         entry,
				                         null);
				...
				if ((type != null) && (type.getOuterName() != null)) {
					...
					if (outerTypeEntry == null) {
						...
						for (Container.Entry e : entry.getParent()
						                              .getChildren()) {
							factory = TypeFactoryService.getInstance()
							                            .get(e);

							if (factory != null) {
								type = factory.make(api,
								                    e,
								                    null);
	...
	protected void onLocationSelected(Set<DelegatingFilterContainer> delegatingFilterContainers,
	                                  URI uri,
	                                  Consumer<Container.Entry> selectedLocationCallback) {
	...
	protected static class ContainerEntryComparator
		...
		@Override
		public int compare(Container.Entry e1,
		                   Container.Entry e2) {
			return e1.getPath()
			         .compareTo(e2.getPath());
		}
	}
}
