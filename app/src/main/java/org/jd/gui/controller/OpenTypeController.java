...
public class OpenTypeController
	...
	public OpenTypeController(API api,
		...
		// Create UI
		openTypeView = new OpenTypeView(api,
		                                mainFrame,
		                                this::updateList,
		                                this::onTypeSelected);
		selectLocationController = new SelectLocationController(api,
		                                                        mainFrame);
		// Create result cache
		cache = new LinkedHashMap<String, Map<String, Collection>>(CACHE_MAX_ENTRIES * 3 / 2,
		                                                           0.7f,
		                                                           true) {
			@Override
	...
	protected static void match(char c,
	...
	@SuppressWarnings("unchecked")
	protected static void match(Pattern regExpPattern,
	                            Map<String, Collection> index,
	                            Map<String, Collection> result) {
		for (Map.Entry<String, Collection> mapEntry : index.entrySet()) {
			String                      typeName = mapEntry.getKey();
			...
			if (regExpPattern.matcher(typeName.substring(lastIndex))
			                 .matches()) {
				add(result,
				    typeName,
				    entries);
	...
	@SuppressWarnings("unchecked")
	protected static void add(Map<String, Collection> map,
	                          String key,
	                          Collection value) {
				...
				try {
					for (Future<Indexes> futureIndexes : collectionOfFutureIndexes) {
						if (futureIndexes.isDone()) {
							...
							String                  key             = indexes.hashCode() + "***" + pattern;
