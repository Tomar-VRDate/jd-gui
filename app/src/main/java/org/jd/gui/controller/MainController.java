...
public class MainController
	...
	public MainController(Configuration configuration) {
		...
		SwingUtil.invokeLater(() -> {
			...
			// Create main frame
			mainView = new MainView(configuration,
			                        this,
			                        history,
			                        e -> onOpen(),
			                        e -> onClose(),
	...
	public void show(List<File> files) {
		SwingUtil.invokeLater(() -> {
			// Show main frame
			mainView.show(configuration.getMainWindowLocation(),
		...
		executor.schedule(() -> {
			                  ...
			                  SwingUtil.invokeLater(() -> {
				                  ...
				                  saveAllSourcesController = new SaveAllSourcesController(MainController.this,
				                                                                          mainFrame);
				                  containerChangeListeners.add(openTypeController = new OpenTypeController(MainController.this,
				                                                                                           executor,
				                                                                                           mainFrame));
				                  containerChangeListeners.add(openTypeHierarchyController =
								                                               new OpenTypeHierarchyController(MainController.this,
				                                                                                                             executor,
				                                                                                                             mainFrame));
				                  goToController = new GoToController(configuration,
				                                                      mainFrame);
				                  containerChangeListeners.add(
								                  searchInConstantPoolsController =
												                  new SearchInConstantPoolsController(MainController.this,
								                                                                                        executor,
								                                                                                        mainFrame));
				                  preferencesController = new PreferencesController(configuration,
				                                                                    mainFrame,
				                                                                    PreferencesPanelService.getInstance()
				                                                                                           .getProviders());
				                  selectLocationController = new SelectLocationController(MainController.this,
				                                                                          mainFrame);
				                  ...
				                  new JFileChooser().addChoosableFileFilter(new FileNameExtensionFilter("",
				                                                                                        "dummy"));
				                  FileSystemView.getFileSystemView()
				                                .isFileSystemRoot(new File("dummy"));
				                  new JLayer();
			                  });
		                  },
		                  400,
	...
	protected void onOpen() {
		Map<String, FileLoader> loaders = FileLoaderService.getInstance()
		                                                   .getMapProviders();
		...
		chooser.addChoosableFileFilter(new FileNameExtensionFilter("All files (" + description + ")",
		                                                           array));

		for (String extension : extensions) {
			FileLoader loader = loaders.get(extension);
			chooser.addChoosableFileFilter(new FileNameExtensionFilter(loader.getDescription(),
	...
	protected void onSaveSource() {
		if (currentPage instanceof ContentSavable) {
			...
			chooser.setSelectedFile(new File(configuration.getRecentSaveDirectory(),
			                                 ((ContentSavable) currentPage).getFileName()));
	...
	protected void save(File selectedFile) {
		try (OutputStream os = new FileOutputStream(selectedFile)) {
			((ContentSavable) currentPage).save(this,
			                                    os);
		} catch (IOException e) {
	...
	protected void onSaveAllSources() {
		if (!saveAllSourcesController.isActivated()) {
			...
			if (currentPanel instanceof SourcesSavable) {
				...
				chooser.setSelectedFile(new File(configuration.getRecentSaveDirectory(),
				                                 sourcesSavable.getSourceFileName()));
	...
	protected void onPaste() {
		try {
			...
			if ((transferable != null) && transferable.isDataFlavorSupported(DataFlavor.stringFlavor)) {
				...
				PasteHandler pasteHandler = PasteHandlerService.getInstance()
				                                               .get(obj);

				if (pasteHandler != null) {
					pasteHandler.paste(this,
					                   obj);
				}
			}
		} catch (Exception e) {
	...
	protected void onFindCriteriaChanged() {
		if (currentPage instanceof ContentSearchable) {
			mainView.setFindBackgroundColor(((ContentSearchable) currentPage).highlightText(mainView.getFindText(),
			                                                                                mainView.getFindCaseSensitive()));
		}
	}

	protected void onFindNext() {
		if (currentPage instanceof ContentSearchable) {
			((ContentSearchable) currentPage).findNext(mainView.getFindText(),
			                                           mainView.getFindCaseSensitive());
		}
	}

	protected void onOpenType() {
		openTypeController.show(getCollectionOfFutureIndexes(),
		                        uri -> openURI(uri));
	}
	...
	protected void onOpenTypeHierarchy() {
		if (currentPage instanceof FocusedTypeGettable) {
			...
			openTypeHierarchyController.show(getCollectionOfFutureIndexes(),
			                                 ftg.getEntry(),
			                                 ftg.getFocusedTypeName(),
			                                 uri -> openURI(uri));
		}
	}
	...
	protected void onGoTo() {
		if (currentPage instanceof LineNumberNavigable) {
			...
			goToController.show(lnn,
			                    lineNumber -> lnn.goToLineNumber(lineNumber));
		}
	}

	protected void onSearch() {
		searchInConstantPoolsController.show(getCollectionOfFutureIndexes(),
		                                     uri -> openURI(uri));
	}
	...
	protected void onFindPrevious() {
		if (currentPage instanceof ContentSearchable) {
			...
			cs.findPrevious(mainView.getFindText(),
			                mainView.getFindCaseSensitive());
		}
	}
	...
	protected void checkPreferencesChange(JComponent page) {
		if (page instanceof PreferencesChangeListener) {
			...
			if (!currentHashcode.equals(lastHashcode)) {
				...
				page.putClientProperty("preferences-hashCode",
				                       currentHashcode);
			}
		}
	}
	...
	protected void checkIndexesChange(JComponent page) {
		if (page instanceof IndexesChangeListener) {
			...
			Integer                     lastHashcode
			                                                      = (Integer) page.getClientProperty(
																																		"collectionOfFutureIndexes-hashCode");

			if (!currentHashcode.equals(lastHashcode)) {
				((IndexesChangeListener) page).indexesChanged(collectionOfFutureIndexes);
				page.putClientProperty("collectionOfFutureIndexes-hashCode",
				                       currentHashcode);
			}
		}
	}
	...
	public void openFiles(List<File> files) {
		...
		for (File file : files) {
			...
			if (file.exists()) {
				...
				if ((loader != null) && !loader.accept(this,
				                                       file)) {
	...
	public boolean openURI(int x,
		...
		if (entries != null) {
			if (entries.size() == 1) {
				...
				Container.Entry entry = entries.iterator()
				                               .next();
				return openURI(UriUtil.createURI(this,
				                                 getCollectionOfFutureIndexes(),
				                                 entry,
				                                 query,
				                                 fragment));
			} else {
				...
				Collection<Future<Indexes>> collectionOfFutureIndexes = getCollectionOfFutureIndexes();
				selectLocationController.show(new Point(x + (16 + 2),
				                                        y + 2),
				                              entries,
				                              entry -> openURI(UriUtil.createURI(this,
				                                                                 collectionOfFutureIndexes,
				                                                                 entry,
				                                                                 query,
				                                                                 fragment)),
				                              // entry selected closure
				                              () -> {});                                                                                    // popup close closure
				return true;
			}
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public <T extends JComponent & UriGettable> void addPanel(String title,
	                                                          Icon icon,
	                                                          String tip,
	                                                          T component) {
		mainView.addMainPanel(title,
		                      icon,
		                      tip,
		                      component);
		...
		if (component instanceof ContentIndexable) {
			...
			component.putClientProperty("indexes",
			                            futureIndexes);
		}
	}

	@Override
	public Collection<Action> getContextualActions(Container.Entry entry,
	                                               String fragment) {
		return ContextualActionsFactoryService.getInstance()
		                                      .get(this,
		                                           entry,
		                                           fragment);
	}
	...
	public FileLoader getFileLoader(File file) {
		return FileLoaderService.getInstance()
		                        .get(this,
		                             file);
	}
	...
	public UriLoader getUriLoader(URI uri) {
		return UriLoaderService.getInstance()
		                       .get(this,
		                            uri);
	}
	...
	public PanelFactory getMainPanelFactory(Container container) {
		return PanelFactoryService.getInstance()
		                          .get(container);
	}

	@Override
	public ContainerFactory getContainerFactory(Path rootPath) {
		return ContainerFactoryService.getInstance()
		                              .get(this,
		                                   rootPath);
	}
	...
	public TreeNodeFactory getTreeNodeFactory(Container.Entry entry) {
		return TreeNodeFactoryService.getInstance()
		                             .get(entry);
	}

	@Override
	public TypeFactory getTypeFactory(Container.Entry entry) {
		return TypeFactoryService.getInstance()
		                         .get(entry);
	}
	...
	public SourceSaver getSourceSaver(Container.Entry entry) {
		return SourceSaverService.getInstance()
		                         .get(entry);
	}
	...
	public String getSource(Container.Entry entry) {
		return sourceLoaderService.getSource(this,
		                                     entry);
	}

	@Override
	public void loadSource(Container.Entry entry,
	                       LoadSourceListener listener) {
		executor.execute(() -> {
			String source = sourceLoaderService.loadSource(this,
			                                               entry);
	...
	public File loadSourceFile(Container.Entry entry) {
		return sourceLoaderService.getSourceFile(this,
		                                         entry);
	}
	...
	protected class FilesTransferHandler
		...
		public boolean importData(TransferHandler.TransferSupport info) {
			if (info.isDrop() && info.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
				try {
					openFiles((List<File>) info.getTransferable()
