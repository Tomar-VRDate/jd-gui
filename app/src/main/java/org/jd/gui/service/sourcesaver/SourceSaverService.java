...
public class SourceSaverService {
	...
	protected SourceSaverService() {
		Collection<SourceSaver> providers = ExtensionService.getInstance()
		                                                    .load(SourceSaver.class);
	...
	public SourceSaver get(Container.Entry entry) {
		SourceSaver saver = get(entry.getContainer()
		                             .getType(),
		                        entry);
		return (saver != null)
		       ? saver
	...
	protected SourceSaver get(String containerType,
	...
	protected static class SourceSavers {
		...
		void add(SourceSaver saver) {
			if (saver.getPathPattern() != null) {
				savers.put(saver.getPathPattern()
				                .pattern(),
				           saver);
			} else {
				defaultSaver = saver;
