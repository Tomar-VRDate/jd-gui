...
public class TreeNodeFactoryService {
	...
	protected TreeNodeFactoryService() {
		Collection<TreeNodeFactory> providers = ExtensionService.getInstance()
		                                                        .load(TreeNodeFactory.class);
			...
			for (String selector : provider.getSelectors()) {
				...
				if (factories == null) {
					mapProviders.put(selector,
					                 factories = new TreeNodeFactories());
				}
	...
	public TreeNodeFactory get(Container.Entry entry) {
		TreeNodeFactory factory = get(entry.getContainer()
		                                   .getType(),
		                              entry);
		return (factory != null)
		       ? factory
	...
	protected TreeNodeFactory get(String containerType,
	...
	protected static class TreeNodeFactories {
		...
		public void add(TreeNodeFactory factory) {
			if (factory.getPathPattern() != null) {
				factories.put(factory.getPathPattern()
				                     .pattern(),
				              factory);
			} else {
